...
One Person Lab App is maintained in [`one-person-lab-app`](https://github.com/gaofeng21cn/one-person-lab-app). Its current stable GUI shell is `shells/aionui`, an upstream-backed AionUI adapter, while this repository remains the OPL Framework and release discovery surface. Users download One Person Lab App packages from this repository’s GitHub Releases; first-time macOS arm64 users can choose the `One-Person-Lab-Full-<version>-mac-arm64.dmg` asset with MAS/MAG/RCA, the current family runtime provider payload, `officecli`, and recommended companion skill payloads, while in-app updates continue to use the standard App assets and `latest*.yml` metadata. This repository provides the shared workbench contracts and product surfaces consumed by the app and operator environment.
