...
function assertAppReleaseUserPathPayloadWorkorder(workorder: JsonRecord) {
  ...
  assert.equal(
    workorder.accepted_payload_paths.release_owner_verdict_path
      .success_claimed_by_opl,
    false,
  );
  assert.equal(
    workorder.accepted_payload_paths.release_owner_verdict_path
      .closes_release_ready,
    false,
  );
...
export function assertOwnerDeltaFirstReadinessProjection(readiness: JsonRecord) {
  ...
  assert.equal(
    readiness.stage_run_cockpit_summary.current_owner_delta_owner,
