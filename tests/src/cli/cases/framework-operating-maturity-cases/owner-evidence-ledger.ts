...
test('framework operating maturity projects owner evidence ledger refs without ready claims', () => {
  ...
  try {
    ...
    assert.equal(
      ...
      assert.equal(brandWorkOrder.missing_owner_action_ids.length, 37);
      assert.equal(
    ...
    const charterBrandChecklist = brandWorkOrder.owner_action_checklist.find(
      ...
      assert.equal(charterBrandChecklist.owner_action_required_count, 4);
      ...
      assert.equal(charterBrandChecklist.blocked_requirement_count, 0);
      assert.equal(charterBrandChecklist.missing_requirement_class_ids.length, 4);
      assert.equal(
    ...
    assert.equal(
      ...
      assert.equal(charterBrandChecklist.observed_refs_not_l5_claim_action_ids.length, 9);
      assert.equal(charterBrandChecklist.observed_refs_not_l5_claim_work_order_ids.length, 9);
      assert.equal(
    ...
    const charterOwnerAcceptanceRef =
      'owner_acceptance_ref:opl-brand-l5/charter/owner_acceptance/opl-framework-owner-accepted-20260615';
