...
export const PACK_COMPILER_READY_DOMAIN_ALIASES = ['mas', 'mag', 'rca', 'opl-meta-agent'];
...
export function assertReadyPackCompilerSummary(summary: {
  ...
  const expectedSurfaceCount = PACK_COMPILER_READY_DOMAIN_ALIASES.length
    * PACK_COMPILER_GENERATED_SURFACE_COUNT_PER_DOMAIN;
  assert.equal(summary.total_domain_count, PACK_COMPILER_READY_DOMAIN_ALIASES.length);
  assert.equal(summary.ready_domain_count, PACK_COMPILER_READY_DOMAIN_ALIASES.length);
  ...
  assert.equal(summary.generated_artifact_drift_aligned_count, PACK_COMPILER_READY_DOMAIN_ALIASES.length);
