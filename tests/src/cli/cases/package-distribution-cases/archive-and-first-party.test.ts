...
test('package archive builder writes channel manifest checksums git source and release discipline gate', () => {
  ...
  const frameworkArchivePath = path.join(outDir, 'framework', 'one-person-lab-framework-0.3.4.tar.gz');
  ...
  assert.equal(runtimePackageJson.version, '0.3.4');
  ...
  assert.equal(manifest.packages.framework_core.homebrew_formula.version, '0.3.4');
  assert.equal(
  ...
  assert.equal(manifest.packages.framework_core.artifact, 'ghcr.io/gaofeng21cn/one-person-lab-framework:0.3.4');
  ...
  assert.match(checksums, /one-person-lab-framework-0\.3\.4\.tar\.gz/);
...
test('MAS Scholar Skills provider manifest separates core Skill exports from module contract ids', () => {
  ...
  assert.equal(manifest.content_lock.digest, 'sha256:9f5358f141fc45051651e82ea573e7c779112e25eb95d1e074b5dd5249288451');
  ...
  assert.equal(payload.source_commit, '6b1ab913239059783c446772d1763e2ed355272f');
  assert.deepEqual(
...
test('OPL Flow is a workflow-profile Package without Agent identity', () => {
  ...
  assert.equal(manifest.codex_surface.carrier_source_commit, 'ea94cbc5d660caf870fefe2842fd0157fca52bd0');
  assert.equal(schema.properties.codex_surface.required.includes('carrier_source_commit'), true);
  ...
  assert.equal(payload.surface_kind, 'opl_package_payload_manifest.v2');
  assert.equal(payload.source_commit, 'ea94cbc5d660caf870fefe2842fd0157fca52bd0');
  assert.equal(
