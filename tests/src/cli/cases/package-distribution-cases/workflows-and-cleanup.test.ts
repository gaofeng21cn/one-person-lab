...
test('framework packages workflow is release-gated and manually repairable without WebUI publishing', () => {
  ...
  assert.equal(workflow.match(/push-to-registry:\s*true/g)?.length, 2);
  assert.equal(workflow.match(/create-storage-record:\s*false/g)?.length, 2);
  assert.doesNotMatch(workflow, /name:\s*Attest exact Release Set files/);
  assert.doesNotMatch(workflow, /name:\s*Attest Release Set SBOM/);
