...
test('packages manifest exposes canonical Release Set coordinates for Package install updates', () => {
  ...
  assert.equal(
    ...
    'ghcr.io/gaofeng21cn/one-person-lab-packages/mas:0.2.16',
  );
  assert.equal(output.packages_manifest.packages.package_artifacts.mas.package_id, 'mas');
  assert.equal(output.packages_manifest.packages.package_artifacts.mas.package_version, '0.2.16');
  ...
  assert.equal(
    ...
    'ghcr.io/gaofeng21cn/one-person-lab-packages/oma:0.4.3',
  );
  assert.equal(
