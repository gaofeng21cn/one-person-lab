...
export function assertOwnerDeltaTopline(drilldown: any) {
  ...
  const ownerAnswerRecorded =
    typeof drilldown.current_owner_delta.latest_owner_answer_ref === 'string'
    || typeof drilldown.current_owner_delta.hard_gate?.owner_answer_ref === 'string';
  ...
  assert.equal(
    drilldown.stage_run_next_missing_input_refs.includes('provider_attempt_ref'),
    !ownerAnswerMissing && !stageRunClosed,
  );
  assert.equal(
    drilldown.stage_run_next_missing_input_refs.includes('attempt_lease_ref'),
    !ownerAnswerMissing && !stageRunClosed,
  );
  assert.equal(
  ...
  assert.equal(
    drilldown.stage_run_next_missing_input_refs.includes('owner_answer_ref'),
    !ownerAnswerRecorded,
  );
