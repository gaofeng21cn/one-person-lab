...
test('agent-lab run returns refs-only MAS production evidence gate results from an external suite', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'opl-agent-lab-mas-production-evidence-suite-'));
  const suitePath = path.join(tmpDir, 'mas_production_evidence_suite.json');
  const suite = {
    suite_id: 'mas-production-evidence-suite',
    ...
    mas_production_evidence_gate: {
      surface_kind: 'mas_production_evidence_gate_refs',
      gate_ids: [
        'mas-production-evidence-gate:dm002/runtime-watch',
        'mas-production-evidence-gate:dm002/publication-eval',
      ],
      gate_result_refs: ['gate-result-ref:mas/dm002/production-evidence/agent-lab-run'],
      owner_route_ref: 'owner-route:mas/publication-quality-owner',
      no_forbidden_write_proof_refs: ['no-forbidden-write:mas/dm002/agent-lab-production-evidence'],
      typed_blocker_refs: ['typed-blocker-ref:mas/dm002/owner-receipt-required'],
      required_owner_receipt_refs: ['required-owner-receipt-ref:mas/dm002/production-evidence-closeout'],
      domain_verdict_claimed: false,
    },
    tasks: [
      {
        task_id: 'agent-lab-task:mas/dm002-production-evidence',
        domain_id: 'med-autoscience',
        task_family: 'mas_production_evidence_gate',
        environment: {
          environment_kind: 'provider_hosted',
          workspace_locator_ref: 'workspace-locator:mas/dm002',
          sandbox_policy: 'refs_only_no_artifact_mutation',
          network_policy: 'production_evidence_refs_only',
        },
        instructions_ref: 'instructions-ref:mas/dm002/production-evidence-suite',
        agent_entry_ref: 'domain-agent-entry:med-autoscience',
        stage_refs: ['stage-ref:mas/dm002/runtime-watch', 'stage-ref:mas/dm002/publication-eval'],
        oracle_refs: ['oracle-ref:mas/dm002/production-evidence-contract'],
        scorer_refs: ['scorer-ref:mas/publication-quality-owner'],
        recovery_probes: [
          {
            probe_ref: 'recovery-probe:mas/dm002/runtime-watch-resume',
            probe_kind: 'resume_after_interruption',
            expected_status: 'passed',
            observed_status: 'passed',
            source_refs: ['runtime-watch-ref:mas/dm002/latest'],
          },
        ],
        trajectory: {
          trajectory_ref: 'trajectory-ref:mas/dm002/production-evidence',
          run_ref: 'run-ref:mas/dm002/production-evidence',
          agent_executor: 'codex_cli',
          stage_attempt_refs: ['stage-attempt-ref:mas/dm002/production-evidence'],
          tool_call_refs: ['tool-call-ref:mas/dm002/read-production-evidence'],
          artifact_refs: ['artifact-locator-ref:mas/dm002/current-package'],
          receipt_refs: [],
          repair_refs: ['repair-ref:mas/dm002/no-current-repair'],
        },
        scorecard: {
          scorecard_ref: 'quality-scorecard-ref:mas/dm002/production-evidence',
          domain_owned: true,
          opl_scorecard_role: 'scorecard_ref_projection_only',
          passed: true,
          metric_refs: ['metric-ref:mas/dm002/production-evidence-present'],
          evidence_refs: ['evidence-ref:mas/dm002/production-evidence-suite'],
          review_refs: ['review-ref:mas/dm002/owner-review-required'],
          quality_gate_refs: ['quality-gate-ref:mas/dm002/publication-owner'],
        },
        improvement_candidate: {
          candidate_ref: 'improvement-candidate-ref:mas/dm002/no-op-production-evidence-handoff',
          candidate_kind: 'test_metadata',
          target_ref: 'handoff-ref:mas/dm002/production-evidence',
          evidence_refs: ['failure-taxonomy:mas/dm002/no-current-failure'],
          allowed_change_scope: 'candidate_config_only',
          promotion_gate_ref: 'promotion-gate-ref:mas/dm002/production-evidence',
        },
        promotion_gate: {
          gate_ref: 'promotion-gate-ref:mas/dm002/production-evidence',
          gate_status: 'passed',
          required_refs: ['quality-scorecard-ref:mas/dm002/production-evidence'],
          regression_suite_refs: ['regression-suite-ref:mas/dm002/production-evidence'],
          no_forbidden_write_proof_refs: ['no-forbidden-write:mas/dm002/agent-lab-production-evidence'],
        },
      },
    ],
  ...
  try {
    ...
    assert.equal(output.version, 'g2');
    assert.equal(output.agent_lab_run.suite_result.suite_kind, 'mas_production_evidence_suite');
    assert.equal(output.agent_lab_run.suite_result.status, 'passed');
    assert.equal(gateResult.surface_kind, 'opl_agent_lab_mas_production_evidence_gate_result');
    ...
    assert.deepEqual(gateResult.owner_route_refs, ['owner-route:mas/publication-quality-owner']);
    assert.deepEqual(gateResult.no_forbidden_write_proof_refs, [
      'no-forbidden-write:mas/dm002/agent-lab-production-evidence',
    ]);
    assert.deepEqual(gateResult.typed_blocker_refs, ['typed-blocker-ref:mas/dm002/owner-receipt-required']);
    assert.deepEqual(gateResult.required_receipt_refs, [
      'required-owner-receipt-ref:mas/dm002/production-evidence-closeout',
    ]);
    assert.deepEqual(gateResult.gate_result_refs, ['gate-result-ref:mas/dm002/production-evidence/agent-lab-run']);
    assert.deepEqual(output.agent_lab_run.ref_summary.production_evidence_gate_result_refs, [
      'gate-result-ref:mas/dm002/production-evidence/agent-lab-run',
    ]);
    assert.deepEqual(output.agent_lab_run.ref_summary.production_evidence_owner_route_refs, [
      'owner-route:mas/publication-quality-owner',
    ]);
    assert.deepEqual(output.agent_lab_run.ref_summary.production_evidence_typed_blocker_refs, [
      'typed-blocker-ref:mas/dm002/owner-receipt-required',
    ]);
