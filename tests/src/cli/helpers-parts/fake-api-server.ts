    ...
    if (request.method === 'GET' && url.pathname === '/api/opl/progress') {
      ...
      response.end(JSON.stringify({
        progress: {
          ...
          session_id: taskId ? 'sess-product-entry-001' : 'sess-progress',
          ...
          task_cards: {
            running: [
              {
                task_id: 'task-product-entry-001',
                title: '刷新投稿包',
                status: 'running',
              },
            ],
          ...
          ...(taskId
            ? {
                task: {
                  ...
                  session_id: 'sess-product-entry-001',
                },
              }
            : {}),
        },
    ...
    if (request.method === 'GET' && url.pathname === '/api/opl/sessions') {
      ...
      response.end(JSON.stringify({
        sessions: {
          ...
          raw_output: 'sess-product-entry-001 opl-product-entry Resume 004 paper progression',
          ledger: {
    ...
    if (request.method === 'POST' && url.pathname === '/api/opl/sessions/resume') {
      ...
      response.end(JSON.stringify({
        session_resume: {
          ...
          mode: 'resume',
          resume: {
            session_id: String(body?.session_id ?? 'sess-product-entry-001'),
