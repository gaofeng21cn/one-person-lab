...
test('Agent Lab exposes a refs-only RCA 40-slide cost estimate without domain authority', async () => {
  const agentLabModule = await import('../../src/agent-lab.ts') as Record<string, any>;
  const buildAgentLabCostEstimate = agentLabModule.buildAgentLabCostEstimate;
