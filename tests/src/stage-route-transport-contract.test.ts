...
test('stage route transport is passive and Codex owns every semantic route decision', () => {
  ...
  assert.equal(
    contract.codex_semantic_route_boundary.framework_can_reject_abi_valid_route_on_domain_semantic_merit,
    false,
  );
  assert.equal(
  ...
  assert.equal(contract.authority_boundary.opl_must_reject_non_authoritative_or_malformed_route_output, true);
  assert.equal(contract.authority_boundary.opl_can_reject_abi_valid_route_on_domain_semantic_merit, false);
  assert.equal(
    Object.hasOwn(
      contract.codex_semantic_route_boundary,
      'framework_can_accept_reject_rank_select_reconcile_or_override_route',
    ),
    false,
  );
