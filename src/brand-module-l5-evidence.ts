...
import {
  ...
  nextActionSummary,
  type BrandModuleL5OwnerEvidenceRoute,
} from './brand-module-l5-evidence-parts/followthrough-summary.ts';
...
function statusEnvelope(
  ...
  const compactModules = modules.map((entry) =>
    compactModule(contract, entry, evidenceLedger.receipts)
  );
  const evidenceClassFollowthrough = evidenceClassFollowthroughSummary(
    contract,
    compactModules.flatMap((entry) => entry.owner_evidence_routes),
  );
  return {
    ...
    evidence_class_followthrough_summary: evidenceClassFollowthrough,
