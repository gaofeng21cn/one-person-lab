...
export function buildStageAttemptUsageProjection(input: StageAttemptUsageInput) {
  ...
  return {
    ...
    availability: hasObservedResourceUsage ? 'usage_observed' : 'retry_budget_observed',
    token: {
    ...
    cost: {
      ...
      estimated_cost_usd_observed: Number(estimatedCostUsd.toFixed(6)),
      source_refs: uniqueStrings(costSourceRefs),
    },
    api_calls: {
      observed_count: apiCallObservedCount,
      count_observed: apiCallCount,
  ...
  return {
    ...
    token: {
      ...
      total_tokens_observed: projections.reduce((count, projection) =>
        count + projection.token.total_tokens_observed, 0
      ),
    },
    cost: {
      ...
      estimated_cost_usd_observed: Number(projections.reduce((count, projection) =>
        count + projection.cost.estimated_cost_usd_observed, 0
      ).toFixed(6)),
    },
    api_calls: {
      observed_count: projections.reduce((count, projection) => count + projection.api_calls.observed_count, 0),
      count_observed: projections.reduce((count, projection) => count + projection.api_calls.count_observed, 0),
    },
    duration: {
      ...
      duration_ms_observed: projections.reduce((count, projection) =>
        count + projection.duration.duration_ms_observed, 0
      ),
    },
    cadence: {
