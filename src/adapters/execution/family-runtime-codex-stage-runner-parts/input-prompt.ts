...
export function runnerPromptFor(input: {
  ...
  return [
    ...
    'If this Attempt is the decisive one, express the route only as route_impact.stage_route_decision. A stage_route_recommendation is advisory and does not become a semantic route decision; any missing-route fallback remains controller-owned and restricted to the declared route. When the next declared Stage genuinely does not apply to this target, that is decision_kind=skip naming that Stage as target_stage_id, not a recommendation; reserve complete for a terminal Stage that truly ends the operation.',
