...
export function domainStageRoutePromptLines(input: DomainStageRoutePromptInput) {
  ...
  return [
    ...
    'Return the best available domain artifact, raw/free-text/partial/negative result, failed-attempt evidence, or no-output/failure diagnostic. A typed closeout improves lineage and quality/ready evidence; progress does not depend on it, but a cross-Stage route declaration can only travel in it.',
