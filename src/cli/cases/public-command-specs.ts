...
export function buildPublicCommandSpecs(
  ...
  const publicCommandSpecs: Record<string, CommandSpec> = {
    ...
    'family-runtime': cloneCommandSpec(commandSpecs['family-runtime'], {
      usage:
        'opl family-runtime status|doctor|install|repair|provider repair|provider-slo tick|provider-worker supervisor|intake|tick|enqueue|scheduler status|scheduler install|scheduler trigger|scheduler remove|scheduler tick|evidence-worklist|paper-autonomy supervisor readback|queue list|queue inspect|queue redrive|queue hold|queue release|queue retire|attempt list|attempt inspect|attempt query|attempt cancel|approve|notify list|events export [options]',
      examples: [
