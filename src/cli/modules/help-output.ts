...
const COMMAND_GROUP_SUMMARIES: Record<string, string> = {
  ...
  'scholar-skills': '读取 OPL ScholarSkills 能力模块目录、descriptor、runtime env bridge 和 false-authority 边界。',
  ...
  return {
    ...
    help: {
      ...
      examples: [
        ...
        'opl connect sync-skills',
        'opl update status',
        'opl update plan --component agent_package_channel',
...
function formatHumanRootHelp(payload: ReturnType<typeof buildRootHelp>) {
  const lines = [
    ...
    '  opl foundry agents list        List the standard OPL Foundry Agent series members',
