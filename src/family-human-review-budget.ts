...
function stageHasActionCatalogHumanGate(stage: FamilyStageDescriptor, actionCatalog: FamilyActionCatalog | null) {
...
function implicitGateFromStage(
  ...
  const requiredRefs = uniqueStrings(requiredRefsForStage(stage, gateType));
  const hasActionGate = stageHasActionCatalogHumanGate(stage, actionCatalog);
  const missingRefs = requiredRefs.length > 0 || hasActionGate ? [] : [`human_review_required_refs:${stage.stage_id}`];
  return {
    ...
    status: missingRefs.length > 0 ? 'blocked' : 'ready',
    source: 'trust_boundary',
    admission_blocking: !hasActionGate,
  };
}
...
function mergeGates(gates: FamilyHumanReviewBudgetGate[]) {
  ...
  for (const gate of gates) {
    ...
    byKey.set(key, {
      ...
      admission_blocking: existing.admission_blocking || gate.admission_blocking,
    });
  }
  return [...byKey.values()];
}
...
export function buildStagePackHumanReviewBurdenBudget(
  ...
  return buildFamilyHumanReviewBurdenBudget({
    ...
    gates: [
      ...stageGates,
      ...gatesFromActionCatalog(plane, actionCatalog),
    ],
  });
}
