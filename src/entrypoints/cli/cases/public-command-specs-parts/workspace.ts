...
export function buildWorkspaceCommandSpecs(
  ...
  const workspaceCommandSpecs: Record<string, CommandSpec> = {
    workspace: {
      usage:
        'opl workspace projects|list|fleet report|root|init|ensure|validate|doctor|adopt|upgrade|artifact-lifecycle|source ingest|project lifecycle|project archive|project delete|export-map|health|inspect|inventory|report|interfaces|sync|bind|activate|archive [options]',
      summary:
