...
const RETIRED_COMMAND_REPLACEMENTS = [
  ...
  {
    tokens: ['agents', 'foundry'],
    command: 'opl agents foundry',
    replacement: 'opl help foundry',
    usage: 'opl foundry <status|approve|reject|cancel|versions|rollback> [options]',
    examples: ['opl help foundry', 'opl foundry status --run-id <run_id>'],
  },
  {
    tokens: ['agents', 'scaffold'],
    command: 'opl agents scaffold',
    replacement: 'opl agents run --domain oma --action engineer-agent',
    usage: 'opl agents run --domain oma --action engineer-agent --workspace <absolute_path> --payload-file <design-request.json>',
    examples: [
      'opl agents run --domain oma --action engineer-agent --workspace /path/to/workspace --payload-file design-request.json',
    ],
  },
  {
    tokens: ['agent-lab'],
