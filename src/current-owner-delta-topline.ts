...
function currentOwnerDeltaWithClosedStageRunAnswer(
  currentOwnerDelta: JsonRecord,
  preliminaryStageRunCockpit: JsonRecord,
) {
  const closedAnswer = closedStageRunOwnerAnswer(preliminaryStageRunCockpit);
  if (!closedAnswer) {
    return currentOwnerDelta;
  }
  return {
    ...currentOwnerDelta,
    latest_owner_answer_ref: closedAnswer.ownerAnswerRef,
    latest_owner_answer_kind: closedAnswer.ownerAnswerKind,
    stage_run_closeout_binding_ref: closedAnswer.stageRunCloseoutBindingRef,
    stage_run_closeout_binding_policy: closedAnswer.stageRunCloseoutBindingPolicy,
    hard_gate: {
      ...record(currentOwnerDelta.hard_gate),
      state: 'domain_owner_answer_recorded',
      human_or_domain_owner_required: false,
      owner_answer_ref: closedAnswer.ownerAnswerRef,
      owner_answer_kind: closedAnswer.ownerAnswerKind,
...
function readModelWithClosedStageRunAnswer(
