...
function expectBoolean(value: unknown, field: string, filePath: string): boolean {
    ...
    domains: domains.map((entry, index) => {
      ...
      if (!Array.isArray(backendCompanions)) {
        throw new GatewayContractError(
          ...
          { file: filePath, field: 'runtime_dependency_boundary.backend_companions' },
        );
      }
      ...
      return {
        ...
        independent_domain_agent: {
          agent_id: expectString(independentDomainAgent.agent_id, 'independent_domain_agent.agent_id', filePath),
          status: expectString(independentDomainAgent.status, 'independent_domain_agent.status', filePath),
        ...
        single_app_skill: {
          skill_id: expectString(singleAppSkill.skill_id, 'single_app_skill.skill_id', filePath),
          plugin_name: expectString(singleAppSkill.plugin_name, 'single_app_skill.plugin_name', filePath),
          activation_kind: expectString(singleAppSkill.activation_kind, 'single_app_skill.activation_kind', filePath),
          entry_command: expectString(singleAppSkill.entry_command, 'single_app_skill.entry_command', filePath),
        ...
        domain_truth_owner: expectStringArray(entry.domain_truth_owner, 'domain_truth_owner', filePath),
        opl_projection_role: expectStringArray(entry.opl_projection_role, 'opl_projection_role', filePath),
        runtime_dependency_boundary: {
          ...
          backend_companions: backendCompanions.map((companion, companionIndex) => {
            if (!isRecord(companion)) {
              throw new GatewayContractError(
                'contract_shape_invalid',
                'Each backend companion entry must be an object.',
                { file: filePath, field: 'runtime_dependency_boundary.backend_companions', index, companionIndex },
              );
            }

            return {
              project: expectString(companion.project, 'backend_companion.project', filePath),
              role: expectString(companion.role, 'backend_companion.role', filePath),
              controlled_by: expectString(companion.controlled_by, 'backend_companion.controlled_by', filePath),
              opl_top_level_domain_agent: expectBoolean(
        ...
        standalone_allowed: expectBoolean(entry.standalone_allowed, 'standalone_allowed', filePath),
        ...
        legacy_boundary_terms: {
          role: expectString(legacyBoundaryTerms.role, 'legacy_boundary_terms.role', filePath),
