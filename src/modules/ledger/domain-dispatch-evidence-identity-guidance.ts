...
export function buildDomainDispatchEvidenceIdentityGuidance(input: {
  ...
  const targetFields = [
    ...
    ['stage_manifest_ref', stringValue(input.targetIdentity.stage_manifest_ref)],
    ['current_pointer_ref', stringValue(input.targetIdentity.current_pointer_ref)],
    ['provider_attempt_ref', stringValue(input.targetIdentity.provider_attempt_ref)],
    ['attempt_lease_ref', stringValue(input.targetIdentity.attempt_lease_ref)],
    [
      'execution_authorization_decision_ref',
      stringValue(input.targetIdentity.execution_authorization_decision_ref),
    ],
  ...
  return {
    ...
    payload_identity_fields_checked_when_present: [
      ...
      'attempt_lease_ref',
      'execution_authorization_decision_ref',
      'owner_delta_result.closeout_binding.stage_run_id',
      'owner_delta_result.closeout_binding.stage_manifest_ref',
      'owner_delta_result.closeout_binding.current_pointer_ref',
      'owner_delta_result.closeout_binding.source_fingerprint',
      'owner_delta_result.closeout_binding.idempotency_key',
      'owner_delta_result.closeout_binding.provider_attempt_ref',
      'owner_delta_result.closeout_binding.attempt_lease_ref',
      'owner_delta_result.closeout_binding.execution_authorization_decision_ref',
      'profile',
      'profile_name',
    ],
