...
function buildWorkflowProfileComponent(channel: string): ManagedUpdateComponent {
  ...
  const route = ownerRoute({
    owner: 'opl-flow',
    authority_surface: 'OPL Flow package policy and Framework package receipt',
    route_kind: 'manual_owner_route',
    readback_ref: 'opl packages optimize opl-flow --json',
    apply_owner: 'opl_flow_package_lifecycle',
    forbidden_claims: [
  ...
  return managedUpdateComponent({
    ...
    coordination_role: 'owner_handoff',
    policy_id: 'codex_semantic_merge_with_packet_fallback',
    owner_route: route,
    owner_execution_boundary: ownerExecutionBoundary(route, {
      owner_executor_id: 'opl_flow_package_lifecycle',
      executor_kind: 'manual_owner_route',
      runner_can_execute: false,
      allowed_operations: [],
      receipt_projection: 'external_owner_receipt_required',
      diagnostic_only: false,
      notes: [
        'OPL Flow package install, update, and optimize run Codex semantic merge with hash, backup, receipt, and packet fallback checks.',
      ],
    }),
    ...
    current: {
      ...
      semantic_merge_mode: 'codex_auto_with_reviewable_packet_fallback',
    ...
    conditions: [
      condition(
        'SemanticMergeAvailable',
        'True',
        'PackageLifecycleOwned',
        'Existing user AGENTS.md is merged by Codex inside the OPL Flow package transaction; scripts only remove known marker blocks deterministically.',
      ),
      condition(
    ...
    lifecycle: KERNEL_LIFECYCLE,
    postApplyHooks: ['codex_semantic_merge', 'target_hash_check', 'profile_receipt'],
    auto_apply: {
      mode: 'controlled_apply',
      eligible: false,
      app_background_safe: false,
      scope: 'explicit_opl_flow_package_or_post_app_update_reconcile',
      command_ref: 'opl packages optimize opl-flow --json',
      blocked_reasons: ['explicit_opl_flow_or_app_update_required'],
    },
    status_detail: detail,
    post_apply_guidance: {
      required: false,
      command_refs: ['opl packages optimize opl-flow --json'],
      reload_guidance: reloadGuidance,
    },
    plan: {
      action: 'none',
      summary: 'Workflow profile optimization is owned by the OPL Flow package transaction, with Codex merge and packet fallback.',
      command_refs: [
        manualCommand(
          'workflow_profile_optimize',
          'opl packages optimize opl-flow --json',
          'Optimize workflow conflicts and profile semantics through the Framework package transaction.',
        ),
      ],
    },
    receipt: componentReceipt({
    ...
    authority_boundary: {
      ...
      package_lifecycle_can_apply_verified_semantic_merge: true,
      can_mutate_runtime_substrate: false,
      can_write_domain_truth: false,
      can_create_owner_receipt: false,
      can_claim_domain_ready: false,
    ...
    notes: [
      'Workflow profile status is projected in the managed update plane so Settings can show OPL Flow optimization and rollback guidance.',
      'The managed update kernel does not duplicate profile logic; install, update, optimize, and rollback stay in the Framework package lifecycle.',
    ],
  });
}
