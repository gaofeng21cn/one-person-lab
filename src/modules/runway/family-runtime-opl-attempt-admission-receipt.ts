...
export function domainRouteOplAttemptAdmissionNeedsProviderFollowthrough(input: {
  taskPayload: Record<string, unknown> | null | undefined;
  output: Record<string, unknown> | null | undefined;
}) {
  return isDomainRouteOplAttemptAdmissionRequested(input.output)
    && !masDomainOwnerAnswerObservationFromRecords([
      { source: 'task_payload', value: input.taskPayload },
      { source: 'domain_handler_output', value: input.output },
    ]);
}
