...
- MAS (medautoscience) added the `family_orchestration.action_graph` payload in its manifest fixture at the time, but the CLI tests then did not assert that the MAS clip consumed those fields.
...
1. Extend the then-current `domain-manifests resolves real family manifest fixtures ...` test to assert that the medautoscience manifest contains `family_orchestration.action_graph_ref.ref`, the `graph_id`, and the expected node/edge counts from the fixture.
