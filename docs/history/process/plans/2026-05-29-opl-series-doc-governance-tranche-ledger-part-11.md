...
- `opl family-runtime evidence-worklist --family-defaults --provider temporal --executor-kind codex_cli --detail full --json`：`open_worklist_item_count=2`、payload-required item `2`、domain-dispatch workorder `2`、closed refs-only item `310`，`domain_ready_authorized=false`、`production_ready_authorized=false`。两条 open workorder 均为 MAS domain-dispatch refs-only record route，等待 domain/App/live owner receipt、owner-chain/no-regression ref 或 domain-owned typed blocker；OPL 不自造 owner refs。
- `opl runtime app-operator-drilldown --json`：provider cadence/capability satisfied；current control state 仍投影 `152` 个 running provider attempts；domain-dispatch evidence 当前暴露 `2` 个 payload-required record route；App user path、Developer Mode、OMA 和 Codex App runtime evidence 等 refs-only gates 的状态只能从 fresh drilldown 读取，均不授权 App release ready、domain ready 或 production ready。
