...
- `opl runtime app-operator-drilldown --json`：provider cadence/capability satisfied；current control state 仍投影 `150` 个 running provider attempts；domain-dispatch evidence 当前暴露 `2` 个 payload-required record route；App user path、Developer Mode、OMA 和 Codex App runtime evidence 等 refs-only gates 的状态只能从 fresh drilldown 读取，均不授权 App release ready、domain ready 或 production ready。
