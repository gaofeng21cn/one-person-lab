# OPL series docs governance tranche ledger part 44

Owner: `One Person Lab`
Purpose: `opl_series_docs_governance_tranche_ledger_part_44`
State: `history_provenance`
Machine boundary: 本文是人读过程归档，不是 operating-model authority、shared runtime contract、shared domain contract、runtime provider proof、App release gate、domain truth、quality verdict、artifact authority、owner receipt 或 production readiness oracle。当前 truth 回到 `docs/public/operating-model.md`、核心五件套、active gap plan、`docs/specs/shared-runtime-contract.md`、`docs/specs/shared-domain-contract.md`、contracts、source、tests、CLI/read-model、runtime ledger 和 domain-owned receipts。
Date: `2026-05-29`

## Scope

本轮处理 `OPL 运行模型` public support doc 的 currentness drift：

- `docs/public/operating-model.md`
- process ledger index

目标是让 public operating model 保持用户视角运行模型、owner 分层和 activation / shared-boundary 读法，不再把 Shared Runtime / Shared Domain 合同写成“逐步承接”的旧过渡状态，也不让本文冻结 provider readiness、App release evidence、domain coverage、worklist counter 或 production readiness。
...
- `docs/specs/README.md`
  - `shared-runtime-contract.md` 与 `shared-domain-contract.md` 均是 `active_support` 规格支撑，不是 future landing queue。
...
- `docs/public/unified-harness-engineering-substrate.md`
  - UHS 已把 Shared Runtime / Shared Domain 读为 current active-support contracts，机器真相回到 contracts/source/tests/read-model/runtime ledger/domain-owned evidence。
- `docs/public/README.md`
  - `operating-model*` 是公开支撑文档；公开产品 truth 继续归 README、核心五件套、contracts、source、CLI/API 行为、release artifacts、runtime ledger 和真实 App evidence。

## Changes

- `docs/public/operating-model.md`
  - Added a currentness rule limiting the document to public operating-model and user-facing owner / activation / shared-boundary readings.
  - Replaced stale "逐步承接" wording with active-support contract wording for Shared Runtime Contract and Shared Domain Contract.
...
- `docs/public/operating-model.md`
- `docs/history/process/plans/2026-05-29-opl-series-doc-governance-tranche-ledger-part-44.md`
