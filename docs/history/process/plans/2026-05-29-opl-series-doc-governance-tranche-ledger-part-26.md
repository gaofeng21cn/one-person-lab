...
- `opl packages manifest`：manifest 读取为 OPL `26.5.28`；`module_install_update_source=git_checkout`；`package_consumption_status=packages_defined_not_consumed_by_install_update`；release automation 为 `prepared_not_consumed_by_module_install_update`；channel manifest kind 为 `opl_release_channel_manifest.v1`；latest source until packages consumed 仍是 `git_checkout_upstream_default_branch`；module keys 为 `medautogrant`、`medautoscience`、`oplmetaagent`、`redcube`；WebUI image 为 `ghcr.io/gaofeng21cn/one-person-lab-webui:26.5.28`；native helper image 为 `ghcr.io/gaofeng21cn/one-person-lab-native-helper`。
- `/tmp` package dry-run：用 `node --experimental-strip-types ./scripts/package-module-archives.mjs --version 26.5.28 --owner gaofeng21cn --out-dir /tmp/...` 和 sibling module paths 生成 `opl-release-manifest.json`、`opl-channel-manifest.json`、`SHA256SUMS` 与 four module source archives；随后 `node ./scripts/package-release-discipline.mjs --manifest /tmp/.../opl-release-manifest.json` 验证通过，modules 为 `medautoscience`、`medautogrant`、`redcube`、`oplmetaagent`。该 dry-run 只证明 manifest / discipline shape；因为 RCA local checkout dirty/ahead，它不是 release readiness evidence，只 archives committed HEAD。
...
- `scripts/package-module-archives.mjs` 与 `src/package-distribution.ts`：manifest build 会生成 `opl-release-manifest.json`、`opl-channel-manifest.json`、`SHA256SUMS` 和 module source archive，并写入 `source_archive.size`、`source_archive.sha256`、`checksum` 与 `source_git.head_sha`；本轮只写入 `/tmp` 输出目录，未在 repo checkout 写入 dist/package outputs。
...
- `/Users/gaofeng/workspace/one-person-lab-app` live status：root repo 在 `main...origin/main`，head 与 origin 均为 `d6f60d2`，但有 5 个外部本地修改文件；dirty remote-backed worktree `.worktrees/codex/full-first-run-stable-gate-20260525` also remains active. 本轮只读 App repo，不修改、不验证、不吸收这些 dirty lane。
