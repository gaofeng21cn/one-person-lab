...
- 但**不**把当时仓库推进到真正的 routed-action implementation。
...
- 当时不抽共享执行内核。
- 当时所有 routed-action 相关 schema / prose / examples，都只能被解释为 **planning dependency / contract pre-freeze**，不能被解释为当时可执行入口。
...
换句话说，当时 routed handoff 的硬规则就是：**不得绕过 domain gateway**。
...
- `docs/opl-routed-action-gateway*.md`：当时继续承担长程 `G3` contract prose，但在该历史阶段只能被解释为 planning-level contract。
- `contracts/opl-framework/routed-actions.schema.json`：当时作为 `thin handoff planning` 的 schema dependency，不等于 routed-action runtime 已激活。
- `docs/references/opl-gateway-acceptance-test-spec*.md`：当时只应把 `G3` 写成 contract / planning gate，不应写成已落地 runtime。
- `docs/references/opl-gateway-rollout*.md`：当时需要显式写明只到 `thin handoff planning` 预冻结。
...
4. 当时文档没有把 `OPL` 写成 runtime owner、shared truth store 或 mutation gateway。
5. 当时文档没有把 planned workstream / planned routed-action 写成已实现能力。
...
- 不把 shared execution core 提前写成当时主线
