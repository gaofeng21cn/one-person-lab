...
- `redcube-ai`
  ...
  - 当前 absorbed 主线锚点仍是 `Phase 2 / runtime watch locator integrity hardening`；`Phase 2 / source-readiness deep research trigger + gate convergence` 与 `Phase 2 / workspace operator quickstart convergence` 继续作为 absorbed provenance 保留，且 `fd01266` 已把 quickstart 测试与 source-intake bootstrap flow 对齐
  ...
  - 当前 same-mainline 已预冻结 `phase_2_family_parity_autopilot_continuation_board`，第一条 tranche 是 `phase_2_family_parity_governance_surface_convergence`；当前不得再把 RedCube 写成“没有下一棒”
- `med-autogrant`
  ...
  - `98df81f` 已把 control-plane state 迁回 repo-tracked truth；当前 truthful continuation 仍是 post-`R5.A` local runtime hardening，而不是 actual hosted runtime 或 `P5` 联邦
- `med-autoscience`
  - 主线不重开新的架构 tranche
