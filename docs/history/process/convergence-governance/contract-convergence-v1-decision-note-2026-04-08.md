...
Purpose: `historical_contract_convergence_v1_decision_note_2026_04_08`
State: `history_only`
...
- `Unified Harness Engineering Substrate` 是当时系列项目的总架构名称。
- 它首先是共享的架构语义、共享的合同语义、共享的五个平面与共享的工程方法，而不是已经独立交付的共享代码框架。
- 当时阶段统一的重点是共享 substrate 语言，而不是强行宣称四仓已经共享同一份运行内核。

因此，后续文档应优先把 `Unified Harness Engineering Substrate` 作为上位名词；`host-agent` 只适合描述当前本地默认部署形态，不应被继续写成未来商业化形态的唯一总称。

### 3. 历史开发控制面语义

2026-04-08 当时曾冻结下面这套开发控制面分工：

- `Codex` 负责规划、真相冻结、阶段裁决、最终集成判断
- 旧 `OMX` control plane 当时被记录为长时间连续执行、验证、报告回写与受控推进层

这套分工现在只保留为历史背景；当前 OPL 开发环境与执行入口以 Codex-only / repo-tracked truth 口径为准。
...
### 5. 三个业务仓当时都是 `Auto-only` 的 `Domain Harness OS`

当时 `med-autoscience`、`redcube-ai`、`med-autogrant` 的仓库主线，统一按下面方式理解：
...
因此，当时三个业务仓不再按“同仓双模”理解，也不应被要求在当时仓内同时收敛出一套完整的人机回环顶层判断逻辑。
