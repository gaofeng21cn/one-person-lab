...
1. 家族默认执行器在当时统一之后，主线还剩哪些 honest next steps。
2. 四仓与 `MedDeepScientist` 在文档层面还要补哪些同步。
3. `Hermes-Agent` 作为备选执行器时，2026-04 当时能力上限和与 `Codex CLI` 的差异是什么。
...
`OPL` 当时冻结的是 family-level executor contract，而不是 domain execution owner：

- 家族默认执行器正式名称：`Codex CLI`
- 默认执行模式：`autonomous`
- 默认模型：`inherit_local_codex_default`
- 默认 reasoning effort：`inherit_local_codex_default`
- `Hermes-Agent` 当时路线状态是 `experimental`，完整 `Hermes AIAgent` agent loop 是这条路线的硬 guardrail
...
`Med Auto Science` 自己并不直接发模型请求。2026-04 当时 repo-tracked 真实链路是：
...
`Med Auto Grant` 当时 critique 路线的真实最底层调用已经是：
...
也就是说，`RedCube AI` 当时已经不再是“待吸收默认执行器”的状态；当时 honest next step 变成：
...
`OPL` 顶层当时最容易继续漂移的不是 core truth，而是 central reference sync 面。
...
## Hermes-Agent 作为备选执行器：当时已经具备什么

### 它不是只有 chat relay

从上游 `hermes-agent` 当时实现看，至少已经有三种“直接调用 agent loop”的方式：
...
当时上游代码和文档明确显示，`Hermes-Agent` 已经具备：
...
## Hermes-Agent 与 Codex CLI 的当时关键差异

### 一、当时 family 落地成熟度：`Codex CLI` 更强

在 `OPL` 这一系列仓里，当时已经 repo-verified、质量稳定、路径清楚的默认执行器，仍然是 `Codex CLI`，默认模式是 `autonomous`。

`Hermes-Agent` 当时在这几个仓里的主要已落地点，仍然是：
...
1. 当时批次先把 `RedCube AI` 已 absorbed truth 写回 `OPL` 顶层 central reference sync 面。
