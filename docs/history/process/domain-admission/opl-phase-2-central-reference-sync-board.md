...
1. `phase_2_admitted_domain_delta_intake_refresh`
   - 先把当时最新 absorbed admitted-domain delta 收进中央执行板、状态矩阵与任务板
   - 当时已知 delta：
...
4. `phase_2_reference_regression_refresh`
   - 把当时 tranche 的中央 sync 改动压回 tests，维持 machine-verified closeout
...
前提是这些文件确实已经和 admitted-domain 当时 latest absorbed delta 发生 repo-tracked drift。
