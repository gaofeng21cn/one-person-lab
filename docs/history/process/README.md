...
| Retired active vocabulary and compatibility surfaces | Active `frontdoor` machine fields, old owner-map route vocabulary, App release-boundary prose oracle, Agent Lab RHO/workflow docs-prose oracle, Homebrew tap PR compatibility wording, and similar compatibility surfaces are retired or guarded. Narrative docs may point to contracts, but tests must keep machine acceptance on contracts/source/CLI/read-model rather than exact Markdown wording. | Current contracts/source/tests/validators, App release owner surfaces, Agent Lab contracts/CLI/read-model, history/tombstone provenance |
