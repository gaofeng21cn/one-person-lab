...
Purpose: `historical_runtime_substrate_family_lightweight_direct_entry_rollout_board`
State: `history_only`
...
历史读法：本文中的“当前”“下一棒”“推进顺序”和 `frontdoor_surface` / `operator_loop_surface` 语言只描述 2026-04 的 direct-entry 迁移板。当前 product/operator/agent entry taxonomy、handoff envelope 与 domain authority boundary 回到 `docs/specs/opl-domain-onboarding-contract.md`、核心五件套和 OPL stage-led framework roadmap；本文不作为 active roadmap、runtime contract、App release plan 或 product-entry 计划执行。
...
### 历史 D3. 接入真实 Hermes-backed runtime
