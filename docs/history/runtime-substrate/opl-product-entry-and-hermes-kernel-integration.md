...
Purpose: `historical_runtime_substrate_opl_product_entry_hermes_kernel_integration`
State: `history_only`
...
历史读法：本文中的 `Hermes Kernel`、`OPL Gateway`、`Domain Gateway`、hosted / web 化实施顺序和 `external kernel, managed by OPL product packaging` 决策只保留为 Hermes-first product-entry migration provenance。当前 production online runtime substrate 是 Temporal-backed provider；`hermes_agent` 只作为显式非默认 executor adapter/backend 评估，不作为 provider、readiness path、默认 executor 或 compatibility surface。
...
- 历史上文档曾把上游 `Hermes-Agent` 设想为优选未来 runtime substrate，但当时还没有把“到底怎么接入、由谁维护、用户需不需要自己理解 Hermes”冻结成不可误解的决策。
...
### 历史方案 A：把 Hermes-Agent 代码 fork / vendor 到 OPL 里，长期自己维护
...
### 历史方案 B：要求用户自己安装和理解独立 Hermes-Agent，再让 OPL 去调用
...
- `docs/history/process/convergence-governance/gui-mainline-pivot-to-aionui-2026-04-21.md`
