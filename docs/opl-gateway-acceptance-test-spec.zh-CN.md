...
这些配套 surface 只承担 illustrative 或 reference-only 角色。gateway corpus 展示跨层组合；operating corpus 把独立的 `P5.M1` / `P5.M2` record materialize 成 example；operating-record catalog 则集中索引全部已冻结 record kind。它们帮助人类与 Agent 理解已冻结 surface，但不替代上面的 contracts 与 acceptance gates。
...
只要其中任何一条不成立，这套体系就还没有达到 post-P13 discoverability surface 的 acceptance-green 状态。
