...
When `Hermes-Agent` appears in this context, it refers to the upstream external runtime project/service. `hermes_agent` is a canonical explicit non-default executor adapter/backend; old Hermes provider / Gateway / readiness / compatibility surfaces remain historical provenance, diagnostic vocabulary, or negative guards. The Temporal-backed provider is the required production online runtime substrate, while real production soak remains a separate acceptance gate.
