...
| 条目 | 用途 | 边界 |
| --- | --- | --- |
| [OPL 白皮书](./whitepaper/README.md) | 面向外部读者分享 OPL 定位、设计哲学、品牌模块和 Foundry Agents；PDF 用于转发和离线阅读。 | 用户可读公开材料。白皮书内容源是同目录 `opl-whitepaper.source.json`，Markdown/PDF/verification JSON 是生成物，不声明 runtime、release、domain 或 production ready。 |
