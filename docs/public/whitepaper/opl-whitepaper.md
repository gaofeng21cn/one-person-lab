...
OPL 的九个模块来自长周期知识工作的真实摩擦：一次性回答需要延伸为持续交付，自动化脚本需要连接专家判断，界面可见性需要连接质量完成。
...
九个模块是 OPL Framework 的能力分区。它们把顶层理念落实到真实运行：先建立语言和边界，再组织目录和工作空间，再设计阶段、长跑执行、证据回执、用户控制台、智能体工坊和外部连接。
