...
OPL 的设计原则不是一组抽象口号，而是一套面向复杂交付的品牌判断：先从成果反推系统，再把 AI 执行、阶段推进、责任边界、证据回执和用户可见进度组织成同一条工作线。AI 在阶段内完成开放式专家工作，系统提供可审计、可恢复、可交接的边界。

### 成果优先

**从交付物反推系统**

OPL 先问用户要交付什么：论文、基金、汇报、书稿、专利，还是一个可接力的智能体。目录、状态、运行时、队列和验证都服务这个目的；不能产出下一版成果、责任方、证据或交接的设计面，都应收薄、下沉或归档。

- 用户目标决定系统形状，不让历史实现反向定义用户路径。
- 真实进度按成果、决策、交接和可接力增量计算。

### AI 执行优先

**能力来自更强执行者**

复杂知识工作交给 AI 执行者做开放式判断、写作、评审和修订。系统提供目标、上下文、权限、工具和质量边界，把专家行为留给最合适的执行者；当前默认且第一公民执行者是 Codex CLI，其他执行者必须显式接入并留下回执和审计。
...
OPL 把阶段作为可观察、可恢复、可审计的工作单元。它启动的是带 owner、输入、输出、权限、预期回执和质量边界的 stage pack，而不是一段自由形态脚本。这样 AI 可以在阶段内自由判断，系统仍能在启动、执行和收口时知道该检查什么、该交给谁、该留下什么证据。

- 每个阶段有目标、输入、输出、知识、工具边界和质量门。
- 阶段内策略留给执行者，阶段外边界留给系统。

### 轻合同

**合同只守安全和可恢复下限**

合同负责责任方、权限、安全动作、回执、阻塞、审计、恢复和状态投影，只守系统下限。工具、技能、知识和评价标准是可用能力和边界，不是流程脚本；执行者可以选择、组合、跳过或替代工具，系统只固定权限、凭据、可写范围、副作用和禁止越权。
...
schema 完整、provider 完成、read model 可读、generated surface ready 或 checklist 通过，都只能说明某个机器面可用。论文质量、基金可资助性、视觉交付、书稿完成度和最终导出判断，必须回到独立 AI reviewer、领域质量门、负责人回执或结构化阻塞。
...
OPL 用 requires / ensures 表达阶段之间的前置和产出关系。下游阶段需要什么，上游阶段承诺了什么，证据是否新鲜，责任方是否匹配，都必须能被看见。可静态验证的身份、责任方、引用、范围和禁止越权属于启动安全；AI 输出、人类决策、外部系统结果、产物修改、记忆写回和领域裁决属于运行时边界。

- 组合关系在启动前检查。
- 缺口回到结构化阻塞、人工确认或路线回退，不由系统猜测补齐。
...
OPL 只把启动安全、越权、关键运行事件缺失、组合关系不满足、硬性人工确认和执行者绑定缺失这类问题设为硬阻塞。容量、监控、假设、回放和 reviewer attention 进入建议或路线回退，避免诊断细节反向卡住普通推进。
...
descriptor ready、read model 可读、generated surface proof、provider proof、cleanup proof 或测试通过，都不能被包装成 domain ready、artifact ready 或 production evidence complete。每个未闭合边界都必须返回结构化阻塞、人工确认、回执冲突或路线回退，而不是用 fallback verdict 补语义。

- ready claim 必须绑定对应责任方证据。
- OPL 宁可暴露缺口，也不制造第二真相源。
