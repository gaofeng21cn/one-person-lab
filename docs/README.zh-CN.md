...
- 默认的 `OPL Cortex` 前台桥现在刻意收成 8 个高频工具：论文/项目进度、执行请求、任务状态、最近会话、恢复会话、运行日志、项目列表、切换 workspace
- 真正的前台执行请求现在采用“异步受理”语义：先快速返回 `task_id`，再通过任务状态面持续追踪，而不是把一次长任务塞进单个阻塞式 tool call
