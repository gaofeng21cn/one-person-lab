...
Fresh runtime evidence（2026-05-20）显示：`opl agents evidence apply` 已提供 OPL-owned refs-only external evidence receipt ledger，可对 domain 声明的 evidence request 和 evidence gate 记录或验证 receipt；`runtime app-operator-drilldown.domain_evidence_request_refs` 会把 request、remaining gate、replacement coverage、observed receipt status 和 verified gate receipt 投给 App/operator。App/operator 现在还会把 open external evidence request / remaining evidence gate 转成 `external_evidence_receipt_record`、`external_evidence_receipt_verify`、`evidence_gate_receipt_record` 或 `evidence_gate_receipt_verify` OPL safe action route；`runtime action execute` 只能用这些 route 记录或验证 OPL external-evidence refs-only ledger，record 时必须由 operator payload 提供 evidence refs、domain receipt refs、typed blocker refs、no-regression refs、release/dist refs、direct/hosted parity refs 或 owner-chain refs，verify 只会验证既有 OPL receipt。OPL 同时新增 `opl_meta_agent_registry` / `opl_meta_agent_workbench_refs` 消费面，把 OMA 的 `opl_domain_manifest_registration`、`app_workbench_projection` 和 `real_target_agent_scaleout_evidence` 作为 refs-only extension 暴露给 `domain manifests` 与 App/operator drilldown；summary 默认展示 `target_brief`、`candidate_package`、`agent_lab_results`、`developer_work_order`、`mechanism_patch_proposal`、`scaleout_evidence` 六个 section，完整 refs 仍通过 `--detail full` 显式展开，且 authority boundary 全部保持 no domain ready / no quality verdict / no default promotion。`runtime app-operator-drilldown.domain_dispatch_evidence` 现在会按 domain 汇总 stage attempt 中的 owner receipt refs、typed blocker refs、no-regression evidence refs、memory writeback receipt refs 和 domain-ready 非授权状态；`stage_production_evidence` 会按 domain/stage 汇总 selected executor binding、生产 caller attempt、expected receipt refs、source/artifact/workspace scope refs、runtime event refs、monitor refs、monitor freshness refs 与 observed evidence refs，并把 expected receipt / monitor 分成 declared、observed 和 unobserved 状态，避免把 stage pack obligation 缺口与 live receipt/monitor evidence 缺口混在一起。App/operator 会把 admitted-but-missing production caller stage 生成 `stage_production_attempt_request` safe action route；full detail 与 `app_execution_bridge.safe_action_routes` 保留 `action_ref`、结构化 `opl_cli_args`、target domain/project、missing production evidence 和 expected receipt refs，供 App 做 drilldown 审计、命令复制和安全提交；`runtime action execute` 可用该 route 调用 `opl family-runtime attempt create --provider temporal --executor-kind codex_cli --require-stage-admission`，创建 OPL-owned stage attempt request。本轮又把 OPL provider scheduler 与 legacy cleanup 纳入同一 safe action shell；`runtime action execute` 可执行 scheduler status/install/trigger/tick 和 cleanup apply/verify，但它们只管理 OPL provider scheduler 或 OPL cleanup ledger，不安装 domain daemon、不删除 domain repo 文件。动态 summary 和 stage drilldown 一律读 `opl framework readiness --family-defaults --json`、`opl runtime app-operator-drilldown --json` 和 `opl runtime app-operator-drilldown --detail full --json`，本文不冻结 action route、receipt、missing stage 或 tail item 数字。两个 projection 都默认 summary-first，完整 attempt/stage evidence 与 route 只在 `--detail full` 展开，并持续声明 `provider_completion_is_domain_ready=false`、`projection_can_authorize_domain_ready=false`；attempt-create route 只请求 OPL stage attempt，不执行 domain action、不写 domain truth、不生成 domain owner receipt。verified receipt 或 gate receipt 只证明 refs-only roundtrip 可用，不声明 grant-ready、fundability-ready、generated/hosted caller default 化、App workbench consumption、stage completion 或 live soak 完成；OMA scaleout closeout 只证明 MAS/MAG 两目标 refs-only evidence loop 和 work order/proposal handoff 可被 OPL/App 消费，不声明 MAS/MAG domain ready；真实 default caller、release/dist、owner-chain、expected receipt instance、monitor freshness 和 long-soak 证据仍需由 MAS/MAG/RCA 或 App 生产并作为 receipt/ref 回填。

2026-05-20 safe-action 执行后的 fresh closeout 读法是：`opl framework readiness --family-defaults --json` 仍显示 framework hard blocker 为 0，Temporal provider cadence / capability SLO 为 satisfied，`stage_production_caller_tail_open_item_count=0`；`opl family-runtime production-closeout --family-defaults --provider temporal --executor-kind codex_cli --json` 已把 OPL-owned 可执行 closeout 清到 `production_closeout_open_safe_action_item_count=0`。provider scheduler 4 项和 legacy cleanup 6 项已 closed by receipt；MAG 6 个 external evidence request 与 RCA 3 个 evidence gate 已由 domain-owned typed blocker refs record+verify 到 OPL refs-only ledger，并在 full closeout 中显示为 `closed_by_domain_owned_typed_blocker`。这些 typed blocker 只说明 request/gate 已被 domain owner 以 refs-only blocker 回应；release/dist、App/default caller、direct/hosted parity、owner-chain、expected receipt instance、monitor freshness 和 long-soak 等真实外部/domain evidence 仍未由此变成成功，不能由 OPL safe action 伪造为 domain ready 或 production ready。
