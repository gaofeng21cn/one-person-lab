...
- `OPL GUI` 预编译包指 Electron-builder 产出的 OPL 品牌 `.dmg` / `.exe` / `.deb` 分发文件及 `latest*.yml` updater metadata；这些 release artifact 由 `opl-aion-shell` 构建，再通过 `npm run gui:release` 上传到 `one-person-lab` GitHub Release。macOS arm64 另有 `One-Person-Lab-Full-<version>-mac-arm64.dmg` 首次安装资产，随包带 MAS/MAG/RCA、可选 Hermes provider-adapter payload、`officecli` CLI binary 和推荐 companion skill payload，但不得写入 `latest*.yml` 或成为 App 自动更新目标；Hermes payload 不得写成 required/default runtime。
