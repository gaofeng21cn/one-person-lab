...
- 当前重点：把三个业务仓新增的 `operator_loop_actions` 收口为同一套家族级用户面 contract，并让 `domain-manifests / dashboard / handoff-envelope / opl web` 一起消费，不再只告诉用户“当前 loop 是什么”，而是直接告诉用户“下一步能做哪几件事”
