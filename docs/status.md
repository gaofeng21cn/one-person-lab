...
- `opl family-runtime` 已能把 MAS sidecar 导出的 `paper_autonomy/repair-recheck`、`paper_autonomy/ai-reviewer-recheck`、`paper_autonomy/gate-replay` 与 `paper_autonomy/route-decision` 入队、去重、派发，并在 task / dispatch JSON 中保留 `paper_autonomy` 投影：study id、next owner、callable surface、source refs、source fingerprint 与 idempotency key。`opl family-runtime attempt create|list|inspect|start|query|signal` 现在把 domain stage 执行登记为 provider-backed stage attempt，记录 provider kind、workflow id、workspace locator、checkpoint/closeout/human gate/user instruction/resume refs、retry budget、task binding、provider run/activity event/heartbeat、route impact 与 provider receipt，并可对 Temporal workflow 执行 start/query/signal；缺少 Temporal 地址时 fail-closed。attempt query/workbench 会把 typed closeout 缺失的 domain dispatch 视为 checkpointed，而不是 completed，并投影 human gate ledger、user instruction ledger、resume ledger 与 dead-letter task ledger。该投影只服务 OPL Runtime Manager 和本地 inbox/event 可见性；OPL/provider 不写 MAS study truth、publication quality、artifact gate 或 `current_package`。
- 当前完成边界：OPL 已落地 MAS paper autonomy task 的 family queue / projection / guarded dispatch bridge；MAG/RCA domain adapters 已在各自 repo main 上落地，并可通过 `pending_family_tasks[]` 进入同一 OPL typed queue；MAS/MAG/RCA skeleton adapters 已能被 `opl agents list|inspect` 真实发现并校验为 aligned。Temporal provider code、worker lifecycle contract、typed closeout ingestion、Codex stage runner repo/test harness 与 stage attempt workbench 可见性已落地，Hermes/local provider 路径仍是迁移期可用实现信号。OPL 侧已用 fixture E2E 覆盖 gateway stopped -> repair -> ready、跨仓 MAS/MAG/RCA notification / approval / retry / dead-letter，以及 typed closeout gate；Codex runner 已具备 `dry_run`、`live_dry_run` 与 `codex_cli` process-supervision harness。剩余重型验收是真实 Temporal server/worker residency proof、长时 domain activity soak、provider cutover、真实 token/cost/progress 观测校准，以及真实 paper / grant / visual line 的 controlled apply 到最终交付。
...
- 2026-05-11 fresh assessment：stage-led / provider-backed 计划已经落到 `shared contracts + domain descriptors + local queue / attempt ledger + Temporal provider code + standard skeleton discovery + runtime snapshot / Aion workbench projection + Codex runner repo/test harness`，但没有完全落到生产闭环。实测 `opl agents list --json` 为 `aligned_count=3`、`missing_count=0`、`drift_detected_count=0`；`opl stages list --json` 为 `resolved_planes_count=3`、`stages_count=18`；`opl domain-memory list --json` 为 `resolved_memory_descriptor_count=3`、`missing_memory_descriptor_count=0`。OPL 层当前可实测的 shared 面包括 `python/opl-harness-shared`、`contracts/family-orchestration/`、`contracts/opl-framework/`，以及 root `src/` 下 family runtime、Temporal provider、Codex stage runner、stage control、action catalog、handoff、product-entry、domain catalog、runtime manager、native helper、skill sync、runtime tray 相关 TypeScript surface。MDS 保持较早 shared pin，符合 archive / diagnostic / upstream-intake 角色。普通 `Product Entry` / `opl resume` / `opl session resume` 路径已经收敛到 Codex-default executor；`opl session list/logs` 与 runtime status 中的 Hermes 字段仅作为 `hermes_legacy` diagnostics / provenance / fixture 可见性存在。`opl status runtime` 顶层 runtime substrate 已按 provider-backed family runtime 报告，Hermes status、sessions 与 process usage 只保留在 legacy diagnostics 语境。已新增 typed closeout required-for-completion gate、human gate / user instruction / resume / dead-letter ledger 投影；Aion stage attempt workbench 已可发送 human gate / resume / dead-letter repair provider-level signal。仍未完成的是真实 Temporal server/worker residency proof、真实长时 domain activity soak，真实 paper / grant / visual line 的 provider-hosted soak，三仓 workspace/runtime memory apply receipt，以及旧默认面物理退役。后续文档和实现可以把“完整智能体运行框架、全自动交付”写成目标定位，但不得把当前状态表述成“全量生产可用”。
