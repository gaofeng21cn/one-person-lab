...
2026-05-19 的系统评估把当前完善主线收窄为 OPL 自身语义治理：先关闭 provider readiness 多真相、generated surface drift/block、App/operator drilldown 默认过重、`family-runtime` parser 语义堆叠和 legacy vocabulary active leakage 五类冗余/污染点，再继续放大 MAS/MAG/RCA 的 production evidence。本轮并行 worktree 已把 runtime single truth、generated surface gate、App drilldown summary-first、CLI parser split 和主文档 hygiene 合入集成线；后续验收以 main 上 fresh CLI/test 结果为准。
...
0. `semantic_single_truth_closeout`
   本轮已在集成线完成 OPL 自身多重语义面治理：provider readiness 改为 lifecycle-aware 单一 payload 派生；generated surfaces 从 canonical action/stage metadata 与 pack compiler input 对齐，MAG transition oracle 降为 evidence gate 而非 generated drift；App/operator 默认面改为 summary-first，完整 refs/routes 走显式 full detail；`family-runtime` parser 按子域拆分；旧 Hermes/Gateway/frontdoor/MDS/default-compat 词汇继续只留在 history/tombstone/provenance、fixture、negative guard 或 cleanup plan。该项的完成证据以最终 main 验证命令为准。
