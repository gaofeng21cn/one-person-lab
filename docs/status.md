...
- `opl system initialize` 是当前一键配置安装的聚合面：同时暴露 workspace root、Codex default runtime、optional Hermes provider adapter / online-management readiness、domain modules、推荐 companion skills、OPL 品牌 GUI shell、local support service 与下一步动作；没有显式 workspace root 时默认使用用户 Home 目录。首启 readiness 分成 core/domain readiness 与 optional online-management readiness：Codex 与已准入 domain 模块可用时，首屏不应因为 Hermes 缺失或 gateway 尚未 loaded 而阻塞核心工作。
