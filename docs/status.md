...
Fresh conformance evidence（2026-05-19）显示：`opl agents conformance --family-defaults --json` 当前为 `total_repo_count=4`、`passed_count=3`、`blocked_count=1`、`structural_conformance_status=blocked`。MAS、MAG 与 OPL Meta Agent 通过；RCA 被 `required_domain_pack_path_must_not_be_readme:agent/README.md` 阻断。这是 RCA physical source morphology cleanup tail，说明 required domain pack path 不能只指向 README；它不改变 RCA 现有 descriptor / generated / hosted shell 消费边界，也不能写成 production/domain/artifact readiness。
...
MAS/MAG/RCA 当前均可被 OPL 识别为 standard domain agent consumer。这个状态说明 descriptor、read model、handoff、generated/handler target proof 和 replacement surface 已经存在；不能写成 full conformance pass、production domain owner chain、App 真实用户路径或 long-soak evidence 已全部闭合。Fresh OPL proof-bundle evidence（2026-05-18）显示 `opl stages proof-bundle --domain mas|mag|rca` 均返回 `admission_status=admitted`、`blockers_count=0`、`warnings_count=0`，`opl stages list` 汇总为 18/18 stages admitted。Fresh OPL cohort-loop evidence（2026-05-19）显示三仓 `family_stage_control_plane` 已为 18 个 stage 声明 `source_scope_refs`、`cohort_query_refs`、OPL queue `trigger_refs`、`monitor_refs` 和 `dashboard_metric_refs`；`opl stages cohort-loop --domain mas|mag|rca` 均返回 6/6 `closed_loop_ready`、`blocker_count=0`。Fresh conformance evidence（2026-05-19）则显示 family defaults 当前是 3/4 structural pass：MAS、MAG 与 OPL Meta Agent 通过，RCA 因 `required_domain_pack_path_must_not_be_readme:agent/README.md` 被阻断。这只证明闭环声明与 OPL refs-only projection 可消费，不等于真实 provider launch、cohort execution、owner receipt、full conformance pass 或 long-soak 已完成。MAS 当前机器面已关闭未分类 generic owner 回流、effect-boundary stage admission 和 5 个结构 follow-through gate，`classification_gap_count=0`、`active_private_generic_residue_count=0`、`functional_structure_gap_count=0`；2026-05-19 MAS main 又新增 `runtime_transport_handoff_projection`，把 `mas_runtime_core`、turn runner、worker lease、runtime supervisor 与 `runtime_lifecycle_store.py` 逐项约束为 OPL generic runtime 的 domain bridge / refs-only sidecar / diagnostic，不再允许 MAS 声明 queue、attempt ledger、worker residency、transition runner 或 persistence/lifecycle engine owner。MAS 剩余是真实 paper-line provider apply、memory/artifact/lifecycle receipt scaleout、human gate/resume、provider SLO long-soak evidence gate，以及 no-active-caller / OPL parity / domain receipt parity 成立后的物理删除、archive 或 tombstone。MAG 当前 repo-side handler / refs-only / authority boundary 已闭合；剩余主要是外部默认 caller、App 消费、live soak 和 owner receipt scaleout 证据。RCA 当前 generated/hosted shell 消费边界已由 OPL `agents interfaces --repo-dir` read model 承接，`generated_wrapper_bundle` 机器面显式列出 `cli`、`mcp`、`skill`、`product_entry`、`product_status`、`product_session`、`sidecar`、`workbench` 的 OPL-owned descriptor scope，并把 RCA 限定为 domain handler target 或 refs-only adapter；RCA 8 项结构差距和 legacy physical cleanup 已按 OPL consumer 口径闭合。RCA 剩余是 conformance blocker 指出的 physical source morphology cleanup tail、production evidence tail 与 naming hygiene tail：required domain pack path 不能只指向 README，controlled visual-stage long soak、真实 artifact-producing owner receipt、visual memory body reuse、workspace receipt scaleout、cross-family repeated no-regression evidence，以及历史 `managed` 命名的 compatibility-free contract/tombstone 清理。
