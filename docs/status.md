...
2026-06-11 transport foldback：Temporal activity completion 与 `stage_progress_log` 的边界已经收敛为 refs-only transport / projection。`codexStageActivity` 返回给 Temporal 的 closeout packet 只保留 stage attempt id、idempotency key、closeout refs、consumed refs、writeback refs、rejected writes summary、next owner、domain-ready verdict、route impact 和 authority boundary；`paper_stage_log` / `user_stage_log` / `stage_log_summary`、transcript、paper/artifact/memory body 和大 detail arrays 留在文件/ledger。`stage_progress_log.user_stage_log` 只能投影 domain closeout 明确给出的语义摘要和 duration/token/cost observed/missing 状态；缺 domain summary 时显示 missing，不由 OPL 编写 domain typed blocker 或论文语义。该落地关闭的是 Temporal 4MB completion payload 造成的 provider terminal/currentness 卡点；它不声明 domain ready、owner receipt observed、typed blocker created、quality/export verdict、artifact ready、paper repaired、publication ready、App release ready 或 production ready。active stage attempt 存在时，`worker_source_stale` 仍按 supervisor guard fail closed，不能为了加载新代码重启 worker。

2026-06-11 Runway foldback：MAS default-executor 的常规 tick / dispatch 路径在 `same dispatch`、`same study` 或 `same task` live-skip 前，必须先通过 Temporal read-model 尝试同步 terminal stage attempt。若同步后仍是 running / checkpointed / human_gate，或仍处于已 claim 的 queued admission window，才刷新 lease 并 skip；若同步观察到 completed typed closeout / failed / canceled / blocked terminal 状态，则先收敛 OPL stage attempt ledger 和 linked task，再允许当前候选继续 claim/start。这个规则让 `attempt inspect` 不再是唯一能解开假 running 的路径，避免旧 running ledger 长期阻塞新的 owner task；它仍然只处理 OPL provider transport currentness，不生成 domain owner receipt、typed blocker、quality verdict 或 domain ready。

2026-06-11 queued current-control foldback：同一 dedupe-key 下仍处于 `queued` / `waiting_approval` 的 MAS current-control provider admission 若被更新的 work-unit/source currentness identity 取代，`family-runtime enqueue` 会原地刷新 task payload、重置 attempts 并记录 `task_requeued_from_mas_current_control_provider_admission`。若旧 task 已在 `waiting_approval`，刷新必须保留 approval gate 和 hold reason，不得把新 provider admission 降成自动 queued。该规则只消除 stale queued admission 卡住 fresh MAS work unit 的 OPL queue/read-model 卡点；不签 MAS owner answer、不创建 MAS typed blocker、不声明 provider completion 等于 domain ready。当前 fresh audit 中 `sat_c0348bcfa41849926ebb46f9` 的 MAS typed blocker 缺 provider attempt / active lease / execution authorization decision / source fingerprint / idempotency 匹配的 closeout binding，不能被 OPL 硬绑定为 current owner answer。

2026-06-11 Stage-route substrate foldback：`contracts/opl-framework/stage-route-scheduler-contract.json#stage_route_arbiter_substrate_contract` 固定 OPL 基座对 MAS/Foundry Agent 的五项保障：统一 `stage_run_currentness_identity`、terminal closeout 优先 live projection、schema-backed no-progress budget、`worker_source_stale` supervisor guard、refs-only trace/span correlation。`src/family-runtime-stage-run-currentness-identity.ts#buildStageRunCurrentnessIdentity` 已把 `stage_run_currentness_identity` 从合同声明推进为可复用机器对象，并由 `tests/src/family-runtime-stage-run-currentness-identity.test.ts` 验证完整 identity、mismatch、route-level currentness match 和缺字段 fail-closed；该测试已登记到 `scripts/test-lanes.mjs` fast lane。`sameStageRunRouteCurrentnessIdentity` 是 live skip、terminal closeout reconcile 和 current-control projection 共用的 route identity gate：同一业务路由可跨不同 `stage_attempt_id` 对账，stale work-unit/source/currentness basis 必须 fail closed。OPL Runway 负责 durable execution / repair / reconcile，Console 负责 operator read-model，Vault 负责 refs-only evidence；这些 surface 可以选择 transport task、调度 stage attempt、同步 attempt ledger 和投影 blocker/ref 状态，但 OPL 不持有 MAS owner receipt、MAS typed blocker 或 MAS quality authority，也不能创建 domain owner receipt、domain typed blocker、quality/publication ready、paper delta、App release verdict 或 production-ready claim。DM003 这类运行态闭环仍按 `OPL terminal -> MAS DHD consume -> fresh current_owner_delta -> OPL scoped tick if needed` 验证，不能用旧 active/run 字段或 transport 状态判断论文进展。
