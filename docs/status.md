...
- 当前开发宿主：`Codex` 长线自治会话
- 当前产品入口真相：`OPL` 已经落下本地 direct product-entry shell，默认前台命令就是 `opl`；`opl <request...>` 可直接作为 quick ask，而 `opl doctor / ask / chat / resume / sessions / logs / repair-hermes-gateway / frontdesk-manifest / frontdesk-service-*` 构成显式入口与 runtime 运维命令面。`opl projects / workspace-status / runtime-status / dashboard` 补上了第一版顶层管理面，而 `opl web` 则把本地 web front desk pilot 与 hosted-friendly 的 `health / manifest / sessions / resume / logs` 表面也一并落地；现在又进一步补上了基于 launchd 的 service-safe 本地包装层。用户在本机上不再必须先通过 `Codex` 才能触达顶层 `OPL`
...
2. 把 `LibreChat-first` 的 hosted / web pilot 选择固定为当前最快可用路线，同时明确长期仍是 `OPL` 自有 web front desk。
...
6. 当前已经落地的是本地 direct product-entry shell、本地 web pilot、hosted-friendly shell contract，以及 service-safe 的本地 front desk packaging；不把它们误写成 hosted / web runtime；在上游 pilot 继续成立后，再推进真正的 hosted shell 接壳与更完整的 direct product entry。
