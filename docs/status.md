...
- 当前重点：三个业务仓的 repo-tracked `product-entry manifest` 现在还会统一带出 `product_entry_quickstart` companion；`OPL` 顶层不再只知道 manifest 和 loop，而是能直接展示“先从哪进、接着怎么续跑、看哪条进度面”
