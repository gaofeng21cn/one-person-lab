...
- 当前开发宿主：`Codex` 长线自治会话
- 当前产品入口真相：`OPL` 已经落下本地 direct product-entry shell，默认前台命令就是 `opl`；`opl <request...>` 可直接作为 quick ask，而 `opl doctor / ask / chat / resume / sessions / logs / repair-hermes-gateway / frontdesk-manifest` 构成显式入口与 runtime 运维命令面。`opl projects / workspace-status / runtime-status / dashboard` 补上了第一版顶层管理面，而 `opl web` 则把本地 web front desk pilot 与 hosted-friendly 的 `health / manifest / sessions / resume / logs` 表面也一并落地。用户在本机上不再必须先通过 `Codex` 才能触达顶层 `OPL`
...
6. 当前已经落地的是本地 direct product-entry shell、本地 web pilot，以及 hosted-friendly shell contract；不把它们误写成 hosted / web runtime；在上游 pilot 继续成立后，再推进 service-safe packaging 与更完整的 direct product entry。
