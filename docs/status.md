...
| `Private platform retirement` | Default-caller deletion evidence worklist 仍有 32 项 refs-only audit surface；private residue cleanup lane 为空时也输出 `owner_decision_work_order.status=no_private_residue_classified_not_delete_ready`，`physical_delete_authorized=false`。 | 只有 domain owner 给出 physical delete authorization 才能物理删除；empty cleanup lane、structural evidence、tombstone/provenance refs 或 OPL work order 都不授权删除；否则保持 authority adapter / tombstone / typed blocker 路径。 |
| `Brand module L5` | 当前十个品牌模块的 L5 evidence matrix、可执行 read/validate surface 和 refs-only runtime evidence ledger 已落地；`current_owner_delta_default_read`、`domain_authority_false_boundary`、`no_second_truth_regression`、`pack_compile_parity` 已记录 40 条 OPL-controlled refs-only evidence refs，其余 90 个仍需 owner resolution 的 requirement 已记录 owner-needed typed blocker refs，Runway `long_soak_recovery` 已有 1 条 verified refs-only ledger receipt。真实 L5 运营证据仍缺，剩余 open requirement count 为 120。 | 各模块用真实用户路径、跨 agent scaleout、long-soak、release/install、owner acceptance 和运营闭环证明；L4/L3、contract validation、provider completion、verified ledger、contract refs-only evidence、owner-needed typed blocker refs 和 App projection 只能作为输入。 |
