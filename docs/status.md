...
- 当前 hosted / web 前台真相：默认本机 GUI 路径已经切到 `OPL Atlas` Desktop，本地 web front desk pilot 作为 companion surface 可直接打开；hosted-friendly contract surface 与 self-hostable hosted pilot package 已落地。`opl frontdesk librechat install` 与 `opl frontdesk-librechat-package` 只保留 optional compatibility / fallback lane；actual managed hosted runtime 仍未落地。`OPL Cortex` MCP bridge 已把 hosted shell bootstrap 所需的 `workspace binding + session attribution` wiring 明文落成 machine-readable surface；repo 内部 `frontdesk_*` 继续是 machine-readable contract / API 命名
