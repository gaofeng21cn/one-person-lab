...
- `OPL Runtime Manager` 已冻结为 Hermes-first 产品控制面：它管理外部 `Hermes-Agent` online runtime substrate 的 provision、profile、typed family queue、domain dispatch、task registration、诊断、恢复入口、Rust native helper catalog 与高频状态索引，但不承担 domain scheduler/session/memory kernel。
- `Codex` 是默认具体交互与执行宿主；`Hermes-Agent` 是默认 online runtime substrate 与 gateway，负责 24h wakeup、session、delivery、approval、notification 与 profile isolation。具体执行语义只在显式切换 executor 或 domain route 选择时进入。
...
- `opl install` 是当前最短一键安装入口：默认安装或复用 `Codex CLI`、Hermes online runtime、`MAS`、`MAG`、`RCA`，同步短名 Codex skills，并以保守 managed 模式同步推荐 companion skills 和 `officecli` CLI 工具；MAS 声明的 MDS 相关 surface 只作为显式可选 companion diagnostic / oracle / intake 引用读取，不再作为 OPL 默认安装依赖。Hermes online-management gateway 由 Hermes installer/gateway command 管理；OPL 通过 `opl family-runtime install|repair`、`opl runtime manager action --apply`、`opl runtime repair-gateway` 或 engine install 路径触发、检查 readiness 并报告状态，不把 gateway service lifecycle 变成 OPL 自有实现。若 OPL 品牌 GUI 已安装则尝试打开，macOS 上未安装时会自动下载并安装匹配当前平台的 release DMG 后再打开；历史 8787 Product API service 模块已退役。
...
- `opl system initialize` 是当前一键配置安装的聚合面：同时暴露 workspace root、Codex default runtime、Hermes online runtime readiness、domain modules、推荐 companion skills、OPL 品牌 GUI shell、local support service、developer supervisor 用户级配置与下一步动作；没有显式 workspace root 时默认使用用户 Home 目录。首启 readiness 分成 Core、Domain modules、Hermes online runtime 三层：Full OPL readiness 需要三层都 ready；Hermes 缺失或 gateway 尚未 loaded 会报告 degraded / attention needed。
- Fresh install 与 GUI 首启测试已有机器可读矩阵：`contracts/opl-gateway/fresh-install-test-matrix.json` 冻结本机 clean-room、离线 blocker、干净 macOS VM 首启、首启 JSONL 日志和 GUI accessibility labels；本机验证入口是 `npm run test:fresh-install` / `./scripts/verify.sh fresh-install`，OPL Verify workflow 已纳入该 lane。真实 GUI 首启由 `gaofeng21cn/opl-aion-shell` 的 `scripts/opl-first-run-vm-smoke.mjs`、`scripts/opl-first-run-tart-smoke.mjs` 与 `.github/workflows/opl-first-run-vm.yml` 执行，目标 runner labels 为 `self-hosted`、`macOS`、`opl-gui-vm`。用户可见文案应把 Hermes 缺失或 gateway 未 loaded 写成 Full online runtime degraded，不再写成 optional online-management。
...
- `opl runtime manager` 是当前 Runtime Manager 的机器可读 projection：它展示 OPL 管理 required `Hermes-Agent` online substrate 的 owner split、非目标、v1 domain registration registry、family runtime queue、Rust native helper lifecycle、native helper target 与 state index target；Rust helper 源码、doctor/repair/prebuild 脚本和 Cargo workspace 已进入 npm package surface；当 helper 可发现时，它会调用 native doctor / state indexer / artifact indexer 与 runtime watch，并把 `opl_runtime_manager_native_state_projection` 写入 `OPL_STATE_DIR/runtime-manager/native-state-index.json`，同时报告 TTL、history、failure、last-success、freshness、结构化 diff 与 history GC preserved/removed 状态。
- `opl family-runtime` 已能把 MAS sidecar 导出的 `paper_autonomy/repair-recheck`、`paper_autonomy/ai-reviewer-recheck`、`paper_autonomy/gate-replay` 与 `paper_autonomy/route-decision` 入队、去重、派发，并在 task / dispatch JSON 中保留 `paper_autonomy` 投影：study id、next owner、callable surface、source refs、source fingerprint 与 idempotency key。该投影只服务 OPL Runtime Manager 和本地 inbox/event 可见性；OPL/Hermes 不写 MAS study truth、publication quality、artifact gate 或 `current_package`。
- 当前完成边界：OPL 已落地 MAS paper autonomy task 的 family queue / projection / guarded dispatch bridge；MAG/RCA domain adapters 已在各自 repo main 上落地，并可通过 `pending_family_tasks[]` 进入同一 OPL typed queue；Hermes Full App packaging 已恢复为 Full first-install asset。OPL 侧已用 fixture E2E 覆盖 Hermes gateway stopped -> repair -> ready、跨仓 MAS/MAG/RCA notification / approval / retry / dead-letter。剩余重型验收是长时真实 Hermes gateway 24h soak，以及三篇 MAS live paper controlled apply 到最终交付。
...
- `OPL GUI` 预编译包指 Electron-builder 产出的 OPL 品牌 `.dmg` / `.exe` / `.deb` 分发文件及 `latest*.yml` updater metadata；这些 release artifact 由 `opl-aion-shell` 构建，再通过 `npm run gui:release` 上传到 `one-person-lab` GitHub Release。macOS arm64 另有 `One-Person-Lab-Full-<version>-mac-arm64.dmg` 首次安装资产，随包带 MAS/MAG/RCA、Hermes online runtime、`officecli` CLI binary 和推荐 companion skill payload，但不得写入 `latest*.yml` 或成为 App 自动更新目标。
...
- 自有完整长期常驻 runtime sidecar 不是当前 active work；当前通过 Runtime Manager 冻结可迁移边界，并默认依赖外部 `Hermes-Agent` online substrate。只有 Hermes 无法表达 OPL 必需的 task/wakeup/approval/audit/product isolation contract 时，才评估 OPL sidecar promotion。
