...
- `opl runtime manager` 是当前 Runtime Manager 的机器可读 projection：它展示 OPL 管理外部 `Hermes-Agent` substrate 的 owner split、非目标、v1 domain registration registry、Rust native helper lifecycle、native helper target 与 state index target；Rust helper 源码、doctor/repair/prebuild 脚本和 Cargo workspace 已进入 npm package surface；当 helper 可发现时，它会调用 native doctor / state indexer / artifact indexer / runtime watch，并把 `opl_runtime_manager_native_state_projection` 写入 `OPL_STATE_DIR/runtime-manager/native-state-index.json`，同时报告 TTL、history、failure、last-success 与 freshness 状态。
- Rust native helper 的生产化门禁已经进入仓库验证面：`native:doctor`、prebuild check、`npm pack --dry-run`、Rust tests/build、state cache 与 MAS/MAG family smoke 共同构成 native lane；CI 使用 fixture family smoke，本地集成机继续可以对真实 MAS/MAG sibling workspace 做端到端 indexing smoke。
