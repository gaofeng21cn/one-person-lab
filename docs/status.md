...
当前长期智能体原则是 AI-first, contract-light。OPL 依靠 `Codex CLI` 等 AI executor 和 domain stage pack / prompt / skill / knowledge / quality gate 的演进获得智能体能力提升；合同只固定边界、安全、审计、receipt、阻塞、恢复、projection 和 fail-closed 语义，不把开放式规划、创作、评审、路线判断或修订逻辑写死。
...
Fresh conformance evidence（2026-05-19）显示：`opl agents conformance --family-defaults --json` 当前为 `total_repo_count=4`、`passed_count=4`、`blocked_count=0`、`structural_conformance_status=passed`、`production_evidence_tail_count=8`。MAS、MAG、RCA 与 OPL Meta Agent 都通过 structural conformance；RCA 的 README-only required pack path blocker 已关闭。该报告只证明标准 Agent 源码形态、generated surface owner、private generic-owner guard 和 active path scan 通过，不证明 production/domain/artifact readiness 或 live owner-chain evidence。
