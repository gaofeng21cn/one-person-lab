...
- 当前开发宿主：`Codex` 长线自治会话
- 当前产品入口真相：`OPL` 已经落下本地 direct product-entry shell，默认前台命令就是 `opl`；`opl <request...>` 可直接作为 quick ask，`opl start --project <project_id> [--mode <mode_id>]` 会把某个 admitted domain 的 `product_entry_start` 直接解析成推荐启动模式，而 `opl doctor / ask / chat / resume / sessions / logs / repair-hermes-gateway / frontdesk-manifest / frontdesk-entry-guide / frontdesk-readiness / frontdesk-domain-wiring / frontdesk-hosted-bundle / frontdesk-hosted-package / frontdesk-librechat-package / session-ledger / handoff-envelope / domain-manifests / frontdesk-service-* / paperclip-config / paperclip-bind / paperclip-status / paperclip-open-task / paperclip-open-gate / paperclip-sync / paperclip-operator-loop` 构成显式入口与 runtime 运维命令面。`opl frontdesk-bootstrap --path <workspace>` 现在是本机 GUI 前台的主入口：它会一次性落下 `OPL Atlas` 前台壳、`OPL Agent` 默认聊天配置、默认挂载的 `OPL Cortex` MCP bridge，并把当前 workspace 接入 OPL 顶层 registry。`opl projects / workspace-status / workspace-catalog / workspace-bind|activate|archive / domain-manifests / runtime-status / session-ledger / dashboard / paperclip-status` 已把顶层管理面补成“可观察 + 可写 workspace 管理 + 会话归因 + 下游 control-plane bridge”的第一版；其中 `workspace-catalog` 继续只是 registry，会输出 project-level binding summary、active direct-entry locator、可写 action 提示，以及可选的 domain-owned `manifest_command`；`workspace-bind` 现在除了保留手写 `entry_command / manifest_command / entry_url` 路径，也支持结构化 locator 推导：`medautoscience` 用 `--profile`，`medautogrant` 用 `--input`，`redcube` 用 `--workspace-root` 或绑定路径本身，随后自动得到 honest `entry_command + manifest_command`；`domain-manifests` 则会实际解析这些 active binding 上的 `manifest_command`，把 family wiring 指向 routed domain 的 machine-readable product-entry discovery surface，并把 routed domain 的 `product_entry_start`、`frontdesk_surface`、`operator_loop_surface`、`operator_loop_actions`、`product_entry_shell`、`shared_handoff`、可选的 `family_orchestration.action_graph`、recommended shell / command、repo mainline 摘要与 `product_entry_status` 状态摘要一起回灌到 `dashboard` 与 `handoff-envelope`；`frontdesk-entry-guide` 则把 family workspace taxonomy、domain workspace mapping、每个 admitted domain 的 start / preflight / readiness 推荐收成单一 machine-readable entry layer，供 AI / GUI 壳直接消费，并且现在已经在 `frontdesk-manifest`、`opl web` startup payload 与 `OPL Cortex` MCP bridge 里冻结成 `shell_bootstrap.primary_surface`；`dashboard` 则显式退回 operator debug / aggregate view，而不是壳层默认 discovery 起点。`frontdesk-domain-wiring` 则把 hosted shell 与本地 front desk 最直接需要的 `hosted_runtime_readiness / domain_entry_parity / domain_binding_parity / recommended_entry_surfaces` 收成独立的 family wiring truth surface，并直接暴露修复 parity 所需的 workspace registry endpoints；`frontdesk-readiness` 则进一步把本地 service 状态、hosted pilot readiness、domain `product_entry_readiness / preflight` 与下一步修复建议收成单一 operator-facing board；`session-ledger` 现在会输出 session aggregate 级别的归因视图。`paperclip-sync` 现在会先把下游 issue / approval 状态回流到本地 tracked projection，再决定是否写 comment；`paperclip-operator-loop` 则把这条 reconcile + sync 闭环自动化，并把运行摘要落到 `paperclip-status`。`opl web` 现在不只会暴露本地 web front desk pilot 与 hosted-friendly 的 `health / manifest / frontdesk-entry-guide / frontdesk-readiness / frontdesk-domain-wiring / domain-manifests / hosted-bundle / hosted-package / librechat-package / sessions / resume / logs / handoff-envelope / paperclip/control-plane` 表面，还会直接消费 routed domain 的 `product_entry_start`，在浏览器里给出 start panel、`Frontdesk Readiness` / `Domain Wiring` 视图与 `/api/start` resolution surface；同时也保留了基于 launchd 的 service-safe 本地包装层。用户在本机上不再必须先通过 `Codex` 才能触达顶层 `OPL`
...
- 当前长任务语义真相：前台 `ask` 已改成 `async_accept`；真实执行请求会先返回 `task_id`，后台再异步启动 Hermes ask，随后通过 `/api/task-status` 与 `opl_task_status` 回报阶段、最近输出与已解析出的 `session_id`。`dry_run` 仍保持同步预演
- 当前 hosted / web 前台真相：这轮已经有了可直接打开的本地 web front desk pilot，也已经把 future hosted shell 可消费的 hosted-friendly contract surface 冻结出来，并导出了带 service-install / healthcheck helper 的 self-hostable hosted pilot package，以及真实可部署的 `LibreChat-first` hosted shell pilot package；但 actual managed hosted runtime 仍未落地。真正的 hosted / web 入口选型继续冻结为短期 `LibreChat-first`、长期 `OPL` 自有 web front desk。`OPL Cortex` MCP bridge 现在也已把 hosted shell bootstrap 所需的 `workspace binding + session attribution` wiring 明文落成 machine-readable surface。若另一条 GUI 线把用户前台命名为 `OPL Cortex`，那是产品壳名称；repo 内部 `frontdesk_*` 继续是 machine-readable contract / API 命名。`Chatbot UI` 不作为主 hosted 基座
...
- 当前共享实现 owner 真相：从 `2026-04-17` 开始，`OPL` 不再只冻结 shared contract，也开始显式持有 family-shared implementation substrate。当前已冻结的中央 helper 包括 JS 侧的 `managed-runtime-contract`、`product-entry-companions` 与 `family-orchestration` export，以及可被 Python domain 仓消费的 `python/opl-harness-shared` 子包中的 `managed_runtime`、`hermes_supervision`、`product_entry_companions`、`family_orchestration`。这意味着 `family_orchestration` 的 preview shell、runtime companion、人审 gate helper 与 `program_id / active_run_id` 解析 helper 也已经进入 OPL 的单一 owner surface；domain 仓应直接复用这些 family helper，而不是继续各写一套同类 substrate
