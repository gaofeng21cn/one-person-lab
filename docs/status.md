...
- GraphFlow / GFL 的吸收口径已经限定为 OPL stage pack admission、requires / ensures composition、trust lane、verified static core 与 runtime-enforced boundary；当前最小机器读模型已落到 `family-stage-admission` 和 `opl stages list|inspect` admission projection。OPL 不引入 GraphFlow / GFL runtime dependency，不把它写成 provider、executor、planner、stage runner 或 domain authority。
...
2. `stage_pack_admission_requires_ensures`
   OPL 已把 GraphFlow/GFL 可借鉴模式收敛为 stage pack 准入、`requires` / `ensures` 组合、trust lane 和静态/运行时边界拆分，并落成 `family-stage-admission` 读模型：`opl stages list` 汇总 admission 状态，`opl stages inspect` 投影 plane-level admission review、composition blocker、effect-boundary blocker 与 OPL non-authority boundary。当前仍不能写成每条 production launch 都已强制执行 admission gate、selected executor binding proof 和 App/operator blocker projection。
