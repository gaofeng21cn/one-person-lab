...
Standard Agent landing acceptance：`contracts/opl-framework/standard-agent-landing-acceptance-contract.json` 是当前 OPL family 对“标准化智能体彻底落地”的机器验收定义。它把 MAS DM002/DM003 暴露的错误路径提升成七个 family-level gate：真实 agent pack semantics、generated surface production consumption、private-platform residue owner decision、`current_owner_delta` single ordinary route、stage-route arbiter / stop-loss、OMA target-agent work-order guard、cross-agent negative conformance。当前只声明 `acceptance_definition_landed`，不是 family completion：`descriptor_ready`、`generated_interface_ready`、`standard_pack_conformance_passed`、`suite_pass`、`Agent_Lab_pass`、`functional_structure_gap_count_zero`、`provider_completed`、`verified_refs_only_ledger`、`App_projection_ready` 和 `contract_landed` 都不能声明 domain ready、caller migration、physical retirement、target owner acceptance、Brand L5 或 production readiness。后续 OMA / new-agent builder / takeover path 必须用该合同把缺 target owner route、source morphology、generated surface consumption、private residue decision、no-forbidden-write proof 或 owner answer shape 的情况转成 typed blocker / developer work order。
