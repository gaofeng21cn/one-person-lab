...
OPL Developer Mode 已有系统配置基础面：`opl system developer-supervisor` 读写 OPL state 下的 developer supervisor 配置，并默认以 `gaofeng21cn` 作为安装期自动开启的 GitHub login 候选。目标产品语义是 App 设置页暴露 Developer Mode 开关，普通用户可手动开启或关闭；开启后任务默认可以启用外围 AI 巡检，Agent Lab 负责把问题归因、evidence、owner route、candidate fix 和 PR refs 组织成可审计控制面。repo developer / collaborator 可以走对应 repo 的修复提交路径；非 repo developer 只能走 fork / PR 路径。当前仍缺 GitHub 身份/权限检测、App 设置开关消费、Agent Lab 巡检自动挂载和 repo fix / PR route 的端到端产品化。
...
6. `developer_mode_agent_lab_repair_route`
   Developer Mode 已有系统配置入口，但还未闭合成 App 设置开关、GitHub login / repo permission 检测、外围 AI 巡检默认挂载、Agent Lab owner route、repo developer 直接修复提交、非 developer fork / PR 的完整产品路径。该能力必须保持 managed runtime truth 不变，只授权开发修复和 PR 路由。
...
- `opl system developer-supervisor` 管理 Developer Mode 的系统级配置；App 设置页应消费同一配置面，而不是另建本地偏好。
