...
- 当前共享实现 owner 真相：从 `2026-04-17` 开始，`OPL` 不再只冻结 shared contract，也开始显式持有第一批 family-shared implementation substrate：JS 侧的 `managed-runtime-contract` builder/validator export，以及可被 Python domain 仓消费的 `python/opl-harness-shared` 子包。domain 仓应直接复用这些 family helper，而不是继续各写一套同类 substrate
