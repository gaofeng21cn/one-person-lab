...
- `OPL Runtime Manager` 已冻结为 thin product-managed adapter：它只在显式启用时管理外部 `Hermes-Agent` hosted/runtime provider adapter / online-management gateway 的 provision、profile、task registration、诊断、恢复入口、Rust native helper catalog 与高频状态索引，但不承担 scheduler/session/memory kernel。
- `Codex` 是唯一默认交互与执行宿主；`Hermes-Agent` 保留为显式可选 hosted/runtime provider adapter 与 online-management gateway，只在显式切换或长跑托管语境中参与执行。
...
- `opl install` 是当前最短一键安装入口：默认安装或复用 `Codex CLI`，安装 `MAS`、`MAG`、`RCA`，同步短名 Codex skills，并以保守 managed 模式同步推荐 companion skills 和 `officecli` CLI 工具；MAS 声明的 MDS 相关 surface 只作为显式可选 companion diagnostic / oracle / intake 引用读取，不再作为 OPL 默认安装依赖。Hermes online-management gateway 是显式可选 provider adapter，由 Hermes installer/gateway command 管理；OPL 只在 `opl engine install --engine hermes`、`opl runtime repair-gateway` 或已安装 Hermes repair 路径中触发、检查 readiness 并报告状态，不把 gateway service lifecycle 变成 OPL 自有实现。若 OPL 品牌 GUI 已安装则尝试打开，macOS 上未安装时会自动下载并安装匹配当前平台的 release DMG 后再打开；历史 8787 Product API service 模块已退役。
...
- `opl system initialize` 是当前一键配置安装的聚合面：同时暴露 workspace root、Codex default runtime、optional Hermes provider adapter / online-management readiness、domain modules、推荐 companion skills、OPL 品牌 GUI shell、local support service、developer supervisor 用户级配置与下一步动作；没有显式 workspace root 时默认使用用户 Home 目录。首启 readiness 分成 core/domain readiness 与 optional online-management readiness：Codex 与已准入 domain 模块可用时，首屏不应因为 Hermes 缺失或 gateway 尚未 loaded 而阻塞核心工作。
- Fresh install 与 GUI 首启测试已有机器可读矩阵：`contracts/opl-gateway/fresh-install-test-matrix.json` 冻结本机 clean-room、离线 blocker、干净 macOS VM 首启、首启 JSONL 日志和 GUI accessibility labels；本机验证入口是 `npm run test:fresh-install` / `./scripts/verify.sh fresh-install`，OPL Verify workflow 已纳入该 lane。真实 GUI 首启由 `gaofeng21cn/opl-aion-shell` 的 `scripts/opl-first-run-vm-smoke.mjs`、`scripts/opl-first-run-tart-smoke.mjs` 与 `.github/workflows/opl-first-run-vm.yml` 执行，目标 runner labels 为 `self-hosted`、`macOS`、`opl-gui-vm`。用户可见文案应把 Hermes 缺失或 gateway 未 loaded 写成 optional online-management 状态，避免把首屏 blocker 表述成底层 Hermes runtime 未就绪。
...
- `OPL GUI` 预编译包指 Electron-builder 产出的 OPL 品牌 `.dmg` / `.exe` / `.deb` 分发文件及 `latest*.yml` updater metadata；这些 release artifact 由 `opl-aion-shell` 构建，再通过 `npm run gui:release` 上传到 `one-person-lab` GitHub Release。macOS arm64 另有 `One-Person-Lab-Full-<version>-mac-arm64.dmg` 首次安装资产，随包带 MAS/MAG/RCA、`officecli` CLI binary 和推荐 companion skill payload，但不得写入 `latest*.yml` 或成为 App 自动更新目标；Hermes 只通过显式可选安装进入 hosted/online task management 路径。
...
- 自有完整长期常驻 runtime sidecar 不是当前 active work；当前只通过 Runtime Manager 冻结可迁移边界。只有显式可选外部 `Hermes-Agent` provider adapter 不能表达 OPL 必需的 task/wakeup/approval/audit/product isolation contract 时，才评估 promotion。
