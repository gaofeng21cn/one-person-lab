...
- `opl packages manifest` 现在暴露 OPL 中央 Packages 坐标雏形：WebUI Docker 镜像、native helper GHCR 包，以及 MAS/MDS/MAG/RCA 模块源码归档坐标；当前 `opl install`、App 首启协调、`opl system reconcile-modules` 与环境管理仍以 git checkout / sibling repo 更新到远端最新为正式路径。
...
- Fresh install 与 GUI 首启测试已有机器可读矩阵：`contracts/opl-gateway/fresh-install-test-matrix.json` 冻结本机 clean-room、离线 blocker、干净 macOS VM 首启、首启 JSONL 日志和 GUI accessibility labels；本机验证入口是 `npm run test:fresh-install` / `./scripts/verify.sh fresh-install`，OPL Verify workflow 已纳入该 lane。真实 GUI 首启由 `gaofeng21cn/opl-aion-shell` 的 `scripts/opl-first-run-vm-smoke.mjs`、`scripts/opl-first-run-tart-smoke.mjs` 与 `.github/workflows/opl-first-run-vm.yml` 执行，目标 runner labels 为 `self-hosted`、`macOS`、`opl-gui-vm`。
