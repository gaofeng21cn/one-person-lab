...
2026-06-14 Runway readout foldback：`opl runway readiness --json` 与 `opl runway recovery-repair --json` 当前 wrapper 分别是 `.opl_runway_readiness` 与 `.opl_runway_recovery_repair`。`recovery-repair` 现在区分 provider repair plan 与 worker restart authorization：stale worker restart 只有在 running blocking attempt 存在、developer supervisor 缺失、Temporal unreachable 或 ledger unreadable 时才输出 `repair_blocked_by_worker_restart_guard`、`repair_can_mutate_worker=false` 和 restart guard blocker refs；queued / checkpointed / human_gate 只进入 diagnostic backlog。worker source stale 的理想验收标准是：Runway 可以投影唯一 OPL repair owner，但真正 stop/start 必须由 provider repair / provider-slo tick 的 restart guard 证明 explicit developer supervisor、Temporal reachable、ledger readable 且 running blocking attempts 为 0。若当前 attempt ledger 仍有 running attempt，ordinary operating model 必须保持 runtime currentness blocker，而不是为了加载新源码杀 worker 或把 repair action 写成 domain progress。
