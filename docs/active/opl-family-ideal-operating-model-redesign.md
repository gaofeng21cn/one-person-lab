...
2026-06-13 refresh：近期 OPL family 重构主要改变的是 read-model wrapper、Agent Lab / OPL Flow / owner-evidence foldback、App drilldown 顶层投影、provider long-soak 的 live counter 读取方式，以及 App operator drilldown 的源码维护边界，不改变本文 north-star。需要从 fresh payload 读取当前事实：framework readiness 在 `.framework_readiness`，其默认 summary 是 attention-first payload；`framework operating-maturity` 在 `.framework_operating_maturity`，App operator drilldown 在 `.app_operator_drilldown`，Brand L5 在 `.brand_module_l5_status`。最新 readiness readout 仍是 `framework_control_plane_available_with_open_production_tail`：`.framework_readiness.current_owner_delta` 的当前 identity 是 `medautoscience:frt_80051b2d7726a6b3d77ad5be` / `sat_cfb833131bfa30d6661c26c2` / `domain_owner/default-executor-dispatch`，hard gate 为 `owner_delta_open`，`latest_owner_answer_ref=null`，`latest_owner_answer_kind=null`；`.framework_readiness.owner_delta_handoff_summary.status=needs_domain_or_app_live_owner_payload`，accepted payload paths 仍要求 domain/app owner refs 或 typed blocker refs，且不关闭 domain ready / production ready。`framework operating-maturity.current_owner_delta_bridge` 已观测并验证同一 target identity 的 refs-only typed blocker summary，`observed_owner_answer_ref_shapes=["typed_blocker_ref"]`，`owner_payload_summary_closure_state=verified_owner_payload_summary_observed_not_current_pointer_claim`，`readiness_current_pointer_owner_answer_ref=null`，`current_pointer_update_still_required=true`；fresh summary 仍同步显示 `current_owner_delta_owner_answer_missing=true` / `current_owner_delta_owner_answer_still_required=true`，`unresolved_owner_gates.gate_count=7`，第一项为 `owner-gate:current_owner_delta_owner_answer`，`owner_repo=med-autoscience`，accepted refs 为 `domain_owner_receipt_ref`、`quality_gate_receipt_ref` 或 `typed_blocker_ref`。这表示 owner payload summary 已可读、可验证、可接力，但还没有折成 readiness current pointer closeout；重复写同一 refs-only ledger 不是进展。最新 provider long-soak readout 已观察到 `provider_blocker_ref` + `typed_blocker_ref`，但 `.framework_operating_maturity.summary.provider_long_soak_open_count=1`、`long_evidence_ready=false`、`provider_completion_counts_as_production_ready=false`。App drilldown 的 memory/artifact/lifecycle evidence 已提升到 `.app_operator_drilldown.memory_artifact_lifecycle` 顶层，fresh readout 是 `status=refs_observed`、`readiness_status=typed_blocker_work_order_required_not_ready`、`ready_claim_authorized=false`、`observed_ref_count=47`、`lifecycle_reconcile_status=reconciled`；它让 App/operator 更容易读到 owner work order 和 forbidden claims，但不关闭 domain-owned memory/artifact/lifecycle receipt。App drilldown 的 cleanup-retirement 顶层 fresh readout 是 `deletion_evidence_worklist_count=8`、`open_deletion_evidence_requirement_count=0`、`physical_delete_authorized=false`；framework maturity 同时保留 default-callers broader cleanup evidence worklist `32`，两个计数面都不能授权删除。App drilldown core refs 已拆入 `runtime-tray-app-operator-drilldown-parts/core-refs.ts`，主入口约 `612` 行，新增语义 part 约 `420` 行；fresh structure advisory 不再把 `runtime-tray-app-operator-drilldown.ts` 列为 source-shape advisory。这是 Console / App operator read model 的维护性落地，不是 owner-evidence closure。Brand L5 默认全模块 readout 是 compact row：十个模块仍 `evidence_required`、`l5_complete_module_count=0`、每个模块 `open_requirement_count=12`，per-requirement owner routes 需要用 `--module <module_id>` 读取；空 `requirements=[]` 不是完成信号。任何 `open_count=0/1`、verified refs-only ledger、provider blocker、typed blocker ref、conformance pass、completion audit contract pass、App 顶层投影、source split 或 docs foldback 都只能作为 owner-evidence 输入，不能升级为 domain ready、App release ready、Brand L5、production ready 或 physical delete authorization。

2026-06-13 objective update：原“一步到位落地 OPL family ideal operating model”的长期目标不取消，也不降级为文档任务；它的 active 执行解释必须收窄为 owner-evidence closure。当前 OPL 基座、read-model wrapper、completion audit contract、App drilldown 顶层投影、Brand L5 refs-only ledger、provider long-soak refs-only ledger 和结构维护切片都只能支撑更清楚的 owner handoff。当前 MAS typed blocker 已经作为 current target identity 的 verified refs-only summary 被 maturity bridge 读到；真正的 closure 仍按 `framework readiness.current_owner_delta` 和 `framework operating-maturity.unresolved_owner_gates.gates` 分到 domain/App/brand/runtime owner：当前 owner delta current pointer closeout、domain owner-chain scaleout、Brand L5、App release verdict、provider long-soak/recovery/dead-letter、private platform retirement decision、memory/artifact/lifecycle receipt。本文只能保留 north-star 和审计标准；执行目标、owner repo、accepted ref shape、验证命令和下一步 baton 必须折回 [OPL Family 当前状态与理想目标差距](./current-state-vs-ideal-gap.md)。

2026-06-14 workspace currentness foldback：本轮文档更新把近期重构后的 currentness 规则和真实 workspace binding 折回到 active owner-map 与状态文档，不改变本文 north-star。`redcube` 和 `medautogrant` active bindings 已回到真实 repo 路径，fresh readiness 的 `domain_manifest_stale_binding_project_ids=[]` / `domain_manifest_not_configured_project_ids=[]` 说明 workspace binding currentness 尾项已清零。fresh wrapper-aware readback 仍显示 `framework operating-maturity.status=evidence_required`、`ready_claim_authorized=false`、7 个 unresolved owner gates 未闭合；`current_owner_delta_bridge.latest_owner_answer_kind=typed_blocker_ref` 只是 refs-only owner payload summary observed，`readiness_current_pointer_owner_answer_ref=null` 仍要求 current pointer 可消费的 owner answer、route-back 或 updated currentness evidence。`app_operator_drilldown.memory_artifact_lifecycle` 和 `cleanup_retirement` 的顶层投影提升了 Console 可读性；Brand L5 ledger 的 3 条 verified receipts 提升了 evidence transport 可见性；这些都不是 MAS paper ready、App release ready、Brand L5 complete、physical delete authorization 或 production ready。
...
W7 owner-evidence docs foldback 只能记录 `operating-maturity`、Brand L5 和 operating-model projection 的 fresh 判断：owner-route work order 是 refs-only 投影和字段/guard，不是 closing authority。当前 owner answer gate 仍回 `med-autoscience` / route-back owner，并且必须绑定当前 `frt_80051b2d7726a6b3d77ad5be` / `sat_cfb833131bfa30d6661c26c2` / `domain_owner/default-executor-dispatch` identity；已验证的 current-target typed blocker summary 只能说明 bridge 读到 evidence，不能替代 readiness current pointer。Brand L5、App release/user-path、provider long-soak、cleanup 和 memory/artifact/lifecycle evidence 都按各自 owner lane 关闭。近期 wrapper-aware readout 中 provider long-soak 为 open owner-evidence tail，已观察到的 provider blocker / typed blocker refs 只说明 OPL runtime owner tail 被解释或 parked，不关闭 real long-window recovery / dead-letter evidence；Brand L5 仍是 10 个 evidence-required modules / 0 个 L5 complete，App same-cohort user path evidence 不授权 release-ready；这些都不能由本文或 OPL 投影生成 ready claim。

结构维护 foldback 只能说明某个 OPL read model 或 runtime queue coordinator 更易维护。`runtime-tray-app-operator-drilldown.ts` 拆出 `core-refs.ts` 后，App drilldown 入口不再是 source-shape advisory；`existing-dedupe-reconcile.ts` 拆出 `existing-dedupe-requeue.ts` 后，fresh scoped advisory 显示 OPL 自身 `needs_design_pass=0`。这不改变 7 个 owner gates，也不允许把 source-boundary cleanup、line budget pass、structure advisory clean 或 semantic part split 写成 MAS/MAG/RCA/OMA owner answer、App release verdict、Brand L5、physical delete authorization 或 production readiness。
