...
2026-06-14 Runway readout foldback：`opl runway readiness --json` 与 `opl runway recovery-repair --json` 当前 wrapper 分别是 `.opl_runway_readiness` 与 `.opl_runway_recovery_repair`。`recovery-repair.repair_status=repair_action_available` 只说明 OPL runtime owner 有 provider repair plan；它不是 worker restart authorization，也不是 active attempt drain 证明。worker source stale 的理想验收标准是：Runway 可以投影唯一 OPL repair owner，但真正 stop/start 必须由 provider repair / provider-slo tick 的 restart guard 证明 explicit developer supervisor、Temporal reachable、ledger readable 且 active attempts 为 0。若当前 attempt ledger 仍有 queued / checkpointed / running / human_gate，ordinary operating model 必须保持 runtime currentness blocker，而不是为了加载新源码杀 worker 或把 repair action 写成 domain progress。
