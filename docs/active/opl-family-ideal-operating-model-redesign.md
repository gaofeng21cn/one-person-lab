...
2026-06-13 refresh：近期 OPL family 重构主要改变的是 read-model wrapper、Agent Lab / OPL Flow / owner-evidence foldback 和 provider long-soak 的 live counter 读取方式，不改变本文 north-star。需要从 fresh payload 读取当前事实：`framework operating-maturity` 在 `.framework_operating_maturity`，App operator drilldown 在 `.app_operator_drilldown`，Brand L5 在 `.brand_module_l5_status`。最新 provider long-soak readout 已验证 2 条 refs-only receipt，observed ref shapes 为 `provider_blocker_ref` + `typed_blocker_ref`，但 `.framework_operating_maturity.summary.provider_long_soak_open_count=1`、`long_evidence_ready=false`、`provider_completion_counts_as_production_ready=false`。任何 `open_count=0/1`、verified refs-only ledger、provider blocker、typed blocker ref、conformance pass、completion audit contract pass 或 docs foldback 都只能作为 owner-evidence 输入，不能升级为 domain ready、App release ready、Brand L5、production ready 或 physical delete authorization。
