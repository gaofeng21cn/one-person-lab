...
近期 foldback 读法：read-model wrapper、Agent Lab / OPL Flow / owner-evidence foldback、App drilldown 顶层投影、Runway wrapper、provider long-soak evidence、workspace currentness 和源码维护切片都不改变本文 north-star。当前状态、计数、workspace binding、receipt refs、source split、Runway repair plan 与下一步 owner gate 必须回到 [OPL Family 当前状态与理想目标差距](./current-state-vs-ideal-gap.md)、[OPL 当前状态](../status.md)、核心五件套、contracts、source、tests 和 fresh CLI/read-model。本文只保留审计标准：这些 surface 可以让 owner handoff 更清楚，但不能升级为 domain ready、App release ready、Brand L5 complete、physical delete authorization 或 production ready。

2026-06-14 objective update：原“一步到位落地 OPL family ideal operating model”的长期目标不取消，也不降级为文档任务；它的 active 执行解释必须收窄为 owner-evidence closure。当前 OPL 基座、read-model wrapper、completion audit contract、App drilldown 顶层投影、Brand L5 refs-only ledger、provider long-soak refs-only ledger、workspace currentness 和结构维护切片都只能支撑更清楚的 owner handoff。MAS current owner identity、readiness current pointer、verified / stale owner payload summaries 和 unresolved owner gates 都是 live readout；本文不保存 `task_or_study_ref`、`lineage_ref`、receipt refs 或 counter 样本，也不把 verified refs-only summary 写成 ready claim。当前 owner delta current pointer closeout 可由 fresh readiness / StageRun execution authorization / maturity bridge 证明 closed for current identity，但 identity 可能随 MAS `current_work_unit` 切换；每次 claim 都必须重新 live-read。真正未闭合的 closure 仍按 `framework operating-maturity.unresolved_owner_gates.gates` 分到 domain/App/brand/runtime owner：domain owner-chain scaleout、Brand L5、App release verdict、provider long-soak/recovery/dead-letter、private platform retirement decision、memory/artifact/lifecycle receipt。本文只能保留 north-star 和审计标准；执行目标、owner repo、accepted ref shape、验证命令和下一步 baton 必须折回 [OPL Family 当前状态与理想目标差距](./current-state-vs-ideal-gap.md)。

2026-06-14 owner-route audit foldback：current-control residue、accepted closeout evidence、domain-owned typed blocker ref、provider admission state 和 managed execution envelope 必须分开读。若 MAS dry-run 的 managed envelope 是 `executable_owner_action`，例如 `gate_clearing_batch / publication_gate_replay`，而 provider admission current-control 仍携带旧 `typed_blocker_ref`，后者只能作为 history/audit/currentness evidence；它不能覆盖 managed envelope，也不能由 OPL 写成 readiness current pointer owner answer。Fresh readout 也可能直接给出 `current_execution_envelope.state_kind=typed_blocker` / `current_work_unit.status=typed_blocker`，例如 `opl_execution_authorization_required`：这类 blocker 的 canonical current owner / recovery owner 应是 OPL runtime owner `one-person-lab`，embedded typed blocker / PaperRecovery obligation owner 继续保留原 domain work unit owner（例如 `gate_clearing_batch`）作为 provenance。这类 blocker 可以被 maturity 观察为 `typed_blocker_ref` 形状。current pointer owner answer 的验收标准是 owner answer 进入 readiness current pointer，并满足 currentness / closeout binding；当前 identity 可满足该标准，但后续 identity 漂移必须重新证明。理想 operating model 的后续验收标准因此收紧为：ordinary progress 只从 managed current owner delta / execution envelope 推进；current-control residue 只能触发 route-back、updated currentness evidence、execution authorization repair、typed blocker binding 修复或 owner-needed work order，不能抢占 6 个 production-tail owner gates。

2026-06-14 MAS current-work-unit projection foldback：理想 operating model 的 ordinary route 应优先消费 domain-owned `current_work_unit` / `current_execution_envelope`，再把 raw workstream、default-executor dispatch、provider current-control residue 和 evidence worklist 下沉为 full-detail provenance。OPL 可以把 MAS `current_work_unit` 作为 refs-only projection 进入 App/operator 和 readiness 的 owner-delta-first，但只能读 `study_id`、`stage_id`、`stage_attempt_id`、`work_unit_id`、`work_unit_fingerprint`、accepted return shapes 和 source refs；它不能写 MAS truth、不能创建 owner receipt 或 typed blocker、不能关闭 domain ready。若多个 study 同时有 work unit，默认 ordinary pointer 应优先暴露 OPL-owned typed blocker / execution-authorization blocker，再按 domain currentness 序处理同类 owner action；这个策略只保护 current pointer 不被 stale dispatch 抢占，不关闭任何 production-tail gate。

2026-06-14 Runway readout foldback：`opl runway readiness --json` 与 `opl runway recovery-repair --json` 当前 wrapper 分别是 `.opl_runway_readiness` 与 `.opl_runway_recovery_repair`。`recovery-repair` 现在区分 provider repair plan、worker restart authorization 与 blocked/failed attempt operator repair queue：stale worker restart 只有在 running blocking attempt 存在、developer supervisor 缺失、Temporal unreachable 或 ledger unreadable 时才输出 `repair_blocked_by_worker_restart_guard`、`repair_can_mutate_worker=false` 和 restart guard blocker refs；queued / checkpointed / human_gate 只进入 diagnostic backlog；blocked / failed backlog 则进入 `attempt_repair_queue`，输出 `family-runtime attempt list --status blocked|failed --json` 这类只读 operator 查询入口。worker source stale 的理想验收标准是：Runway 可以投影唯一 OPL repair owner，但真正 stop/start 必须由 provider repair / provider-slo tick 的 restart guard 证明 explicit developer supervisor、Temporal reachable、ledger readable 且 running blocking attempts 为 0。若当前 attempt ledger 仍有 running attempt，ordinary operating model 必须保持 runtime currentness blocker；若只有 blocked/failed backlog，ordinary operating model 必须给出可接力查询与 owner review path，而不是为了加载新源码杀 worker、自动 redrive、创建 domain typed blocker 或把 repair action 写成 domain progress。

2026-06-14 provider long-soak false-ready foldback：provider long-soak readout 可以同时出现 `long_evidence_ready=true`、`capability_status=capability_slo_satisfied`、owner work order `open_count=0` 和 `status=owner_evidence_recorded`；这些只说明 OPL runtime owner evidence / refs-only intake 与 capability observation 可接力。稳定验收字段必须继续暴露 `ready_claim_authorized=false`、`provider_completion_counts_as_production_ready=false`、`verified_refs_only_ledger_counts_as_production_ready=false`、`long_evidence_ready_counts_as_production_ready=false` 和 `capability_slo_satisfied_counts_as_production_ready=false`。因此 provider completion、verified refs-only ledger、long-evidence-ready 或 capability SLO satisfied 不能被 App、Console、maturity 或文档写成 provider production ready、domain ready 或 family production ready。

2026-06-14 文档落地吸收规则：本文的“已落地”只表示 ideal operating model 的审计标准已经被 current-state/status/contracts/tests 消费，不能表示 runtime closure。后续新增内容进入本文的条件是它改变 north-star、体验合同、外部成熟实践吸收分类、plane/primitive 责任边界或 forbidden-claim 标准；如果只是 fresh readout、owner gate 状态、receipt refs、worktree closeout、测试结果、release 证据或具体下一步执行顺序，必须写入 `current-state-vs-ideal-gap.md`、`docs/status.md`、history、runtime ledger 或对应 owner repo。本文允许声明 `active_reference_current`；不允许声明 `owner_gate_closed`、`current_pointer_closeout_done`、`App_release_ready`、`Brand_L5_complete`、`provider_long_soak_complete`、`physical_delete_authorized`、`memory_or_artifact_ready` 或 `production_ready`。
...
2026-06-11 implementation foldback：同一 dedupe-key 下 stale `queued` / `waiting_approval` MAS current-control provider admission 被 fresh work-unit/source currentness identity 取代时，属于 OPL `durable_runway_plane` / `reconciler_plane` 的 queue currentness 修复，可原地刷新 payload、重置 attempts，并保留既有 approval gate。owner-answer bridge 仍只接受带完整 StageRun closeout binding 的 domain-owned owner receipt / typed blocker；binding 必须匹配 provider attempt、active lease、execution authorization decision、source fingerprint 与 idempotency。当轮 MAS typed blocker 可作为真实业务阻塞证据，但具体 StageRun identity 与 owner payload summary 都只能从 live `framework readiness` / `framework operating-maturity` 或 history/provenance 读取；缺少匹配该 StageRun 的 closeout binding 时，OPL 不能把 typed blocker 绑定成当前 owner answer 或 domain ready。
...
| `owner_route_residue_separation` | managed execution envelope、current owner delta、provider admission current-control residue 和 accepted closeout evidence 分开读。 | executable owner action 需要 owner delta / receipt / typed blocker；旧 typed blocker residue 只能做 audit/currentness evidence，不能覆盖 current pointer。 |
...
2026-06-14 落地文档更新后的审计口径：当 fresh readout 显示 readiness current pointer 已有 `owner_answer_ref`，StageRun execution authorization `authorized`，closeout binding blockers 为空，maturity bridge `owner_answer_missing=false` / `owner_answer_still_required=false`，本文可以把 current pointer owner-answer handoff 评为 `meets_target` for current identity。Runway `repair_blocked_by_worker_restart_guard` / `repair_can_mutate_worker=false` 同理只能评为 `meets_target` for fail-closed repair guard、`needs_runtime_owner_action` for provider currentness. 这些状态不能被本文升级为 domain ready、App release ready、Brand L5、provider long-soak complete、physical delete authorized、memory/artifact ready 或 `production_ready`；完成度必须回到 active gap 的 Plan Completion Audit 分账。
