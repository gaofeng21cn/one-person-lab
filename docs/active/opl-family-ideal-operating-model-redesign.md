...
结构维护 foldback 只能说明某个 OPL read model 更易维护。`runtime-tray-app-operator-drilldown.ts` 拆出 `core-refs.ts` 后，fresh advisory 只剩 `existing-dedupe-reconcile.ts` 一个 OPL source-shape design-pass 项；这不改变 six owner gates，也不允许把 source-boundary cleanup、line budget pass、structure advisory clean 或 semantic part split 写成 MAS/MAG/RCA/OMA owner answer、App release verdict、Brand L5、physical delete authorization 或 production readiness。
