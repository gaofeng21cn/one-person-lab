...
| `med-autoscience` | MAS 已是 Research Foundry pack；AI reviewer record-only handoff/currentness、request persistence、provider admission、provider handoff 后 terminal materialize refresh、request-bound current record consumption、clean-migration gate replay routing、provider-hosted gate-clearing authorization 和 Progress-first owner-action/liveness arbitration 已从 owner-route 角度推进；paper-line authority 留在 MAS。 | 平台 repair、read-model currentness、storage/index maintenance、gate replay routing 和 liveness arbitration 仍不能写成 research progress；真实 paper owner receipt / reviewer receipt / publication gate 仍是证据尾项。 | `meets_target` for structure/currentness, `needs_domain_evidence` |
...
| `opl-meta-agent` | OMA 边界已经防止第二 Framework：只产出 candidate package、work order、mechanism proposal 或 typed blocker；active docs 已改为 fresh-read / live evidence 口径；全部 TS scripts 已被 script-to-pack retirement/retention gate 覆盖；developer work-order policy 与 standard Foundry policy bundle 已迁入 contract-backed projection；`stage-decomposition-pack-draft.ts` 已收薄为 public entry，builder / validator / materializer / shared parts 已拆开并纳入 machine gate。 | 后续新增或继续稳定下来的 policy 继续迁入 `agent/`、contracts、authority functions 或 OPL primitive；保留脚本只能作为 refs/smoke/work-order/helper；retained helper 退役仍需 OPL primitive parity 和 no-active-caller 证据。 | `meets_target` for script gate coverage, `stable_policy_contracts_and_split_parts_landed` |
