...
| `mag_controlled_soak` | MAG manifest、standard skeleton、controlled memory apply proof 与 controlled soak typed blocker surface 已存在。 | `opl_temporal_controlled_stage_attempt_apply_contract` 未开放前，不能声明 production soak success。 | MAG controlled attempt 返回 domain receipt、no-regression evidence 或 typed blocker；grant truth、fundability verdict 与 submission-ready authority 仍归 MAG。 |
| `rca_controlled_soak` | RCA product-entry、sidecar、standard skeleton、controlled memory apply proof、runtime architecture alignment 与 controlled soak typed blocker surface 已存在。 | `opl_temporal_controlled_visual_stage_attempt_apply_contract` 未开放前，不能声明 production soak success；RCA repo 仍有既有 line-budget 工程债。 | RCA controlled visual attempt 返回 domain receipt、no-regression evidence 或 typed blocker；visual verdict、export gate 与 canonical artifact authority 仍归 RCA。 |
