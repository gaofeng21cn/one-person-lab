...
| `med-autoscience` | MAS active plan source-morphology readout | `docs/active/mas-ideal-state-gap-plan.md` | `owner_lane` | `active_plan` | high-risk active owner doc; large plan | none this tranche | source/contracts/runtime/live evidence | L1/L3 depending edit | future focused docs audit | mas-owner | deferred |
| `med-autoscience` | medical-display portfolio overlap | `docs/delivery/medical-display/README.md` + delivery contracts/catalogs | `owner_lane` | `delivery_support` | delivery authority and large support family | none this tranche | domain artifact authority, visual templates, contracts | L1/L3 depending edit | future theme map first | mas-delivery | deferred |
| `med-autogrant` | active/status false-ready repetition | `docs/active/mag-ideal-state-cross-repo-gap-plan.md` | `governance_ssot` | `active_plan` | none for docs-only, but no live/readiness claims | no broad edit this tranche; keep current owner split | contracts/source/runtime/receipts | L1 | future claim-boundary scan | mag | deferred |
...
| `redcube-ai` | status field-level follow-through longlist | `docs/active/rca-ideal-state-gap-plan.md` + `contracts/physical_source_morphology_policy.json` | `governance_ssot` | `active_truth` | no runtime/readiness claim | `docs/status.md` summary compression | contracts/source/tests/history bulk | L1 | `git diff --check`; targeted stale scan | rca | selected |
...
| `opl-meta-agent` | script-to-pack/source-purity repetition | `docs/active/opl-private-implementation-migration-inventory.md` + machine contracts | `governance_ssot` | `active_truth` | no script retirement/readiness claim | `docs/status.md`; active gap summary only if needed | contracts/source/tests/script gates | L1 | `git diff --check`; targeted duplicate scan | oma | selected |
...
| `med-autoscience` | 2 | 294 | Runtime README duplicate link was fixed; active plan prompt fields normalized to executable OPL Doc baton; runtime/source/delivery README support boundaries clarified; medical-display README and portfolio map now point to the correct active board, generated status, catalogs, contracts, plans and provenance owners. | MAS active source-morphology plan detail, `docs/decisions.md`, history indexes, deeper medical-display contract/catalog leaves. | Next MAS tranche should either compress source-morphology active baton or inspect history indexes; do not turn delivery/source/runtime README into active backlog or authority verdict. |
| `med-autogrant` | 2 | 51 | Active plan prompt fields normalized; specs lifecycle SSOT clarified in `docs/specs/specs_lifecycle_map.md` and specs README now points direct-file readers away from dated-support backlogs. | Active/status repetition, private inventory, history coverage ledgers, product/runtime/delivery/source thin indexes. | Inbound-ref scan before compressing private inventory or active/status repetition. |
| `redcube-ai` | 2 | 93 | Active gap explicit remainder compressed into SSOT table; prompt fields normalized; `docs/history/process/README.md` now has a re-expansion guard and a compressed 2026-06-12 docs lifecycle batch row. | Private inventory, product/runtime/source support docs, reference ideal-state detail, retained history/provenance bodies. | Inspect private inventory only after source/ref owner map is clear; do not re-expand process history into dated proof logs. |
| `opl-meta-agent` | 2 | 14 | Active plan prompt fields normalized; private inventory dated cleanup batches compressed into current migration gates; status source-purity/readback detail thinned back to private inventory and machine contracts. | History process index, docs README rule density, future script-to-pack changes that alter active inventory or contracts. | Keep private inventory as gate owner; move future dated cleanup batches to history/provenance. |
