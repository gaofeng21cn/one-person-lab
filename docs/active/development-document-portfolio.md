...
| `one-person-lab-app` | 2 | 47 | Active plan prompt fields normalized; prior GUI spec link fix remains selected batch output. | Publishing/generated user-guide dirty write set, untracked `docs/architecture/`, release README, GUI inventory/spec long support docs, status/release evidence routing. | Owner lane must resolve/absorb App publishing/generated/public write set and `docs/architecture/` report before governance writes under `docs/publishing/**`, generated guide assets, public guide artifacts, or architecture support docs. |
