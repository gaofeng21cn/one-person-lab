...
| `5` | `domain_soak_and_acceptance` | OPL roadmap plus MAS/MAG/RCA status/active/runtime owner docs | Run real or controlled domain soak on the migrated target shape. | MAS/MAG/RCA produce real progress delta, quality gate movement, human gate, stop-loss, or typed blocker. |
...
| Runtime-substrate history docs | Hermes-first, direct-entry, Gateway-era, host-agent-only, managed-runtime checklist, Hermes runtime benchmark, MAS cutover, and online-platform migration material | Absorbed content lives in active owners; whole historical documents live in `docs/history/runtime-substrate/`. Do not expand them as active planning. |
...
| MAS top-level cutover board | No longer the current MAS migration order. MAS migration now follows OPL framework-first plus the MAS active portfolio. |
