...
当前执行目标、Redesign Backlog 状态和下一轮 baton 只回到 [OPL Family 当前状态与理想目标差距](./current-state-vs-ideal-gap.md)。本文不再给出独立执行顺序，只定义旧开发文档内容应归到哪个 owner surface，避免多条 active 线并行复活。
