...
resource execution 可由 E2B / Daytona / Modal 这类 external sandbox provider 承接。
这些 provider 不替换 OPL Framework、Runway 或 Temporal：Temporal 仍负责 durable
workflow / wakeup / retry / human gate，external sandbox provider 只负责 selected stage
executor 的隔离执行环境。
...
- E2B / Daytona / Modal：成熟 sandbox provider 已把隔离 workspace、process、resource
  和 provider-side execution 当作外部 substrate 提供。对 OPL 的含义是不要在
  runtime env lane 里自研 VM/container sandbox；只定义 provider profile、credential
  preflight、run-context binding、receipt projection 和 no-authority flags。
...
- `External sandbox provider binding`：选择 E2B / Daytona / Modal 等 provider 时，
  OPL 只持有 provider profile、credential preflight、run-context binding、receipt
  projection 和 false-ready guard。

OPL Framework 不拥有底层 VM/container sandbox implementation。provider-side filesystem、
process、network 和 resource isolation 由 external sandbox provider 承担；OPL 不因
sandbox run 成功而获得 domain truth、owner receipt、quality/export verdict、App release
verdict 或 production readiness。
...
- live E2B / Daytona / Modal credential run 已通过。
...
- External sandbox provider 接入不得回落到 host process / host workspace 伪装成功；
  provider credential 缺失或 sandbox receipt 缺失时只能 fail closed 或输出 repair /
  preflight work order。
