...
Current machine slice: `runtime environment substrate` 已落成 contract 与 fail-closed
readback skeleton。可执行入口是 `opl runtime env inspect|lock|cache status|doctor|run-context|contract
--json`；这些入口只证明 descriptor / lock / cache / run-context / false-ready 边界可读，不
materialize runtime root、不生成 lock、不检查真实 host packages、不签 receipt、不证明 runtime、
domain 或 App release ready。
...
`src/runtime-environment-substrate.ts` 和 `opl runtime env inspect|lock|cache status|doctor|run-context|contract`
已提供 fail-closed readback skeleton。Gate：contract/readback tests pass；docs 不宣称
runtime/domain/release ready。
...
| --- | --- | --- | --- | --- | --- |
| OPL runtime environment contract/readback skeleton | `contracts/opl-framework/runtime-environment-substrate-contract.json`; `src/runtime-environment-substrate.ts`; `opl runtime env inspect|lock|cache status|doctor|run-context|contract --json` | Contract 与 fail-closed readback skeleton 已落地；不 materialize runtime root | `done` | `100%` for skeleton only | 后续进入 lock/materializer/cache inventory |
| Shared layer key library | OPL `src/runtime-environment-*` | App repo 有局部实现 | `partial` | `25%` | 抽取 key/manifest/receipt/exclusion policy |
...
| Runtime cleanup convergence | OPL CLI + App Settings | App Storage 有 runtime/toolchain cleanup；OPL env cache 未统一 | `partial` | `25%` | 统一 inventory/prune receipt |
