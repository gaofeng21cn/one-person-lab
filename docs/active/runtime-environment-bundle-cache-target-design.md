...
| MAS environment intent | MAS display dependency profile and run-context consumer | MAS display profile 声明 renderer dependency requirements，并由 OPL `prepare`/run-context 消费；ggconsort/reporting-flow 仍由 MAS display lane 验收 | `partial` | `70%` | 吸收 MAS display lane 后用 MAS focused tests 和 gallery readback 验证 |
| App Full consumer boundary | App Full build scripts and release contract | App Full manifest 记录 OPL runtime environment substrate refs，role 是 consumer-only，false-ready flags 保持 false | `partial` | `80%` | 吸收 App lane 后用 focused release-boundary readback 验证；App release ready 仍需 release cohort |
