...
- E2B：当前已有 executor 的显式 remote sandbox provider；其他 provider 只保留 generic external adapter 接入边界，不能在实现落地前声明支持。
...
- live Docker/devcontainer run 或 live E2B credential run 已通过。
