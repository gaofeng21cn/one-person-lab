...
- `OPL Framework` 持有通用 provider runtime、stage attempt、stage-attempt request/projection、attempt ledger、retry/dead-letter、workspace/source/artifact/memory locator、generated surface、operator/App workbench shell、Agent Lab 和 lifecycle/projection ledger；Codex CLI 独占语义 stage route。
...
| Stage route context transport | StageRun、declared stage set、artifact refs、quality debt refs | 非权威 route context、route-back hint、阴性结果 lineage | Codex CLI 可启动任意 declared stage；OPL 不执行 transition spec、oracle 或 route gate。 |
