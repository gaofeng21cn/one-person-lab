...
- `OMA` 当前 structural clean：`functional_structure_gap_count=0`、`domain_repo_retained_generic_surface_count=0`、`source_shape=landed`。其 `domain_handler` default-caller gate 已消费 `domain_action_adapter_export_dispatch` 的 refs-only evidence；scripts 只允许 authority implementation、smoke helper、fixture/proof helper 或 work-order materializer，不得变成 Agent Lab runner、promotion gate、queue、attempt ledger、App/workbench 或 target truth writer。
