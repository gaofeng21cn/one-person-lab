...
2026-05-25 18:50 CST fresh repo audit 结论：MAS/MAG/RCA/OPL Meta Agent 的持续、全自动运行默认 owner 仍是 OPL/Temporal、OPL attempt ledger、typed queue、retry/dead-letter、wakeup/resume 与 App/operator read model；未发现四仓应接管 domain truth、quality/export verdict、artifact authority、memory body 或 owner receipt 的 OPL surface。当前不干净风险按三类读取：

- `MAS` 已完成本轮 strict source shape cutover：`contracts/functional_privatization_audit.json` 读为 `functional_structure_gap_count=0`、`repo_local_wrapper_tail_count=0`、`source_purity_cutover_status=standard_agent_source_shape_landed`，但 `domain_repo_physical_delete_authorized=false`。原 `generic_cli_mcp_product_wrappers`、`owner_route_reconcile_materialize_dispatch_shell`、`workbench_portal_generic_shell` 只作为 `former_repo_local_wrapper_tail_module_ids` / deletion-gate provenance 读取；剩余项是 live provider paper-line、memory receipt、artifact lifecycle receipt 与 provider SLO evidence gates。
- `MAG`、`RCA` 按 strict source-purity 仍不能标成 physical/source-pure：二者 legacy generic runtime owner 已回到 OPL，合同中的 functional_structure gap 为 0，但 active repo-local wrapper/adapter bridge-exit tails 仍存在，且 `domain_repo_physical_delete_authorized=false`。这类风险是 strict source-purity / physical delete dirty，不是 domain repo 接管 OPL runtime owner。
- `OMA` 当前不标 structural dirty：`functional_structure_gap_count=0`、`domain_repo_retained_generic_surface_count=0`、`source_shape=landed`。剩余是 `opl_generated_default_caller_consumption_tail`、`domain_refs_only_adapter_thinning`、`script_to_pack_hygiene` 和 production evidence tail；这些不允许写成长期标准组成，也不授权 target domain ready 或 family production ready。

当前 OPL read-model 也保持相同边界：`opl agents conformance --family-defaults --json` 为 `passed_count=4`、`blocked_count=0`；`opl agents default-callers --family-defaults --json` 为 `missing_domain_owner_receipt_or_typed_blocker_count=0`、`missing_no_forbidden_write_proof_count=0`、`missing_tombstone_or_provenance_ref_count=0`，但 `physical_delete_authorized_by_this_report=false`；`opl runtime app-operator-drilldown --json` 为 `functional_privatization_action_required_count=0`、`functional_privatization_active_private_generic_residue_count=0`、`default_caller_deletion_evidence_open_requirement_count=0`、`domain_legacy_cleanup_blocked_plan_count=0`、`lifecycle_domain_physical_delete_can_execute=false`。这些读数说明 OPL 有 structural replacement/read-model，不授权 domain repo 物理删除，也不声明 production ready。

按严格目标态，domain repo 不能继续把 refs-only adapter、developer work-order materializer、status/workbench/sidecar shell 或 tombstone/provenance code path 当作标准组成；它们是待迁移或删除污染。剩余风险按 strict source-purity deletion gate 与 production evidence tail 读取，不写成 production ready。
...
| --- | --- | --- | --- |
| `MAS` | Study truth、publication quality、source readiness、artifact/package authority、AI reviewer judgment、owner receipt、typed blocker。 | CLI/MCP/product wrapper、workspace/source intake shell、status/read-model assembly、progress portal/workbench、owner-route handoff、sidecar domain-ref adapter。旧 `runtime_transport/`、`mas_runtime_core*`、turn runner、worker lease 和 lifecycle refs SQLite writer 已 no-alias retired。 | 当前 MAS 合同读为 `functional_structure_gap_count=0`、`repo_local_wrapper_tail_count=0`、`source_purity_cutover_status=standard_agent_source_shape_landed`、`domain_repo_physical_delete_authorized=false`。原三类 wrapper tail 只保留为 former tail / deletion-gate provenance；剩余工作是 real paper-line、memory/artifact/lifecycle receipt 和 provider SLO evidence gates，不是功能/结构 gap。 |
| `MAG` | Fundability / authoring quality / export verdict、grant strategy memory body / accept-reject、package authority、transition oracle、owner receipt、typed blocker。 | Product-entry/status/sidecar/grouped CLI shell、product user-loop route-command shell、runtime report locator shell、runtime registration、lifecycle/package/memory projection envelope、autonomy loop shell、source-layout/scaffold scan/read-model shell。 | 当前合同读为 `mag_functional_structure_gap_count=0`、`standard_agent_source_shape_status=landed`、`claims_opl_replacement_exists=true`、`claims_domain_repo_physical_delete_authorized=false`。MAG 不再作为正向私有控制面读取，但 bridge-exit tails、product/status/sidecar/runtime wrapper、local journal/attempt ledger、scheduler/probe、flat alias、patch bridge 和 compatibility-only tests 仍是 strict physical-delete / evidence tail，不是长期组成。 |
| `RCA` | Source readiness、visual direction、review/export verdict、artifact authority、visual memory accept/reject、native helper implementation、owner receipt、typed blocker。 | Product-entry/session/status/sidecar/MCP wrapper、guarded action metadata wrapper、executor adapter shell、runtimeWatch/operator evidence/stability read model、workspace/run envelope、native-helper generic envelope、review/repair transport、artifact gallery/handoff shell。 | 当前合同读为 `functional_structure_gap_count=0`、`unclassified_private_generic_residue_count=0`，但 strict wrapper/source-purity cutover 仍 pending，module / bridge-exit gates 的 `physical_delete_authorized=false`。RCA 不再作为 active generic runtime owner 读取；generic wrapper、executor shell、operator projection、stability read model 和 workspace/source lifecycle shell 必须继续迁到 OPL generated/hosted surface 或删除。 |
| `OPL Meta Agent` | Agent-building semantics、candidate package/work-order/proposal materialization refs、target-agent typed blocker refs。 | Script-level materializers、bootstrap contract pack writer、external suite work-order/blocker output assembly、default-caller deletion evidence contracts。 | 当前 OMA 合同读为 `source_shape=landed`、`functional_structure_gap_count=0`、`domain_repo_retained_generic_surface_count=0`，remaining tails 是 `opl_generated_default_caller_consumption_tail`、`domain_refs_only_adapter_thinning`、`script_to_pack_hygiene`、`evidence_tail`。scripts 只允许 authority implementation、smoke helper、fixture/proof helper 或 work-order materializer；不得变成 Agent Lab runner、promotion gate、queue、attempt ledger、App/workbench 或 target truth writer。 |
