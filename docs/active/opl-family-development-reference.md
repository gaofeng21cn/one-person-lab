...
- `user_stage_log`：对用户说明“问题是什么、这个 stage 要做什么、实际对 domain deliverable 做了什么、结果、耗时、token/费用”。OPL 只提供 `stage_progress_log` 容器、时间/usage/refs 和 missing/null 状态；domain agent 必须提供 `stage_work_done` / `changed_stage_surfaces` 等人话语义。MAS 可以继续用 `paper_work_done` / `changed_paper_surfaces` alias，MAG/RCA 应用同一通用字段描述 grant / visual deliverable 改动；缺失时必须暴露 `missing_domain_semantic_summary`。
