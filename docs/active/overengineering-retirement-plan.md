...
| P2 | 测试与 fixture surface | 体量目标仍未完成。当前推进范围排除 MedOPL；只统计 OPL、MAS、Aion shell、RCA、MAG、App、OMA。2026-07-09 本轮连续 landing 已吸收 17 个 mainline 测试瘦身提交，净删 `29,737` 行：OPL `-6,733`、MAS `-11,902`、Aion `-3,596`、MAG `-1,387`、App `-2,717`、RCA `-1,388`、OMA `-2,016`。当前 tracked test surface 仍有 `2,378` 个文件、`670,839` 行：OPL `160,500`、MAS `336,871`、Aion `58,008`、RCA `52,205`、MAG `35,704`、App `15,516`、OMA `12,035`。按 `10-18 万` 首批目标折算，本轮只覆盖约 `17-30%`；剩余约 `7.0-15.0 万` 行必须继续分类为 `delete / merge / rewrite / keep / owner-blocked`，不能把旧测试会红当成保留理由。 | focused P2 tests + `test-lanes assert-coverage` + sibling App/Aion/RCA/MAS repo-native validators；旧测试变红不是最终否决理由，必须区分必要覆盖缺失、旧 wrapper/文案断言被删除、实现细节绑定、或 helper 改坏行为；同写集 dirty 或需要真实 owner 降低覆盖粒度的 lane 只能记 blocker/owner decision。 |
...
| 测试与 fixture surface | partial | 17-30% | 以 10-18 万行目标区间折算，本轮可确认的 mainline 净删为 `29,737` 行，仍不足以声明完成。已吸收并验证的提交：OPL `86e691c44`、`fcf524851`、`b09cdabae`；MAS `42403289d`、`ed67a7d23`、`cf1818f82`、`922417a03`；Aion `62f8d226c`、`c032fdb94`；MAG `4745dba`、`99140d2`；App `6209229`、`17db5a8`；RCA `ecbd2375`、`3f902c81`；OMA `22b3c8c`、`a0f13e2`。当前剩余 tracked test surface：`2,378` 文件 / `670,839` 行。 | 10-18 万目标剩余差距约 `7.0-15.0 万` 行，主要在 active semantic coverage。后续每个候选必须分类为 `delete / merge / rewrite / keep / owner-blocked`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
