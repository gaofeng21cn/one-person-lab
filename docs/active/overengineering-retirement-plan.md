...
| P2 | 测试与 fixture surface | 体量目标仍未完成：2026-07-08 当前 verified test-tree/support-doc 安全落地量已从首批 `2,412` 行推进到约 `2.8-5.0 万` 行区间；其中已吸收的 mainline evidence 包括 MAS dead-file prune `-23,746`、OPL orphan support `-349`、Aion skipped/todo placeholders `-549`、App release-boundary shadow assertions `-29`、RCA unused fixtures `-172`，再加上前序已吸收的 fixture/assertion shrink。2026-07-09 继续落地的吸收量为 `-2,147` 行：Aion 删除重复 `conversation-full-cycle` E2E `-1,571`，OPL pass3 收敛 runtime/native helper fixture `-71`，MAS 重复 active aggregation `-14`，Aion pass3、MAS duplicate prune、RCA/MAG/MedOPL/App/OMA 小切口合计 `-491`。2026-07-09 fresh tracked `tests/**`/`test/**` 计数为 `722,839` 行。10-18 万行目标仍未完成；后续只计入有 fresh verification 且不删除 active authority/currentness coverage 的净删。 | focused P2 tests + `test-lanes assert-coverage` + sibling App/Aion/RCA/MAS repo-native validators；baseline 红、active authority/currentness tests、同写集 dirty、active Makefile/contract/source refs 或需要降低覆盖粒度的 lane 只能记 blocker/owner decision，不能计入净删或完成。 |
...
| 测试与 fixture surface | partial | 17-52% | 以 10-18 万行目标区间折算，当前可确认的安全落地量仍不足以声明完成；2026-07-08 已追加 MAS 67 个非收集死测试文件删除（hygiene `20 passed`、representative collect-only `233 collected`、line-budget advisory-only，`-23,428`）、MAS pass2 dead marker 删除（同组验证 + `tests.*` import check，`-318`）、OPL MAS tombstone duplicate scenario（focused node + test-lanes，`-59`）、OPL orphan support 安全版（保留 active Temporal diagnostic module，`test-lanes assert-coverage` + `npm run typecheck`，`-290`）、App release notes shadow assertions（release-boundary validator，`-29`）、Aion skipped/todo placeholders（Playwright list + web-host Vitest，`-549`）和 RCA unused fixtures（typecheck，`-172`）。2026-07-09 继续吸收 OPL pass3（affected node tests `53/53`、`test-lanes assert-coverage`、`npm run typecheck`）、MAS pass3（focused pytest `25 passed`）、Aion duplicate full-cycle E2E 删除（Playwright list `207 tests in 31 files` 与代表性 list `12 tests in 5 files`）、App/OMA/RCA/MAG/MedOPL focused validators，新增 verified net `-2,147` 行；fresh tracked `tests/**`/`test/**` 计数为 `722,839` 行。 | 10-18 万目标剩余差距主要在 active semantic coverage，而不是孤儿文件：MAS 大块 lane 证明无入边且默认不收集的整文件候选只有 `2,318` 行；复核后 6 个候选均被 Makefile、contracts 或 source active refs 命中，已拒绝删除且不计入成果。更大的 `15,827` 行候选有聚合入口。OPL 大块 lane 证明点名 16,629 行簇内最大安全收敛约 `182` 行。RCA 大块尝试 `-778` 会打坏 native PPT tests。App release lane 的 jscpd 理论重复上限仅 `907` 行。继续大幅下降必须先修 baseline collection errors、做 aggregate-surface redesign / table-driven equivalence proof，或由 owner 明确批准降低覆盖粒度。 |
