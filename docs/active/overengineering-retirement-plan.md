...
| P2 | 测试与 fixture surface | 体量目标仍未完成：2026-07-08 当前 verified test-tree/support-doc 安全落地量已从首批 `2,412` 行推进到约 `2.8-5.0 万` 行区间；其中已吸收的 mainline evidence 包括 MAS dead-file prune `-23,746`、OPL orphan support `-349`、Aion skipped/todo placeholders `-549`、App release-boundary shadow assertions `-29`、RCA unused fixtures `-172`，再加上前序已吸收的 fixture/assertion shrink。2026-07-09 继续落地的吸收量为 `-2,147` 行：Aion 删除重复 `conversation-full-cycle` E2E `-1,571`，OPL pass3 收敛 runtime/native helper fixture `-71`，MAS 重复 active aggregation `-14`，Aion pass3、MAS duplicate prune、RCA/MAG/App/OMA 小切口合计 `-491`。当前推进范围排除 MedOPL；后续只统计 OPL、MAS、Aion shell、RCA、MAG、App、OMA。10-18 万行目标仍未完成；后续只计入有 fresh verification 且必要 coverage 仍被更小测试/contract/readback 保留的净删。 | focused P2 tests + `test-lanes assert-coverage` + sibling App/Aion/RCA/MAS repo-native validators；旧测试变红不是最终否决理由，必须区分必要覆盖缺失、旧 wrapper/文案断言被删除、实现细节绑定、或 helper 改坏行为；同写集 dirty 或需要真实 owner 降低覆盖粒度的 lane 只能记 blocker/owner decision。 |
...
| 测试与 fixture surface | partial | 17-52% | 以 10-18 万行目标区间折算，当前可确认的安全落地量仍不足以声明完成；2026-07-08 已追加 MAS 67 个非收集死测试文件删除（hygiene `20 passed`、representative collect-only `233 collected`、line-budget advisory-only，`-23,428`）、MAS pass2 dead marker 删除（同组验证 + `tests.*` import check，`-318`）、OPL MAS tombstone duplicate scenario（focused node + test-lanes，`-59`）、OPL orphan support 安全版（保留 active Temporal diagnostic module，`test-lanes assert-coverage` + `npm run typecheck`，`-290`）、App release notes shadow assertions（release-boundary validator，`-29`）、Aion skipped/todo placeholders（Playwright list + web-host Vitest，`-549`）和 RCA unused fixtures（typecheck，`-172`）。2026-07-09 已吸收 OPL pass3、MAS pass3、Aion duplicate full-cycle E2E、App/OMA/RCA/MAG 小切口，新增 verified net `-2,147` 行。当前继续推进范围排除 MedOPL，按 OPL、MAS、Aion shell、RCA、MAG、App、OMA 重新统计。 | 10-18 万目标剩余差距主要在 active semantic coverage，不应再用“删了旧测试会红”作充分否决；后续每个候选必须分类为 `delete / merge / rewrite / keep`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
