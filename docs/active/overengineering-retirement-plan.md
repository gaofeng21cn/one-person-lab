...
| P2 | 测试与 fixture surface | `10 万行`最低线已完成，`18 万行`上沿仍未完成。当前推进范围排除 MedOPL、已归档 DeepScientist/med-deepscientist，以及上游拥有的 `opl-aion-shell` fork；Aion 只允许 OPL-owned overlay 目标进入，非我们设计的上游测试/实现不再作为瘦身对象。2026-07-08 起非 Aion mainline slim/shrink 提交按 test-path numstat 净删 `144,685` 行。最新补充吸收：OPL `a983454d5`、`1671aa7e1`、`f19c3c59e`、`da1068253`；MAS `35f6164e2`、`598eb4b03`、`500a6f649`；MAG `4e7cf59`、`4d64b2b`、`eccaf87`；App `4610eb1`、`6b67a56`、`4537402`；OMA `15450c6`、`d57a84d`、`b738486`；RCA `0a986912`。按 fresh tracked-test 统计口径（`tests/`/`test/` 下文件 + 根级 `test_*.py` / `.test.*` / `.spec.*`，排除 Aion），非 Aion 当前仍有 `517,636` 行：OPL `135,539`、MAS `297,637`、RCA `45,711`、MAG `22,267`、App `9,300`、OMA `7,182`。按 `10-18 万` 首批目标折算，本轮完成最低线 `100%`，上沿约 `80%`；若继续追 18 万，还需净删 `35,315` 行，必须继续分类为 `delete / merge / rewrite / keep / owner-blocked`，不能把旧测试会红当成保留理由。 | focused P2 tests + `test-lanes assert-coverage` + sibling App/RCA/MAS/MAG/OMA repo-native validators；Aion fork 只做 read-only ownership/readback 或回退误动，不做测试瘦身；旧测试变红不是最终否决理由，必须区分必要覆盖缺失、旧 wrapper/文案断言被删除、实现细节绑定、或 helper 改坏行为；同写集 dirty 或需要真实 owner 降低覆盖粒度的 lane 只能记 blocker/owner decision。 |
...
| 测试与 fixture surface：10 万最低线 | done | 100% | 2026-07-08 起非 Aion mainline slim/shrink 提交按 test-path numstat 净删 `144,685` 行，超过 `10 万`最低线。最新已推远端：OPL `da1068253`（72/72 touched tests、`test-lanes assert-coverage`、`npm run typecheck`）、MAS `500a6f649`（101/101 focused pass）、MAG `eccaf87`（47 pass / 20 subtests）、App `4537402`（62/62 release tests）、OMA `b738486`（touched 15/15、behavior 37/37、smoke 28/28、typecheck）。当前非 Aion tracked test surface：`517,636` 行。 | 最低线已关闭；后续不能再用“离 10 万还差”作为 blocker。 |
| 测试与 fixture surface：18 万上沿 | partial | 80% | 同一 fresh numstat 口径已完成 `144,685 / 180,000`。剩余主要在 MAS/OPL active semantic coverage，而不是 Aion 上游、MedOPL 或已归档 DeepScientist。RCA pass6 因 unresolved `tests/**` ownership 和当前 unrelated dirty overlay/runtime write set 暂列 owner-blocked。 | 若继续追上沿，还需净删 `35,315` 行。后续每个候选必须分类为 `delete / merge / rewrite / keep / owner-blocked`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
