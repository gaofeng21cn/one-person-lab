...
- `未处理项`：`src/modules/connect/managed-shell-command-env.ts` 属于本 lane 禁止写集；`src/modules/console/app-state-mas-owner-answer-projection.ts` 是 owner-answer projection surface，仍需单独 owner/caller 复核，不在本轮低风险 barrel 删除范围。`hygiene:fallow` 允许继续非零；本轮完成的是可证无消费者的 Console re-export surface 收薄和剩余 allowed public/runtime surfaces 的保留判定。
- 新鲜证据：`npm run typecheck` 通过；`npm run source:modules -- --format json` 为 `status=ok`、`module_entrypoints.expected_count=10`、`deep_import_violations.count=0`、`forbidden_dependency_violations.count=0`；`npm run hygiene:fallow` 仍为 exit 1，unused files 仍为 5，残留均有上述保留、禁写集或待 owner 复核理由；`git diff --check` 通过。

## 2026-07-07 Codex carrier / Skill 暴露收薄

- `OPL Connect Codex Plugin carrier`：family plugin registry 不再把 `OPL_STATE_DIR/codex-plugin-marketplaces/<marketplace-id>/plugins/<plugin-id>` 做成指向 domain checkout 的 symlink；改为 OPL-owned canonical wrapper，读取 active repo tracked plugin source 后物化 `.codex-plugin/plugin.json`、`skills/<plugin-id>/SKILL.md`、icon refs 和 prompt refs，并把 Codex-visible id 规范为 `mas`、`mag`、`rca`。这避免要求 domain repo 把 package/source identity 改成 Codex 短名，也避免在 domain repo 写 `.agents/plugins/marketplace.json`。
...
| Fallow entrypoint / dead-export / cycle hygiene | partial | 95% | `.fallowrc.json` stale entrypoint 已纠正，`foreground-paths.ts` cycle tail 已切断；`2026-07-07` 追加删除私有 unused wrapper/source hygiene 文件后，public/shared-release lane 把 advisory unused files 降到 5，public/fallow lane 删除 `console/index.ts` 无消费者 re-export barrel并判定模块 aggregate、Console public entry、GitHub action runtime surface 保留；Runway lane 追加切断 `family-runtime-providers -> family-runtime-temporal-provider -> family-runtime-temporal-activities -> family-runtime-scheduler -> family-runtime-providers` direct cycle。 | fallow 仍是 advisory cleanup gate，不是 physical delete authority；剩余 unused files、dead exports、clone groups 与 Runway worker-repair / provider-SLO / current-control / Temporal client-visibility cycles 要按 active caller 和 owner route 分批处理，不能批量删除或写成 runtime/provider ready。 |
