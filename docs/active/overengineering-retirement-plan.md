...
| 测试与 fixture surface | partial | 20-36% | 以 10-18 万行目标区间折算，2026-07-08 起非 Aion mainline slim 提交按 test-path numstat 净删 `35,848` 行，仍不足以声明完成。已吸收并验证的最新补充提交包括：OPL `ec44edd1a`，MAS `984b89f8d`，RCA `9f4866e3d`，App `2c43aec07`；既有非 Aion slim 提交继续由各仓 `git log --grep slim --since 2026-07-08 main`、提交 diff、repo-native tests 和 ops ledger 持有。当前非 Aion tracked test surface：`2,046` 文件 / `611,423` 行。 | 10-18 万目标剩余差距约 `6.4-14.4 万` 行，主要在 active semantic coverage。后续每个候选必须分类为 `delete / merge / rewrite / keep / owner-blocked`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
