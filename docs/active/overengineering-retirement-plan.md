...
| P2 | 测试与 fixture surface | `10 万行`最低线与 `18 万行`上沿均已完成。当前推进范围排除 MedOPL、已归档 DeepScientist/med-deepscientist，以及上游拥有的 `opl-aion-shell` fork；Aion 只允许 OPL-owned overlay 目标进入，非我们设计的上游测试/实现不再作为瘦身对象。2026-07-08 起非 Aion mainline slim/shrink 提交按 commit numstat（含必要 test-lane metadata/source references）净删 `209,529` 行。已吸收：OPL `5593d2749`、`9341c0a9c`、`a16e10a86`、`751ed6c93`、`01299a7f`；MAS `0e7b005da`、`7d2badb2f`、`2f9eab3fd`、`3721050ae`、`2d9c2f2`、`1b23dbc66`；MAG `4e7cf59`、`4d64b2b`、`eccaf87`、`9db7a0f0`、`1731640`、`c6939f3`；App `4610eb1`、`6b67a56`、`4537402`、`bcd4acdd`、`b29d9e4`、`8d19320`、`5f59efa`；OMA `15450c6`、`d57a84d`、`b738486`、`ebf41e24`、`9f0840a`、`1c8832f`；RCA `0a986912`、`4c379d42`、`0bd339e`。按 fresh tracked-test 统计口径（`tests/`/`test/` 下文件 + 根级 `test_*.py` / `.test.*` / `.spec.*`，排除 Aion），非 Aion 当前仍有 `456,753` 行：OPL `125,805`、MAS `248,173`、RCA `45,644`、MAG `21,137`、App `8,823`、OMA `7,171`。该 current surface 已包含同期非瘦身提交 OPL `fe2356e14` 和 OMA `7131e55` 的测试新增，不能用它和 slim/shrink commit numstat 做简单相减。`10-18 万` 首批目标已关闭；继续瘦身只能按新候选分类为 `delete / merge / rewrite / keep / owner-blocked`，不能把旧测试会红当成保留理由。MAS product/CLI 候选未吸收：focused product-entry validation 为 `69 failed, 167 passed`，按 rejected 处理，不计入完成行数；RCA pass9 helper 候选因 focused node tests `12 pass / 2 fail` 被回退并关闭。 | focused P2 tests + `test-lanes assert-coverage` + sibling App/RCA/MAS/MAG/OMA repo-native validators；Aion fork 只做 read-only ownership/readback 或回退误动，不做测试瘦身；旧测试变红不是最终否决理由，必须区分必要覆盖缺失、旧 wrapper/文案断言被删除、实现细节绑定、或 helper 改坏行为；同写集 dirty 或需要真实 owner 降低覆盖粒度的 lane 只能记 blocker/owner decision。 |
...
| 测试与 fixture surface：10 万最低线 | done | 100% | 2026-07-08 起非 Aion mainline slim/shrink 提交按 commit numstat（含必要 test-lane metadata/source references）净删 `209,529` 行，超过 `10 万`最低线。最新已推远端：OPL `01299a7f`（workspace lifecycle cleanup focused node test `7/7`）、MAS `1b23dbc66`（stage artifact index `18/18`、collect-only `18`）、MAG `c6939f3`（hosted contract bundle `16 passed, 30 subtests`）、App `5f59efa`（release boundary `116 pass / 2 skip`）、OMA `1c8832f`（external-suite `16/16`、`npm run typecheck`）、RCA `0bd339e`（native PPT focused node tests `13/13`、`npm run typecheck`）。当前非 Aion tracked test surface：`456,753` 行；该数包含同期其他主线测试新增，因此不是 slim/shrink commit numstat 的简单相减。 | 最低线已关闭；后续不能再用“离 10 万还差”作为 blocker。 |
| 测试与 fixture surface：18 万上沿 | done | 100% | 同一 fresh commit-numstat 口径已完成 `209,529 / 180,000`，超过上沿目标。剩余测试代码仍可继续按新候选瘦身，但不再属于首批 `10-18 万` 目标缺口。MAS/App/OMA/RCA/MAG/OPL 已验证净删 lanes 已吸收并清理；MAS product/CLI 候选因 focused product-entry validation `69 failed, 167 passed` 被拒绝，RCA pass9 helper 候选因 focused node tests `12 pass / 2 fail` 被回退，二者均未吸收。 | 后续如继续追更深瘦身，必须重新列候选并分类为 `delete / merge / rewrite / keep / owner-blocked`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
| Aion upstream fork 边界 | done | 100% | 用户明确 `opl-aion-shell` 是 fork 的 AionUI，上游主要维护；`opl-aion-shell/main` 已精确对齐 `gh-https/main` HEAD `ea56f2eed`，fresh readback 显示无本地 diff。误动本地提交仅保留在本地备份分支 `codex/backup-aion-d685b0b9-before-upstream-reset` 与 `codex/backup-aion-e1e18a05-before-upstream-reset`。后续统计与执行排除 `opl-aion-shell` 和 App 内 `shells/aionui/**` 上游体，除非目标明确是 OPL-owned overlay。 | 不再对 Aion upstream body 做测试瘦身、结构重写、样式交互重构、依赖升级或实现清理；只允许 read-only 核查、误动回退、或 OPL overlay 边界内改动。 |
