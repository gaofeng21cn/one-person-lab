...
| --- | --- | --- | ---: | --- |
| OPL | `47913390a` + `555a7f531` + `63bdc710e` + `b3615ce96` + `6afbda9b7` + `f2c3f5fe` | domain progress transition runtime；runtime manager native cases；Codex default shell generated-surface helpers；runtime drilldown helper fixtures；workspace domain transitions coverage | `-387` | domain progress transition runtime：`22/22 pass`；runtime manager native：`11/11 pass`；Codex default shell：`21/21 pass`；runtime drilldown：`2/2 pass`；workspace transitions：`11/11 pass`；`npm run typecheck` |
| MAS | `2e361d8ec` + `3ccd05fcb` + `66c94dbac` + `c7e7311f4` | currentness / provider admission owner identity tests；AI reviewer routeback cases；runtime protocol study runtime cases；runtime health kernel tests | `-429` | owner/currentness：`27 passed`；routeback focused pytest：`21 passed`；runtime protocol focused pytest：`42 passed`；runtime health kernel focused pytest：`25 passed`；display QC helper candidate was reverted on MAS `main` and is not counted |
| RCA | `fb1cb34c` + `a630314a` + `f00f2b18` + `3eff3f75` | native PPT Python layout fixture parts；RedCube Playwright mock helper；native PPT layout fixtures；source intake cases | `-378` | native PPT focused tests：`11/11 pass`；helper-referencing focused tests：`60/60 pass`；source intake：`17/17 pass`；`npm run typecheck` |
| MAG | `c5cab6a` + `8f6a8fb` + `f7b8ad3` + `8858cbf` | product entry / workspace summary / critique executor tests；domain handler receipt assertion preservation；workspace summary cases；critique executor tests | `-382` | affected product/workspace/critique focused tests：`51 passed, 60 subtests passed`；workspace summary focused pytest：`4 passed, 51 subtests passed`；critique executor focused pytest：`7 passed, 2 subtests passed`；`./scripts/verify.sh`：`234 passed, 351 subtests passed` |
| App | `23d450e` + `d44dd99` + `72b490a` | release boundary tests；release closeout cases；release helper core | `-446` | `OPL_APP_SHELL_ROOT=/Users/gaofeng/workspace/one-person-lab-app/shells/aionui npm run test:release-boundary`：`116 pass, 2 skip` |
...
Strict test-only landed total after main-session semantic repairs and reverted candidates: `-2,123` lines.
...
| P2 | 测试与 fixture surface | 历史 `10-18 万`口径已撤销为 mixed-commit snapshot，不能作为测试瘦身完成证据。本轮严格 test-only 已落地并推送 OPL/MAS/RCA/MAG/App/OMA 六仓净删 `2,123` 行；所有计入候选均限制在 `tests/**` 或等价测试文件，且通过 focused repo-native 验证；MAS display QC helper candidate 与 OMA bootstrap-loop candidate 均因净增被拒绝，不计入收益。当前推进范围继续排除 MedOPL、已归档 DeepScientist/med-deepscientist、`opl-aion-shell` upstream fork body 和 App `shells/aionui/**` vendor copy。当前 tracked-test surface 库存必须另跑 path-filtered counter 重算，不作为本快照完成证据。 | focused P2 tests + sibling App/RCA/MAS/MAG/OMA repo-native validators；Aion fork 与 App `shells/aionui/**` 只做 read-only ownership/readback 或误动回退，不做测试瘦身；继续瘦身必须按新候选分类为 `delete / merge / rewrite / keep / owner-blocked`，不能把旧测试会红当成保留理由，也不能把 mixed commit 当测试收益。 |
...
| 测试与 fixture surface：严格 test-only 吸收 | partial | 2% | 本轮已吸收并推送六仓严格测试瘦身：OPL `47913390a` / `555a7f531` / `63bdc710e` / `b3615ce96` / `6afbda9b7` / `f2c3f5fe`、MAS `2e361d8ec` / `3ccd05fcb` / `66c94dbac` / `c7e7311f4`、RCA `fb1cb34c` / `a630314a` / `f00f2b18` / `3eff3f75`、MAG `c5cab6a` / `8f6a8fb` / `f7b8ad3` / `8858cbf`、App `23d450e` / `d44dd99` / `72b490a`、OMA `6cbda67`，合计净删 `2,123` 行；所有计入候选均由主会话复核 write set 与 focused verification。 | 相对于历史 `10-18 万`目标，只能确认 `2,123 / 100,000` 的严格 test-only 证据；剩余大体量必须重新按语义候选审计，不能从旧 mixed snapshot 继承完成度。 |
