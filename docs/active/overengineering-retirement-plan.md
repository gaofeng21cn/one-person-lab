...
| P2 | 测试与 fixture surface | 体量目标仍未完成：2026-07-08 当前 verified test-tree/support-doc 安全落地量已从首批 `2,412` 行推进到约 `2.8-5.0 万` 行区间；其中本轮新增已推送的 mainline evidence 包括 MAS dead-file prune `-23,746`、OPL orphan support `-349`、Aion skipped/todo placeholders `-549`、App release-boundary shadow assertions `-29`、RCA unused fixtures `-172`，再加上前序已吸收的 fixture/assertion shrink。2026-07-09 OPL pass3 继续压缩 active CLI 测试内容：把 runtime manager/native helper 重复 shell fixture 收到单一小 helper，并把 runtime app-operator dispatch workorder/group 重复 shape 断言局部表述化，保持同一 CLI payload coverage。这个数字仍只是 10-18 万行候选池的安全落地子集，不是目标完成。继续优先处理可由现有 validator、contract reader 或行为测试覆盖的重复 fixture、快照和聚合用例。 | focused P2 tests + `test-lanes assert-coverage` + sibling App/Aion/RCA/MAS repo-native validators；baseline 红、active authority/currentness tests、同写集 dirty 或需要降低覆盖粒度的 lane 只能记 blocker/owner decision，不能计入净删或完成。 |
...
| 测试与 fixture surface | partial | 16-50% | 以 10-18 万行目标区间折算，当前可确认的安全落地量仍不足以声明完成；2026-07-08 已追加 MAS 67 个非收集死测试文件删除（hygiene `20 passed`、representative collect-only `233 collected`、line-budget advisory-only，`-23,428`）、MAS pass2 dead marker 删除（同组验证 + `tests.*` import check，`-318`）、OPL MAS tombstone duplicate scenario（focused node + test-lanes，`-59`）、OPL orphan support 安全版（保留 active Temporal diagnostic module，`test-lanes assert-coverage` + `npm run typecheck`，`-290`）、App release notes shadow assertions（release-boundary validator，`-29`）、Aion skipped/todo placeholders（Playwright list + web-host Vitest，`-549`）和 RCA unused fixtures（typecheck，`-172`）。2026-07-09 OPL pass3 本地候选继续收敛 active CLI 测试重复内容，净删约 70 行，受影响 node tests 为 `51/51` pass；它只证明测试内容收薄，不替代 broader runtime/live evidence。 | 10-18 万目标剩余差距主要在 MAS paper-mission / study-progress、OPL runtime drilldown broader assertions、App/Aion UI duplication 等 active authority/currentness coverage；若要继续大幅下降，必须先修 baseline-red suites、证明等价 table-driven fixture，或由 owner 明确批准降低覆盖粒度。 |
