...
| P2 | 测试与 fixture surface | 体量目标仍未完成：2026-07-08 当前 verified test-tree/support-doc 净删累计约 `2,353` 行（前序 MAS outer-loop `-679`、前序 OPL workspace-domain `-300`、本轮 OPL descriptor/drilldown assertions `-1,170`、MAS paper-mission duplicate collection `-123`、App release-boundary shadow assertions `-65`、Aion e2e placeholder/docs `-16`）。这只是 10-18 万行候选池的安全落地首批，不是目标完成。继续优先处理可由现有 validator、contract reader 或行为测试覆盖的重复 fixture、快照和聚合用例。 | focused P2 tests + `test-lanes assert-coverage` + sibling App release-boundary validators；baseline 红的 lane（MAS consume-submission-package、MAS display materialization、RCA helper、OPL runtime-drilldown broader lane）只能记 blocker，不能计入净删或完成。 |
...
| 测试与 fixture surface | partial | 1.3-2.4% | 以 10-18 万行目标区间折算，当前可确认的安全落地量仍明显不足；2026-07-08 已追加 OPL descriptor/drilldown assertion 瘦身（typecheck、focused node tests、test-lanes coverage 通过，`-1,170`）、MAS paper-mission duplicate pytest collection 删除（9 pytest pass，`-123`）、App release-boundary shadow assertions 删除（6 node tests + release-boundary validator pass，`-65`）和 Aion e2e 空占位/README 收薄（Playwright list pass，`-16`）。 | 继续按语义 owner 分批删除 shadow coverage；10-18 万目标仍需 MAS paper-mission / study-progress、OPL runtime drilldown broader assertions、App/Aion UI duplication 等更大簇，但 baseline 红或同写集 dirty 的 lane 不能计入完成。 |
