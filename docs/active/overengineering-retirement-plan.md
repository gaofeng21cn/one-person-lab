...
| --- | --- | --- | ---: | --- |
| OPL | `47913390a` + `555a7f531` + `63bdc710e` + `b3615ce96` + `6afbda9b7` | domain progress transition runtime；runtime manager native cases；Codex default shell generated-surface helpers；runtime drilldown helper fixtures | `-329` | domain progress transition runtime：`22/22 pass`；runtime manager native：`11/11 pass`；Codex default shell：`21/21 pass`；runtime drilldown：`2/2 pass`；`npm run typecheck` |
| MAS | `2e361d8ec` + `3ccd05fcb` + `66c94dbac` | currentness / provider admission owner identity tests；AI reviewer routeback cases；runtime protocol study runtime cases | `-392` | owner/currentness：`27 passed`；routeback focused pytest：`21 passed`；runtime protocol focused pytest：`42 passed`；display QC helper candidate was reverted on MAS `main` and is not counted |
| RCA | `fb1cb34c` + `a630314a` + `f00f2b18` | native PPT Python layout fixture parts；RedCube Playwright mock helper；native PPT layout fixtures | `-324` | native PPT focused tests：`11/11 pass`；helper-referencing focused tests：`60/60 pass`；`npm run typecheck` |
| MAG | `c5cab6a` + `8f6a8fb` + `f7b8ad3` | product entry / workspace summary / critique executor tests；domain handler receipt assertion preservation；workspace summary cases | `-353` | affected product/workspace/critique focused tests：`51 passed, 60 subtests passed`；workspace summary focused pytest：`4 passed, 51 subtests passed`；`./scripts/verify.sh`：`234 passed, 351 subtests passed` |
...
Strict test-only landed total after main-session semantic repairs and reverted candidates: `-1,945` lines.
...
| 测试与 fixture surface：严格 test-only 吸收 | partial | 15% | 本轮已吸收并推送六仓严格测试瘦身：OPL `47913390a` / `555a7f531` / `63bdc710e` / `b3615ce96` / `6afbda9b7`、MAS `2e361d8ec` / `3ccd05fcb` / `66c94dbac`、RCA `fb1cb34c` / `a630314a` / `f00f2b18`、MAG `c5cab6a` / `8f6a8fb` / `f7b8ad3`、App `23d450e` / `d44dd99` / `72b490a`、OMA `6cbda67`，合计净删 `1,945` 行；所有计入候选均由主会话复核 write set 与 focused verification。 | 相对于历史 `10-18 万`目标，只能确认 `1,945 / 100,000` 的严格 test-only 证据；剩余大体量必须重新按语义候选审计，不能从旧 mixed snapshot 继承完成度。 |
