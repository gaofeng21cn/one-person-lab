...
| Fallow entrypoint / dead-export / cycle hygiene | partial | 98% | `.fallowrc.json` stale entrypoint 已纠正，`foreground-paths.ts` cycle tail 已切断；`2026-07-07` 追加删除私有 unused wrapper/source hygiene 文件后，public/shared-release lane 把 advisory unused files 降到 5，public/fallow lane 删除 `console/index.ts` 无消费者 re-export barrel，Connect lane 删除 `managed-shell-command-env.ts` wrapper 并收窄 registry part exports，Runway lane 追加切断 `family-runtime-providers -> family-runtime-temporal-provider -> family-runtime-temporal-activities -> family-runtime-scheduler -> family-runtime-providers` direct cycle；本轮 integration 把 GitHub action runtime、aggregate modules entrypoint 和 Console public entrypoint 加入 `.fallowrc.json` entry，删除 owner-answer one-line shim，CLI entrypoint support 删除 16 个 unused value exports 和 10 个 unused type exports，并把 `family-runtime-residency-proof.ts` 改为读取 Temporal worker lifecycle / production-proof thin surfaces。整合 `hygiene:fallow -- --format json` 读回 `unused_files=0`、`unused_exports=363`、`unused_types=109`、`circular_dependencies=25`、`total_issues=507`。 | fallow 仍是 advisory cleanup gate，不是 physical delete authority；剩余 dead exports 多为 public/dynamic/runtime/owner surface，剩余 SLO cycles 经 `family-runtime-provider-worker-repair.ts -> family-runtime-temporal-provider.ts`，clone/complexity groups 要按 active caller 和 owner route 分批处理，不能批量删除或写成 runtime/provider ready。 |
