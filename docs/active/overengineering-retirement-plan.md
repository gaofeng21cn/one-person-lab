...
- `2026-07-08`：P1 改为破后立执行：`family-runtime enqueue|queue|tick|intake`、queue hold/release/retire/redrive、default-executor local dispatch 和 scheduler tick CLI/App action 不再作为成功入口保留；Temporal external workflow/activity history、provider SLO/cadence readback 和 stage-attempt projection 是唯一可继续承接 runtime 读回的路径。
- `2026-07-08`：收薄剩余 false-ready readback：`runtime observability-export` 不再把 retired provider 或未配置 Temporal 输出成 ready。JSON readback 与 OpenMetrics 均 fail-closed：`provider_ready=false`、`diagnostic_provider_ready=false`、`local_sqlite_counts_as_provider_ready=false`，OpenMetrics 对当前 provider 输出 `opl_provider_ready 0`。
...
- `2026-07-08`：Temporal `SchedulerTickWorkflow/Activity` 只作为 provider cadence external history 名称保留；OPL 内部 result surface 改为 `opl_temporal_provider_cadence_readback`，activity compact receipt 返回 provider cadence summary 与 `retired_queue_tick=null`，不再返回 `family_runtime_scheduler_tick` 或本地 queue tick body。
- `2026-07-08`：`family-runtime attempt signal|cancel|start` 进入 Temporal-only fail-closed 边界；缺少 Temporal address 或 attempt provider 非 `temporal` 时不写本地 SQLite signal/cancel projection，只有 Temporal update/cancel/start 成功后才落 stage-attempt projection/readback index。
...
- 当前完成口径：P1 只允许把本地 SQLite 写成 stage-attempt projection/readback index 和 retired-provider negative guard；不能再把 local provider、本地 queue、scheduler tick、enqueue/intake/redrive 或 tests fixture 成功路径写成 active runtime。该结构证据仍不声明 external Temporal production readiness、domain ready、App release ready 或 owner receipt。
...
- `family-runtime CLI option parsing`：retired Runway family-runtime queue / scheduler 命令的局部分散 option loop 收到邻近 `parseCliOptions` thin helper。该 helper 保留现有 unknown option 与 error payload 行为；未引入新 CLI framework，也未把业务语义藏入 parser abstraction。
...
- 新鲜证据：lane 已 rebase 到 root `main` `2de376bd5` 并吸收到 root `main` commit `034e79f9b`；`runtime-environment-substrate-command-surface.test.ts` 为 `11/11` 通过；`npm run typecheck`、`git diff --check`、`npm run reuse-first:scan:diff -- --format json` 通过；fallow target 读回不再列出该文件的 parser clone/refactor target，仅剩既有 coverage/CRAP advisory。本记录只证明 parser 样板收薄和 focused CLI behavior 未回归，不声明 runtime/provider/domain/App ready。

## 2026-07-08 four-lane cleanup absorption

- `OPL Connect / Runway low-risk wrappers`：吸收 commit `b5623681b`，删除 `runtime-mag-manifest-sustained-consumption-command-spec.ts`、`local-codex-defaults.ts`、`opl-runtime-paths.ts` 三个薄 wrapper / alias surface；active imports 改为直连 canonical kernel / module owner；`family-runtime-current-control-provider-admission.test.ts` 拆出邻近 case module，关闭当日 test line-budget drift。
- `OPL Connect manifest / Runway production proof exports`：吸收 commit `03417a4db`，将无外部消费者的 first-party manifest collection、MAS convenience accessors、Temporal production probe helper 收回文件内部；保留 active `getAgentPackageManifestByModuleId()` 与 capability dependency read path。
- `OPL Connect managed update / ACP cycle`：吸收 commit `f2ccd2f08`，新增 `managed-update-owner-primitives.ts` 作为 leaf primitive owner，`managed-update-component-receipts.ts` 不再反向 import `managed-update-owner-boundary.ts`；`opl-acp-bridge.ts` 直接复用 kernel `JsonRecord`，不再从 ACP types 维护重复 JSON alias。
- 主线读回：`git diff --check HEAD~4..HEAD` 通过；`line-budget` 为 `status=ok`、`oversize_count=0`；`source:modules` 为 `status=ok`、`deep_import_violations.count=0`；`hygiene:fallow -- --summary` 仍为 advisory exit 1，但当前读回降到 `480 issues`、`355 unused exports`、`93 unused types`、`8 duplicate exports`、`24 circular dependencies`、`464 clone groups`。本记录只证明本轮四个 cleanup candidates 已吸收到 root `main` 且结构验证通过，不声明 release/currentness/runtime/provider/domain/App ready。
...
| 一行 wrapper / re-export | done | 100% | `Merge OPL wrapper cleanup` / `refactor: retire OPL wrapper re-exports` 已进入 `main`；`2026-07-07` 追加删除 Connect managed-shell 薄 wrapper 和 Console public re-export barrel；semantic cleanout follow-up 将 Agent Lab complete 收成 public facade，并把 product-entry / scaffold constants star barrel 改为显式 export；`2026-07-08` 再删除 runtime MAG manifest alias、Connect local Codex defaults wrapper、Runway runtime path wrapper，并收回一批 unused export surface。 | 无本会话继续项；production/live evidence 不适用。 |
| Runway local scheduler / queue tail | partial | 95% | `2026-07-08` lane 正在物理删除 local scheduler / queue / intake / enqueue / tick / redrive 成功路径，并把 SQLite sidecar 改成 stage-attempt projection/readback index；CLI/help/contracts/tests 以 retired/unknown-command guard 覆盖旧入口。 | 需要本轮 final fresh verification 和 main absorption 后才能改成 done；外部 Temporal worker/owner receipt/live long-soak 仍是后置证据，不能由结构删除替代。 |
...
| CLI parser tail | done | 100% | `2026-07-07` retired family-runtime queue / scheduler / attempt / provider / service-worker / lifecycle / paper-autonomy / evidence-worklist / stage-artifact option parsing 收到邻近 thin helper，focused parser tests 与 typecheck 作为行为证据；`2026-07-08` runtime env command parser 继续收薄为本文件局部 visitor / target-option helper，focused CLI surface `11/11` 通过。 | 仍需继续按新增 diff 守住 command registry；不为统一指标引入新 CLI framework。 |
| Fallow entrypoint / dead-export / cycle hygiene | partial | 98% | `.fallowrc.json` stale entrypoint 已纠正，`foreground-paths.ts` cycle tail 已切断；`2026-07-07` 多轮删除私有 unused wrapper/source hygiene 文件、收窄 Connect registry part exports、切断 Runway direct cycle、删除 owner-answer shim、收回 CLI support/type barrel、Connect package internals、Runway runtime env parts 与 Ledger MAG TS alias；semantic cleanout follow-up 再把两个 star barrel 改为显式 export；`2026-07-08` 主线继续收回 unused exports、删除低风险 wrappers，并把 Connect managed-update receipt/boundary cycle 降为 leaf primitive import。最新 `npm run hygiene:fallow -- --summary` 仍为 advisory exit 1：`480 issues`、`355 unused exports`、`93 unused types`、`8 duplicate exports`、`24 circular dependencies`、`464 clone groups`。 | fallow 仍是 advisory cleanup gate，不是 physical delete authority；`reuse-first:scan --summary` 的剩余 `529` 项全为已决策 worklist，其中 `512` 个 `allowed_projection_boundary`、`17` 个 `owner_decision_required`。继续物理删除需要 owner receipt、typed blocker、route-back 或 live evidence，不能批量删除或写成 runtime/provider ready。 |
