...
| P2 | 测试与 fixture surface | 体量目标仍未完成。当前推进范围排除 MedOPL；只统计 OPL、MAS、Aion shell、RCA、MAG、App、OMA。2026-07-09 本轮连续 landing 已吸收 25 个 mainline 测试瘦身提交，按 commit diff 净删约 `30,732` 行。最新补充吸收：OPL `4be5ed0d7`、`2deed4a13`；MAS `79837791e`、`e6413a568`；MAG `11971c8`；App `a6c8b9f`、`08e44ad`；OMA `5528454`。按 fresh tracked-test 统计口径（`tests/` 下文件 + 根级 `test_*/.test/.spec` 文件），当前仍有 `2,377` 个文件、`670,067` 行：OPL `160,375`、MAS `336,532`、Aion `58,007`、RCA `52,388`、MAG `35,450`、App `15,291`、OMA `12,024`。按 `10-18 万` 首批目标折算，本轮只覆盖约 `17-31%`；剩余约 `6.9-14.9 万` 行必须继续分类为 `delete / merge / rewrite / keep / owner-blocked`，不能把旧测试会红当成保留理由。 | focused P2 tests + `test-lanes assert-coverage` + sibling App/Aion/RCA/MAS repo-native validators；旧测试变红不是最终否决理由，必须区分必要覆盖缺失、旧 wrapper/文案断言被删除、实现细节绑定、或 helper 改坏行为；同写集 dirty 或需要真实 owner 降低覆盖粒度的 lane 只能记 blocker/owner decision。 |
...
| 测试与 fixture surface | partial | 17-31% | 以 10-18 万行目标区间折算，本轮可确认的 mainline 净删约 `30,732` 行，仍不足以声明完成。已吸收并验证的提交：OPL `86e691c44`、`fcf524851`、`b09cdabae`、`4be5ed0d7`、`2deed4a13`；MAS `42403289d`、`ed67a7d23`、`cf1818f82`、`922417a03`、`79837791e`、`e6413a568`；Aion `62f8d226c`、`c032fdb94`；MAG `4745dba`、`99140d2`、`11971c8`；App `6209229`、`17db5a8`、`a6c8b9f`、`08e44ad`；RCA `ecbd2375`、`3f902c81`；OMA `22b3c8c`、`a0f13e2`、`5528454`。当前剩余 tracked test surface：`2,377` 文件 / `670,067` 行。 | 10-18 万目标剩余差距约 `6.9-14.9 万` 行，主要在 active semantic coverage。后续每个候选必须分类为 `delete / merge / rewrite / keep / owner-blocked`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
