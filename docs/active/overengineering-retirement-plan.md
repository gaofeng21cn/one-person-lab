...
| P2 | 测试与 fixture surface | 体量目标仍未完成：2026-07-08 继续落地只新增 verified test-tree 净删 `979` 行（MAS outer-loop `-679`、OPL workspace-domain `-300`）；Aion / RCA 前序 smoke fixture 收薄已落地，但距离 10-18 万行测试内容首轮瘦身仍是 partial。继续优先处理可由现有 validator、contract reader 或行为测试覆盖的重复 fixture、快照和聚合用例。 | focused P2 tests + `test-lanes assert-coverage` + sibling App release-boundary validators；baseline 红的 lane（MAS paper-mission command、MAS study-progress、RCA helper、OPL runtime-drilldown）只能记 blocker，不能计入净删或完成。 |
...
| 测试与 fixture surface | partial | 30-54% | 以 10-18 万行目标区间折算，当前可确认的安全落地量仍低于目标上界；2026-07-08 追加落地证据为 MAS `aa738373c`（outer-loop publication eval fixtures，30 pytest pass，`-679`）和 OPL `9ce9ade36`（workspace-domain stage fixtures，23 node tests pass，`-300`）。 | 继续由 `OPL family 合理重构巡检` 按 validator/contract/behavior owner 删除 shadow coverage；先修 baseline 红 lane，再处理 MAS paper-mission / study-progress、RCA helper、OPL runtime-drilldown 等高体量簇。 |
