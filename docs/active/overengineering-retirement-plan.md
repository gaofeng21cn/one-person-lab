...
| Fallow entrypoint / dead-export / cycle hygiene | partial | 97% | `.fallowrc.json` stale entrypoint 已纠正，`foreground-paths.ts` cycle tail 已切断；`2026-07-07` 追加删除私有 unused wrapper/source hygiene 文件后，public/shared-release lane 把 advisory unused files 降到 5，public/fallow lane 删除 `console/index.ts` 无消费者 re-export barrel，Connect lane 删除 `managed-shell-command-env.ts` wrapper 并收窄 registry part exports，Runway lane 追加切断 `family-runtime-providers -> family-runtime-temporal-provider -> family-runtime-temporal-activities -> family-runtime-scheduler -> family-runtime-providers` direct cycle；本轮 fallow-entrypoints lane 把 GitHub action runtime、aggregate modules entrypoint 和 Console public entrypoint 加入 `.fallowrc.json` entry，并删除 owner-answer one-line shim。 | fallow 仍是 advisory cleanup gate，不是 physical delete authority；dead exports、clone groups 与 Runway worker-repair / provider-SLO / current-control / Temporal client-visibility cycles 要按 active caller 和 owner route 分批处理，不能批量删除或写成 runtime/provider ready。 |
