...
| 测试与 fixture surface：10 万最低线 | done | 100% | 2026-07-08 起非 Aion mainline slim/shrink 提交按 commit numstat（含必要 test-lane metadata/source references）净删 `207,095` 行，超过 `10 万`最低线。最新已推远端：OPL `751ed6c93`（focused node tests `138/138`、`test-lanes assert-coverage`、`npm run typecheck`）、MAS `3721050ae`（final focused `20/20`，此前 combined `220/220`）、MAG `eccaf87`（47 pass / 20 subtests）、App `4537402`（62/62 release tests）、OMA `b738486`（touched 15/15、behavior 37/37、smoke 28/28、typecheck）。当前非 Aion tracked test surface：`458,780` 行。 | 最低线已关闭；后续不能再用“离 10 万还差”作为 blocker。 |
| 测试与 fixture surface：18 万上沿 | done | 100% | 同一 fresh commit-numstat 口径已完成 `207,095 / 180,000`，超过上沿目标。剩余测试代码仍可继续按新候选瘦身，但不再属于首批 `10-18 万` 目标缺口。RCA further pass 仍因 `tests/**` ownership/owner-route 不清暂列 owner-blocked；MAS product/CLI 候选因 focused product-entry validation `69 failed, 167 passed` 被拒绝、未吸收。 | 后续如继续追更深瘦身，必须重新列候选并分类为 `delete / merge / rewrite / keep / owner-blocked`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
| Aion upstream fork 边界 | done | 100% | 用户明确 `opl-aion-shell` 是 fork 的 AionUI，上游主要维护；`opl-aion-shell/main` 已精确对齐 `gh-https/main` HEAD `ea56f2eed`，误动本地提交仅保留在本地备份分支 `codex/backup-aion-d685b0b9-before-upstream-reset` 与 `codex/backup-aion-e1e18a05-before-upstream-reset`。后续统计与执行排除 `opl-aion-shell` 和 App 内 `shells/aionui/**` 上游体，除非目标明确是 OPL-owned overlay。 | 不再对 Aion upstream body 做测试瘦身、结构重写或实现清理；只允许 read-only 核查、误动回退、或 OPL overlay 边界内改动。 |
