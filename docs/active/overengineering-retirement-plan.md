...
| P2 | 测试与 fixture surface | 体量目标仍未完成：已删除一批 prose / workflow / field-echo / shadow assertion 测试，但 10-18 万行测试内容的首轮瘦身还没有达到可关闭状态；继续优先处理可由现有 validator、contract reader 或行为测试覆盖的重复 fixture、快照和聚合用例。 | focused P2 tests + `test-lanes assert-coverage` + sibling App release-boundary validators；不把 line-budget 清零、局部测试绿或小额净删写成测试体量目标完成。 |
...
| 测试与 fixture surface | partial | 5% | P2 仍是 refactor_patrol 主线；本文件只保留目标和 forbidden claims，不冻结行数、净删、test count 或 advisory snapshot。 | 继续由 `OPL family 合理重构巡检` 按 validator/contract/behavior owner 删除 shadow coverage。 |
