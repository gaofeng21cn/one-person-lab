...
| 测试与 fixture surface：10 万最低线 | done | 100% | 2026-07-08 起非 Aion mainline slim/shrink 提交按 commit numstat（含必要 test-lane metadata/source references）净删 `208,517` 行，超过 `10 万`最低线。最新已推远端：OPL `751ed6c93`（focused node tests `138/138`、`test-lanes assert-coverage`、`npm run typecheck`）、MAS `3721050ae`（final focused `20/20`，此前 combined `220/220`）、MAG `9db7a0f0`（5 pytest pass、`make test-source-purity-strict`）、App `bcd4acdd`（release boundary `115 pass / 2 skip`）、OMA `ebf41e24`（owner-evidence tests `5/5`、`npm run typecheck`）、RCA `4c379d42`（functional audit tests `2/2`、private-platform readback passed）。当前非 Aion tracked test surface：`457,358` 行。 | 最低线已关闭；后续不能再用“离 10 万还差”作为 blocker。 |
| 测试与 fixture surface：18 万上沿 | done | 100% | 同一 fresh commit-numstat 口径已完成 `208,517 / 180,000`，超过上沿目标。剩余测试代码仍可继续按新候选瘦身，但不再属于首批 `10-18 万` 目标缺口。RCA/MAG/App/OMA 本轮补充 lanes 已吸收并清理；MAS product/CLI 候选因 focused product-entry validation `69 failed, 167 passed` 被拒绝、未吸收。 | 后续如继续追更深瘦身，必须重新列候选并分类为 `delete / merge / rewrite / keep / owner-blocked`：旧 wrapper/文案/实现细节断言可删或改写，必要 authority/currentness/no-authority 覆盖要用更小测试或 contract/readback 替代，真实 owner 降低覆盖粒度才需要 human decision。 |
