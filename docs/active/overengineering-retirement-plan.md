...
| Fallow entrypoint / dead-export / cycle hygiene | partial | 94% | `.fallowrc.json` stale entrypoint 已纠正，`foreground-paths.ts` cycle tail 已切断；`2026-07-07` 追加删除 3 个私有 unused wrapper/source hygiene 文件后，`hygiene:fallow` unused files 为 6；public surface / shared-release 收薄后 advisory unused files 降到 5；public/fallow hygiene lane 删除 `console/index.ts` 无消费者 re-export barrel，并用 source-module contract/workflow 证据判定 `src/modules/index.ts`、`src/modules/console/index.ts` 和 `.github/actions/quality-details/emit-quality-details.mjs` 文件本体保留。 | fallow 仍是 advisory cleanup gate，不是 physical delete authority；剩余 5 个 unused files 中，Connect 文件属禁写集，owner-answer projection 需单独 owner/caller 复核，模块 aggregate / Console public entry / GitHub action runtime surface 已 rejected for physical delete；dead exports、clone groups 仍需按 owner surface 分批处理。 |
