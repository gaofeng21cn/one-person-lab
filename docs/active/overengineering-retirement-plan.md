...
- `未处理项`：`src/modules/console/app-state-mas-owner-answer-projection.ts` 是 owner-answer projection surface，仍需单独 owner/caller 复核，不在本轮低风险 barrel 删除范围。`hygiene:fallow` 允许继续非零；本轮完成的是可证无消费者的 Console re-export surface 收薄和剩余 allowed public/runtime surfaces 的保留判定。
- 新鲜证据：`npm run typecheck` 通过；`npm run source:modules -- --format json` 为 `status=ok`、`module_entrypoints.expected_count=10`、`deep_import_violations.count=0`、`forbidden_dependency_violations.count=0`；`npm run hygiene:fallow` 仍为 exit 1，后续 Connect lane 删除 `src/modules/connect/managed-shell-command-env.ts` 后 unused files 降到 4，残留均有上述保留或待 owner 复核理由；`git diff --check` 通过。
