...
| Fallow entrypoint / dead-export / cycle hygiene | partial | 90% | `.fallowrc.json` stale entrypoint 已纠正，`foreground-paths.ts` cycle tail 已切断；`2026-07-07` 追加删除 3 个私有 unused wrapper/source hygiene 文件后，`hygiene:fallow` unused files 为 6。 | fallow 仍是 advisory cleanup gate，不是 physical delete authority；剩余 public/dynamic/action/owner surfaces、dead exports 与 clone groups 要按 active caller 和 owner route 分批处理，不能批量删除。 |
