...
| 测试与 fixture surface | done | 100% | P2 wrapper tests、line-budget readback、focused App drilldown/App release/evidence worklist tests、family-runtime provider-hosted-attempts tests、`test-lanes assert-coverage` 和 `reuse-first-scan --summary` 证明已删除一批重复 prose/assertion/projection surface；剩余 `phase10-test-and-fixture-projections=387` 全部为 `allowed_projection_boundary`，不是未分类 backlog。 | residual watch-only：package/update 相关项由另会话处理，runtime protocol fixture 保留真实 queue 语义，真实 CLI/schema/projection 名不为清零 scan 而重命名。不声明 runtime/domain/App release ready。 |
