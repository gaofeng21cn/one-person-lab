...
因此不存在通用“防跳”流程：readback evaluator 只接受并投影 Codex 已选择的 declared stage、raw/typed artifact、quality debt 和 route-back refs，不拒绝 skip、逆序、repeat 或缺前序 receipt。它不能调度 Stage、创建 domain blocker，也不能用 provider completion、测试通过或单个 Stage closeout 冒充 quality/delivery/ready claim。
