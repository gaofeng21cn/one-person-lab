...
当前不是“只剩跑真实论文”。OPL 已具备完整生产级智能体框架的控制面骨架；2026-05-13 本轮已经把主要 functional closure surface 收口到机器可读 contracts、refs、focused tests、typed blockers 和跨仓 status/docs。但原计划里有几项“功能性 follow-through”仍不能写成已完成：破坏性物理目录迁移、真实 memory body / writeback apply、legacy 最终物理删除、统一 domain-agent descriptor CLI 和真实生产 provider readiness 仍需要进一步 evidence gate。
...
- OPL agents read model 能区分 descriptor readiness、physical skeleton layout audit 与 production closure gaps；2026-05-13 fresh `opl agents list --json` 为 `aligned_count=3`、`missing_count=0`、`drift_detected_count=0`、`physical_skeleton_audit_pending_count=3`、`production_closure_gap_count=15`。这表示三仓 descriptor 已 aligned，domain repos 已提供/声明 skeleton anchor 或 follow-through proof surfaces；不表示 OPL 或三仓已完成破坏性物理目录迁移。
...
- `functional follow-through gates`：physical skeleton 的破坏性目录搬迁、legacy residue 的最终物理删除、真实 workspace/runtime memory body migration、统一 domain-agent descriptor CLI，以及长时 provider/operator SLO 校准。RCA review helper 的 line-budget baseline removal 与 geometry/summary/report 拆分已完成；后续 screenshot capture 可自然拆分，但不再是本轮 blocker。

## 距理想状态还有多远

工程判断保持：`65-75% framework/control-plane landed`，`25-35% production closure remaining`。如果只看“排除真实论文/真实交付长时 soak 后，可由工程实现闭合的功能性缺口”，本轮后多数功能性 surface 已落地，但不能声明全部完成；剩余约 `5-10%` follow-through 主要是破坏性物理目录迁移、真实 memory body / writeback apply、legacy 物理删除、统一 descriptor CLI 和 provider readiness / SLO 校准这类必须等 no-active-caller、path parity、owner receipt、provenance 或 live readiness 证据的收尾。

| 理想目标 | 当前距离 |
| --- | --- |
| OPL 成为完整生产级智能体框架 | 控制面、contract、attempt、provider code、workbench、descriptor discovery、controlled apply projection 和 production closeout gate 已落地；剩余距离主要是真实 production residency、长时 operator SLO、统一 descriptor CLI 和真实 domain attempt evidence。 |
...
| domain repo 目录结构标准化 | OPL 已定义 `agent/ contracts/ runtime/ docs/` skeleton；三仓已声明/提供 skeleton anchor 或 follow-through proof surface。fresh OPL read model 仍显示 `physical_skeleton_audit_pending_count=3`，完整物理重组仍是 follow-through，workspace artifacts、memory body 和 receipt instances 不进 repo source。 |
...
| `physical-skeleton-follow-through` | MAS/MAG/RCA | `descriptor_aligned_physical_layout_pending` | 三仓已有 physical skeleton audit/proof surface，OPL read model 显示三仓 descriptor aligned 但 physical layout pending；破坏性 path move 后续按 parity/provenance/no-forbidden-write 证据推进。 |
...
- `unified_domain_agent_descriptor_cli`：`agents list`、`stages list`、`domain-memory list` 可用；`node dist/cli.js agents descriptors --json` 当前不是稳定子命令，统一聚合入口仍待补。
- `physical_path_migration`：三仓 physical skeleton anchor / follow-through proof surface 已存在；完整物理 `agent/ contracts/ runtime/ docs/` 目录迁移仍需 direct skill path、OPL-hosted path、restore/provenance proof 和 no-forbidden-write proof。
