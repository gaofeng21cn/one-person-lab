...
当前不是“只剩跑真实论文”。OPL 已具备完整生产级智能体框架的控制面骨架；2026-05-13 本轮已经把主要 functional closure surface 收口到机器可读 contracts、refs、focused tests、typed blockers 和跨仓 status/docs。canonical `./bin/opl agents descriptors --json` 统一 domain-agent descriptor CLI 已可用。但原计划里仍有几项“功能性 follow-through”不能写成已完成：破坏性物理目录迁移、真实 memory body / writeback apply、legacy 最终物理删除、release/dist 构建口径同步和真实生产 provider readiness 仍需要进一步 evidence gate。
...
- OPL agents read model 能区分 descriptor readiness、physical skeleton layout audit、physical skeleton evidence 与 production closure gaps；2026-05-13 fresh `./bin/opl agents list --json` 为 `aligned_count=3`、`missing_count=0`、`drift_detected_count=0`、`physical_skeleton_evidence_observed_count=2`、`physical_skeleton_audit_pending_count=1`、`production_closure_gap_count=15`。这表示三仓 descriptor 已 aligned，MAS/MAG 已有 repo-source anchor evidence，RCA 仍 pending；不表示 OPL 或三仓已完成破坏性物理目录迁移。
...
- MAG/RCA 已在各自 main 上落地 functional closure proofs：domain owner receipt envelope、domain memory accepted/rejected receipt refs、lifecycle guarded apply refs、controlled soak typed blocker 和文档/status 对齐。MAG 已有 minimum repo-source anchor evidence；RCA 仍需补 physical skeleton anchor evidence。
...
- `functional follow-through gates`：physical skeleton 的破坏性目录搬迁、RCA repo-source anchor evidence、legacy residue 的最终物理删除、真实 workspace/runtime memory body migration、release/dist CLI 口径同步，以及长时 provider/operator SLO 校准。RCA review helper 的 line-budget baseline removal 与 geometry/summary/report 拆分已完成；后续 screenshot capture 可自然拆分，但不再是本轮 blocker。

## 距理想状态还有多远

工程判断保持：`65-75% framework/control-plane landed`，`25-35% production closure remaining`。如果只看“排除真实论文/真实交付长时 soak 后，可由工程实现闭合的功能性缺口”，本轮后多数功能性 surface 已落地，但不能声明全部完成；剩余约 `5-10%` follow-through 主要是破坏性物理目录迁移、真实 memory body / writeback apply、legacy 物理删除、RCA physical skeleton evidence、release/dist CLI 同步和 provider readiness / SLO 校准这类必须等 no-active-caller、path parity、owner receipt、provenance 或 live readiness 证据的收尾。

| 理想目标 | 当前距离 |
| --- | --- |
| OPL 成为完整生产级智能体框架 | 控制面、contract、attempt、provider code、workbench、descriptor discovery、统一 descriptor CLI、controlled apply projection 和 production closeout gate 已落地；剩余距离主要是真实 production residency、长时 operator SLO、release/dist CLI 同步和真实 domain attempt evidence。 |
...
| domain repo 目录结构标准化 | OPL 已定义 `agent/ contracts/ runtime/ docs/` skeleton；fresh OPL read model 显示 `physical_skeleton_evidence_observed_count=2`、`physical_skeleton_audit_pending_count=1`，MAS/MAG 已有 repo-source anchor evidence，RCA 仍 pending。完整物理重组仍是 follow-through，workspace artifacts、memory body 和 receipt instances 不进 repo source。 |
...
| `domain-memory-apply-generalization` | MAS/MAG/RCA | `descriptor_and_receipt_projection_landed` | MAS/MAG/RCA 都暴露 descriptor、receipt locator/projection 或 typed blocker；真实 memory body、retrieval apply 和 writeback truth 仍归 domain owner。 |
...
| `physical-skeleton-follow-through` | MAS/MAG/RCA | `partial_anchor_evidence_observed` | MAS/MAG 已有 repo-source anchor evidence，RCA 仍 pending；三仓 descriptor aligned，破坏性 path move 后续按 parity/provenance/no-forbidden-write 证据推进。 |
...
- `temporal_production_readiness_and_long_residency_evidence`：当前 fresh family runtime 仍选中 `local_sqlite`，`full_online_ready=false`、`durable_online_ready=false`；需要真实本机/外部 managed Temporal service + worker 的 readiness、长时 residency、restart/re-query、signal/history 和 operator repair loop 证据。
...
- `domain_memory_body_apply`：三仓 descriptor/receipt projection 已存在，但 OPL fresh read model 的 retrieval/writeback/body migration 仍不是 OPL-applied success；真实 body、accept/reject 和 writeback apply 继续由 domain workspace/runtime owner 闭合。
- `unified_domain_agent_descriptor_cli`：canonical `./bin/opl agents descriptors --json` / `./bin/opl agents descriptor --domain mas --json` 已可用；后续是保持 release/dist 构建输出、App consumption 和专题 drilldown 口径同步。
- `physical_path_migration`：MAS/MAG physical skeleton anchor evidence 已存在，RCA 仍 pending；完整物理 `agent/ contracts/ runtime/ docs/` 目录迁移仍需 direct skill path、OPL-hosted path、restore/provenance proof 和 no-forbidden-write proof。
