...
Machine boundary: 本文是人读执行计划与验收矩阵。机器真相继续归 `contracts/opl-framework/brand-module-registry.json`、`opl brand-modules * --json`、`opl contract validate`、source、测试、runtime ledger、provider receipt、domain-owned manifest 和真实 App/workspace evidence。
...
| `cli_surface_ref` | 至少一个真实 CLI 或统一 `opl brand-modules ...` inspection surface。 |
...
`opl brand-modules validate --json` 是本计划的最小机器验收；`opl contract validate --json` 证明 registry 已进入 required framework contract set。
