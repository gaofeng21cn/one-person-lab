...
| 优先级 | 重构点 | 理想目标 |
| --- | --- | --- |
| P0 | `owner_delta_contract_as_single_root` | 把 `current_owner_delta` 从 policy / projection 进一步提升成所有 default read surface 的唯一 canonical object；framework readiness、App fast state、runtime tray、evidence-worklist summary 和 Foundry Kernel improvement input 都从它派生。 |
...
| OMA | `DesignRequest -> AgentBlueprint/EvalSpec`；`EvidenceBundle -> EvolutionProposal`。 | 第二 Framework / 第二 Foundry Kernel、评测执行 owner、worktree lifecycle owner、版本/晋级 owner、物化器。 |
...
| `authority_boundary` | OPL/App/Foundry Kernel 只做 transport、projection、guard、generated surface、候选物化、FoundryRun / 评测执行、`EvidenceBundle` 与版本治理；MAS/MAG/RCA verdict 留给对应 domain owner，target Agent 的保护测试正文、最终验收、权限与生产采用留给 target owner。 | OPL/App/OMA/doctor/schema/provider completion 机械替代 paper、grant、visual 或 target Agent 的质量、ready、artifact authority 或 adoption decision。 |
...
```text
User chooses a MAS/MAG/RCA domain purpose
...
failed/blocked attempt
  -> Foundry Kernel reads artifact/receipt/audit refs
  -> OPL creates an exact-version improve DesignRequest
  -> Foundry Kernel freezes an EvidenceBundle for that exact baseline or candidate
  -> Foundry Kernel invokes OMA through engineer-agent
  -> OMA internally diagnoses EvidenceBundle and returns EvolutionProposal with a complete next blueprint
  -> Foundry Kernel materializes and evaluates a new immutable candidate
  -> target owner returns receipt/typed blocker
  -> no domain truth written by Foundry Kernel or OMA
```
...
```text
DesignRequest(create|takeover|improve)
  -> User/Codex calls OMA engineer-agent
  -> OMA internally performs design
  -> AgentBlueprint + EvalSpec
  -> Foundry Kernel materializes and evaluates an immutable candidate
  -> EvidenceBundle
  -> Foundry Kernel invokes OMA through the same engineer-agent entry when diagnosis is needed
  -> OMA internally performs diagnose
  -> EvolutionProposal + complete next blueprint
  -> target owner approves, rejects, or requests revision
  -> Foundry Kernel qualifies, canaries, activates, or rolls back according to the owner decision
```

Kernel/operator-only surfaces, not OMA affordances:

- provider implementation diagnostics；
- operator FoundryRun/version inspection；
- reviewer/test execution and orchestration；
- candidate materialization and worktree lifecycle；
- version, qualification, canary, activation and rollback。

OMA 的唯一 public action 是 `engineer-agent`，`design` / `diagnose` 只是该入口背后的内部 operation。OMA 只拥有目标理解、设计依据、`AgentBlueprint`、`EvalSpec`、证据诊断和 `EvolutionProposal`；它不执行测试、不编排 reviewer、不修改 target repo、不管理 worktree、不物化候选，也不持有版本、qualification、canary、activation 或 rollback。Foundry Kernel 承担这些执行与生命周期职责，target owner 持有保护测试正文、最终验收、权限授权和生产采用决定。
...
CLI、App、runtime tray、Foundry Kernel、safe action shell 都是 thin adapters。它们不各自判断 readiness，也不各自读取 raw state 拼结论。
...
| Phase | 当前读法 |
| --- | --- |
| Design target | 目标架构已冻结为 `OPL resource model + current_owner_delta + Cognitive Computation Kernel + Stage Artifact Unit + Passive passive evidence ledger + App Cockpit + Foundry Kernel improvement control plane`。 |
...
| Foundry Kernel loop | OMA 只产出完整 blueprint/eval semantics 或 evolution proposal；Foundry Kernel 产出 immutable candidate、EvidenceBundle、risk gate、version/canary/activation/rollback records；target owner 关闭领域质量与生产采用决定。 |
