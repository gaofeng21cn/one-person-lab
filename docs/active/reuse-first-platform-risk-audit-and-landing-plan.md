...
本文最初按复用优先原则给出结构风险审计和一步到位计划；截至 2026-07-05，已按该计划并行吸收多批 first-slice 代码 / 合同 / readback / source-boundary lanes 回 `main`。这些切片把 reuse-first gate、schema/CLI boundary、Runway Temporal-first readback 与 live test-server residency proof、reconciler safe action / safe-action source convergence、managed update owner boundary、Pack/Workspace content-addressed policy 与 hard-blocker precedence、domain-tail matrix、App/Aion consumer-only readback、observability semantic conventions、HTTP `/metrics` endpoint、Collector smoke readback、Connect provider receipts、Connect external skill metadata、Connect agent package registry / manifest / lock / receipt readback、Agent Lab self-evolution work orders、Ledger artifact provenance、source-module owner alignment、family action catalog owner、source-ref kernel helper、runtime snapshot provider injection，以及 Charter / Console / managed update / OMA JSON boundary helper 迁移进可执行维护面；但它们仍不是 OPL production ready、domain ready、App release ready、owner acceptance、full historical risk eliminated 或 physical delete authorization。后续真正关闭每个风险项仍必须逐项替换 active caller，并用 source/contract/CLI/runtime/live owner evidence 分账验收。
...
Fresh readback：full scan 当前为 `finding_count=709`、`hard_gate_finding_count=242`、`advisory_finding_count=467`、`open_worklist_finding_count=0`、`blocking_worklist_finding_count=0`；目标三类为 `custom_runtime_queue=147`、`custom_update_or_package_manager=93`、`custom_observability_ledger=467`。`phase4-src-modules-runtime-queue=0`、`phase6-src-modules-update-package=0`、`phase10-src-modules-observability=0`，含义只是 active `src/modules` 已没有被 scanner 归到这三条 migration item 的剩余命中；不等于 Temporal durable lifecycle、App release owner、package owner、OTLP/exporter/live endpoint 或 domain owner acceptance 完成。
...
| `capability_package_channel_owner_route` | OPL Pack / Connect package owners | 无 owner package lifecycle receipt。 | live owner receipt / typed blocker 仍缺；但 `connect agent-packages {validate-manifest,install,update,repair,rollback,uninstall,list,status}` 已落统一 `owner_route_readback`，显式暴露 descriptor/digest/lock/materializer/no-domain-authority/no-package-manager 结构读回。 | 继续通过 Pack/Connect owner surface 路由 package lifecycle；结构读回已就位，不得把它包装成 owner receipt。 |
...
Fresh owner evidence audit：`contracts/opl-framework/reuse-first-historical-worklist.json#owner_live_evidence_preflight_2026_07_05.fresh_evidence_audit_2026_07_05` 已按本仓 scan、App release/product surfaces、Pack/Connect/Observability/Domain-tail refs 执行本地读回；结果为 `audit_result=no_owner_live_evidence_found`，5/5 `evidence_found=false`，`can_mark_preflight_closed=false`。该审计只证明本轮没有发现可关闭 owner-live-evidence 的 authority 证据，不替代 owner receipt、typed blocker、route-back 或 live endpoint proof。
...
| --- | --- | ---: | --- | --- | --- |
| 1 | Reuse-first governance gate | 97% | `partial` | 已落 `contracts/opl-framework/reuse-first-governance.json`、`contracts/opl-framework/reuse-first-historical-worklist.json`、`scripts/reuse-first-scan.mjs`、`npm run reuse-first:scan`、`npm run reuse-first:scan:diff` 与 `./scripts/verify.sh reuse-first`；full scan 现在输出 historical decision/worklist readback；maintainer scripts 已新增共享 JSON/helper 与标准参数解析 helper，`phase0-maintenance-script-boundaries` 当前为 0；`scripts/test-lanes.mjs` 的 23 个 advisory observability hits 已单列为 test lane file catalog projection，不再混入 update/package owner-decision worklist；Agent Lab developer-mode split 后对应 test lane 已注册；本轮已把 8 条超线测试 reviewed baseline 全部拆到 `cases/` 子文件，`source-structure-budget` 当前 `reviewed_baselines=[]`；GitHub Verify 的 lint/structure job 会跑 strict diff gate；本轮 `reuse-first-scan` 继续保持 `open_worklist_finding_count=0`、`blocking_worklist_finding_count=0`，full scan 当前为 `finding_count=709`。 | 继续逐项消化 worklist 中的 migration / owner-decision findings；不要把 worklist 分类或 clean diff gate 当作全仓历史风险已清零。 |
...
| 3 | CLI parser/command registry | 79% | `partial` | 已落最小 `CommandSpec.registry` 与 `validateCommandRegistryCoverage` adapter，`connect pubmed search`、`connect references verify`、`connect external-skills list/search/inspect/sync`、`connect install/update/reinstall/remove` module actions、`status workspace/runtime/dashboard`，以及 `update status/check/plan/apply/repair/rollback` 都进入 registry；status 与 update 命令已复用 registry parser adapter；`runtime manager` / `runtime manager action` 与 `runtime app-operator-drilldown` 已进入 command registry，`runtime manager action` 与 `runtime app-operator-drilldown` 参数走现有 `node:util parseArgs` registry adapter；maintainer scripts 的重复参数解析已集中到 `scripts/required-value-options.mjs`；private runtime command specs 已拆到 `parts/runtime.ts` / `parts/agents.ts`，总入口降到 650 行；本轮又把 Stagecraft `stages list/inspect/readiness/proof-bundle/graph/assumptions/cohort-loop/runtime-budget/registry/source-spec/replay-certification` 纳入 `cli-command-registry.json` protected / required command set，Stage command specs 复用 registry parser adapter 拦截未知参数，registry 测试读取合同改为复用 shared JSON boundary；本轮继续删除已无 active caller 的 `parseRuntimeAppOperatorDrilldownArgs` / `RuntimeAppOperatorDrilldownCliInput` 手写 parser/type/export 尾巴；`phase2-cli-entrypoint-boundaries` 当前为 0，owner-routed rollback command projection 已由 scanner allow 收敛为 0，entrypoint 层剩余 diagnostic drilldown 词项从 20 降到 18，单列为 refs-only command projection。 | 继续把 remaining public/runtime diagnostic command specs 纳入 registry；只有当 Commander/Yargs 能减少现有 parser 分散度时再引入依赖。 |
...
| 5 | Kubernetes-style reconciler | 68% | `partial` | `family-runtime lifecycle reconcile` 已输出 desired_state / observed_state / reconcile_decision / next_safe_action，mutation 只允许进入 lifecycle apply receipt projection，禁止写 domain truth、artifact body、owner receipt 或 typed blocker；`opl runway reconcile --json` 现在把 `queue_lifecycle_boundary` 作为 observed_state，在 local SQLite lifecycle 与 Temporal 竞争时只输出 `observe_queue_lifecycle_boundary` readback，不给 scheduler tick mutation；Runway control-loop 在 queue handoff gate attention 时只给 read-only observation；本轮新增 `action_source_convergence` readback，明确 scheduler、worker、App 与 domain helper 都不能绕过 canonical safe-action source。 | 继续把 remaining worker/App/domain helper active caller 的 mutation 入口收敛到同一 Reconciler safe-action source，并补真实 owner/readback evidence。 |
| 6 | Managed update split | 80% | `partial` | App release channel 与 managed update plane 边界存在；已增加 `owner_route_contract`、`owner_execution_boundary_contract`、组件级 `owner_route` / `owner_execution_boundary` 与 receipt `owner_projection`，把 app binary / runtime substrate / capability packages 明确路由到 owner/readback/apply owner，并保留 no-package-manager forbidden claims；receipt JSON 读取已复用共享 record boundary，不再在 managed update 模块私写 `optionalString` / `optionalStringArray` helper；`ManagedUpdateOperation`、provider ids、component model、owner route、owner execution boundary、receipt projection required fields、no-auto-apply / status-detail builders、component filter/summary、operation mode、receipt write policy、post-apply/reload status、receipt input assembly、rollback/app-state owner decision vocabulary 与 receipt type source 已从 `managed-update-kernel.ts` / runner / Console / Foundry Lab / Runway projections 拆到 `managed-update-owner-boundary.ts`，kernel 只保 projection 组装与 component readback；Console、Foundry Lab 与 Runway 内剩余 previous-version / fallback 指针已改为复用 owner boundary 或标明 refs-only recovery；`connect agent-packages` 已把 registry discovery、manifest validation、install lock 与 lifecycle receipt 固定到 Framework `OPL_STATE_DIR` refs-only writer，并显式保留 no-domain-authority boundary；本轮进一步把 capability package canonical owner-route 收到共享 boundary，并把 `connect agent-packages` 返回面统一补成 `owner_route_readback`，显式投影 descriptor/digest/lock/materializer/no-package-manager 读回；agent package physical Codex surface 已复用 `system-installation/codex-plugin-registry.ts` 的 local marketplace / config helper，不再在 package registry 内手写第二套 plugin materializer。Full scan 当前为 `custom_update_or_package_manager=93`，其中 `phase6-src-modules-update-package=0`、`phase2-update-command-registry-projection=0`。 | 继续把 runtime materializer、App release updater、capability package channel 与 companion/workflow manual owner route 的 active caller 边界继续拆薄；不要声明 release/currentness ready。 |
| 7 | Pack/Workspace standardization | 70% | `partial` | pack/workspace CLI 与 descriptors 已存在；Pack OS lock/cache/registry 已增加 OCI descriptor/digest 字段，并统一投影 `content_addressed_lock_policy`：OCI media type、sha256 digest、refs-only lock、no registry push/pull、no body、no stage close、no domain truth write；Workspace shared-resource manifest/inventory 已增加 sha256 content-addressing policy；本轮把 Workspace diagnostic policy 的 overlap precedence 收紧为 hard blocker fail-closed，避免同一 code 同时进入 repairable list 时误降级。 | 继续把 remaining pack/workspace transport/cache 历史 findings 归入 standard descriptor/digest/lock，并补 owner package lifecycle evidence。 |
| 8 | Domain private platform retirement | 85% | `partial` | 已落 machine-readable `domain-private-platform-tail-matrix.json` 和 executable `domain_private_platform_tail_matrix` readback，覆盖 MAS/MAG/RCA/OMA/BookForge/ScholarSkills tail class、replacement primitive、retained authority、delete/tombstone gate、forbidden claims、`physical_delete_authorized=false` 与 owner decision gate；OMA live path / production consumption / long-soak ledger JSON readback 已复用共享 OPL JSON boundary；fresh readback 显示 matrix `row_count=6`、全部 domain `physical_delete_authorized=false`，ScholarSkills 仍是 refs-only capability package；MAG/RCA 多轮 retained-tail 收薄已吸收；本轮 `opl-bookforge` main `a7bc73d` 又把 publication/export helper、image asset helper、project hygiene helper 和 runtime/session/update absence 固化进 capability map / functional privatization audit / generated handoff / Temporal StageRun consumption policy，明确 BookForge retained helper 只保 domain-specific artifact operation 或 diagnostic helper，不承担 generic runtime、session、workbench、update、package manager、publication readiness authority 或 final export acceptance。 | 继续开 sibling capability-map / conformance / default-caller-thinning lane，逐项补 no-active-caller、no-forbidden-write、tombstone/provenance、direct/hosted parity 或 owner decision refs；RCA/BookForge 物理删除继续 owner-gated。 |
| 9 | App/Aion consumer-only | 78% | `partial` | App/Aion 边界已有合同和 release channel；Phase 8 lane 已在 `settings_control_center` 增加 executable consumer-only readback/validator，明确 local scheduler 只允许 refresh/UI/poll，forbidden truth-only paths 可见；fresh OPL readback 中 `app_settings_read_model.consumer_only_readback == app_aion_consumer_only_readback`；本轮 `opl-aion-shell` main `3cd2926af` 进一步把 background managed update `check/plan` 收成 refresh/projection-only，不再自动 apply `capability_packages`，显式 Settings apply 仍走既有 allowlist；`one-person-lab-app` main `36d900b` 已把 App release channel / GUI product contract / page-state matrix / active-shell validator / release-boundary test 同步到 `no_background_auto_apply_refresh_projection_only`，background auto-apply components 为空，`capability_packages` 进入 never-auto-apply；focused DOM test 5/5 pass、App release-boundary 260/260 pass 2 skip、active-shell quick pass、validate:release-boundary pass。 | release/currentness claim 仍必须来自 App owner receipt 或 typed blocker；后台 refresh-only 结构不等于 App release ready。 |
| 10 | OpenTelemetry-style observability | 92% | `partial` | 已落 `contracts/opl-framework/observability-semantic-conventions-contract.json` 与 `src/modules/ledger/observability-semantic-conventions.ts`，把 current owner delta / stage attempt / provider attempt 投影到 trace、metric、log/event vocabulary；`runtime observability-export` 现在直接输出 `semantic_conventions` refs-only readback seed，并在 OpenMetrics 中追加 `opl_queue_length`、`opl_observability_export_boundary`、`opl_observability_exporter_signal_mapping`、`opl_observability_collector_export_boundary` 与 `opl_observability_collector_consumption_config`；JSON readback 已携带 Prometheus receiver / OpenMetrics input / batch processor / debug exporter 的 Collector YAML 等价配置片段，`--format collector-config-json` 可直接输出同一 Collector config JSON；`runtime observability-endpoint` 已用 Node 标准 HTTP server 暴露 read-only OpenMetrics `/metrics` endpoint，并通过 CLI registry 保护 `runtime observability-*` 命令；`runtime observability-collector-smoke` 默认 no-endpoint 路径使用 bounded OPL OpenMetrics smoke endpoint，并用 `--collector-command` / `OPL_OTELCOL_COMMAND` / PATH 上的 `otelcol-contrib|otelcol` 执行 smoke readback；真实 `otelcol-contrib v0.155.0` 已 fresh 观察到 OPL metric，`collector_consumption_observed=true`、`external_collector_connected=true`，同时 authority boundary 仍固定 `can_claim_runtime_ready=false`；Collector binary 缺失时仍输出 `collector_binary_missing` typed blocker readback；`evidence-envelope` projection 与 `app-operator-drilldown` 已绑定同一 semantic convention signal model；Console runtime tray / stage attempt drilldown 已复用 Ledger-owned semantic vocabulary，保持 no-body / no-ready-claim boundary；observability projection field / label vocabulary 已集中到 `src/kernel/observability-projection-vocabulary.ts`，source-module diagnostic drilldown 词项已收为 refs-only command/readback projection，且扫描器仍会保留同一行里的 private ledger terms 风险。Full scan 当前为 `custom_observability_ledger=467`，其中 `phase10-src-modules-observability=0`。 | 继续补外部 runtime endpoint / OTLP SDK exporter owner lane 和部署环境证据；本阶段不声明 runtime、domain、artifact 或 production readiness。 |
| 11 | No-resurrection scan | 99% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback，并新增 `open_worklist_finding_count` / `blocking_worklist_finding_count` / `allowed_projection_finding_count`，避免把已决历史投影误报成真 blocker。本轮进一步把 package rollback allow 限定到 owner-routed lifecycle projection / managed update owner boundary，并把 `private-command-specs-parts/runtime.ts` 中已注册的 runtime diagnostic command projection 归入 allowed worklist。当前 full scan 为 `finding_count=709`、`hard_gate_finding_count=242`、`advisory_finding_count=467`、`decisioned_finding_count=709`、`undecisioned_finding_count=0`、`open_worklist_finding_count=0`、`blocking_worklist_finding_count=0`，全部归入 `allowed_projection_boundary`；当前 category 为 `handwritten_json_boundary=2`、`custom_runtime_queue=147`、`custom_update_or_package_manager=93`、`custom_observability_ledger=467`；`src/kernel/contract-validation.ts`、共享 JSON boundary helpers、`queue-projection-vocabulary.ts`、`observability-projection-vocabulary.ts`、`family-runtime-queue-projection-boundary.ts` 与 `managed-update-owner-boundary.ts` 被限定为允许的共享 boundary/owner helper；source-module diagnostic drilldown 只在不夹带 private ledger terms 时允许，不能扩大到 generic observability ledger；owner-routed rollback command projection 只在 CLI command projection 文件中允许；`phase1-src-modules-schema-cli-boundaries`、`phase0-maintenance-script-boundaries`、`phase2-cli-entrypoint-boundaries`、`phase2-update-command-registry-projection`、`phase4-src-modules-runtime-queue`、`phase6-src-modules-update-package` 与 `phase10-src-modules-observability` 已降为 0；`phase2-diagnostic-drilldown-command-projection` 从 20 降到 18；`residual_readback_2026_07_05.owner_route_worklist` 仍列出 Runtime / Update-package / Observability / Domain owner-live evidence 缺口。 | 继续处理 tests / contracts / entrypoints 里的历史 projection findings；`phase10-test-and-fixture-projections=575`，worklist 分类不是 risk eliminated / release ready；owner-route worklist 未清零前本文整体保持 `partial`。 |
...
## Phase 9 OTLP Exporter Owner-Route Structural Foldback 2026-07-05

本轮只收口 `otlp_exporter_live_endpoint` 的纯代码/结构面，不补 live endpoint、真实 OTLP SDK exporter、外部 collector owner 证明，也不把 focused tests/contract/readback 包装成 runtime ready。

- Source：`observability-export.ts` 的 `semantic_conventions` readback 现在显式携带 `exporter_seed` 与 `owner_route`；`runtime_export_binding` 同步给出 `exporter_seed_ref` / `owner_route_ref`。`observability-export-parts/shared.ts` 为 HTTP endpoint 与 collector smoke 统一补齐 `exporter_seed`、`owner_route`、`endpoint_readback_boundary`、`collector_consumption_readback_boundary`，避免 export / endpoint / smoke 各自手写第二套 OTLP gap 语义。
- Boundary：`exporter_seed` 固定为 `openmetrics_stdout_and_http_seed_only`，`otlp_sdk_exporter_configured=false`、`live_endpoint_configured=false`、`external_collector_connected=false`，并把后续 live owner 路径固定到 `owner_route.worklist_item=otlp_exporter_live_endpoint`。HTTP endpoint readback 只证明 bounded OpenMetrics scrape shape；collector smoke readback 只证明 bounded/local 或显式外部 endpoint probe 的消费结果；两者都显式列出 `does_not_prove`，避免把 collector/endpoint diagnostic readback 误当成 OTLP live endpoint、runtime ready、production ready 或 owner acceptance。
- Contract：`contracts/opl-framework/observability-semantic-conventions-contract.json` 新增 `exporter_seed`、`owner_route`、`endpoint_readback_boundary`、`collector_consumption_readback_boundary` 四个 machine-readable surface，和 `reuse-first-historical-worklist.json#otlp_exporter_live_endpoint` 保持同一 owner-live-evidence 语义，但不改 worklist authority 本身。
- Fresh evidence：`node --test --experimental-strip-types tests/src/cli/cases/runtime-observability-export.test.ts` pass；`npm run typecheck` pass；`git diff --check` pass。该证据只证明 exporter/endpoint owner-route 结构和 focused readback 对齐，不证明 live OTLP exporter、外部 runtime endpoint、runtime ready、domain ready、production ready 或 owner acceptance。
...
- CLI registry projection：`update status/check/plan/apply/repair/rollback` 已作为 owner-routed managed update command projection 保留；scanner 现在只允许 CLI command projection 文件中的 `rollback` wording，full scan 中 `phase2-update-command-registry-projection=0`，不再落入 active migration worklist。`runtime app-operator-drilldown` 作为 diagnostic refs-only projection 保留；本轮删除旧 request parser 尾巴后 full scan 中 `phase2-diagnostic-drilldown-command-projection=18`。
...
- Owner-live preflight：`contracts/opl-framework/reuse-first-historical-worklist.json#owner_live_evidence_preflight_2026_07_05` 逐项固化 5 个 open item 的 missing evidence、legal next owner/command、forbidden completion evidence 与 stop condition；本轮 fresh evidence audit 读回仍为 5/5 `evidence_found=false`，`current_evidence_available=false`、`open_item_count=5`、`can_claim_*_ready=false`。
- Fresh evidence：focused JSON/readiness tests `58/58 pass`；`node --test --experimental-strip-types tests/src/reuse-first-scan.test.ts` 为 `16/16 pass`；full scan 当前为 `finding_count=699`、`hard_gate_finding_count=242`、`advisory_finding_count=457`、`decisioned_finding_count=699`、`undecisioned_finding_count=0`、`open_worklist_finding_count=0`、`blocking_worklist_finding_count=0`、`allowed_projection_finding_count=699`、`handwritten_json_boundary=2`、`custom_runtime_queue=147`、`custom_update_or_package_manager=93`、`custom_observability_ledger=457`、`owner_route_worklist_count=5`、`owner_live_evidence_required_count=5`、`owner_route_open_count=5`、`owner_live_preflight_open_item_count=5`、`owner_live_preflight_current_evidence_available=false`、`owner_live_preflight_can_claim_runtime_ready=false`；`npm run reuse-first:scan:diff -- --strict` 返回 `finding_count=0`；`git diff --check` pass。该证据只证明 historical worklist 已决、测试 JSON boundary 收薄、no-resurrection diff gate 保持和 owner-route / owner-live-preflight worklist 清晰；不证明 runtime ready、release ready、production ready、domain ready 或 owner acceptance。
- Scan audit exposure：full scan 现在同步输出 `owner_live_evidence_audit_result=no_owner_live_evidence_found`、`owner_live_evidence_audit_item_count=5`、`owner_live_evidence_found_count=0`、`owner_live_evidence_audit_can_mark_preflight_closed=false`，避免只读 contract 才能看到本轮 owner evidence audit 结论。

## Reuse-first Structural Convergence Follow-through 2026-07-05

本轮继续按文档中的低完成度结构项开四条隔离 worktree lane，并由主会话逐条检查 diff、复跑 focused verification、cherry-pick 吸收回 `main`。这些切片只推进 Framework 本仓的结构/readback/scan 口径，不写 runtime DB/provider queue、不写 domain truth、不创建 owner receipt / typed blocker，也不声明 runtime ready、release ready、production ready、domain ready 或 owner acceptance。

- CLI command registry：`main` commit `b07687ea` 把 `runtime manager` 与 `runtime manager action` 注册进 `contracts/opl-framework/cli-command-registry.json`，并让 action 参数走现有 registry parser adapter；fresh evidence 为 `node --experimental-strip-types --test tests/src/cli/cases/cli-command-registry.test.ts` 10/10 pass。
- Runway reconciler：`main` commit `072a0de0` 在 lifecycle reconcile readback 中新增 `action_source_convergence`，把 scheduler、worker、App 与 domain helper 的 direct mutation 全部收束到 canonical safe-action source；fresh evidence 为 `node --test tests/src/family-runtime-lifecycle-index.test.ts` 10/10 pass。
- Pack / Workspace：`main` commit `3e57d930` 收紧 Workspace diagnostic severity precedence，policy overlap 时 hard blocker fail-closed，不被 repairable list 降级；fresh evidence 为 `node --test --experimental-strip-types tests/src/workspace-diagnostics-policy.test.ts tests/src/cli/cases/workspace-domain.initializer.test.ts` 20/20 pass。
- No-resurrection scan：`main` commit `65c64a75` 收紧 package rollback owner-route projection allow，并同步 `reuse-first-historical-worklist.json` 的 residual readback counts；fresh evidence 为 `node --test --experimental-strip-types tests/src/reuse-first-scan.test.ts` 16/16 pass，`npm run reuse-first:scan:diff -- --strict` 返回 `finding_count=0`，`npm run reuse-first:scan` 返回 `finding_count=699`、`open_worklist_finding_count=0`、`blocking_worklist_finding_count=0`、`owner_live_evidence_required_count=5`。
- Integration guard：主会话吸收后补齐 Workspace / Runway public module exports 与 test-lane registry，避免新 focused test 形成 source-module deep import 或 active-test-owner 漂移；fresh evidence 为 `npm test` 85/85 pass、`npm run typecheck` pass、`npm run reuse-first:scan:diff -- --strict` `finding_count=0`、`node ./scripts/reuse-first-scan.mjs --max-findings 0` full scan summary 保持 `finding_count=699` / `blocking_worklist_finding_count=0` / `owner_live_evidence_required_count=5`。

本轮后，功能/结构面新增关闭的是：runtime manager CLI registry bypass、lifecycle reconcile safe-action source 可见性、workspace hard-blocker 降级风险、package rollback allow 过宽风险。仍不能关闭的是 5 个 owner-live evidence 项：external Temporal durable lifecycle、App release owner route、capability package channel owner route、OTLP/exporter live endpoint、domain tail owner acceptance。

## Phase 2 Runtime Diagnostic Command Registry Follow-up 2026-07-05

本隔离 lane 继续收敛 CLI parser / registry 纯代码尾巴，只覆盖 `runtime app-operator-drilldown` 这个 refs-only diagnostic command；不改变 App/operator drilldown 的 read model 语义，不写 runtime DB/provider queue，不写 domain truth，不创建 owner receipt / typed blocker，也不声明 runtime、release、production、domain ready 或 owner acceptance。

- Contract：`cli-command-registry.json` 增加 `runtime_app_operator_drilldown`，并把 `runtime app-operator-drilldown` 加入 protected / required command set；output schema 只约束 wrapper、`surface_kind=opl_app_operator_drilldown_read_model` 与 no-authority boundary。
- Source：`private-command-specs-parts/runtime.ts` 中该命令改用现有 `parseRegisteredCommandOptions`，保留 `--detail summary|full` 与 `--full` 语义；没有引入 Commander/Yargs 或新 parser abstraction。
- Worklist/readback：`reuse-first-historical-worklist.json` 把拆分后的 `private-command-specs-parts/runtime.ts` 归入既有 `phase2-diagnostic-drilldown-command-projection` allowed item；本轮删除旧 `request-parsers.ts` 手写 parser 尾巴后，full scan 当前为 `finding_count=709`、`open_worklist_finding_count=0`、`blocking_worklist_finding_count=0`、`allowed_projection_finding_count=709`、`phase2-diagnostic-drilldown-command-projection=18`、`owner_live_evidence_required_count=5`。
- Fresh evidence：`node --test --experimental-strip-types tests/src/cli/cases/cli-command-registry.test.ts` 11/11 pass；`npm run typecheck` pass；`npm run reuse-first:scan:diff -- --strict` exit 0 with `hard_gate_finding_count=0` and 12 advisory diagnostic wording findings；`npm run reuse-first:scan -- --max-findings 0` exit 0 with current full-scan counts above；`git diff --check` pass。

## App/Aion And BookForge Structural Follow-up 2026-07-05

本轮同时吸收两个 sibling repo 非 live 结构切片；它们只降低重复造轮子和私有平台尾巴风险，不构成 App release ready、domain ready、publication ready、owner acceptance、final export acceptance 或 physical delete authorization。

- Aion consumer-only：`opl-aion-shell` main `3cd2926af` 删除 background managed update 自动 apply 路径，`check/plan` 只更新 projection / reload guidance；focused evidence 为 `bun run test:dom tests/unit/opl-runtime/managedUpdateMaintenance.dom.test.ts` 1 file / 5 tests pass，`bunx oxfmt --check ...` pass，`git diff --check gh-https/main...HEAD` pass。`bunx tsc --noEmit` 仍受既有 `packages/web-host/src/static-server.ts` 缺 `serve-handler` 类型阻断，不属于本写集。
- App contract foldback：`one-person-lab-app` main `36d900b` 已把 release channel、GUI product contract、page-state matrix、active-shell policy constant、release-boundary test 和 App active/release docs 同步为 `no_background_auto_apply_refresh_projection_only`；background auto-apply components 为空，`capability_packages` 进入 never-auto-apply，显式 apply/repair/rollback 仍走 Framework receipt route。fresh evidence 为 managed-update-plane focused test 5/5 pass、App GUI/release workflow focused tests 8/8 pass、`OPL_APP_SHELL_ROOT=/Users/gaofeng/workspace/opl-aion-shell npm run validate:active-shell -- --quick` pass、`npm run validate:release-boundary` pass、`npm run test:release-boundary` 260/260 pass 2 skip、`git diff --check` pass。
- BookForge private tail：`opl-bookforge` main `a7bc73d` 增加 BookForge private platform retirement matrix、generated/hosted handoff refs 与 Temporal StageRun consumption policy checks，把 publication/export helper、image asset helper、project hygiene helper 和 runtime/session/update absence 明确降为 domain-specific helper / diagnostic helper / absent-and-fail-closed surface；fresh evidence 为 `./scripts/verify.sh` pass、`python3 tests/test_temporal_stage_run_consumption_policy.py` pass、`opl agents scaffold --validate . --json` status passed / blocker_count 0、`git diff --check HEAD~1..HEAD` pass。
...
| 当前完成度和建议落地顺序 | `done` | 100% | `当前完成度与建议顺序` 表逐项给出百分比、状态、证据和下一步，已按 2026-07-05 追加结构 lane 吸收结果更新：`b07687ea`、`072a0de0`、`3e57d930`、`65c64a75`、`9b4a3e82`，并继续记录本轮 parser-tail 删除切片。 | 百分比是审计判断，不是 runtime/readiness evidence。 | 后续每轮都用 fresh evidence 校准。 |
| `runtime_app_operator_drilldown_registry` | `done` | 100% | `one-person-lab` main `813f7023` 已把 `runtime app-operator-drilldown` 纳入 CLI registry 和 protected / required command set；main-session fresh verification 为 CLI registry test 11/11 pass、reuse-first scan test 16/16 pass、typecheck pass、strict diff hard findings 0、full scan `owner_live_evidence_required_count=5`。 | 只关闭该 diagnostic command 的 parser/registry bypass；不证明 observability live endpoint、runtime ready 或 owner acceptance。 | 后续继续迁 remaining runtime/public diagnostic commands。 |
| `stagecraft_stage_command_registry` | `done` | 100% | `one-person-lab` main `9b4a3e82` 已把 Stagecraft `stages list/inspect/readiness/proof-bundle/graph/assumptions/cohort-loop/runtime-budget/registry/source-spec/replay-certification` 纳入 CLI registry、protected / required command set，并让 Stage command specs 调用 registry parser adapter；主会话 fresh verification 为 CLI registry test 12/12 pass、`npm run typecheck` pass、strict diff hard findings 0。 | 只关闭 Stagecraft diagnostic command registry / parser bypass；不证明 domain ready、production ready、owner acceptance 或 live evidence。 | 后续继续迁 remaining public/runtime diagnostic commands。 |
| `runtime_app_operator_drilldown_parser_tail_delete` | `done` | 100% | 本轮已删除无 active caller 的 `parseRuntimeAppOperatorDrilldownArgs`、`RuntimeAppOperatorDrilldownCliInput` 与 support export；`rg -n "parseRuntimeAppOperatorDrilldownArgs|RuntimeAppOperatorDrilldownCliInput" src tests` 无命中；fresh verification 为 CLI registry test 12/12 pass、`npm run typecheck` pass、strict diff hard findings 0、full scan `finding_count=709` / `phase2-diagnostic-drilldown-command-projection=18` / `owner_live_evidence_required_count=5`。 | 只关闭旧 parser/type/export 尾巴；不证明 runtime ready、owner acceptance 或 live evidence。 | 后续继续迁 remaining public/runtime diagnostic commands。 |
| `aion_managed_update_refresh_only` | `done` | 100% | `opl-aion-shell` main `3cd2926af` 已删除 background managed update auto-apply path；focused DOM test 5/5 pass、format check pass、diff check pass。 | Aion refresh-only 结构不等于 App release/currentness owner receipt。 | release evidence 后置，只由 App release authority 给出 owner receipt / typed blocker。 |
| `app_managed_update_refresh_only_contract` | `done` | 100% | `one-person-lab-app` main `36d900b` 已把 App release channel、GUI product contract、page-state matrix、active-shell validator、release-boundary test 和 active/release docs 同步为 background refresh/projection only；focused tests 5/5、8/8、active-shell quick、validate:release-boundary、test:release-boundary 260/260 pass 2 skip、diff check 均通过。 | 只证明 App/Aion 非 live 结构一致；不证明 release/currentness owner receipt、真实 package lifecycle currentness 或 App release ready。 | 后续 release owner 只走 release evidence / owner receipt / typed blocker。 |
| `bookforge_private_platform_tail_mapping` | `done` | 100% | `opl-bookforge` main `a7bc73d` 已落 capability map / functional privatization audit / generated handoff / Temporal StageRun consumption policy / docs；`./scripts/verify.sh` pass、BookForge policy test pass、`opl agents scaffold --validate . --json` pass / blocker_count 0、diff check pass。 | BookForge owner acceptance、live hosted artifact-handoff parity、publication proof human visual acceptance、final export owner acceptance 与 physical delete authorization 仍缺。 | 后续只走 owner/live evidence 或明确 owner decision。 |
| `external_temporal_durable_lifecycle` | `blocked` | 88% | `family-runtime residency proof --provider temporal --live --json` 已证明 Temporal test-server / worker / signal history / typed closeout code path，且显式 `proves_real_codex_cli=false`；full scan `phase4-src-modules-runtime-queue=0`。 | 仍缺 external Temporal history/query、managed worker、真实 Codex executor closeout 和 owner/domain refs；这是 live substrate gap，不是继续写本仓结构代码能关掉的缺口。 | 只能在 owner 提供的 external Temporal 环境继续做 live proof，并由 Runway Runtime owner 产出 receipt、typed blocker 或 route-back。 |
| `app_release_owner_route` | `partial` | 93% | `one-person-lab-app` main `77ee281` 已吸收 settings cognition closeout；`one-person-lab-app` main `36d900b` 又把 managed update background policy 收成 refresh/projection-only，App release channel、GUI contract、page-state matrix、validator、release-boundary tests 和 docs 一致；fresh evidence 包括 settings focused tests、active-shell quick、managed-update-plane focused tests、validate:release-boundary、test:release-boundary 260/260 pass 2 skip。 | 仍缺 App release owner receipt、managed update receipt 被 App release/currentness truth 消费、release/currentness source ref；结构闭合不等于 release ready。 | 保持 App repo 为 release truth owner，后续只走 App release authority 和 owner receipt / typed blocker。 |
| `capability_package_channel_owner_route` | `partial` | 93% | `one-person-lab` main `1a9ac405` 已吸收 canonical package owner-route：`connect agent-packages` 系列命令统一返回 `owner_route_readback`；focused test `tests/src/cli/cases/connect-agent-packages.test.ts` 7/7 pass，`npm run typecheck` pass，lane 已吸收并清理。 | 仍缺 live owner receipt / typed blocker、真实 package lifecycle currentness、安全审计、真实 Codex reload acceptance；当前只证明 descriptor/digest/lock/materializer/no-package-manager 结构读回。 | 继续经 Pack / Connect owner surface 路由 package lifecycle；不得把 `owner_route_readback` 包装成 owner acceptance。 |
| `otlp_exporter_live_endpoint` | `partial` | 93% | `one-person-lab` main `b126bdc1` 已把 exporter seed / owner-route / endpoint readback boundary / collector consumption readback boundary 固化进 contract 和 runtime readback；focused test `tests/src/cli/cases/runtime-observability-export.test.ts` 6/6 pass，`npm run typecheck` pass，lane 已吸收并清理。 | 仍缺 OTLP/OpenTelemetry SDK exporter、真实 live endpoint、外部 collector owner receipt、trace/metric/log/event 生产链路；当前 bounded OpenMetrics endpoint 与 collector smoke 仍只是 diagnostic。 | 继续开 Observability owner lane；只有 live endpoint / exporter / external collector evidence 才能推进 readiness claim。 |
| `domain_tail_owner_acceptance` | `blocked` | 92% | OPL 本仓 domain tail matrix/readback 仍是 refs-only projection；`redcube-ai` main `e854fbc7` / `c841499c` / `520caab5` / `76f8c10d` 已进一步收薄 `getProductEntrySession`、`session-surfaces`、`executor-runtime`、`route-run-record-store`、route recovery/continuation glue 与 session orchestration 上下文，`redcube-ai` main `57364cba` 又把 `executor-runtime.ts` 中 `RUNNING_RUN_STALE_TTL_MS`、`loadRouteRun`、`appendRouteRunEvent` 与 `readRouteRunEvents` 改成直接 re-export，只保留 3 个仍需 runtime-topology 注入的 active wrapper；fresh verification 为 `node --experimental-strip-types --test tests/runtime-topology-regression.test.ts tests/rca-executor-backend-contract.test.ts` 11/11 pass、`node --experimental-strip-types scripts/run-test-group.ts integration --files tests/product-entry-session-checkpoint.test.ts,tests/product-entry.test.ts` 31/31 pass、`node --experimental-strip-types scripts/run-test-group.ts meta --files tests/opl-family-contract-adoption.test.ts` 21/21 pass、`npm run test:default-caller-tail:strict` pass、`node --experimental-strip-types --test tests/runtime-deliverable-route-cases/route-validation.test.ts` 9/9 pass、`npm run build` pass；`med-autogrant` main `be22a11` 已把 `manifest_builder` 的 runtime continuity quartet 统一复用 `runtime_surfaces._build_runtime_continuity_surfaces()`，`med-autogrant` main `675685c` 已把 `build-product-entry` 与 `grant-direct-entry` 的 default runtime continuity 组装统一收到 `runtime_surfaces._build_default_runtime_continuity_surfaces()`，`med-autogrant` main `142bd76` 又把 manifest builder 统一收到同一个 default continuity helper，只通过 override command 注入当前 manifest shell route command，`med-autogrant` main `c75e329` 又把 retained-tail helper glue 继续收薄到共享 forbidden-write proof 与本地 canonical builder，`med-autogrant` main `eef5515` 再把 hosted receipt verification 与 stage-attempt observability 的 forbidden-write proof 读取统一复用 `owner_receipt_common.read_forbidden_write_proof()`，`med-autogrant` main `f0baf7c` 又把 `receipt_observability.py` 与 `continuous_reconciliation.py` 的 receipt shape/proof 常量统一复用 `owner_receipt_common`，`med-autogrant` main `b6ee8f9` 再把 hosted receipt verification 直接复用 `require_owner_receipt_evidence()` 并删除剩余 proof-pass-through helper，`med-autogrant` main `f072a41` 再把 `receipt_readiness.py` 与 `lifecycle_receipt_bundle.py` 的 owner-receipt / forbidden-write 常量读取统一复用 `owner_receipt_common`，`med-autogrant` main `477685d` 再把 lifecycle receipt kind / operation tuple 与 receipt observability / stage-attempt observability 的重复常量统一复用 `owner_receipt_common`；fresh verification 为 `./scripts/run-pytest-clean.sh tests/product_entry_cases/test_manifest_and_status.py tests/product_entry_cases/test_loop_and_readiness.py tests/product_entry_cases/test_domain_handler.py tests/product_entry_cases/test_direct_entry.py tests/test_cli_validate_workspace.py -q` 52 pass / 49 subtests pass、`./scripts/run-pytest-clean.sh tests/product_entry_cases/test_lifecycle_receipt_bundle.py tests/product_entry_cases/test_functional_closure.py tests/product_entry_cases/test_dependency_structure.py -q` 22 pass / 73 subtests pass、`./scripts/run-pytest-clean.sh tests/product_entry_cases/test_hosted_receipt_verification.py tests/product_entry_cases/test_stage_attempt_observability.py tests/product_entry_cases/test_receipt_observability.py tests/product_entry_cases/test_controlled_soak.py tests/product_entry_cases/test_production_live_acceptance.py tests/product_entry_cases/test_continuous_reconciliation.py -q` 25 pass，以及 `./scripts/run-pytest-clean.sh tests/product_entry_cases/test_receipt_readiness.py tests/product_entry_cases/test_lifecycle_receipt_bundle.py tests/product_entry_cases/test_receipt_scaleout_evidence.py -q` 12 pass / 8 subtests pass，外加 `./scripts/run-pytest-clean.sh tests/product_entry_cases/test_lifecycle_receipt_bundle.py tests/product_entry_cases/test_stage_attempt_observability.py tests/product_entry_cases/test_receipt_observability.py -q` 9 pass。 | `domain_tail_owner_acceptance` 本体仍缺 domain owner acceptance / decision / typed blocker；RCA 剩余尾项经 fresh code audit 已确认主要落在 active caller、public/package boundary、runtime authority 与 live/currentness 证据面，不再是高确定性的 reuse-first 小切片。 | 纯代码面先停在当前收口；后续只走 domain owner acceptance、typed blocker、live route 或明确 physical-delete owner decision，不把结构收薄包装成 domain ready。 |
| 可纯代码/结构落地的本轮剩余缺口 closeout | `done` | 100% | 本轮十七条可安全推进的 lane 已吸收回各自 `main` 并清理：`one-person-lab-app` `77ee281`，`one-person-lab` `1a9ac405` / `b126bdc1`，`redcube-ai` `e854fbc7` / `c841499c` / `520caab5` / `76f8c10d` / `57364cba`，`med-autogrant` `be22a11` / `675685c` / `142bd76` / `c75e329` / `eef5515` / `f0baf7c` / `b6ee8f9` / `f072a41` / `477685d`；本轮追加 OPL root 结构 lane 已吸收为 `3e57d930`、`65c64a75`、`072a0de0`、`b07687ea`、`9b4a3e82`，并继续完成 runtime app-operator drilldown parser-tail 删除切片；对应 focused verification 均由主会话 fresh rerun 后再吸收。 | 当前剩余项已不再属于“本轮还能安全靠纯代码/结构一步到位关闭”的集合：5 个 owner-live-evidence 缺口必须依赖外部 owner / live substrate，不能由本仓 docs、scan、readback 或 focused tests 代替。 | 本轮纯代码/结构 closeout 到此为止；后续按各 owner 分别补 external Temporal、App release、package lifecycle、OTLP live endpoint 和 domain owner acceptance 的 live evidence。 |
