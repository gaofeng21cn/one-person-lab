...
| 10 | OpenTelemetry-style observability | 61% | `partial` | 已落 `contracts/opl-framework/observability-semantic-conventions-contract.json` 与 `src/modules/ledger/observability-semantic-conventions.ts`，把 current owner delta / stage attempt / provider attempt 投影到 trace、metric、log/event vocabulary；`runtime observability-export` 现在直接输出 `semantic_conventions` refs-only readback seed，并在 OpenMetrics 中追加 `opl_queue_length` 等 semantic convention metrics 与 `opl_observability_export_boundary`；`evidence-envelope` projection 也已绑定同一 semantic convention signal model，保持 no-body / no-ready-claim boundary。 | 继续接入真实 Collector/exporter 消费面与 operator drilldown 同源导航；本阶段不声明 runtime、domain、artifact 或 production readiness。 |
