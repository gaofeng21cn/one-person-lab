...
- Fresh evidence：focused kernel/App state tests `node --test --experimental-strip-types tests/src/managed-shell-command-env.test.ts tests/src/opl-managed-runtime-three-layer-contract.test.ts tests/src/cli-codex-default-shell.test.ts tests/src/cli/cases/app-state.test.ts` 为 35/35 pass；`npm run typecheck` pass；full scan 从 `1196` 降到 `1185`，`hard_gate_finding_count` 从 `728` 降到 `717`，`handwritten_json_boundary` 从 `458` 降到 `447`，`decisioned_finding_count=1185`、`undecisioned_finding_count=0`。该证据只证明 kernel JSON boundary 收薄，不证明 schema boundary 全量完成、runtime ready、domain ready、release ready、production ready 或 owner acceptance。
