...
- `2026-07-07` 主线继续关闭四个过度设计尾巴：Temporal test-server proof 移出生产 `src/**`，文件型 JSON receipt ledger 复用 `src/kernel/json-file.ts` shared helper，family-runtime queue / scheduler CLI option loops 收到邻近 thin parser helper，超长 tests 按行为簇拆分。
...
- `line-budget` 新鲜 JSON 读回为 `status=ok`、`oversize_count=0`、`failure_count=0`。这只证明 repo-tracked source/test 文件回到默认结构预算以内，不证明 live runtime、App release、domain owner-chain 或 production readiness。
...
| Schema boundary consolidation | `partial` | 84% | shared schema/JSON helper、focused tests、typecheck、diff gate；`2026-07-07` 追加把重复 JSON receipt ledger read/write/upsert 样板收到 `src/kernel/json-file.ts`。 | 继续禁止新增分散 validator；receipt helper 不改变 payload semantics 或 owner receipt shape。 |
| CLI parser/command registry | `partial` | 88% | command registry、parser adapter、protected/required command set；`source-module-boundary`、`source-module-public-imports`、`line-budget` 的 help/json maintainer surface 已落，`native-helper-prebuild` 已回到 stdlib `parseArgs` 并补 `--help/-h`；family-runtime queue / scheduler option loops 已收到邻近 thin helper。 | 继续迁 remaining public/runtime diagnostic commands；避免新增 parser dependency。 |
...
| Source/test size watch-only | `done` | 100% | `node ./scripts/line-budget.mjs --format json` 新鲜读回 `status=ok`、`oversize_count=0`、`failure_count=0`。 | 后续 touched 文件继续按语义拆；不要为指标单独硬拆，也不能把结构预算通过写成 readiness。 |
