...
| --- | --- | ---: | --- | --- | --- |
| 1 | Reuse-first governance gate | 94% | `partial` | 已落 `contracts/opl-framework/reuse-first-governance.json`、`contracts/opl-framework/reuse-first-historical-worklist.json`、`scripts/reuse-first-scan.mjs`、`npm run reuse-first:scan`、`npm run reuse-first:scan:diff` 与 `./scripts/verify.sh reuse-first`；full scan 现在输出 historical decision/worklist readback；maintainer scripts 已新增共享 JSON/helper 与标准参数解析 helper，`phase0-maintenance-script-boundaries` 当前为 27，其中 hard findings 4；GitHub Verify 的 lint/structure job 会跑 strict diff gate。 | 继续逐项消化 worklist 中的 migration / owner-decision findings；不要把 worklist 分类或 clean diff gate 当作全仓历史风险已清零。 |
| 2 | Schema boundary consolidation | 69% | `partial` | Phase 1 seed lane 已引入 Ajv-backed `src/kernel/schema-registry.ts`；`ProgressDeltaReceipt` 与 `DomainProgressTransitionRuntime` live readback 已分别迁到 schema registry；runtime / entrypoint CLI JSON boundary 已新增 centralized `json-boundary.ts`，累计消化 runtime command spec、App release / OMA live path、Agent Lab public command、OKF / ScholarSkills public spec 与 Agent Lab payload parser 的分散 helper；Pack / Foundry JSON file boundary 已集中到 `src/kernel/json-file.ts`；Atlas domain-manifest / memory contract、Charter advisory / L5 evidence / source-structure readback、Console app-state / operator drilldown / runtime tray readback、Connect system-installation JSON/readback、Connect skill-pack / ACP stdio / Developer Mode / generated plugin / managed install-update ledger JSON boundary、Pack privatization / residue gate JSON boundary、Ledger artifact provenance bundle / current-owner / evidence dispatch / production-evidence / domain-owner / long-soak / template-consumption JSON boundary、Foundry Lab work-order execution / readiness / default-caller / Agent Lab suite / production acceptance / owner-evidence intake JSON boundary、Stagecraft stage/control-plane/quality/stage-run JSON boundary、Runway stop-loss successor / agent-executor / runtime-store / runtime evidence / dispatch / redrive / native readback / provider projection / lifecycle / task scope JSON boundary、maintainer script JSON / argument boundary、managed update receipt 与 OMA ledger readback first-slice 已复用 `src/kernel/json-record.ts` / `src/kernel/json-file.ts`；Charter error vocabulary 已从非 Charter 模块收回到 `kernel` owner。Full scan 当前仍有 `handwritten_json_boundary=517`。 | 继续把 pack descriptors、workspace projections、剩余 module JSON outputs 接入 schema registry/shared boundary；禁止新增分散 validator。 |
...
| 11 | No-resurrection scan | 94% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 为 2605 findings，已全部 decisioned，undecisioned=0；`src/kernel/contract-validation.ts` 和共享 JSON boundary helpers 被明确分类为允许的共享 schema / JSON boundary helper。 | 继续把 151 个 `must_migrate`、358 个 `accepted_migration_worklist` 和 152 个 `owner_decision_required` findings 逐批迁移或做 owner 决策；worklist 分类不是 risk eliminated / release ready。 |
...
本轮继续收薄 maintainer scripts，只把重复参数解析和 JSON 读取迁回共享 helper；不改变 release/package/update/currentness truth，也不把脚本包装成包管理器或 release authority。

- Source：`scripts/required-value-options.mjs` 承接 required value option parsing；`line-budget.mjs`、`reuse-first-scan.mjs`、`source-module-boundary.mjs`、`source-module-public-imports.mjs`、fresh-install smoke、package / cleanup / structure advisory / profile export scripts 复用 target CLI / shared helper 和 `script-json-boundary.mjs`。
- Boundary：`--strict` 等用户可见 flag 仍以显式常量保留，避免 helper 收薄把 literal flag 从 source 中抹掉后影响既有 line-budget/governance test。
- Fresh evidence：`npm run typecheck` pass；`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok；focused script/governance tests 18/18 pass；broader package / verification / command-surface tests 59/59 pass；full scan 中 `scripts` path findings 当前为 27，hard findings 4。
...
| 彻底解决所有代码风险 | `partial` | 90% | 最新结构切片已落到源码与文档：Runway runtime core / provider projection / task scope 继续复用 shared JSON boundary；maintainer scripts 复用 `scripts/required-value-options.mjs` 与 `script-json-boundary.mjs`；`opl domain manifests` 通过 Foundry Lab consumer wrapper 增加 `opl_meta_agent_registry` readback，Atlas normalizer 支持 wrapper sidecar skeleton，Connect framework locator 修正为 repo root。Fresh evidence：`npm run typecheck` pass；`npm run source:modules -- --strict-imports` status=ok，deep import violations=0，forbidden dependency violations=0，dependency cycle=0；focused workspace-domain registry/skeleton/descriptor、framework locator、App operator OMA、Connect external skills tests 26/26 pass；`npm run reuse-first:scan:diff -- --strict` gate_status=ok；`git diff --check` pass；`npm test` smoke 76/76 pass；full reuse-first scan `finding_count=2605`、`hard_gate_finding_count=1065`、`advisory_finding_count=1540`、`decisioned_finding_count=2605`、`undecisioned_finding_count=0`、`handwritten_json_boundary=517`、`custom_runtime_queue=298`、`custom_update_or_package_manager=250`。 | 仍不是全量完成：Runway durable lifecycle 尚未完全迁到真实 Temporal history；schema/CLI/readback 边界仍有 517 个历史 handwritten JSON findings；runtime queue 仍有 298 个 must-migrate findings；update/package 仍有 250 个 owner-decision findings；domain private tail 尚未逐 repo physical closeout；App/Aion live validator、真实 Collector/exporter 消费和 owner acceptance 尚未完成。 | 继续按 runtime durable lifecycle、schema/CLI 扩面、domain tail、App/Aion consumer-only、observability Collector/exporter、historical findings worklist 顺序推进；不得把 worklist 分类、first-slice 测试绿或 clean diff gate 包装成 production ready。 |
