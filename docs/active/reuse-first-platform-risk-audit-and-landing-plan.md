...
| --- | --- | ---: | --- | --- | --- |
| 1 | Reuse-first governance gate | 95% | `partial` | 已落 `contracts/opl-framework/reuse-first-governance.json`、`contracts/opl-framework/reuse-first-historical-worklist.json`、`scripts/reuse-first-scan.mjs`、`npm run reuse-first:scan`、`npm run reuse-first:scan:diff` 与 `./scripts/verify.sh reuse-first`；full scan 现在输出 historical decision/worklist readback；maintainer scripts 已新增共享 JSON/helper 与标准参数解析 helper，`phase0-maintenance-script-boundaries` 当前为 0；`scripts/test-lanes.mjs` 的 23 个 advisory observability hits 已单列为 test lane file catalog projection，不再混入 update/package owner-decision worklist；GitHub Verify 的 lint/structure job 会跑 strict diff gate。 | 继续逐项消化 worklist 中的 migration / owner-decision findings；不要把 worklist 分类或 clean diff gate 当作全仓历史风险已清零。 |
| 2 | Schema boundary consolidation | 72% | `partial` | Phase 1 seed lane 已引入 Ajv-backed `src/kernel/schema-registry.ts`；`ProgressDeltaReceipt` 与 `DomainProgressTransitionRuntime` live readback 已分别迁到 schema registry；runtime / entrypoint CLI JSON boundary 已新增 centralized `json-boundary.ts`，累计消化 runtime command spec、App release / OMA live path、Agent Lab public command、OKF / ScholarSkills public spec 与 Agent Lab payload parser 的分散 helper；Pack / Foundry JSON file boundary 已集中到 `src/kernel/json-file.ts`；Atlas domain-manifest / memory contract、Charter advisory / L5 evidence / source-structure readback、Console app-state / operator drilldown / runtime tray readback、Connect system-installation JSON/readback、Connect skill-pack / ACP stdio / Developer Mode / generated plugin / managed install-update ledger JSON boundary、Pack privatization / residue gate JSON boundary、Ledger artifact provenance bundle / current-owner / evidence dispatch / production-evidence / domain-owner / long-soak / template-consumption JSON boundary、Foundry Lab work-order execution / readiness / default-caller / Agent Lab suite / production acceptance / owner-evidence intake JSON boundary、Stagecraft stage/control-plane/quality/stage-run JSON boundary、Runway stop-loss successor / agent-executor / runtime-store / runtime evidence / dispatch / redrive / native readback / provider projection / lifecycle / task scope / command payload / default-executor receipt / provider followthrough / stage attempt closeout / provider-hosted value JSON boundary、route sync / evidence-currentness / stage-runtime utility JSON boundary、source-module boundary script、maintainer script JSON / argument boundary、managed update receipt 与 OMA ledger readback first-slice 已复用 `src/kernel/json-record.ts` / `src/kernel/json-file.ts` / `scripts/script-json-boundary.mjs`；Charter error vocabulary 已从非 Charter 模块收回到 `kernel` owner。`phase1-src-modules-schema-cli-boundaries` 当前为 0；Full scan 当前仍有 `handwritten_json_boundary=476`，主要留在测试夹具、entrypoint CLI、contracts 与允许的 kernel helper。 | 继续把 entrypoint CLI、test fixtures、contract provenance 与 workspace projections 接入 schema registry/shared boundary；禁止新增分散 validator。 |
| 3 | CLI parser/command registry | 64% | `partial` | 已落最小 `CommandSpec.registry` 与 `validateCommandRegistryCoverage` adapter，`connect pubmed search`、`connect references verify`、`connect external-skills list/search/inspect/sync`、`connect install/update/reinstall/remove` module actions、`status workspace/runtime/dashboard`，以及 `update status/check/plan/apply/repair/rollback` 都进入 registry；status 与 update 命令已复用 registry parser adapter；maintainer scripts 的重复参数解析已集中到 `scripts/required-value-options.mjs`；`phase2-cli-entrypoint-boundaries` 当前为 0，entrypoint 层剩余 40 个 update / diagnostic drilldown 词项已单列为 owner-routed command projection。 | 继续把高频 runtime / public command specs 纳入 registry；只有当 Commander/Yargs 能减少现有 parser 分散度时再引入依赖。 |
| 4 | Runway Temporal-first runtime | 78% | `partial` | Temporal SDK 已是一等依赖，docs 已声明 Temporal production substrate；`family-runtime status/queue list` 已暴露 `queue_lifecycle_boundary`；Runway control-loop 已把 competing SQLite queue lifecycle 纳入 Temporal readiness 降级；新增 `family-runtime-temporal-first-contract.json` 与 readback，映射 StageRunWorkflow / StageAttemptActivity / ReconcileWorkflow / HumanGateSignal / OwnerReceiptSignal 到 Temporal workflow/activity/signal/schedule/history；queue lifecycle boundary 现在把旧队列词汇明确降级为 projection / operator handoff readback，并要求 Temporal workflow history/query、retry policy、failure history、stage attempt identity 与 projection repair/retirement receipt；非 Runway durable mutation 的 Console / Foundry Lab / Stagecraft readback 已复用 `queue-projection-vocabulary.ts`；Runway 内 task status、stage attempt status、lease/dead-letter/retry SQL helper、lease select row、retry budget projection、default executor / enqueue / store / harness、dedupe / linked task / PaperMission route / stage-attempt query / repair queue / Temporal observation sync / provider proof 的 queue projection fragment 已集中到 `family-runtime-queue-projection-boundary.ts`。Full scan 当前为 `custom_runtime_queue=147`，其中 `phase4-src-modules-runtime-queue=0`。 | 继续把 stage attempt durable lifecycle 从 SQLite mutation path 迁到真实 Temporal workflow/activity/schedule/history；local provider 保持 dev/CI/offline diagnostic。 |
...
| 10 | OpenTelemetry-style observability | 72% | `partial` | 已落 `contracts/opl-framework/observability-semantic-conventions-contract.json` 与 `src/modules/ledger/observability-semantic-conventions.ts`，把 current owner delta / stage attempt / provider attempt 投影到 trace、metric、log/event vocabulary；`runtime observability-export` 现在直接输出 `semantic_conventions` refs-only readback seed，并在 OpenMetrics 中追加 `opl_queue_length`、`opl_observability_export_boundary`、`opl_observability_exporter_signal_mapping` 与 `opl_observability_collector_export_boundary`；`evidence-envelope` projection 与 `app-operator-drilldown` 已绑定同一 semantic convention signal model；Console runtime tray / stage attempt drilldown 已复用 Ledger-owned semantic vocabulary，保持 no-body / no-ready-claim boundary。Full scan 当前仍为 `custom_observability_ledger=1390`，其中 `phase10-src-modules-observability=128`，说明真实 Collector/exporter 和更广 operator drilldown 消费仍未关闭。 | 继续接入真实 Collector/exporter 消费面与更广的 operator drilldown 同源导航；本阶段不声明 runtime、domain、artifact 或 production readiness。 |
| 11 | No-resurrection scan | 98% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 为 `finding_count=2134`、`hard_gate_finding_count=744`、`advisory_finding_count=1390`、`decisioned_finding_count=2134`、`undecisioned_finding_count=0`；`src/kernel/contract-validation.ts`、共享 JSON boundary helpers、`queue-projection-vocabulary.ts`、`family-runtime-queue-projection-boundary.ts` 与 `managed-update-owner-boundary.ts` 被限定为允许的共享 boundary/owner helper，不能扩大到 generic runtime queue、updater 或 package manager；`phase1-src-modules-schema-cli-boundaries`、`phase0-maintenance-script-boundaries`、`phase2-cli-entrypoint-boundaries`、`phase4-src-modules-runtime-queue` 与 `phase6-src-modules-update-package` 已降为 0。 | 继续把 128 个 `accepted_migration_worklist` findings 逐批迁移；worklist 分类不是 risk eliminated / release ready。 |
...
- Fresh evidence：后续 Console drilldown lane 继续把 runtime tray / stage attempt readback 迁到 Ledger-owned semantic vocabulary；收尾修复将 `appOperatorProjection*` 只通过 `src/modules/ledger/index.ts` 公共入口暴露，避免 Console 深导入 Ledger 内部文件；lane focused tests 18/18 pass；主线 `npm run source:modules -- --strict-imports` status=ok；full scan readback 为 `custom_observability_ledger=1390`、`phase10-src-modules-observability=128`。该证据只证明 readback 同源、public entrypoint boundary 与 private observability worklist 收薄，不证明真实 Collector/exporter、runtime ready、domain ready、artifact ready、production ready 或 owner acceptance。
...
- Fresh evidence：主线继续吸收 `refactor(connect): centralize managed update receipt types` 与 `refactor(connect): reuse managed update owner boundary vocabulary`，把 receipt 类型、rollback/app-state owner decision vocabulary、Foundry Lab promotion/dev-mode fallback vocabulary 与 Runway runtime-environment fallback projection 收到 owner boundary 或 refs-only recovery constant；lane focused tests 39/39 pass；收尾 root 验证 `npm run typecheck` pass、`npm run source:modules -- --strict-imports --strict-cycles` status=ok、`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok；主线 full scan readback 为 `custom_update_or_package_manager=121`、`phase6-src-modules-update-package=0`。该证据只证明 owner-boundary helper / receipt type / fallback projection 收薄，不证明 managed update split 完成、App release ready、runtime currentness ready、production ready 或 owner acceptance。
...
| 彻底解决所有代码风险 | `partial` | 98% | 最新结构切片已落到源码与文档：Runway route sync、evidence-currentness、stage-runtime utility 与 evidence worklist remaining helper 继续复用 shared JSON boundary；maintainer scripts 复用 `scripts/required-value-options.mjs` 与 `script-json-boundary.mjs`，`phase0-maintenance-script-boundaries=0`，`scripts/test-lanes.mjs` 单列为 test harness file catalog projection；`update status/check/plan/apply/repair/rollback` 进入 CLI registry，`phase2-cli-entrypoint-boundaries=0`；Managed Update owner route / execution boundary / receipt vocabulary、component filter/summary、operation mode、receipt write policy、post-apply/reload status、receipt input assembly、receipt type source 与 rollback/app-state owner decision vocabulary 已拆入 `managed-update-owner-boundary.ts`，kernel/runner/receipt ledger/Console/Foundry Lab/Runway projection 不再分别承载这些 owner-boundary helper；Console、Foundry Lab 与 Runway 内 remaining fallback/recovery refs 已改为复用 owner boundary 或 refs-only recovery constant，`phase6-src-modules-update-package=0`；非 durable mutation 的 queue projection vocabulary 已集中到 `queue-projection-vocabulary.ts`，Runway task/stage-attempt status、lease/dead-letter/retry SQL helper、lease select row、retry budget projection、default executor / enqueue / store / harness、dedupe / linked task / PaperMission route / stage-attempt query / repair queue、default executor currentness、MAS route maintenance、PaperMission preflight、queue hold/release、provider-hosted lease select、stage attempt usage/create/progress-log、Temporal observation sync 与 Temporal proof fixture 的 queue projection fragment 已集中到 `family-runtime-queue-projection-boundary.ts`；Observability semantic convention 已固定为 Ledger-owned contract / Runway exporter seed / OpenMetrics boundary，并与 evidence envelope projection、app-operator-drilldown、Console runtime tray / stage attempt drilldown 共享同一 signal model，且 `appOperatorProjection*` 通过 Ledger public entrypoint 暴露；`package-release-discipline.mjs` 将 package-channel verifier 的 rollback 检查标为 contract verifier 例外，`cleanup-ghcr-package-versions.mjs` 将人工保留入口收薄为 protected tag；`opl domain manifests` 通过 Foundry Lab consumer wrapper 增加 `opl_meta_agent_registry` readback，Atlas normalizer 支持 wrapper sidecar skeleton，Connect framework locator 修正为 repo root。Fresh evidence：focused observability exporter tests 6/6 pass；focused Runway family-runtime/provider tests 556/556 pass；managed-update lane focused tests 39/39 pass；observability drilldown lane focused tests 18/18 pass；收尾 root 验证 `npm run typecheck` pass、`npm run source:modules -- --strict-imports --strict-cycles` status=ok、`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok、`git diff --check` pass；本轮 root full reuse-first scan `finding_count=2134`、`hard_gate_finding_count=744`、`advisory_finding_count=1390`、`decisioned_finding_count=2134`、`undecisioned_finding_count=0`、`handwritten_json_boundary=476`、`custom_runtime_queue=147`、`custom_update_or_package_manager=121`、`custom_observability_ledger=1390`、`phase4-src-modules-runtime-queue=0`、`phase6-src-modules-update-package=0`、`phase10-src-modules-observability=128`、`owner_decision_required=0`、`accepted_migration_worklist=128`。 | 仍不是全量完成：Runway durable lifecycle 尚未完全迁到真实 Temporal history；schema/CLI/readback 边界仍有 476 个历史 handwritten JSON findings，主要在 test fixtures、contracts 与允许的 kernel helper；runtime queue 仍有 147 个历史/projection findings，但 src/modules runtime queue migration worklist 已降为 0；update/package 仍有 121 个 category findings，主要是 allowed projection boundary，不再有 src/modules owner-decision worklist；observability 仍有 1390 个历史/advisory findings，其中 src/modules accepted migration worklist 为 128；domain private tail 尚未逐 repo physical closeout；App/Aion live validator、真实 Collector/exporter 消费和 owner acceptance 尚未完成。 | 继续按 runtime durable lifecycle、test fixture/projection cleanup、domain tail、App/Aion consumer-only、observability Collector/exporter、historical findings worklist 顺序推进；不得把 worklist 分类、first-slice 测试绿或 clean diff gate 包装成 production ready。 |
