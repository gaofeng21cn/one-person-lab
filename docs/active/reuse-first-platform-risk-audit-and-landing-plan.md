...
| 8 | Domain private platform retirement | 75% | `partial` | 已落 machine-readable `domain-private-platform-tail-matrix.json` 和 executable `domain_private_platform_tail_matrix` readback，覆盖 MAS/MAG/RCA/OMA/BookForge/ScholarSkills tail class、replacement primitive、retained authority、delete/tombstone gate、forbidden claims、`physical_delete_authorized=false` 与 owner decision gate；OMA live path / production consumption / long-soak ledger JSON readback 已复用共享 OPL JSON boundary；fresh readback 显示 matrix `row_count=6`、全部 domain `physical_delete_authorized=false`，ScholarSkills 仍是 refs-only capability package；`med-autogrant` main `be22a11` 已把 `manifest_builder` 的 runtime continuity quartet 收到共享 `runtime_surfaces` builder，`med-autogrant` main `675685c` 又把 `build-product-entry` 与 `grant-direct-entry` 的 default runtime continuity 组装统一收到 `runtime_surfaces._build_default_runtime_continuity_surfaces()`，`redcube-ai` main `c841499c` 已把 route-run persistence 收薄为 `route-run-record-store.ts` + `route-run-records.ts` policy adapter，`redcube-ai` main `520caab5` 又把 `runWithRecoverableDependencies + continueToStopAfterStage` 统一收到 `recovery.ts::runRouteWithRecoveryAndContinuation()`，`redcube-ai` main `76f8c10d` 再把 `getProductEntrySession` 收成更纯的 orchestrator，并把共享 `sessionFile/runtimeOwner/deliveryIdentity/entryMode` 上下文下沉到 `session-surfaces.ts`。 | 继续开 sibling capability-map / conformance / default-caller-thinning lane，逐项补 no-active-caller、no-forbidden-write、tombstone/provenance、direct/hosted parity 或 owner decision refs；物理删除继续 owner-gated。 |
