...
| 11 | No-resurrection scan | 94% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 为 2602 findings，已全部 decisioned，undecisioned=0；`src/kernel/contract-validation.ts` 和共享 JSON boundary helpers 被明确分类为允许的共享 schema / JSON boundary helper。 | 继续把 151 个 `must_migrate`、358 个 `accepted_migration_worklist` 和 149 个 `owner_decision_required` findings 逐批迁移或做 owner 决策；worklist 分类不是 risk eliminated / release ready。 |
...
- Fresh evidence：`npm run source:modules -- --strict-imports` status=ok；`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok；full scan 中 `scripts` path findings 当前为 24，hard findings 1；full scan 当前为 2602 findings，`custom_update_or_package_manager=248`。
