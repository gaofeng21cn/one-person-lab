...
| 10 | OpenTelemetry-style observability | 45% | `partial` | 已落 `contracts/opl-framework/observability-semantic-conventions-contract.json` 与 `src/modules/ledger/observability-semantic-conventions.ts`，把 current owner delta / stage attempt / provider attempt 投影到 trace、metric、log/event vocabulary，并新增 refs-only JSON / OpenMetrics exporter readback seed，保持 no-body / no-ready-claim boundary。 | 继续接入 operator drilldown 和真实 Collector/exporter 消费面；本阶段不声明 runtime、domain、artifact 或 production readiness。 |
| 11 | No-resurrection scan | 82% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 仍为 3149 findings，其中 decisioned 3148、undecisioned 1。 | 继续把 `must_migrate`、`accepted_migration_worklist`、`owner_decision_required` 和 `undecisioned` 项逐批迁移或做 owner 决策；worklist 分类不是 risk eliminated / release ready。 |
...
| 彻底解决所有代码风险 | `partial` | 56% | Phase 0/1/2/3/4/5/6/8/9/10 都已有 first-slice 代码或合同托底：schema registry seed、ProgressDeltaReceipt schema、DomainProgressTransitionRuntime live-readback schema、runtime CLI JSON boundary helper、CLI registry seed、status command registry coverage、Connect module action registry、Runway lifecycle reconcile/readiness guard、Runway queue lifecycle observed-status safe action、Runway Temporal-first workflow/activity/signal contract readback、Runway durable lifecycle handoff readback、managed update owner routes / owner execution boundary / receipt owner projection、Pack OS content-addressed lock policy、Workspace digest policy、domain private tail matrix executable readback、App/Aion consumer-only contract、artifact provenance bundle、observability semantic conventions/export seed、reuse-first hard/advisory gate、CI strict diff gate，以及 historical decision/worklist readback。第一批 post-absorption focused tests 39/39、typecheck、build、smoke 72/72、reuse-first strict diff gate 已通过；第二批 post-absorption `npm run typecheck`、`npm run build`、`npm test` smoke 72/72、`./scripts/verify.sh reuse-first`、`npm run reuse-first:scan:diff`、`git diff --check` 均通过；historical worklist lane focused test 7/7、Runway durable handoff lane focused tests 30/30、typecheck、reuse-first diff gate、diff check 均通过。 | Runway durable lifecycle 仍未完全迁到真实 Temporal history；大量手写 schema/parser/readback helper 未迁移；domain private tail 尚未逐 repo no-active-caller/owner decision physical closeout；App/Aion consumer-only live validator、real Collector/exporter 消费、full reuse-first scan 仍有 3149 个历史 findings，其中 151 个 `must_migrate`、185 个 `owner_decision_required`、887 个 `accepted_migration_worklist` 和 1 个 `undecisioned` 仍需后续处理。 | 继续按 runtime durable lifecycle、schema/CLI 扩面、domain tail、App/Aion consumer-only、observability collector/exporter 和 historical findings worklist 顺序推进；不得把 worklist 分类、first-slice 测试绿或 clean diff gate 包装成 production ready。 |
