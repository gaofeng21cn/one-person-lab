...
| 11 | No-resurrection scan | 83% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 仍为 3143 findings，其中 decisioned 3142、undecisioned 1。 | 继续把 `must_migrate`、`accepted_migration_worklist`、`owner_decision_required` 和 `undecisioned` 项逐批迁移或做 owner 决策；worklist 分类不是 risk eliminated / release ready。 |
