...
- `2026-07-07` 主线继续关闭过度设计尾巴：Temporal test-server proof 移出生产 `src/**`，文件型 JSON receipt ledger 复用 `src/kernel/json-file.ts` shared helper，family-runtime queue / scheduler CLI option loops 收到邻近 thin parser helper，超长 tests 按行为簇拆分。
...
- 当前 full scan summary 为 `finding_count=535`、`hard_gate_finding_count=211`、`advisory_finding_count=324`、`undecisioned_finding_count=0`、`open_worklist_finding_count=13`、`blocking_worklist_finding_count=5`、`owner_route_open_count=5`；`phase10-test-and-fixture-projections=388` 全部为 `allowed_projection_boundary`。这些剩余项是用户排除、contract provenance、真实 CLI/schema/projection 名或 runtime queue protocol fixture，不得写成 release/currentness/production/domain ready。
- `line-budget` 当前 JSON 读回为 `status=advisory`、`oversize_count=2`、`failure_count=2`：`src/modules/pack/pack-os.ts`、`src/modules/runway/runtime-environment-substrate.ts`。此前 oversize test/fixture 拆分已完成；当前超线来自后续 source work，属于 structure watch-only，不证明 live runtime、App release、domain owner-chain 或 production readiness。
...
| Test / contract projection pruning | `done` | 100% | provider evidence projection test 已删除重复断言；非 package/update 测试词汇继续收薄后 full scan 从 735 降到 535，phase10-test-and-fixture-projections 从 571 降到 388；剩余 phase10 findings 全部为 `allowed_projection_boundary`，不是 undecisioned backlog；`runtime-app-operator-drilldown-route-support` 只保 route support guard，App release evidence gate 回 dedicated tests / shared assertions，App operator drilldown lifecycle/OMA/payload/provider-worker、App release user-path/evidence worklist route support 与 MAS default-executor redrive/stale-admission 删除重复低层 queue / drilldown projection 字段断言；超长测试已拆分到 `line-budget` 的 `oversize_count=0`。 | residual watch-only：package/update 相关项由另会话处理，runtime protocol fixture 保留真实 queue 语义，contract/CLI/schema/projection 名保留真实 surface；不声明 runtime/domain/App release ready。 |
| Fallow / source owner hygiene | `partial` | 85% | `.fallowrc.json` stale source entrypoint 改回真实 `src/entrypoints/**` / `src/modules/**` paths；`foreground-paths.ts` 不再 import family runtime store，切断 Runway Temporal provider cycle tail；source-module readback 作为结构证据。 | `hygiene:fallow` 仍是 advisory cleanup gate；剩余 dead exports / clone groups 需要逐项 active caller 与 owner route 审核，不能按 aggregate count 批量 physical delete。 |
...
| Source/test size watch-only | `partial` | 90% | 原 oversize test/fixture 拆分已完成；当前 `node ./scripts/line-budget.mjs --format json` 读回 `status=advisory`、`oversize_count=2`、`failure_count=2`，超线文件为 `src/modules/pack/pack-os.ts` 与 `src/modules/runway/runtime-environment-substrate.ts`。 | 后续 touched source 文件继续按语义拆；不要为指标单独硬拆，也不能把结构预算通过写成 readiness。 |
