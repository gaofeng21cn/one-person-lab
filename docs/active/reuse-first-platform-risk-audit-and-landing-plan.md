...
| 8 | Domain private platform retirement | 83% | `partial` | 已落 machine-readable `domain-private-platform-tail-matrix.json` 和 executable `domain_private_platform_tail_matrix` readback，覆盖 MAS/MAG/RCA/OMA/BookForge/ScholarSkills tail class、replacement primitive、retained authority、delete/tombstone gate、forbidden claims、`physical_delete_authorized=false` 与 owner decision gate；OMA live path / production consumption / long-soak ledger JSON readback 已复用共享 OPL JSON boundary；fresh readback 显示 matrix `row_count=6`、全部 domain `physical_delete_authorized=false`，ScholarSkills 仍是 refs-only capability package；`med-autogrant` main `be22a11` 已把 `manifest_builder` 的 runtime continuity quartet 收到共享 `runtime_surfaces` builder，`med-autogrant` main `675685c` 又把 `build-product-entry` 与 `grant-direct-entry` 的 default runtime continuity 组装统一收到 `runtime_surfaces._build_default_runtime_continuity_surfaces()`，`med-autogrant` main `142bd76` 再把 manifest builder 统一收到同一个 default continuity helper，只通过 override command 注入 manifest shell 当前 route command，`med-autogrant` main `c75e329` 又把 `lifecycle_receipt_bundle.py` 的 forbidden-write proof 统一复用 `owner_receipt_common.forbidden_write_proof()`，并删除 `retired_surfaces.py` 无实际 caller 的可注入 helper glue，`med-autogrant` main `eef5515` 再把 hosted receipt verification 与 stage-attempt observability 对 `forbidden_write_proof` 的读取统一复用 `owner_receipt_common.read_forbidden_write_proof()`，`med-autogrant` main `f0baf7c` 又把 `receipt_observability.py` 与 `continuous_reconciliation.py` 对 receipt shape/proof 常量的本地重复定义收回到 `owner_receipt_common`，`med-autogrant` main `b6ee8f9` 再把 hosted receipt verification 直接复用 `require_owner_receipt_evidence()` 并删掉 stage-attempt observability 中只做透传的 proof helper，`med-autogrant` main `f072a41` 再把 `receipt_readiness.py` 与 `lifecycle_receipt_bundle.py` 的 owner-receipt / forbidden-write 常量读取统一复用 `owner_receipt_common`，`med-autogrant` main `477685d` 再把 lifecycle receipt kind / operation tuple 与 receipt observability / stage-attempt observability 的重复常量统一复用 `owner_receipt_common`；`redcube-ai` main `c841499c` 已把 route-run persistence 收薄为 `route-run-record-store.ts` + `route-run-records.ts` policy adapter，`redcube-ai` main `520caab5` 又把 `runWithRecoverableDependencies + continueToStopAfterStage` 统一收到 `recovery.ts::runRouteWithRecoveryAndContinuation()`，`redcube-ai` main `76f8c10d` 再把 `getProductEntrySession` 收成更纯的 orchestrator，并把共享 `sessionFile/runtimeOwner/deliveryIdentity/entryMode` 上下文下沉到 `session-surfaces.ts`，`redcube-ai` main `57364cba` 进一步把 `executor-runtime.ts` 中 read-only route-run helper 改成直接 re-export，只保留 3 个仍需 runtime-topology 注入的 active wrapper。 | 继续开 sibling capability-map / conformance / default-caller-thinning lane，逐项补 no-active-caller、no-forbidden-write、tombstone/provenance、direct/hosted parity 或 owner decision refs；RCA 这条 retained tail 的纯代码 ROI 已明显下降，后续只做极小的 helper/glue 收口，物理删除继续 owner-gated。 |
...
| `app_release_owner_route` | `partial` | 91% | `one-person-lab-app` main `77ee281` 已吸收 settings cognition closeout：`tests/release/app-release-boundary-cases/settings-control-plane-validation.ts` 6/6 pass、`tests/release/app-release-boundary-cases/app-gui-product-contract.ts` 1/1 pass、`OPL_APP_SHELL_ROOT=/Users/gaofeng/workspace/opl-aion-shell npm run validate:active-shell -- --quick` pass；`docs/active/app-ideal-state-gap-plan.md` 已把 `settings_user_cognition_reorg` 改为结构闭合、剩余 release evidence 后置。 | 仍缺 App release owner receipt、managed update receipt 被 App release/currentness truth 消费、release/currentness source ref；结构闭合不等于 release ready。 | 保持 App repo 为 release truth owner，后续只走 App release authority 和 owner receipt / typed blocker。 |
