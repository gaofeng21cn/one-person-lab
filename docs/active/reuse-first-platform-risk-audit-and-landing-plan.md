...
| 10 | OpenTelemetry-style observability | 86% | `partial` | 已落 `contracts/opl-framework/observability-semantic-conventions-contract.json` 与 `src/modules/ledger/observability-semantic-conventions.ts`，把 current owner delta / stage attempt / provider attempt 投影到 trace、metric、log/event vocabulary；`runtime observability-export` 现在直接输出 `semantic_conventions` refs-only readback seed，并在 OpenMetrics 中追加 `opl_queue_length`、`opl_observability_export_boundary`、`opl_observability_exporter_signal_mapping`、`opl_observability_collector_export_boundary` 与 `opl_observability_collector_consumption_config`；JSON readback 已携带 Prometheus receiver / OpenMetrics input / batch processor / debug exporter 的 Collector YAML 等价配置片段，`--format collector-config-json` 可直接输出同一 Collector config JSON；`runtime observability-endpoint` 已用 Node 标准 HTTP server 暴露 read-only OpenMetrics `/metrics` endpoint，并通过 CLI registry 保护 `runtime observability-*` 命令；`evidence-envelope` projection 与 `app-operator-drilldown` 已绑定同一 semantic convention signal model；Console runtime tray / stage attempt drilldown 已复用 Ledger-owned semantic vocabulary，保持 no-body / no-ready-claim boundary；observability projection field / label vocabulary 已集中到 `src/kernel/observability-projection-vocabulary.ts`，source-module diagnostic drilldown 词项已收为 refs-only command/readback projection，且扫描器仍会保留同一行里的 private ledger terms 风险。Full scan 当前为 `custom_observability_ledger=536`，其中 `phase10-src-modules-observability=0`。 | 继续补外部 Collector 运行证据或 OTLP SDK exporter owner lane，并把剩余 test / contract / entrypoint projection findings 维持在 allowed boundary 或后续删除；本阶段不声明 runtime、domain、artifact 或 production readiness。 |
| 11 | No-resurrection scan | 99% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 为 `finding_count=1276`、`hard_gate_finding_count=740`、`advisory_finding_count=536`、`decisioned_finding_count=1276`、`undecisioned_finding_count=0`，全部归入 `allowed_projection_boundary`，`accepted_migration_worklist=0`、`owner_decision_required=0`；`src/kernel/contract-validation.ts`、共享 JSON boundary helpers、`queue-projection-vocabulary.ts`、`observability-projection-vocabulary.ts`、`family-runtime-queue-projection-boundary.ts` 与 `managed-update-owner-boundary.ts` 被限定为允许的共享 boundary/owner helper；source-module diagnostic drilldown 只在不夹带 private ledger terms 时允许，不能扩大到 generic observability ledger；`phase1-src-modules-schema-cli-boundaries`、`phase0-maintenance-script-boundaries`、`phase2-cli-entrypoint-boundaries`、`phase4-src-modules-runtime-queue`、`phase6-src-modules-update-package` 与 `phase10-src-modules-observability` 已降为 0。 | 继续处理 tests / contracts / entrypoints 里的历史 projection findings；`phase10-test-and-fixture-projections=1115`，worklist 分类不是 risk eliminated / release ready。 |
...
- Fresh evidence：full scan 当前为 `finding_count=1276`、`hard_gate_finding_count=740`、`advisory_finding_count=536`、`decisioned_finding_count=1276`、`undecisioned_finding_count=0`；`custom_observability_ledger=536`、`phase10-src-modules-observability=0`。该证据只证明 diagnostic projection 分类更准，不证明真实 Collector/exporter、runtime ready、domain ready、artifact ready、production ready 或 owner acceptance。
...
| 彻底解决所有代码风险 | `partial` | 99% | 最新结构切片已落到源码与文档：Runway route sync、evidence-currentness、stage-runtime utility 与 evidence worklist remaining helper 继续复用 shared JSON boundary；`family-runtime attempt query` 已增加 Temporal durable lifecycle identity readback；maintainer scripts 复用 `scripts/required-value-options.mjs` 与 `script-json-boundary.mjs`，`phase0-maintenance-script-boundaries=0`，`scripts/test-lanes.mjs` 单列为 test harness file catalog projection；`update status/check/plan/apply/repair/rollback` 与 `runtime observability-export/endpoint` 进入 CLI registry，`phase2-cli-entrypoint-boundaries=0`；Managed Update owner route / execution boundary / receipt vocabulary、component filter/summary、operation mode、receipt write policy、post-apply/reload status、receipt input assembly、receipt type source 与 rollback/app-state owner decision vocabulary 已拆入 `managed-update-owner-boundary.ts`，kernel/runner/receipt ledger/Console/Foundry Lab/Runway projection 不再分别承载这些 owner-boundary helper，`managedUpdateCommand()` 默认 `--json` 且 Settings delegated surface 可显式保持非 JSON action string，`phase6-src-modules-update-package=0`；非 durable mutation 的 queue projection vocabulary 与 Runway queue projection fragment 已集中到 `queue-projection-vocabulary.ts` 和 `family-runtime-queue-projection-boundary.ts`，`phase4-src-modules-runtime-queue=0`；Observability semantic convention 已固定为 Ledger-owned contract / Runway exporter seed / OpenMetrics boundary，并与 evidence envelope projection、app-operator-drilldown、Console runtime tray / stage attempt drilldown 共享同一 signal model，Collector consumption config 已作为 refs-only readback 露出且 `--format collector-config-json` 可直接输出同一配置；`runtime observability-endpoint` 已暴露真实 HTTP `/metrics` endpoint，CLI one-shot readback 证明 `fetch()` 可命中 OpenMetrics 输出；observability projection field / label vocabulary 已集中到 kernel，source-module diagnostic drilldown 已分类为 refs-only projection 且不豁免 private ledger terms。Fresh evidence：focused observability exporter/endpoint + semantic convention + CLI registry + product projection tests 23/23 pass；CLI endpoint one-shot readback status=ok；focused Temporal CLI/control-loop tests 20/20 pass；focused Temporal provider tests 12/12 pass；focused runtime App/operator drilldown + reuse-first scan tests 139/139 pass；`npm test` smoke 81/81 pass；收尾验证 `npm run typecheck` pass、`npm run source:modules -- --strict-imports --strict-cycles` status=ok、`npm run reuse-first:scan:diff -- --strict` 返回 `finding_count=0`、`hard_gate_finding_count=0`、`advisory_finding_count=0`，`git diff --check` pass；当前 full reuse-first scan `finding_count=1276`、`hard_gate_finding_count=740`、`advisory_finding_count=536`、`decisioned_finding_count=1276`、`undecisioned_finding_count=0`、`allowed_projection_boundary=1276`、`accepted_migration_worklist=0`、`owner_decision_required=0`、`handwritten_json_boundary=472`、`custom_runtime_queue=147`、`custom_update_or_package_manager=121`、`custom_observability_ledger=536`、`phase4-src-modules-runtime-queue=0`、`phase6-src-modules-update-package=0`、`phase10-src-modules-observability=0`。 | 仍不是全量完成：Runway durable lifecycle 尚未完全迁到真实 Temporal history；schema/CLI/readback 边界仍有 472 个历史 handwritten JSON findings，主要在 test fixtures、contracts 与允许的 kernel helper；runtime queue 仍有 147 个历史/projection findings，但 src/modules runtime queue migration worklist 已降为 0；update/package 仍有 121 个 category findings，主要是 allowed projection boundary，不再有 src/modules owner-decision worklist；observability 仍有 536 个历史/advisory findings，source-module observability migration worklist 已降为 0，HTTP endpoint 已补，但外部 Collector 运行证据或 OTLP SDK exporter 未完成；domain private tail 尚未逐 repo physical closeout；App/Aion live validator、真实 Collector/exporter 消费和 owner acceptance 尚未完成。 | 继续按 runtime durable lifecycle、test fixture/projection cleanup、domain tail、App/Aion consumer-only、observability Collector/exporter、historical findings worklist 顺序推进；不得把 worklist 分类、first-slice 测试绿或 clean diff gate 包装成 production ready。 |
