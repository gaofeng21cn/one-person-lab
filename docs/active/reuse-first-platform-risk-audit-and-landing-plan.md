...
- Test projection pruning 继续收薄非 package/update 断言：删除重复 `runtime app-operator-drilldown` registry 词汇测试、合并 current-control repair 中的 lease 字段级断言、删除重复 MAS redrive / provider attempts 低层场景断言，删除 route-support 测试里重复锁 App release evidence gate 的字段级断言，并收薄 App operator drilldown lifecycle、OMA、provider-worker、MAG/MAS/RCA payload、App release user-path 以及 MAS default-executor redrive/stale-admission 中重复低层 projection 字段断言；保留 cross-repo runtime queue E2E、关键 redrive readback、dedicated App release evidence guard 和 authority false-ready guard。
- 当前 full scan summary 为 `finding_count=570`、`hard_gate_finding_count=210`、`advisory_finding_count=360`、`open_worklist_finding_count=13`、`blocking_worklist_finding_count=5`、`owner_route_open_count=5`；这些剩余项仍是历史 / owner-route / live-evidence backlog，不得写成 release/currentness/production/domain ready。
...
| Test / contract projection pruning | `partial` | 86% | provider evidence projection test 已删除重复断言；非 package/update 测试词汇继续收薄后 full scan 从 735 降到 570，phase10-test-and-fixture-projections 从 571 降到 423；`runtime-app-operator-drilldown-route-support` 只保 route support guard，App release evidence gate 回 dedicated tests / shared assertions，App operator drilldown lifecycle/OMA/payload/provider-worker 与 MAS default-executor redrive/stale-admission 删除重复低层 queue / drilldown projection 字段断言。 | 剩余 423 个 phase10 findings 仍说明 test harness projection vocabulary 很厚；其中 package/update 相关项由另会话处理，runtime protocol fixture 只能保留真实 queue 语义。继续删除只锁历史词汇的重复测试，但保留关键 ready-claim guard、runtime queue E2E 和 scanner/contract guard。 |
