...
| 11 | No-resurrection scan | 96% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 为 2546 findings，已全部 decisioned，undecisioned=0；`src/kernel/contract-validation.ts` 和共享 JSON boundary helpers 被明确分类为允许的共享 schema / JSON boundary helper；`phase1-src-modules-schema-cli-boundaries`、`phase0-maintenance-script-boundaries` 与 `phase2-cli-entrypoint-boundaries` 已降为 0。 | 继续把 151 个 `must_migrate`、263 个 `accepted_migration_worklist` 和 125 个 `owner_decision_required` findings 逐批迁移或做 owner 决策；worklist 分类不是 risk eliminated / release ready。 |
...
| 彻底解决所有代码风险 | `partial` | 92% | 最新结构切片已落到源码与文档：Runway route sync、evidence-currentness、stage-runtime utility 与 evidence worklist remaining helper 继续复用 shared JSON boundary；maintainer scripts 复用 `scripts/required-value-options.mjs` 与 `script-json-boundary.mjs`，`phase0-maintenance-script-boundaries=0`，`scripts/test-lanes.mjs` 单列为 test harness file catalog projection；`update status/check/plan/apply/repair/rollback` 进入 CLI registry，`phase2-cli-entrypoint-boundaries=0`；evidence envelope projection 与 app-operator-drilldown 绑定 OpenTelemetry-style semantic convention signal model；`package-release-discipline.mjs` 将 package-channel verifier 的 rollback 检查标为 contract verifier 例外，`cleanup-ghcr-package-versions.mjs` 将人工保留入口收薄为 protected tag；`opl domain manifests` 通过 Foundry Lab consumer wrapper 增加 `opl_meta_agent_registry` readback，Atlas normalizer 支持 wrapper sidecar skeleton，Connect framework locator 修正为 repo root。Fresh evidence：focused CLI/update/observability/reuse-first tests 44/44 pass；`npm run typecheck` pass；`npm run source:modules -- --strict-imports` status=ok，deep import violations=0，forbidden dependency violations=0，dependency cycle=0；`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok；`git diff --check origin/main..HEAD` pass；`npm test` smoke 77/77 pass；full reuse-first scan `finding_count=2546`、`hard_gate_finding_count=1021`、`advisory_finding_count=1525`、`decisioned_finding_count=2546`、`undecisioned_finding_count=0`、`handwritten_json_boundary=476`、`custom_runtime_queue=298`、`custom_update_or_package_manager=247`、`owner_decision_required=125`、`accepted_migration_worklist=263`。 | 仍不是全量完成：Runway durable lifecycle 尚未完全迁到真实 Temporal history；schema/CLI/readback 边界仍有 476 个历史 handwritten JSON findings，主要在 test fixtures、contracts 与允许的 kernel helper；runtime queue 仍有 298 个 must-migrate findings；update/package仍有 247 个 category findings，其中 125 个仍需 owner decision；domain private tail 尚未逐 repo physical closeout；App/Aion live validator、真实 Collector/exporter 消费和 owner acceptance 尚未完成。 | 继续按 runtime durable lifecycle、test fixture/projection cleanup、domain tail、App/Aion consumer-only、observability Collector/exporter、historical findings worklist 顺序推进；不得把 worklist 分类、first-slice 测试绿或 clean diff gate 包装成 production ready。 |
