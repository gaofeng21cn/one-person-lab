...
| 2 | Schema boundary consolidation | 28% | `partial` | Phase 1 seed lane 已引入 Ajv-backed `src/kernel/schema-registry.ts`；`ProgressDeltaReceipt` 与 `DomainProgressTransitionRuntime` live readback 已分别迁到 `progress-delta-receipt.schema.json`、`domain-progress-transition-runtime-live-readback.schema.json` + shared schema registry，并有 focused valid/invalid / false-authority drift tests。大量 hand-written JSON/readback helper 仍未迁移。 | 继续把 managed update receipts、pack descriptors、workspace projections、CLI JSON outputs 接入 schema registry；禁止新增分散 validator。 |
...
| 11 | No-resurrection scan | 70% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；CI lint/structure job 已接入 `./scripts/verify.sh reuse-first`；本轮 domain-tail 新 helper 先被 diff gate 抓出 `isRecord` / `JSON.parse`，随后改为复用 shared `contract-validation.isRecord` 和 JSON import，避免新增分散 JSON boundary。 | 继续为历史 findings 建立采用/拒绝 decision record / expiry，并逐批迁移或标注例外。 |
...
| 彻底解决所有代码风险 | `partial` | 46% | Phase 0/1/2/3/4/5/6/8/9/10 都已有 first-slice 代码或合同托底：schema registry seed、ProgressDeltaReceipt schema、DomainProgressTransitionRuntime live-readback schema、CLI registry seed、status command registry coverage、Connect module action registry、Runway lifecycle reconcile/readiness guard、Runway queue lifecycle observed-status safe action、managed update owner routes / owner execution boundary / receipt owner projection、Pack/Workspace descriptor/digest、domain private tail matrix executable readback、App/Aion consumer-only contract、artifact provenance bundle、observability semantic conventions/export seed、reuse-first hard/advisory gate，以及 CI strict diff gate。本轮四条 lanes 已吸收到 `main`；post-absorption focused tests 39/39、typecheck、build、smoke 72/72、reuse-first strict diff gate 均通过。 | Runway durable lifecycle 仍未完全迁到 Temporal；大量手写 schema/parser/readback helper 未迁移；domain private tail 尚未逐 repo no-active-caller/owner decision physical closeout；App/Aion consumer-only live validator、real Collector/exporter 消费、full reuse-first scan 仍有 3161 个历史 findings。 | 继续按 runtime、schema/CLI 扩面、domain tail、App/Aion consumer-only、observability collector/exporter 和 historical findings 消化顺序推进；不得把 first-slice 测试绿包装成 production ready。 |
