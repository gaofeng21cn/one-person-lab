...
| 11 | No-resurrection scan | 72% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；CI lint/structure job 已接入 `./scripts/verify.sh reuse-first`；本轮 domain-tail 新 helper 先被 diff gate 抓出并修复，第二批 runtime CLI helper / Temporal contract / Pack lock policy 也保持 diff gate 0 findings。 | 继续为历史 findings 建立采用/拒绝 decision record / expiry，并逐批迁移或标注例外；full scan 当前仍有 3149 个历史 findings。 |
