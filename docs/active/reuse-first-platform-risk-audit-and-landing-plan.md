...
| 2 | Schema boundary consolidation | 42% | `partial` | Phase 1 seed lane 已引入 Ajv-backed `src/kernel/schema-registry.ts`；`ProgressDeltaReceipt` 与 `DomainProgressTransitionRuntime` live readback 已分别迁到 schema registry；runtime / entrypoint CLI JSON boundary 已新增 centralized `json-boundary.ts`，累计消化 runtime command spec、App release / OMA live path、Agent Lab public command、OKF / ScholarSkills public spec 与 Agent Lab payload parser 的分散 helper；Pack / Foundry JSON file boundary 已集中到 `src/kernel/json-file.ts`；Atlas domain-manifest / memory contract first-slice 已复用 `src/kernel/json-record.ts`。Full scan 当前仍有 `handwritten_json_boundary=1008`。 | 继续把 managed update receipts、pack descriptors、workspace projections、剩余 module JSON outputs 接入 schema registry/shared boundary；禁止新增分散 validator。 |
...
| 11 | No-resurrection scan | 88% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 为 3097 findings，已全部 decisioned，undecisioned=0；`src/kernel/contract-validation.ts` 被明确分类为共享 schema registry JSON boundary helper。 | 继续把 151 个 `must_migrate`、831 个 `accepted_migration_worklist` 和 182 个 `owner_decision_required` findings 逐批迁移或做 owner 决策；worklist 分类不是 risk eliminated / release ready。 |
...
| 彻底解决所有代码风险 | `partial` | 74% | Phase 0/1/2/3/4/5/6/8/9/10 都已有 first-slice 代码或合同托底：schema registry seed、ProgressDeltaReceipt schema、DomainProgressTransitionRuntime live-readback schema、runtime / entrypoint CLI JSON boundary helper、Atlas JSON helper first-slice、Pack / Foundry JSON file helper、maintainer script JSON helper、CLI registry seed、status command registry coverage、Connect module action registry、Connect external scientific skill source registry / selective sync、Connect reference verification provider receipt readback（含 Publisher metadata provider）、Runway lifecycle reconcile/readiness guard、Runway queue lifecycle observed-status safe action、Runway Temporal-first workflow/activity/signal contract readback、Runway durable lifecycle handoff readback、queue vocabulary Temporal handoff readback、runtime snapshot provider injection、managed update owner routes / owner execution boundary / receipt owner projection、Pack OS content-addressed lock policy、Workspace digest policy、domain private tail matrix executable readback、App/Aion consumer-only contract、artifact provenance bundle、artifact provenance ledger event schema/readback、Agent Lab self-evolution work-order schema/failure token registry/capability routing guard/work-order dry-run receipt、observability semantic conventions/export seed、runtime observability export semantic-convention binding、source module public-boundary cleanup、source module owner alignment / forbidden dependency policy、source-ref kernel helper、reuse-first hard/advisory gate、CI strict diff gate，以及 historical decision/worklist readback。当前 fresh evidence：focused Runway provider-injection tests 30/30 pass，`npm run typecheck` pass，`npm run build` pass，`npm test` smoke 76/76 pass，`npm run source:modules` status=ok 且 `runway -> console` pair 已消失，`npm run reuse-first:scan:diff -- --strict` gate_status=ok，`node ./scripts/reuse-first-scan.mjs --max-findings=0` 返回 `finding_count=3097`、`hard_gate_finding_count=1557`、`advisory_finding_count=1540`、`decisioned_finding_count=3097`、`undecisioned_finding_count=0`；CLI lane focused tests 70/70 pass，Atlas lane focused tests 18/18 pass。 | Runway durable lifecycle 仍未完全迁到真实 Temporal history；大量手写 schema/parser/readback helper 未迁移，full scan 仍有 `handwritten_json_boundary=1008`；domain private tail 尚未逐 repo no-active-caller/owner decision physical closeout；App/Aion consumer-only live validator、real Collector/exporter 消费、full reuse-first scan 仍有 3097 个历史 findings，其中 151 个 `must_migrate`、182 个 `owner_decision_required`、831 个 `accepted_migration_worklist` 仍需后续处理；source dependency cycle 仍是 advisory。 | 继续按 runtime durable lifecycle、schema/CLI 扩面、domain tail、App/Aion consumer-only、observability collector/exporter 和 historical findings worklist 顺序推进；不得把 worklist 分类、first-slice 测试绿或 clean diff gate 包装成 production ready。 |
