...
| 4 | Runway Temporal-first runtime | 75% | `partial` | Temporal SDK 已是一等依赖，docs 已声明 Temporal production substrate；`family-runtime status/queue list` 已暴露 `queue_lifecycle_boundary`；Runway control-loop 已把 competing SQLite queue lifecycle 纳入 Temporal readiness 降级；新增 `family-runtime-temporal-first-contract.json` 与 readback，映射 StageRunWorkflow / StageAttemptActivity / ReconcileWorkflow / HumanGateSignal / OwnerReceiptSignal 到 Temporal workflow/activity/signal/schedule/history；queue lifecycle boundary 现在把旧队列词汇明确降级为 projection / operator handoff readback，并要求 Temporal workflow history/query、retry policy、failure history、stage attempt identity 与 projection repair/retirement receipt；非 Runway durable mutation 的 Console / Foundry Lab / Stagecraft readback 已复用 `queue-projection-vocabulary.ts`；Runway 内 task status、stage attempt status、lease/dead-letter/retry SQL helper、default executor / enqueue / store / harness projection 字段，以及 dedupe / linked task / PaperMission route / stage-attempt query / repair queue 的 queue projection fragment 已集中到 `family-runtime-queue-projection-boundary.ts`。Full scan 当前为 `custom_runtime_queue=168`，其中 `phase4-src-modules-runtime-queue=21`。 | 继续把 stage attempt durable lifecycle 从 SQLite mutation path 迁到真实 Temporal workflow/activity/schedule/history；local provider 保持 dev/CI/offline diagnostic。 |
...
| 6 | Managed update split | 66% | `partial` | App release channel 与 managed update plane 边界存在；已增加 `owner_route_contract`、`owner_execution_boundary_contract`、组件级 `owner_route` / `owner_execution_boundary` 与 receipt `owner_projection`，把 app binary / runtime substrate / capability packages 明确路由到 owner/readback/apply owner，并保留 no-package-manager forbidden claims；receipt JSON 读取已复用共享 record boundary，不再在 managed update 模块私写 `optionalString` / `optionalStringArray` helper；`ManagedUpdateOperation`、provider ids、component model、owner route、owner execution boundary、receipt projection required fields、no-auto-apply / status-detail builders、component filter/summary、operation mode、receipt write policy、post-apply/reload status 与 receipt input assembly 已从 `managed-update-kernel.ts` / runner 拆到 `managed-update-owner-boundary.ts`，kernel 只保 projection 组装与 component readback。Full scan 当前为 `custom_update_or_package_manager=206`，其中 `phase6-src-modules-update-package=85`。 | 继续把 runtime materializer、App release updater、capability package channel 与 companion/workflow manual owner route 的 active caller 边界继续拆薄；不要声明 release/currentness ready。 |
...
| 10 | OpenTelemetry-style observability | 65% | `partial` | 已落 `contracts/opl-framework/observability-semantic-conventions-contract.json` 与 `src/modules/ledger/observability-semantic-conventions.ts`，把 current owner delta / stage attempt / provider attempt 投影到 trace、metric、log/event vocabulary；`runtime observability-export` 现在直接输出 `semantic_conventions` refs-only readback seed，并在 OpenMetrics 中追加 `opl_queue_length`、`opl_observability_export_boundary`、`opl_observability_exporter_signal_mapping` 与 `opl_observability_collector_export_boundary`；`evidence-envelope` projection 也已绑定同一 semantic convention signal model；`app-operator-drilldown` 现在直接暴露同源 `semantic_conventions` refs-only projection，保持 no-body / no-ready-claim boundary。Full scan 当前仍为 `custom_observability_ledger=1525`，说明真实 Collector/exporter 和更广 operator drilldown 消费仍未关闭。 | 继续接入真实 Collector/exporter 消费面与更广的 operator drilldown 同源导航；本阶段不声明 runtime、domain、artifact 或 production readiness。 |
| 11 | No-resurrection scan | 97% | `partial` | `reuse-first-scan` 已支持 strict diff hard/advisory gate，新增 schema/CLI/runtime/update/package 类 hard finding 会失败，observability ledger 类先进入 advisory；diff 模式明确不应用 historical worklist 豁免；full 模式按 decision status / category / path-prefix / owner / phase / action / expiry 输出 historical decision/worklist readback。当前 full scan 为 `finding_count=2364`、`hard_gate_finding_count=839`、`advisory_finding_count=1525`、`decisioned_finding_count=2364`、`undecisioned_finding_count=0`；`src/kernel/contract-validation.ts`、共享 JSON boundary helpers、`queue-projection-vocabulary.ts`、`family-runtime-queue-projection-boundary.ts` 与 `managed-update-owner-boundary.ts` 被限定为允许的共享 boundary/owner helper，不能扩大到 generic runtime queue、updater 或 package manager；`phase1-src-modules-schema-cli-boundaries`、`phase0-maintenance-script-boundaries` 与 `phase2-cli-entrypoint-boundaries` 已降为 0。 | 继续把 21 个 `must_migrate`、263 个 `accepted_migration_worklist` 和 74 个 `owner_decision_required` findings 逐批迁移或做 owner 决策；worklist 分类不是 risk eliminated / release ready。 |
...
- Fresh evidence：focused `runtime-app-operator-drilldown` core assertion 与 reuse-first diff gate覆盖该字段；该证据只证明 readback 同源，不证明真实 Collector/exporter、runtime ready、domain ready、artifact ready、production ready 或 owner acceptance。
...
- Fresh evidence：主线继续吸收 `refactor(connect): centralize managed update receipt types`，把 receipt 类型也集中到 owner boundary；focused managed-update tests 14/14 pass；主线综合验证 `npm run typecheck` pass、`npm run source:modules -- --strict-imports` status=ok、`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok、`npm test` smoke 79/79 pass；full scan readback 为 `finding_count=2394`、`custom_update_or_package_manager=195`、`phase6-src-modules-update-package=74`。该证据只证明 owner-boundary helper / receipt type 收薄，不证明 managed update split 完成、App release ready、runtime currentness ready、production ready 或 owner acceptance。
...
- Fresh evidence：主线继续吸收 `refactor(runway): fold remaining queue projection fields` 与 `refactor(runway): reuse queue projection boundary fragments`，把 default executor / enqueue / store / harness，以及 dedupe / linked task / PaperMission route / stage-attempt query / repair queue 的 queue projection 字段接入同一 queue boundary；focused family-runtime/provider tests 556/556 pass；主线综合验证 `npm run typecheck` pass、`npm run source:modules -- --strict-imports` status=ok、`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok、`git diff --check` pass；full scan readback 为 `finding_count=2364`、`custom_runtime_queue=168`、`phase4-src-modules-runtime-queue=21`。该证据只证明 queue projection boundary 继续收薄，不证明 durable lifecycle 已迁到 Temporal、runtime ready、domain ready、production ready 或 owner acceptance。
...
| 彻底解决所有代码风险 | `partial` | 96% | 最新结构切片已落到源码与文档：Runway route sync、evidence-currentness、stage-runtime utility 与 evidence worklist remaining helper 继续复用 shared JSON boundary；maintainer scripts 复用 `scripts/required-value-options.mjs` 与 `script-json-boundary.mjs`，`phase0-maintenance-script-boundaries=0`，`scripts/test-lanes.mjs` 单列为 test harness file catalog projection；`update status/check/plan/apply/repair/rollback` 进入 CLI registry，`phase2-cli-entrypoint-boundaries=0`；Managed Update owner route / execution boundary / receipt vocabulary、component filter/summary、operation mode、receipt write policy、post-apply/reload status、receipt input assembly 与 receipt type source 已拆入 `managed-update-owner-boundary.ts`，kernel/runner/receipt ledger 不再同时承载这些 owner-boundary helper；非 durable mutation 的 queue projection vocabulary 已集中到 `queue-projection-vocabulary.ts`，Runway task/stage-attempt status、lease/dead-letter/retry SQL helper、default executor / enqueue / store / harness 以及 dedupe / linked task / PaperMission route / stage-attempt query / repair queue 的 queue projection fragment 已集中到 `family-runtime-queue-projection-boundary.ts`；Observability semantic convention 已固定为 Ledger-owned contract / Runway exporter seed / OpenMetrics boundary，并与 evidence envelope projection、app-operator-drilldown 共享同一 signal model；`package-release-discipline.mjs` 将 package-channel verifier 的 rollback 检查标为 contract verifier 例外，`cleanup-ghcr-package-versions.mjs` 将人工保留入口收薄为 protected tag；`opl domain manifests` 通过 Foundry Lab consumer wrapper 增加 `opl_meta_agent_registry` readback，Atlas normalizer 支持 wrapper sidecar skeleton，Connect framework locator 修正为 repo root。Fresh evidence：focused observability exporter tests 6/6 pass；focused Runway family-runtime/provider tests 556/556 pass；focused managed-update tests 14/14 pass；`npm run typecheck` pass；`npm run source:modules -- --strict-imports` status=ok；`npm run reuse-first:scan:diff -- --diff-ref origin/main --strict` gate_status=ok；`git diff --check` pass；full reuse-first scan `finding_count=2364`、`hard_gate_finding_count=839`、`advisory_finding_count=1525`、`decisioned_finding_count=2364`、`undecisioned_finding_count=0`、`handwritten_json_boundary=476`、`custom_runtime_queue=168`、`custom_update_or_package_manager=195`、`custom_observability_ledger=1525`、`must_migrate=21`、`owner_decision_required=74`、`accepted_migration_worklist=263`。 | 仍不是全量完成：Runway durable lifecycle 尚未完全迁到真实 Temporal history；schema/CLI/readback 边界仍有 476 个历史 handwritten JSON findings，主要在 test fixtures、contracts 与允许的 kernel helper；runtime queue 仍有 168 个 findings，其中 21 个 `must_migrate`；update/package 仍有 195 个 category findings，其中 74 个仍需 owner decision；domain private tail 尚未逐 repo physical closeout；App/Aion live validator、真实 Collector/exporter 消费和 owner acceptance 尚未完成。 | 继续按 runtime durable lifecycle、test fixture/projection cleanup、domain tail、App/Aion consumer-only、observability Collector/exporter、historical findings worklist 顺序推进；不得把 worklist 分类、first-slice 测试绿或 clean diff gate 包装成 production ready。 |
