...
这个 pattern 适用于 `MAS`、`MAG`、`RCA` 和 `OMA`。OPL 持有通用运行时、Pack compiler、generated/hosted surfaces、current_owner_delta 默认读面、refs-only Ledger、Console 和 cross-agent conformance；domain 仓只保留无法声明化的领域 authority kernel。
...
调研判断折回：把 OPL family 描述成“个人 AI 操作系统”是合理的产品 / 体验层概括，前提是它只指一个 executor-first 的个人工作 operating model：App / Console 给用户统一入口，OPL Framework 提供 StageRun、Pack、Runway、Ledger、Console、Connect、Workspace 等通用 substrate，MAS/MAG/RCA/OMA/BookForge 作为标准 Foundry Agent 提供 domain pack 与 authority kernel。“Foundry Agent 工厂”也是合理的工程概括，前提是“工厂”只表示用 pack compiler、generated/hosted surfaces、cross-agent conformance、capability registry 和 Agent Lab work-order 生产 / 验收标准 agent surface；它不表示 OPL 可以替 domain owner 生产 owner receipt、typed blocker、quality/export/review verdict、artifact mutation、App release verdict、Brand L5 verdict 或 production-ready claim。

Plan Completion Audit 映射：`W0-W6` 目前只能按结构 / 功能落地审计，已落地项的完成证据是 contract/source/CLI/API/readback/focused tests；这些条目可支撑“Foundry Agent OS 目标 ABI 和工厂式生成 / 验收骨架已建立”。`W7` 只能按 live / owner evidence 后置缺口审计，完成证据必须是真实 owner receipt、typed blocker、human gate、quality/export/review receipt、long-soak、release/install、private-platform owner decision、owner acceptance 或 no-regression refs。任何 focused tests、read-model clean、refs-only ledger、App projection、conformance pass、queue empty、docs foldback 或 commit-only evidence 都只能映射到对应 planning row 的结构证据，不能把 Foundry Agent OS、个人 AI 操作系统、Foundry Agent 工厂、domain ready、App release ready、Brand L5 或 production ready 标为 `100%`。
...
- OPL / Ledger / Console / Runway / Pack / Capability Registry 不能签 `owner_receipt`、不能创建 domain `typed_blocker`、不能写 domain truth、不能授权 quality / export / publication verdict。
- refs-only Ledger 只能记录、校验和投影 domain-owned refs；只有 domain owner 把 refs fold 成 owner delta、owner answer、hard gate、route-back、human gate 或 typed blocker 后，才影响默认下一步。
...
| `current_owner_delta` default read root | `OPL Console` | `Runway`, `Ledger`, `Stagecraft` | `ordinary` | Console 只能投影 current owner / next delta / accepted answer shape，不能签 owner answer。 |
| Durable StageRun execution | `OPL Runway` | `Stagecraft`, `Ledger` | `ordinary` | Provider completion 不能变成 domain completion。 |
| Evidence / lineage | `OPL Ledger` | `Console`, `Runway` | `audit` | Ledger 只存 refs，不存 artifact body / memory body，也不把 ref 变成 owner authority。 |
...
| `BookForge` | `OPL Agent OS + Book Pack + Manuscript Authority Kernel + Artifact Lifecycle Registry` | generic runtime、Pack compiler、generated CLI/MCP/App/status surfaces、source/artifact lifecycle shell、OKF context bundle、Console/workbench、refs-only evidence、capability registry ABI。 | book/manuscript truth、source byproduct interpretation、artifact lifecycle handoff acceptance、export/package authority、editorial quality verdict、owner receipt、typed blocker、human gate。 |

BookForge 是当前 standard Foundry Agent member，但 W7 不在本文维护独立 backlog。它的结构归位由 `opl agents conformance --family-defaults --json`、OKF / pack compiler / generated surface readback 和 BookForge repo-owned contracts 证明；真实 manuscript、artifact lifecycle、export/package、editorial quality 或 production evidence 仍必须由 BookForge owner receipt、typed blocker、human gate、no-regression ref 或 owner acceptance 关闭。
...
- `contracts/opl-framework/brand-module-registry.json` 把 `Atlas / Pack / Stagecraft / Runway / Ledger / Console` 的 target architecture、capability registry 和 false-authority 边界折回 registry。
...
- MAS/MAG/RCA/OMA/BookForge 已 domain ready；
...
| `L6` | Evidence / lineage / Ledger | Ledger 只做 refs-only sidecar，L5 和 production evidence 不抢 default next action。 | evidence refs 只能 fold 成 owner delta / hard gate / owner answer / typed blocker 后影响 default planning。 |
...
- OPL / Ledger / Console / Runway / Pack 不签 owner receipt、不创建 domain typed blocker、不授权 quality/export verdict；
...
User / Agent / App
  ...
  -> OPL StageRun / Pack / Ledger / Console / Capability ABI
...
- Ledger / lineage / observability / L5 evidence 只进入 audit 或 drilldown；只有被 domain owner fold 成 owner delta、owner answer、hard gate 或 typed blocker 后，才影响默认下一步。
...
| `Ledger` | 持有 refs-only evidence、lineage、receipt/blocker refs、audit ledger、retention refs。 | Ledger 不存 artifact body / memory body，不把 ref 升级成 owner authority。 |
...
| `OwnerReceipt` | Domain authority kernel | 关闭 owner action、artifact mutation、memory decision、quality/export gate 或 handoff。 | OPL / Ledger / Console 不能签。 |
...
| `ProvenanceEnvelope` | Ledger refs-only | entity/activity/agent/source/artifact/run lineage refs。 | Lineage 存在不等于 readiness。 |
...
| passive evidence ledger | `record_ref_lineage(envelope)` / `read_owner_folded_refs(delta_ref)` | Ledger / Console / audits |
...
| `ledger-lineage` | Ledger evidence contracts / lineage tests | refs-only evidence policy | lineage fixture tests、body-forbidden tests | Ledger 只保存 refs / digest / locator，不保存 body，不签 owner answer。 |
...
- OPL / Ledger / Console / Runway / Pack / Capability Registry 不得签 owner receipt、创建 domain typed blocker、写 domain truth 或授权 quality/export verdict。
...
- 不把 Ledger ref 写成 owner receipt authority。
