...
OPL Connect 同步的是 MAS Scholar Skills 的本地 Codex discovery surface，不是 MAS stage 主提示词。当前默认 profile 包含一个 required aggregate entry 和八个 default medical-paper professional specialist skills：
...
这八个专业 Skill 是 MAS 消费的专业 playbook；MAS 的 `write`、`review`、`figure`、`data/cohort` 等 stage 主提示词仍在 MAS 仓内维护，负责 stage policy、证据门槛、route-back、owner gate、owner receipt、typed blocker、artifact authority、clinical data body/source readiness 和 publication readiness。
...
这会把 filtered skill pack 写入 `<target>/.codex/skills/mas-scholar-skills/`，并同步 source repo 中已经物化的八个专业 Skill 到同级 `.codex/skills/<skill-id>/` 目录：`medical-research-lit`、`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-statistical-review`、`medical-table-design`、`medical-submission-prep`、`medical-data-governance`。同步同时生成 `.opl-install-receipt.json` 和 profile manifest，记录 source repo path、source HEAD、target scope/root、copy policy、copied/excluded roots、default/required pack 与 authority false flags。复制内容只包括 `SKILL.md` 以及必要 contracts/docs/gallery review refs；`.git`、`outputs`、`build`、`dist`、`node_modules`、`render-cache` 和 heavy gallery assets/intermediate files 不进入目标 workspace / quest。Gallery 的人审文档与 lightweight manifest 应随 `mas-scholar-skills` package 分发；重型生成中间结果、render cache 和批量图片资产不得进入 git/package 默认同步面。
