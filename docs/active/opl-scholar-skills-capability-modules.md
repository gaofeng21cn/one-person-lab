...
OPL Connect 同步的是 MAS Scholar Skills 的本地 Codex discovery surface，不是 MAS stage 主提示词。当前默认 profile 包含一个 required aggregate entry 和七个 default medical-paper professional specialist skills：
...
这七个 `medical-*` Skill 是 MAS 消费的专业 playbook；MAS 的 `write`、`review`、`figure` 等 stage 主提示词仍在 MAS 仓内维护，负责 stage policy、证据门槛、route-back、owner gate、owner receipt、typed blocker、artifact authority 和 publication readiness。
