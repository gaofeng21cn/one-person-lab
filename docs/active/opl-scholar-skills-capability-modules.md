...
OPL Connect 同步的是 MAS Scholar Skills 的本地 Codex discovery surface，不是 MAS stage 主提示词。当前默认 profile 包含一个 required aggregate entry、十个已物化 default medical-paper professional specialist skills，以及可选专科 Skill：
...
这些专业 Skill 是 MAS 消费的专业 playbook；`medical-figure-style` / `medical-figure-composer` 是 Display 子 Skill，不新增 active module。MAS 的 `write`、`review`、`figure`、`data/cohort` 等 stage 主提示词仍在 MAS 仓内维护，负责 stage policy、证据门槛、route-back、owner gate、owner receipt、typed blocker、artifact authority、clinical data body/source readiness 和 publication readiness。
