...
- `opl.scholarskills.data` / `Medical Data Governance`
...
MAS 可以把 active `opl.scholarskills.*` 模块作为 `current_owner_delta` 的 refs-only capability request，桥接到 MAS Scientific Capability Registry 的 candidate artifact refs。Display 仍可复用 MAS Display Pack 的 candidate artifact refs；Tables、Stats、Lit、Write、Review、Submit 和 Data 则通过同一 OPL module descriptor / execution receipt 形状暴露专业候选 refs。但 MAS 仍持有 study truth、publication truth、quality verdict、artifact authority、owner receipt、typed blocker、human gate 和当前 package authority。
