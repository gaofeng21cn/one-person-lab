...
required dependency，但只检查“是否存在且可调用”。安装、更新、删除和物理版本切换
交给 Codex Plugin Manager、Git 或该 Package 实际 carrier 的平台能力。OPL 只保留
跨 carrier 的薄组合接口、动态发现、运行聚合和 App 投影。
...
| --- | --- | --- |
| Package owner | descriptor、entrypoints、required/optional ids、稳定 identity 的兼容演进、domain truth、业务接口。 | Base/App 版本、中央注册表、其他 Package 发布节奏。 |
| Codex Plugin Manager / Git / carrier platform | install、list、update、remove、platform source/currentness 和平台安全校验。 | OPL domain truth、App 展示偏好、Temporal execution truth。 |
| OPL Base | `ensure/list/update/remove/health` 薄 adapter、installed descriptor discovery、Agent Work Item 与 Temporal execution 聚合、typed custom-view proxy。 | 自研 resolver/lock/payload/LKG/receipt/materializer、starter profile、GUI renderer。 |
...
### P0.1 冻结复杂性并取得平台 proof

Owner: Framework

- 禁止新增 Package resolver、lock、payload、LKG、receipt、materializer、scope activation
  和 fixed registry 功能。
- 对 Codex Plugin Manager、Git、local path、Full offline seed 的真实能力做 fresh inventory；
  只选择实际需要的 carrier adapters。
- 在空白和已有安装环境证明平台能完成 install/list/update/remove，并记录无法由平台提供
  的最小差距。不能用 unit test 或模拟 receipt 代替实际 readback。

Exit gate: 至少一个普通 managed carrier 和一个 developer/local carrier 完成全生命周期
readback；差距表证明留下的每个 Base adapter 都有真实 consumer。
...
- Base 先读 installed platform descriptors；缺 descriptor 时只读旧 Package manifest
  并投影为同一内部 shape。dual-read 不能双写，也不能让旧 lock 赢过平台 inventory。
- 为 `entrypoints/runtime/custom_views` 增加最小安全校验；不引入版本和 digest gate。
...
即可被发现；稳定 identity 的旧 consumer 在更新后仍可调用；旧数据仍可读。
...
Exit gate: 单 Package fresh update、失败隔离、删除不回装和 local source 保护全部通过。
...
execution 同屏且 truth owner 分离；未知 view 安全降级。
...
证明无 active caller；旧 manager 物理删除后同一矩阵再次通过。
...
- 平台 `install/list/update/remove` fresh readback。
