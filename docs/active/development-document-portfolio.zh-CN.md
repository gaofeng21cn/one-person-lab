...
| `docs/references/runtime-substrate/hermes-agent-*` 与 `docs/history/runtime-substrate/hermes-agent-runtime-substrate-benchmark.md` | Hermes 评估、truth reset 与归档 benchmark | 当前 Hermes references 只保留边界/评估；runtime-substrate benchmark 因其 gateway-first / Domain Harness OS 执行顺序已被取代而归入历史。`hermes_agent` 本身仍是 canonical 显式非默认 executor adapter/backend，不是兼容接口。 |
...
| `docs/references/runtime-substrate/opl-managed-runtime-three-layer-contract*` | managed runtime 三层边界参考 | 有效的三层分法吸收到 current framework wording；旧 Gateway/Domain Gateway 词只作 historical support，不保留 compatibility surface。 |
...
| `docs/references/operating-governance/*` | surface、quality、operator、memory、audit、publish governance 参考 | 保持 governance support；legacy-derived gateway ids 必须标明为历史审计语境，不得作为 compatibility surface。 |
