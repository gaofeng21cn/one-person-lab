...
| `docs/references/runtime-substrate/hermes-agent-*` | Hermes 评估、benchmark、truth reset | 旧 Hermes provider/Gateway/readiness surface 只保留为 legacy / provenance / diagnostic / negative-guard reference；`hermes_agent` 本身仍是 canonical 显式非默认 executor adapter/backend，不是兼容接口。 |
