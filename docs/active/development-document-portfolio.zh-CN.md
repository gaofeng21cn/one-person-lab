...
1. 先完成 OPL 作为 Codex-first、stage-led 完整智能体框架的 framework foundation。
...
4. 旧 Hermes-first、Gateway/frontdoor、direct-entry、local-manager、MDS-default 等路线在替代证据存在后退役清理。
...
| `6` | `new_domain_admission` | [Domain-Agent Admission Contract](./opl-domain-onboarding-contract.zh-CN.md), domain-admission references | 新 domain 只按标准 skeleton/descriptor/locator/authority boundary 接入。 | 不复制旧 Gateway/frontdoor/direct-entry 路线。 |
...
| `docs/specs/2026-04-20-*`, `docs/specs/2026-04-21-*` | retained path spec/reference | 保留路径稳定；有效结论已吸收到核心五件套、current development lines 和 stage-led roadmap。 |
...
| `docs/references/runtime-substrate/family-product-entry-and-domain-handoff-architecture.md` | 早期 product-entry / handoff 架构参考 | 有效的 product-entry taxonomy 和 handoff envelope 已吸收到 current development lines、active contracts、public docs；Hermes Kernel / Gateway wording 降级为 historical migration context。 |
| `docs/references/runtime-substrate/family-lightweight-direct-entry-rollout-board.md` | 早期 direct-entry 推进板 | 降级为 dated migration board；有效的 entry taxonomy / frontdoor_surface 经验吸收到 public/active/support owner docs。 |
| `docs/references/runtime-substrate/mas-top-level-cutover-board.md` | 早期 OPL->MAS 切换板 | 降级为 MAS migration provenance；当前 MAS 迁移顺序以 OPL roadmap 和 MAS program portfolio/current development lines 为准。 |
| `docs/references/runtime-substrate/opl-product-entry-and-hermes-kernel-integration.md` | Hermes-first product-entry 决策背景 | 降级为 superseded runtime-substrate reference；当前目标是 provider-backed runtime，Temporal 是 production candidate，Hermes 是 legacy/optional/proof lane。 |
...
| `docs/references/runtime-substrate/host-agent-runtime-contract.md` | Codex-default host-agent runtime 历史合同 | 保留为迁移参考；有效的 Codex-default 口径已吸收到 active runtime boundary 和 roadmap。 |
...
| `docs/references/runtime-substrate/opl-vertical-online-agent-platform-roadmap.md` | 早期垂类在线 Agent 平台蓝图 | 降级为 superseded platform reference；当前蓝图由 stage-led framework roadmap 和 public roadmap 承接。 |
...
| Product-entry taxonomy、handoff envelope、frontdoor_surface / operator_loop_surface 区分 | active contracts、public docs、domain admission；旧 direct-entry boards 保留为 provenance |
...
| Gateway/frontdoor/federation/routed-action 旧路线 | history / compatibility / tombstone；active docs 只作为历史上下文引用 |
...
| Lightweight direct-entry 全家族推进板 | 不再作为当前完整 backlog。保留 entry taxonomy、handoff envelope 和 frontdoor/operator 分层经验。 |
...
| Host-agent runtime contract | 不再独立定义当前目标 runtime。有效 Codex-default 口径已合入 runtime boundary 和 framework roadmap。 |
| Product API / ACP native specs | 不再作为实现队列；作为 retained path spec/reference 保留。 |
