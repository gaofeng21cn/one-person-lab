...
- `opl-family-purpose-first-design-audit.md`：从 OPL 系列目标态反推的跨仓顶层设计审计；聚焦每个 repo 还可删减、收薄、上收或降噪的设计负担，不承载瞬时 proof 流水。
- `standard-agent-design-consistency-audit.md`：MAS/MAG/RCA/OMA 是否已经表现为同源 OPL Foundry Agent，以及 active surface / full-detail source-purity 中仍有哪些历史残留风险的审计记录。
