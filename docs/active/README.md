...
`docs/active/current-state-vs-ideal-gap.md` 是 OPL family 当前目标、差距、完成口径和 active-goal baton 的唯一 active owner。其他 active/support 文件只提供标准、矩阵、审计或路线支撑；它们不能独立声明新主线、live readiness、domain ready、App release ready、production ready 或物理删除授权。

当前 active index 只描述文档角色，不记录某次落地 readout。文档线的完成条件是角色归位和 false-completion guard 清楚：`current-state-vs-ideal-gap.md` 持有 owner / gate / next action / Plan Completion Audit；`foundry-agent-os-family-target-implementation-plan.md` 只持有 W0-W7 target ABI 和 false-authority guard；`opl-family-ideal-operating-model-redesign.md` 只持有 north-star / audit standard；`docs/status.md` 只持有当前状态读法和不能声明的边界。Readiness、maturity、App drilldown、Brand L5 或 provider readout 的具体状态必须回 live commands、contracts、source、tests、runtime ledger 或当前 active owner，不能写进目录索引。
...
| `reuse-first-platform-risk-audit-and-landing-plan.md` | 复用优先风险审计与理想落地计划支撑 | 不作为独立 backlog。Phase 0-10 只作 reuse-first 审计标准、风险排序、排除项、当前可落地项和 completion audit 入口。 | 真实落地状态必须回 `current-state-vs-ideal-gap.md`、source/contracts/CLI/runtime evidence、对应 owner repo lane 或 history archive。 |
...
- `reuse-first-platform-risk-audit-and-landing-plan.md`：active_support 复用优先风险审计与落地计划；说明 OPL family 哪里存在手搓通用平台能力风险、应吸收哪些成熟工程模式、理想目标态、Phase 0-10 标准、当前排除项、可落地项、完成度审计和 forbidden claims；历史批次流水回 `docs/history/**`。
...
- 2026-05 dated proof、receipt 流水和一次性 production functional closure 过程已迁入 `docs/history/process/plans/`；当前 follow-through owner 仍是 `current-state-vs-ideal-gap.md`，`production-framework-closure-gap-matrix.md` 只保留 evidence-lane 支撑读法。
...
- 2026-06-03 OPL family purpose-first 设计审计已迁入 [OPL Family Purpose-First Design Audit](../history/process/plans/2026-06-03-opl-family-purpose-first-design-audit.md)；当前 owner、gap 和下一步回到 `current-state-vs-ideal-gap.md` 与核心五件套，production closure 证据门细节只读 `production-framework-closure-gap-matrix.md`。
