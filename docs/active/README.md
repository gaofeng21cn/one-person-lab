...
- `opl-foundry-agent-target-operating-architecture.md`：在不受当前实现分布约束的前提下，重设 OPL / Foundry Agent 的目标操作架构；把 OPL 定义为 `Agent Operating System`，核心 primitive 是 Agent Product Pack、Stage Runtime Kernel、Current Owner Delta Controller、Stage Artifact Unit、Passive Evidence Vault 和 App Cockpit，并给出全面重构的迁移阶段和验收门。
