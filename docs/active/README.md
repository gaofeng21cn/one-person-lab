...
- `current-state-vs-ideal-gap.md`：OPL family 当前目标、完成进度、功能/结构差距、测试/证据差距和下一轮 baton 的唯一 active owner；同时把 MVP friction、purpose-first cross-repo audit 与 cognitive kernel 重构折成同一执行口径：外层 `current_owner_delta`，内层 `Cognitive Computation Kernel`，domain repo 只保留领域 truth、artifact authority、quality verdict、owner receipt / typed blocker。历史 proof、dated closeout、receipt 流水和长清单回到 `docs/history/**` 或动态 CLI/read-model。
...
- `opl-foundry-agent-target-operating-architecture.md`：在不受当前实现分布约束的前提下，重设 OPL / Foundry Agent 的目标操作架构；把目的优先审计与 MVP 阻力审计合并成设计评估，定义 OPL 为 `Cognitive Computation Kernel + Agent Runtime Platform`，核心 primitive 是 Agent Product Pack、Cognitive Computation Kernel、Stage Attempt Runtime、Current Owner Delta Controller、Stage Artifact Unit、Passive Evidence Vault 和 App Cockpit，并给出全面重构的迁移阶段和验收门。
- `opl-family-ideal-operating-model-redesign.md`：按 `目的反推必要性，MVP 检查阻碍性` 重新评估 OPL Framework、App、MAS/MAG/RCA/OMA 和 support repos 的实际情况，吸收外部成熟工程经验，给出理想 operating model、OPL 基座优化方向、跨仓分类和下一轮重构 backlog。
