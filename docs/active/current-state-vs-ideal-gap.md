...
| Standard agent structural conformance | `passed_with_strict_purity_followthrough` | Fresh `opl agents conformance --family-defaults --json` 显示 4 个 repo 通过、0 个 repo blocked，active forbidden-name residue 清零；这只证明当前 descriptor / skeleton / policy 可读。按新的严格标准，domain repo 中仍有 active caller 的手写 product/status/workbench/sidecar/runtime/dispatch/adapter shell 都算标准化 follow-through，不再被写成长期可接受的 refs-only adapter。 |
...
| Domain production evidence | `partial_blocked_by_domain_owner_evidence` | MAS/MAG/RCA 的 owner receipt refs、typed blocker refs、no-regression refs 可进入 OPL refs-only ledger；worklist/readiness 已把重复 domain-owned typed blocker refs 作为 attention group 去重展示，同时保留 raw item / envelope 审计数量。Fresh worklist 当前没有 OPL 可执行 open safe-action 或 domain-dispatch record workorder；剩余 attention 是 domain-owned typed blocker / blocked refs-only envelope 的生产证据尾项，不是 OPL 自闭合、domain ready 或 production ready。真实 paper/grant/visual owner-chain、memory/artifact/lifecycle receipt 和 repeated no-regression 仍需扩面。 |
| Legacy cleanup ledger | `ready_for_strict_retirement_planning` | OPL normalizer 可消费 MAS/MAG/RCA/OMA 当前 standard-agent evidence；fresh App/operator drilldown 读为 cleanup plans ready、blocked cleanup plan count 为 0、domain physical delete can execute 为 0。该状态只证明 OPL cleanup ledger/read-model 可定位删除门，不再作为保留理由。最终标准智能体不保留 repo-local generic adapter / compat facade / hand-written default caller；active caller 必须迁到 OPL generated/hosted surface，域仓只留 domain pack、authority function、handler target 或必要 native implementation。 |
...
| gap | 当前状态 | 完成口径 |
| --- | --- | --- |
| `zero_retained_generic_adapter_purity` | 当前 conformance 只证明标准面可读；MAS/MAG/RCA/OMA 仍有若干 hand-written default caller、sidecar/status/workbench、runtime projection、route shell 或 script materializer 作为 migration input。 | 标准智能体 active source 只允许 `agent/` domain pack、machine-readable contracts、standard authority functions、domain handler target、domain-specific implementation 和必要 native helper。任何 generic control-plane / default-caller / workbench / sidecar / session / queue / lifecycle / wrapper 逻辑都由 OPL 生成或托管；域仓副本删除，不保留兼容 alias、facade、re-export wrapper 或 compatibility-only tests。 |
