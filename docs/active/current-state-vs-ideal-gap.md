...
| `./bin/opl family-runtime status --json` | 默认 `configured_provider=local_sqlite`；attempt ledger 最新可由 `attempt list` / `production-closeout` 读到 `total=18` / `completed=18` | 默认本机 provider 是 dev/offline baseline，不是 production online readiness；attempt ledger 是持久 OPL state，可被 workbench/closeout/domain-memory read model 消费。 |
...
- OPL 已新增 domain-neutral family transition runner 机器面：`contracts/opl-framework/family-transition-runner-contract.json`、`src/family-transition-runner.ts`、`tests/src/family-transition-runner.test.ts` 和 package export `./family-transition-runner`。它把 transition schema、guard 匹配、fail-closed、dead-letter、receipt/projection envelope 和 matrix runner 放在 OPL；MAS/MAG/RCA 继续持有各自 domain transition table、guard 定义、oracle fixtures、质量 verdict、artifact authority 和 owner receipt。当前测试使用 MAS-like fixture 锁定 `bundle_stage_ready`、`bundle_stage_blocked`、`publishability_gate_blocked` 相邻状态不会误转场；这证明通用 runner 边界，不证明 MAS 三篇论文已经被推进。
...
- OPL 已有跨 domain 共用的 transition runner 机器面；MAS 仍最需要 domain transition table 来收口 publication gate / bundle-stage / AI reviewer / owner priority；MAG 和 RCA 后续也需要 fundability / package-ready、visual route / export-ready 的 transition spec。当前分工是 OPL 提供 transition schema、matrix runner、fail-closed/dead-letter/human-gate receipt envelope 和 projection transport，domain repo 提供 transition table、guard、oracle fixture、typed blocker 和 owner action；不能让 OPL 自己推断 domain ready verdict。
...
4. Generic primitives：已落地的 read-model shell 和 family transition runner 继续由 OPL/App 产品化、domain spec 和真实 receipt 驱动；domain agent 只提供 authority refs、projection refs、route nodes/edges、transition table、guard/oracle fixtures、review/quality/export refs、receipt 和 typed blocker。缺口集中在真实 live apply、artifact/memory lifecycle evidence、operator SLO、App drilldown 和 domain transition spec 接入，而不是重新设计一套通用壳。
5. Transition tables：保持 OPL 只运行 domain-declared transition table；MAS/MAG/RCA 必须提供 domain transition spec、guard/oracle fixtures 和 owner receipt，不再把分散状态判断散落在 controller、priority、authorization 和人读文档里。
...
1. 用 DM002、DM003、Obesity 继续跑 live provider-hosted guarded apply；DM002 当前有效结果是 MAS owner `stable_blocker` receipt，下一步仍需 artifact delta、AI reviewer update、route decision、human gate、stop-loss 或跨 paper-line stable blocker scaleout。
...
- 当前 fresh runtime snapshot 没有 running domain attempt，但 `stage_attempt_workbench.summary.total=18` 且 18 条 attempt 均 completed，覆盖 `medautoscience=6`、`medautogrant=11`、`redcube=1`，其中 MAS 最新 guarded-apply attempt 已能展示 owner stable blocker receipt ref；provider proof receipt drilldown 已进入 snapshot 的 attention / recent lane item，stage-attempt operator item 已能投影 domain/stage/blocker/memory/artifact locator refs 与 action route refs，native helper execution envelope 已能投影现有 native index / helper receipt metadata。App 仍缺真实生产运行状态下的 progress delta、artifact mutation receipt、route/decision map drilldown、review/repair queue、artifact gallery/handoff drilldown 与长时运行 drilldown 证据。
