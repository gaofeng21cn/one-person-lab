...
| `oma_script_to_pack_hygiene` | `needs_retirement_with_docs_boundary_landed`。OMA docs 已改成 fresh-read / live evidence / no second Framework 口径；scripts/materializers 仍只可作为 refs/smoke/work-order helper。 | OPL Meta Agent + OPL Framework | 稳定 policy 进入 `agent/`、contracts、authority functions 或 OPL primitive；脚本保留必须有 machine gate。 |
