...
1. Provider SLO：Temporal service/worker/proof 已纳入 runtime ledger、closeout projection、freshness blocker、`operator_slo_repair_loop` drilldown 和 supervised production-proof execution receipt；继续补自动周期性 proof 调度、restart/re-query、query/signal/history、真实 repair execution receipt 和 domain owner chain 的长时稳定性证据。
