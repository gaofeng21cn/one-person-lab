...
2. 补齐 `family-stage-admission` 的严格生产启动门：`attempt create` 已记录 launch admission gate 并阻断声明 stage blocker；下一步把 selected executor binding、idempotency、consumed refs 和 expected receipt refs 也纳入 queue/provider/App 可见门禁。
