...
2026-06-06 OPL follow-through：OPL 新增 refs-only `stage-run-authorization` ledger/CLI/cockpit 接入，用来记录、验证和投影真实 StageRun execution authorization refs。该 ledger 只接受 `evaluateStageRunExecutionAuthorization` 已判定 launch authorized 的 provider attempt、active lease、execution authorization decision、workspace/artifact scope、source fingerprint 和 idempotency refs；recorded / verified receipt 仍不是 MAS owner receipt、不是 MAS typed blocker、不是 closeout binding，也不能声明 domain ready 或 production ready。Fresh live readout 显示 DM002 `sat_483e4945f368296a70170657` 与 DM003 `sat_06d74cae8aad627661aaa8aa` 当前仍是 local ledger `queued` / provider run `registered`，`current_provider_readiness.provider_ready=false`，worker source stale，Temporal query 返回 `temporal_workflow_not_started_or_not_found`。因此本轮不能把这些 queued attempts 写成 active lease 或 execution authorization receipt；MAS owner-answer 闭环仍等待真实 active lease / authorization decision 与 MAS durable owner receipt 或 typed blocker。
