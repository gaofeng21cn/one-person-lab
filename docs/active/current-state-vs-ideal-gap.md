...
2026-06-05 follow-on gate tranche 继续推进 retained-wrapper 防误读：OMA `script_to_pack_retirement_gates` 已覆盖全部 `scripts/**/*.ts`，新增脚本若只有 morphology classification 而没有 retirement/retention gate 会 fail closed；MAG physical morphology guard 已把 `claims_physical_morphology_cleanup_complete` 固定为 false，只在外部证据齐全时声明 `claims_ready_for_owner_receipted_cleanup`，避免把 owner receipt 之前的 evidence completeness 写成 physical cleanup complete；RCA retained wrapper 审计确认 `runtimeWatch` direct surface、session refs、operator projection、neutral route-run record adapter 已有 `physical_delete_authorized=false` / `safe_to_delete_now=false` / typed-blocker guard，不新增重复合同面。该 tranche 仍不授权任何 physical delete。
...
| `real_owner_delta_tail` | `partial_owner_currentness_landed` / `needs_domain_evidence`。MAS AI reviewer handoff/currentness 已能更清楚地回到 owner route；平台 repair、read-model currentness、ledger proof 仍不得写成 domain progress。 | MAS/MAG/RCA/OMA domain owners | 真实 paper/grant/visual/target-agent owner receipt、typed blocker、human gate、review/export receipt、no-regression 或 long-soak refs。 |
| `app_contract_compaction` | `meets_target_for_contract_guard` / `needs_release_user_path_evidence`。App ordinary cockpit surface budget、Home/Runtime/Settings budget refs、Runtime next owner / accepted answer shape / artifact-or-blocker 字段和 active-shell/release-boundary guards 已落地；Full native trust 属 App release hardening，不是 family production ready。 | One Person Lab App | 新 App release cohort 继续提供 screenshot、reload/first-run、provider linkage、operator evidence 和 release-owner blocker path；release detail 不进入 ordinary cockpit。 |
| `oma_script_to_pack_hygiene` | `meets_target_for_script_gate_coverage` / `needs_retirement_for_stable_policy`。OMA docs 已改成 fresh-read / live evidence / no second Framework 口径；全部 repo-local TS scripts 已要求同时具备 morphology classification 和 script-to-pack retirement/retention gate。 | OPL Meta Agent + OPL Framework | 稳定 policy 继续进入 `agent/`、contracts、authority functions 或 OPL primitive；脚本可保留但新增/收薄时必须更新 machine gate。 |
...
| `app_release_user_path` | App 已新增 first conversation warmup、Full runtime native trust 和 release-boundary gate；每个新的 App release cohort 仍需重复提供 release package、screenshot、reload / first-run prompt、provider linkage、operator evidence 和 release-owner typed blocker path。 | App release/user-path refs 只关闭 cohort evidence gate；release-ready verdict 回 App release 流程。 |
