...
| `ideal_operating_model_redesign` | `active_support_landed`。跨 OPL/App/MAS/MAG/RCA/OMA/support repo 的理想 operating model 已按 `目的反推必要性，MVP 检查阻碍性` 折成 `meets_target` / `needs_demotion` / `needs_retirement` 审计口径。 | OPL Framework + domain/App owners | 已落地 OPL default summary de-noise、`current_owner_delta` default root cutover、surface-budget guard 和 active line consolidation；下一步只保留 domain/App/OMA/support tail，不新增第二 worklist、第二 App bridge 或第二 Agent Lab。 |
...
| `domain_wrapper_delete_gate` | `needs_retirement`。Machine-readable delete gate 已有，但 MAG/RCA/MAS/OMA retained wrapper 仍需逐项 owner-side closeout。 | OPL cleanup gate + domain owners | 逐 surface 跑 no-active-caller、owner receipt / typed blocker、no-forbidden-write、tombstone/provenance；满足后删除或 tombstone。 |
| `real_owner_delta_tail` | `needs_demotion_for_platform_ops` / `needs_domain_evidence`。平台 repair、read-model currentness、ledger proof 不得写成 domain progress。 | MAS/MAG/RCA/OMA domain owners | 真实 paper/grant/visual/target-agent owner receipt、typed blocker、human gate、review/export receipt、no-regression 或 long-soak refs。 |
| `app_contract_compaction` | `needs_demotion_for_release_detail`。Framework default payload 已收薄；App repo Home/Runtime/Settings wording 和 release detail 仍需持续隐藏 shell/provider/backend internals。 | One Person Lab App | App ordinary path 只展示 purpose、task status、next owner、artifact/blocker 和 release facts。 |
| `oma_script_to_pack_hygiene` | `needs_retirement`。OMA 不应长出第二 Framework / second Agent Lab；scripts/materializers 只可作为 refs/smoke/work-order helper。 | OPL Meta Agent + OPL Framework | 稳定 policy 进入 `agent/`、contracts、authority functions 或 OPL primitive；脚本保留必须有 gate。 |
| `support_entry_clarity` | `needs_demotion`。Shell/doc support repo 不进入 Foundry Agent truth set。 | App shell / OPL Doc support owners | support repo 首屏只说明 App contract carrier / workflow steward，不反向定义 OPL/App/domain truth。 |
