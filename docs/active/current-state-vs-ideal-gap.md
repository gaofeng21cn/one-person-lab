...
| `provider_long_soak` | `operating-maturity` 现在消费 provider cadence window / capability projection；本轮 fresh readout 为 `open_evidence_count=1`、`long_evidence_ready=false`、`capability_status=capability_slo_blocked` 和 `provider_completion_counts_as_production_ready=false`。Temporal service/worker、provider cadence/capability、domain owner-chain dispatch 和 retry/dead-letter 仍需在更长窗口内持续满足，并能暴露 blocker。 | Long-soak refs、provider state linkage、operator evidence refs 或 typed blocker refs 可重复 record/verify；open count 只说明 provider evidence lane 观测缺口，不外推为 production ready 或 domain ready。 |
