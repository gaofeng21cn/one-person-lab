...
| `./bin/opl domain-memory list --json` | `resolved_memory_descriptor_count=3`、`missing_memory_descriptor_count=0`；`runtime_receipt_evidence.closeout_count=14`、`consumed_memory_ref_count=1`、`writeback_receipt_ref_count=2`、`rejected_write_count=9`、`opl_writes_memory_body=false` | Memory locator/descriptor 已对齐；OPL 现在只读投影 stage attempt closeout ledger 中的 memory receipt refs。MAG 已有 accepted/rejected memory receipt refs 进入 OPL read model；MAS 当前主要是 rejected writes；RCA 有 closeout refs。真实 retrieval/writeback/body migration 仍由 domain owner 后续闭合。 |
...
| MAS guarded-apply live dispatch receipts | 早期 DM002 live dispatch 返回 `blocked_no_mas_owner_apply_receipt`；MAS owner blocker projector 落地后，同一 task `frt_54187384a4fa403abda12ce5` 产生 source fingerprint `a0cdbf341fae623d`，OPL stage attempt `sat_df08b839ce33a33ee92974f2` ingest `artifacts/runtime/opl_family_sidecar/dispatch_receipts/e97b1c8cd25f3a019f90.json`，route impact 为 `mas_owner_apply_receipt_observed`、`provider_attempt_state=mas_owner_receipt_present`、`writes_performed=false`，consumed ref 为 `studies/002-early-residual-risk/artifacts/controller_decisions/latest.json`，content sha256 `afacdd922c4db4a6c92556754f1a6baea196162ec567d16b8e11f054f18a9db3` | OPL/MAS live bridge 已能传递 MAS owner stable blocker receipt；这是有效 owner receipt chain 证据，不是论文推进完成、quality verdict 或 submission-ready。 |
...
| Domain memory | OPL 只持 locator / refs / receipts；domain 持有正文、接受/拒绝、route/quality truth | 三仓 memory descriptor resolved；OPL `domain-memory` read model 已从 stage attempt closeout ledger 投影 `closeout_count=14`、`consumed_memory_ref_count=1`、`writeback_receipt_ref_count=2`、`rejected_write_count=9`；MAS 有 publication-route memory workspace apply closure、body-free receipt inventory、operator grouping 和 stale/deprecated review summary；MAG accepted/rejected receipt refs 已可钻取；RCA 有 closeout refs | Fresh OPL read model 中 retrieval/writeback/body migration 仍不是 OPL-applied success；真实 memory body apply 和跨 domain soak 仍不足 | 三仓产出真实 consumed/writeback accepted/rejected receipt；OPL/App 做 ref-only grouping；memory body 始终留在 workspace/runtime owner |
