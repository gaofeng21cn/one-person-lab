...
| `real_owner_delta_tail` | `needs_domain_owner_evidence_tail`。MAS AI reviewer handoff/currentness、gate replay routing、provider dispatch authorization 和 refs-only typed blocker transport 已让 owner route 更清楚；这些属于 OPL/MAS 路由与接力能力，不是 domain progress 或 paper closure。 | MAS/MAG/RCA/OMA domain owners | 真实 paper/grant/visual/target-agent owner receipt、typed blocker、human gate、review/export receipt、no-regression 或 long-soak refs；下一个 OPL safe action 若仍指向 MAS/domain live payload，必须由 domain owner 提供可记录 ref。 |
