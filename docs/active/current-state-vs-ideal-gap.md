...
| Standard agent structural conformance | `passed_with_strict_purity_followthrough` | Fresh `opl agents conformance --family-defaults --json` 显示 4 个 repo 通过、0 个 repo blocked，active forbidden-name residue 清零；这只证明当前 descriptor / skeleton / policy 可读。按严格标准，domain repo 中仍有 active caller 的手写 product/status/workbench/sidecar/runtime/dispatch/adapter shell 都算标准化 follow-through，不再被写成长期可接受的 refs-only adapter。MAS 当前合同已重新打开 2 个 source-purity functional follow-through gate。 |
...
| Domain production evidence | `partial_blocked_by_domain_owner_evidence` | MAS/MAG/RCA 的 owner receipt refs、typed blocker refs、no-regression refs 可进入 OPL refs-only ledger；worklist/readiness 已把重复 domain-owned typed blocker refs 作为 attention group 去重展示，同时保留 raw item / envelope 审计数量。Fresh evidence worklist 读为 `open_worklist_item_count=0`、`open_safe_action_payload_required_item_count=0`、`open_safe_action_payload_free_item_count=0`，同时仍有 `zero_open_worklist_blocked_refs_only_envelope_count=181`、`domain_ready_authorized=false`、`production_ready_authorized=false`。真实 paper/grant/visual owner-chain、memory/artifact/lifecycle receipt 和 repeated no-regression 仍需扩面。 |
| Legacy cleanup ledger | `ready_for_strict_retirement_planning_refs_only_tail` | OPL normalizer 可消费 MAS/MAG/RCA/OMA 当前 standard-agent evidence；fresh App/operator drilldown 读为 `functional_privatization_action_required_count=0`、`default_caller_deletion_evidence_open_requirement_count=0`、`domain_legacy_cleanup_blocked_plan_count=0`、`lifecycle_domain_physical_delete_can_execute=false`。该状态只证明 OPL cleanup ledger/read-model 可定位删除门，不再作为保留理由或物理删除授权。最终标准智能体不保留 repo-local generic adapter / compat facade / hand-written default caller；active caller 必须迁到 OPL generated/hosted surface，域仓只留 domain pack、authority function、handler target 或必要 native implementation。 |
...
| gap | 当前状态 | 完成口径 |
| --- | --- | --- |
| `zero_retained_generic_adapter_purity` | 当前 conformance 只证明标准面可读；MAS/MAG/RCA/OMA 仍有若干 hand-written default caller、sidecar/status/workbench、runtime projection、route shell 或 script materializer 作为 migration input。其中 MAS live contract 明确 `functional_structure_gap_count=2`、`repo_local_wrapper_tail_count=3`，MAG/RCA/OMA structural shape 为 landed / gap 0 但仍有 physical delete / evidence tail。 | 标准智能体 active source 只允许 `agent/` domain pack、machine-readable contracts、standard authority functions、domain handler target、domain-specific implementation 和必要 native helper。任何 generic control-plane / default-caller / workbench / sidecar / session / queue / lifecycle / wrapper 逻辑都由 OPL 生成或托管；域仓副本删除，不保留兼容 alias、facade、re-export wrapper 或 compatibility-only tests。 |
...
| `MAS_physical_thinning` | MAS 默认 runtime owner 已回到 OPL provider-backed stage runtime；旧 runtime transport、runner、worker lease、SQLite lifecycle writer 只按 retired/no-resurrection 读取。当前 open functional follow-through 是 `standard_agent_purity_guard` 与 `domain_ref_consumer_physical_thinning`，active wrapper tail 是 `generic_cli_mcp_product_wrappers`、`owner_route_reconcile_materialize_dispatch_shell`、`workbench_portal_generic_shell`。 | product/status/workbench、owner-route handoff、progress/domain-ref projection 等仍有 caller 的 shell 迁到 OPL generated/hosted surface 或 domain handler target 后删除；MAS 只留 medical study truth、publication gate、artifact/package authority、owner receipt / typed blocker 和 domain handler。 |
| `MAG_wrapper_shell_retirement` | MAG repo-side handler/ref-only/authority boundary 已闭合；`mag_functional_structure_gap_count=0`、`claims_opl_replacement_exists=true`，但 `claims_domain_repo_physical_delete_authorized=false`。product-entry、status/user-loop、sidecar、runtime registration、lifecycle/projection shell 仍是 active direct path 或 migration / deletion tail。 | OPL generated/default caller 与 App/default consumption 接管后，MAG 删除 product/status/sidecar/runtime shell、legacy flat alias、local journal/attempt ledger、scheduler/probe、patch bridge 和 compat tests；只留 grant authority functions、transition oracle、owner receipt / typed blocker 和 domain handler。 |
| `RCA_naming_and_adapter_hygiene` | RCA generated/hosted shell、legacy cleanup 和 visual authority boundary 已清晰；`functional_structure_gap_count=0`、`unclassified_private_generic_residue_count=0`，但 module bridge gates 仍读为 `physical_delete_authorized=false`。旧 session/store/managed/runtime/gateway 命名已降到 retired/provenance/tombstone 语境。 | active source 继续收敛为 visual authority implementation、native helper、domain handler target 和 artifact/visual pack logic；generic CLI/MCP wrappers、executor adapter shell、operator projection shell、stability read model、workspace/source lifecycle shell 不作为 RCA 长期代码保留。 |
...
| MAS real paper chain | MAS guarded-apply、owner-route、aftercare、default-executor typed blocker payload 可被 OPL record/verify 消费；当前 refs-only accounting 没有 open domain-dispatch workorder，已记录/验证或 typed-blocker 化的 MAS evidence 只作为 ledger / blocked attention 投影。 | 多条真实 paper line 产生 progress delta、AI reviewer update、artifact delta、human gate、stop-loss、owner receipt、owner-chain / no-regression refs 或 stable typed blocker。 |
