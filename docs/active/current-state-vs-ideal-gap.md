...
| `active_line_consolidation` | `meets_target_for_docs_owner`。`current-state-vs-ideal-gap.md` 是唯一 active owner；路线图、production closure matrix、purpose-first audit、target architecture 和 docs portfolio 降为 active support。 | OPL docs governance | active docs 只写当前 owner / gate / next action；dated proof、receipt 流水和 worktree closeout 进 history 或 runtime ledger。 |
