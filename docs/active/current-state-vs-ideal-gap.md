...
| `opl framework operating-maturity --family-defaults --json` | baseline 是 `L4_executable_baseline`，target 是 `L5_production_operating_maturity`；current owner bridge 仍指向 `med-autoscience` owner answer gate；`owner_evidence_intake` 先读取 domain owner payload summary、Brand L5、App release 和 Codex App runtime evidence 的 refs-only ledger，再把 observed refs 回投到 domain owner-chain、brand L5、release/user-path、provider long-soak、cleanup 和 memory/artifact/lifecycle evidence lane。 | Evidence lane 只能定位下一 owner、observed ref shape 和 false-authority boundary；不能授权 domain ready、App release ready、Brand L5、physical delete 或 production ready。 |
...
| `brand_module_l5_operating_maturity` | `l5_evidence_matrix_landed` / `l5_refs_only_ledger_landed` / `needs_live_operating_evidence`。十个品牌模块的 L5 证据矩阵、contract validation、聚合 CLI/read-model、模块级 `l5-status` 和 runtime refs-only ledger 已落地；当前仍缺真实用户路径、跨 agent scaleout、长跑/恢复证据、release/install evidence、operator repair loop、owner acceptance 和 no-second-truth 回归证据。 | L5 只能由模块 owner 提供可重复的 live path、release/install、long-soak、owner receipt / typed blocker / acceptance 和运营闭环证据关闭；Workspace L4、docs foldback、contract/conformance pass、provider completion、verified ledger、accepted module id guard 或 App projection只能作为 L5 输入，不能作为 L5 结论。 |
