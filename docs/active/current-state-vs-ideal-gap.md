...
| Standard agent structural conformance | `passed_with_delete_authority_tail` | Fresh `opl agents conformance --family-defaults --json` 显示 4 个 repo 通过、0 个 repo blocked，active forbidden-name residue 清零；这只证明当前 descriptor / skeleton / policy 可读。MAS/MAG/RCA/OMA 当前都按 strict source-shape landed 读取：MAS wrapper tail 已收成 former tail / deletion-gate provenance；MAG 顶层合同读为 `standard_agent_source_shape_status=landed`、`claims_domain_repo_physical_delete_authorized=false`；RCA bridge gate 读为 `source_shape_status=landed`、`functional_structure_gap_count=0`、`remaining_bridge_module_ids=[]`、`declares_no_active_bridge_modules=true`；OMA 读为 `source_shape=landed`、`functional_structure_gap_count=0`、`domain_repo_retained_generic_surface_count=0`。这不授权物理删除或 production/domain ready。 |
...
| OMA/new agent consumption | `production_consumption_ready` | New-agent scaffold consumption 现在重复穿过 scaffold validation、standard conformance、agent readiness 和 App/operator projection；managed install/update、App live path、owner receipt/typed blocker scaleout 和 verified long-soak 已进入读面；OMA production-consumption ready 不授权 MAS/MAG/RCA 或 family production ready。 |
| Domain production evidence | `partial_blocked_by_domain_owner_evidence` | MAS/MAG/RCA 的 owner receipt refs、typed blocker refs、no-regression refs 可进入 OPL refs-only ledger；worklist/readiness 已把重复 domain-owned typed blocker refs 作为 attention group 去重展示，同时保留 raw item / envelope 审计数量。Fresh evidence worklist 读为 `open_worklist_item_count=0`、`open_safe_action_payload_required_item_count=0`、`open_safe_action_payload_free_item_count=0`，同时仍有 `zero_open_worklist_blocked_refs_only_envelope_count=196`、`domain_ready_authorized=false`、`production_ready_authorized=false`，且 not-authorized claims 仍包含 `domain_repo_physical_delete_authorization` 和 `default_caller_delete_ready`。domain-dispatch 默认 record route 只对最新且已有 accepted typed closeout 的 stage attempt 开放；未 closeout attempt 只作为 blocked / superseded provenance。真实 paper/grant/visual owner-chain、memory/artifact/lifecycle receipt 和 repeated no-regression 仍需扩面。 |
| Legacy cleanup ledger | `ready_for_strict_retirement_planning_refs_only_tail` | OPL normalizer 可消费 MAS/MAG/RCA/OMA 当前 standard-agent evidence；fresh App/operator drilldown 读为 `functional_privatization_action_required_count=0`、`default_caller_deletion_evidence_open_requirement_count=0`、`domain_legacy_cleanup_plan_count=3`、`domain_legacy_cleanup_ready_plan_count=2`、`domain_legacy_cleanup_blocked_plan_count=1`、`lifecycle_domain_physical_delete_can_execute=false`。该状态只证明 OPL cleanup ledger/read-model 可定位删除门，不再作为保留理由或物理删除授权。最终标准智能体不保留 repo-local generic adapter / compat facade / hand-written default caller；active caller 必须迁到 OPL generated/hosted surface，域仓只留 domain pack、authority function、handler target 或必要 native implementation。 |
...
| gap | 当前状态 | 完成口径 |
| --- | --- | --- |
| `zero_retained_generic_adapter_purity` | 当前 conformance 只证明标准面可读。MAS/MAG/RCA/OMA 都已从功能/结构 gap 收敛到 source-shape landed / replacement-readable 状态：MAS wrapper tail 为 former provenance；MAG `claims_domain_repo_physical_delete_authorized=false`；RCA strict bridge gate 已无 remaining bridge module，但仍有 `repo_local_wrapper_delete_tail_evidence_gated_only` delete-tail class，且 generated surface consumption / production consumption 均不能声明 complete；OMA 不标 structural dirty，只保留 default-caller consumption、refs-only adapter thinning、script-to-pack hygiene 和 production evidence tail。剩余风险统一按 deletion authority / physical delete / production evidence tail 读取。 | 标准智能体 active source 只允许 `agent/` domain pack、machine-readable contracts、standard authority functions、domain handler target、domain-specific implementation 和必要 native helper。任何 generic control-plane / default-caller / workbench / sidecar / session / queue / lifecycle / wrapper 逻辑都由 OPL 生成或托管；域仓副本删除，不保留兼容 alias、facade、re-export wrapper 或 compatibility-only tests。 |
| `generated_surface_default_consumption` | OPL 已能从 descriptor/stage/action/memory/artifact metadata 生成或投影 CLI、MCP、Skill/product-entry、sidecar、status、session、workbench、harness 等 surface。 | MAS/MAG/RCA/OMA 的生产默认 caller 使用 OPL generated/hosted surfaces；repo-local wrapper 不再作为最终形态存在，迁移后只保留 domain handler target 或 authority function。 |
| `domain_private_platform_retirement` | OPL 已能审计 standard pack、authority functions、private platform residue、semantic equivalence 和 forbidden generic owner flags。 | 任何 repo-local scheduler、queue、attempt ledger、session store、workspace/source shell、artifact/memory lifecycle shell、workbench、generated wrapper owner、diagnostic cleanup shell 或 default caller 都完成上收或删除；历史只允许在 `docs/history/**` 或外部 provenance 中作审计材料，不属于标准智能体组成。 |
| `MAS_physical_thinning` | MAS 默认 runtime owner 已回到 OPL provider-backed stage runtime；旧 runtime transport、runner、worker lease、SQLite lifecycle writer 只按 retired/no-resurrection 读取。本轮 MAS source shape 已 landed：原 `generic_cli_mcp_product_wrappers`、`owner_route_reconcile_materialize_dispatch_shell`、`workbench_portal_generic_shell` 只作为 former wrapper tail / deletion-gate provenance，当前不再是 open functional follow-through。 | MAS 只留 medical study truth、publication gate、artifact/package authority、owner receipt / typed blocker 和 domain handler；不能据此声明 physical delete authorized、paper closure、artifact authority ready 或 production ready，剩余关闭条件是 real paper-line owner-chain、memory/artifact/lifecycle receipt 与 provider SLO evidence。 |
| `MAG_wrapper_shell_retirement` | MAG repo-side handler/ref-only/authority boundary 已闭合；顶层合同读为 `standard_agent_source_shape_status=landed`、`claims_opl_replacement_exists=true`、`claims_domain_repo_physical_delete_authorized=false`。按 strict purity，product/status/sidecar/runtime 等手写 shell 不再是标准组成；当前只作为 deletion authority / evidence tail 读取。 | OPL generated/default caller 与 App/default consumption 接管后，MAG 删除 product/status/sidecar/runtime shell、legacy flat alias、local journal/attempt ledger、scheduler/probe、patch bridge 和 compat tests；只留 grant authority functions、transition oracle、owner receipt / typed blocker 和 domain handler。 |
| `RCA_wrapper_adapter_retirement` | RCA generated/hosted shell、legacy cleanup 和 visual authority boundary 已清晰；bridge gate 当前读为 `source_shape_status=landed`、`functional_structure_gap_count=0`、`remaining_bridge_module_ids=[]`、`declares_no_active_bridge_modules=true`，但 delete tail 仍要求 `domain_authority_refs_preserved`、`no_regression_proof_recorded` 和 `explicit_owner_receipt_authorizes_physical_delete`，且 production/default-caller consumption 不能声明 complete。旧 session/store/managed/runtime/gateway 命名已降到 retired/provenance/tombstone 语境。 | active source 继续收敛为 visual authority implementation、native helper、domain handler target 和 artifact/visual pack logic；generic CLI/MCP wrappers、executor adapter shell、operator projection shell、stability read model、workspace/source lifecycle shell 不作为 RCA 长期代码保留。 |
...
3. 先推进 strict standard-agent source purity：把 MAS/MAG/RCA/OMA 的 repo-local generic default caller、sidecar/status/workbench、runtime projection、queue/ledger/session/lifecycle shell、compat aliases 和 compatibility-only tests 迁到 OPL generated/hosted surface 或直接删除。
4. 再推进 MAS/MAG/RCA repo-owned production caller、owner receipt、typed blocker、memory/artifact/lifecycle receipt、no-regression 和 live long-soak evidence。
...
- Refs-only adapter、diagnostic shell、tombstone/provenance ref 或 active direct path 可以作为标准智能体长期组成。
