...
| Codex App runtime evidence | `open_temporal_long_soak_followthrough` | `codex_app_runtime_role` 已把 `temporal_hosted_long_soak_refs` 投影为 refs-only production evidence follow-through gate，并在 `evidence_next_steps` 暴露真实 Temporal-hosted long-soak refs、provider state linkage、operator evidence refs 或 typed blocker refs 的 operator payload workorder。该面只提供观察与回填 guidance，后续仍需要 dedicated refs-only intake、provider-owner follow-through 或 typed blocker；不驱动长跑任务、不创建 owner receipt、不关闭 long-soak、不声明 production ready。 |
...
| Codex App Temporal long soak | `codex_app_runtime_role.production_evidence_followthrough` 暴露 open `temporal_hosted_long_soak_refs` gate 和 operator payload workorder，可回填真实 long-soak refs、provider state linkage、operator evidence refs 或 typed blocker refs。 | observation workorder、空 payload、event-only log 或 OPL 自造 ref 都不能关闭 long-soak；必须有真实 Temporal-hosted long-soak、dedicated refs-only intake、provider-owner receipt 或 typed blocker follow-through，并继续保持 Codex App 不驱动长跑任务。 |
