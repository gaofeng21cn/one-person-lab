...
当前仍不能写成“family 全部功能/结构差距为 0”。Fresh OPL CLI 证据显示，MAS/MAG/RCA 的 `opl_generated_agent_interface_bundle.status=ready`，并且 generated blocks 覆盖 CLI、MCP、Skill、product-entry、OpenAI tool 和 AI SDK。`opl stages proof-bundle --domain mas|mag|rca` 也已返回三仓各 6 个 stage 全部 `admitted`、`blockers_count=0`、`warnings_count=0`，family stage admission 汇总为 18/18 admitted。MAS 机器合同中的未分类 generic owner 回流和结构 follow-through gate 已关闭，MAG/RCA repo-side boundary 也更干净；但严格按“物理纯净 OPL 标准 Agent 模板”审计，MAS 仍有手写 turn lifecycle / runner / runtime-supervisor / workbench projection / SQLite lifecycle writer 等 repo-local generic-adjacent 实现，需要继续收薄为 domain guard、owner receipt、typed blocker、refs-only adapter 或直接迁到 OPL generated/hosted surface。family 层剩余功能/结构工作集中在 App/workbench 产品路径、managed environment startup、Developer Mode live route、MAS 物理收薄、MAG/RCA legacy cleanup evidence 和新 agent scaffold 消费；真实 provider / owner-chain long soak 继续作为 evidence gate。
...
| Domain private platform residue | `functional_privatization_audit` 已分类 MAS/MAG/RCA 的标准 pack、authority function 与 private platform residue，且 App drilldown 已把 domain-declared request/gate/coverage 投到 operator 面。MAS 合同层禁回流和结构 follow-through 已清零，但物理源码仍有手写 generic-adjacent runner/supervisor/workbench/SQLite writer，需要继续收薄；MAG/RCA repo-side boundary 更干净，但 live evidence / physical cleanup 仍未全部闭合。 | 对 MAS/MAG/RCA 剩余 private platform residue 执行 OPL 上收、generated surface 替换、refs-only 收薄、diagnostic cleanup 或 tombstone；不保留兼容 alias/facade。 |
...
MAS 理想形态是医学研究 `Declarative Medical Research Pack + OPL generated/hosted surfaces + minimal authority functions`。当前 MAS 机器合同已关闭未分类 generic owner 回流、effect-boundary stage admission 和结构 follow-through，`classification_gap_count=0`、`active_private_generic_residue_count=0`、`functional_structure_gap_count=0`。这表示 MAS 的声明边界和 gate 分类已经对齐；按“纯净 OPL 模板智能体”的物理标准，MAS 仍需要继续收薄手写 turn lifecycle / runner / worker lease、runtime supervisor scan/consume/execute、product/status/workbench projection builder、SQLite lifecycle writer 等通用平台实现。它们应迁为 OPL generated/hosted surface、refs-only adapter、domain guard/receipt 或 tombstone，不能作为 MAS 长期 generic owner。
...
- `mas_runtime_core` / turn runner / worker lease 只应保留 MAS owner-route prompt、domain guard 和 receipt；generic residency、retry、auto-continue 与 runner monitor 应迁到 OPL provider-backed stage runtime。
- Runtime supervisor 的 scan / consume / dispatch / rescan 控制环应迁到 OPL queue / attempt / scheduler / repair projection；MAS 只保留 owner-route 判定、domain typed blocker 和 receipt。
...
- SQLite lifecycle writer 应降为 refs-only adapter 或由 OPL lifecycle index 接管 generic persistence；MAS 只持 domain artifact authority 和 memory body / accept-reject receipt。
...
| MAS real paper apply / physical thinning | 已有 read-only / stable blocker / receipt consumption 证据；这证明边界可读，不证明 production caller、App/workbench、lifecycle/memory/artifact 对账或物理源码收薄已结构闭合。 | 多条真实 paper line 产生 progress delta、AI reviewer update、artifact delta、human gate、stop-loss 或 stable typed blocker；MAS 手写 generic runner/supervisor/workbench/SQLite writer 已迁为 OPL surface、refs-only adapter 或 tombstone，并保留 no-forbidden-write proof。 |
...
4. 按 `functional_privatization_audit` 逐项清 MAS/MAG/RCA private platform residue：MAS 先处理手写 runner/supervisor/workbench/SQLite lifecycle writer 的物理收薄；MAG/RCA 继续按 request/gate 做 no-active-caller、replacement parity、owner receipt 和 live evidence；能由 OPL 承接的迁走，不能声明化的收成最小 authority function，无 active caller 的旧面直接删除或 tombstone。
