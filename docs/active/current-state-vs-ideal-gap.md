...
GraphFlow / GFL 论文中可吸收的模式已经被定位为 OPL 自有 contract pattern：stage pack 先做 admission，stage 之间用 `requires` / `ensures` 组合，可信部分拆成 `verified_static_core`，AI / 人 / 外部系统 / domain verdict 等边界进入 `runtime_enforced_boundary`，再由 selected executor 启动已准入 stage pack。这个吸收不引入 GraphFlow / GFL runtime dependency，也不把 GraphFlow 写成 OPL provider、planner、stage runner、executor 或 domain authority。
...
| Stage pack admission / requires-ensures composition | 当前已有 stage descriptor、action/stage discovery、authority boundary、typed queue、attempt ledger 与 receipt projection；GraphFlow/GFL 可借鉴模式已在文档口径中收敛为 stage pack admission、`requires` / `ensures` 组合、trust lane、`verified_static_core` 与 `runtime_enforced_boundary`。这仍是叙述性 contract / gap definition，不等于生产启动路径已经逐条执行机器级 admission、composition 和 executor-binding proof。 | 在 OPL launch / queue / provider / App projection 路径中形成机器级 admission gate：每个可启动 stage pack 都有 `requires`、`ensures`、trust lane、authority boundary、selected executor binding、idempotency、consumed refs、expected receipt refs；组合不满足时产出 typed blocker / human gate / route-back。 |
...
2. 把 stage pack admission 变成生产启动前的明确 gate：每个可启动 pack 都声明 `requires` / `ensures`、trust lane、authority boundary 和 selected executor binding；组合失败时形成 typed blocker / human gate / route-back。
...
- 不能把 GraphFlow / GFL 的参考模式写成 OPL 已引入 runtime dependency、provider、stage runner、planner 或 executor，也不能把叙述性 admission 口径写成生产启动路径已有完整机器 gate。
