...
| `stage_native_kernel` | StageRun Kernel 默认面只回答 current owner delta、缺什么 role/receipt/blocker 和下一 owner；launch/closeout blockers 与 advisory/audit/drilldown 分层，避免把新 Kernel 变成更厚 admission、worklist 或 read-model 补偿链。 |
...
| `real_owner_delta_tail` | `needs_domain_owner_evidence_tail`。MAS Stage Native live canary 已把 DM002 / DM003 stage artifact read-model 推进到 terminal publication handoff owner gate；MAS AI reviewer handoff/currentness、gate replay routing、provider dispatch authorization 和 refs-only typed blocker transport 已让 owner route 更清楚。这些属于 OPL/MAS 路由与接力能力，不是 publication verdict、paper closure 或 production readiness。 | MAS/MAG/RCA/OMA domain owners | MAS 下一步是 `publication_handoff_owner_receipt_or_typed_blocker`；MAG/RCA/OMA 下一步是真实 grant/visual/target-agent owner receipt、typed blocker、human gate、review/export receipt、no-regression 或 long-soak refs。下一个 OPL safe action 若仍指向 domain live payload，必须由 domain owner 提供可记录 ref。 |
...
- Required actions:
  ...
  3. 若当前工作是 StageRun Kernel / conformance / App cockpit surface，先保持 launch admission 与 closeout admission 分层：strategy refs 缺失只进 advisory / route-back，只有 identity、owner、scope、executor、authority、required role slot、receipt/blocker shape、forbidden write 或 replay/audit lineage 缺口才阻断 launch；只有 role artifact、manifest、owner receipt / typed blocker、current pointer、hash、generation 或 lineage 缺口才阻断 closeout。
