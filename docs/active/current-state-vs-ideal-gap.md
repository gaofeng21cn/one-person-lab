...
| Standard agent structural conformance | `passed_with_strict_purity_followthrough` | Fresh `opl agents conformance --family-defaults --json` 显示 4 个 repo 通过、0 个 repo blocked，active forbidden-name residue 清零；这只证明当前 descriptor / skeleton / policy 可读。按严格标准，domain repo 中仍有 active caller 的手写 product/status/workbench/sidecar/runtime/dispatch/adapter shell 都算标准化 follow-through，不再被写成长期可接受的 refs-only adapter。MAS 当前合同已落到 `source_purity_cutover_status=standard_agent_source_shape_landed`、`repo_local_wrapper_tail_count=0`；MAG/RCA 仍保留 strict physical/source-purity bridge-exit tail，OMA 只剩 hygiene/evidence/default-caller tail。 |
...
| Domain production evidence | `partial_blocked_by_domain_owner_evidence` | MAS/MAG/RCA 的 owner receipt refs、typed blocker refs、no-regression refs 可进入 OPL refs-only ledger；worklist/readiness 已把重复 domain-owned typed blocker refs 作为 attention group 去重展示，同时保留 raw item / envelope 审计数量。Fresh evidence worklist 读为 `open_worklist_item_count=0`、`open_safe_action_payload_required_item_count=0`、`open_safe_action_payload_free_item_count=0`，同时仍有 `zero_open_worklist_blocked_refs_only_envelope_count=194`、`domain_ready_authorized=false`、`production_ready_authorized=false`。domain-dispatch 默认 record route 只对最新且已有 accepted typed closeout 的 stage attempt 开放；未 closeout attempt 只作为 blocked / superseded provenance。真实 paper/grant/visual owner-chain、memory/artifact/lifecycle receipt 和 repeated no-regression 仍需扩面。 |
...
| gap | 当前状态 | 完成口径 |
| --- | --- | --- |
| `zero_retained_generic_adapter_purity` | 当前 conformance 只证明标准面可读；MAS 已从功能/结构 gap 收敛到 `functional_structure_gap_count=0`、`repo_local_wrapper_tail_count=0`、`source_purity_cutover_status=standard_agent_source_shape_landed`，但 `domain_repo_physical_delete_authorized=false`，剩余为 live provider paper-line、memory/artifact/lifecycle receipt 和 provider SLO evidence gates。MAG/RCA 按 strict source-purity 仍 dirty：functional gap 0 只表示 legacy generic runtime owner 闭合，active bridge-exit / wrapper delete tails 尚未物理清零。OMA 不标 structural dirty，只保留 default-caller consumption、refs-only adapter thinning、script-to-pack hygiene 和 production evidence tail。 | 标准智能体 active source 只允许 `agent/` domain pack、machine-readable contracts、standard authority functions、domain handler target、domain-specific implementation 和必要 native helper。任何 generic control-plane / default-caller / workbench / sidecar / session / queue / lifecycle / wrapper 逻辑都由 OPL 生成或托管；域仓副本删除，不保留兼容 alias、facade、re-export wrapper 或 compatibility-only tests。 |
