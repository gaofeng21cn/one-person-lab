...
| Codex App Temporal long soak | `codex_app_runtime_role.production_evidence_followthrough` 暴露 `temporal_hosted_long_soak_refs` gate、operator payload workorder、专用 refs-only ledger、record/verify safe-action route 和专用 `codex-app-runtime-evidence long-soak start|event|finish` observation surface。recorded receipt 只显示 verify-pending；verified `temporal_hosted_long_soak_refs` 会让该 refs-observed gate 清零；provider state linkage 与 operator evidence refs 只作为支撑审计 refs，单独记录或验证不能关闭 gate；typed blocker refs 只作为 provider-owner 阻塞信号继续暴露。 | observation workorder、空 payload、event-only log、未 verify receipt、support-only payload 或 OPL 自造 ref 都不能关闭 long-soak；`finish` 物化的 payload 仍需经过 `record` / `verify`。必须有真实 Temporal-hosted long-soak refs、provider-owner receipt 或 typed blocker follow-through，并继续保持 Codex App 不驱动长跑任务、不创建 owner receipt、不声明 production ready。 |
