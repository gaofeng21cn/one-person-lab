...
North-star 目标态回到 [OPL 与 Foundry Agents 理想目标态](../references/runtime-substrate/opl-family-agent-ideal-state.md)。公开角色、runtime 边界和默认入口回到 `project/status/architecture/invariants/decisions`。文档生命周期规则回到 [文档组合治理](../docs_portfolio_consolidation.md) 和 [文档生命周期政策](../policies/docs-lifecycle-policy.md)。
...
| `stage_artifact_unit_domain_adoption` | MAS `stage-artifact-materialize` 已成为当前首个真实 domain adoption tail；MAG/RCA/OMA 仍需要把各自 authoring/export、visual review/export、agent work-order stage 的 stable artifact roles、tool affordance refs、quality gate refs 和 owner receipt / typed blocker refs 做成可重复 evidence。 | 每个 domain 至少有一条真实 stage artifact unit 从 legacy/current workspace refs 物化为 manifest + owner receipt + current pointer / index projection；非终局 stage 不要求终局 ready，终局 ready 只归 domain gate。 |
