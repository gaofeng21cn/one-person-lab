...
- OPL 已新增 domain-neutral family transition runner 机器面：`contracts/opl-framework/family-transition-runner-contract.json`、`src/family-transition-runner.ts`、`src/family-transition-oracle-ingestion.ts`、`tests/src/family-transition-runner.test.ts` 和 package export `./family-transition-runner`。它把 transition schema、guard 匹配、fail-closed、dead-letter、receipt/projection envelope 和 matrix runner 放在 OPL；MAS/MAG/RCA 继续持有各自 domain transition table、guard 定义、oracle fixtures、质量 verdict、artifact authority 和 owner receipt。当前测试使用 MAS-like fixture 锁定 `bundle_stage_ready`、`bundle_stage_blocked`、`publishability_gate_blocked` 相邻状态不会误转场，并使用 MAG-like `mag_grant_transition_oracle` fixture 证明 OPL 能把 MAG-owned transition table / oracle fixtures 适配成通用 matrix cases；这证明通用 runner / ingestion 边界，不证明 MAS 三篇论文已经被推进，也不证明 MAG grant-stage production soak 已完成。
