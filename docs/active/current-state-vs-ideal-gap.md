...
MAS/MAG/RCA 当前均已被 OPL 识别为标准 domain agent 的消费方。三仓 effect-boundary stage 已补齐 machine-readable `runtime_event_refs` 并通过 OPL proof-bundle admission。MAS 机器合同已关闭未分类 generic owner 回流和 5 个结构 follow-through gate，`classification_gap_count=0`、`active_private_generic_residue_count=0`、`functional_structure_gap_count=0`；这说明声明边界正确，不说明物理 repo 已清到最小模板。MAS 源码中的 turn lifecycle / runner / supervisor / workbench / SQLite lifecycle 等实现需要作为上收输入继续处理。MAG 当前 repo-side handler / refs-only / authority boundary 已闭合，剩外部默认 caller、App 消费、live soak 和 owner receipt scaleout 证据；RCA 当前 generated/hosted shell 消费边界已闭合，剩 controlled visual-stage long soak、naming hygiene 和 live owner-chain evidence。不能以“已有 active caller”为理由保留通用平台面。保留在 domain 内的程序面必须能解释为领域 truth、quality/export verdict、artifact mutation authorization、memory accept/reject、source readiness、owner receipt signer 或 domain-native helper implementation，并走 OPL 标准 ABI。
