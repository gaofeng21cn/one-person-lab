...
| Domain production evidence | `partial_blocked_by_domain_owner_evidence` | MAS/MAG/RCA 的 owner receipt refs、typed blocker refs、no-regression refs 可进入 OPL refs-only ledger；worklist/readiness 已把重复 domain-owned typed blocker refs 作为 attention group 去重展示，同时保留 raw item / envelope 审计数量。Fresh evidence worklist 读为 `open_worklist_item_count=0`、`open_safe_action_payload_required_item_count=0`、`open_safe_action_payload_free_item_count=0`，同时仍有 `zero_open_worklist_blocked_refs_only_envelope_count=222`、`domain_ready_authorized=false`、`production_ready_authorized=false`，且 not-authorized claims 仍包含 `domain_repo_physical_delete_authorization` 和 `default_caller_delete_ready`。domain-dispatch 默认 record route 只对最新且已有 accepted typed closeout 的 stage attempt 开放；未 closeout attempt 只作为 blocked / superseded provenance。MAG legacy lifecycle refs 已作为 standalone verified external evidence receipt 投影进 App/operator：external verified artifact mutation refs 读为 2、restore proof refs 读为 3。真实 paper/grant/visual owner-chain、accepted/rejected memory writeback、package/lifecycle receipts 和 repeated no-regression 仍需扩面。 |
...
| MAG grant soak | MAG transition oracle、owner receipt contract、refs-only handoff 边界清晰；MAG legacy lifecycle cleanup 的 artifact mutation refs 与 restore proof refs 已作为 verified standalone external evidence 被 App/operator 消费。 | 真实 OPL-hosted grant-stage attempt 持续返回 grant-owned receipt、typed blocker 或 no-regression evidence；legacy lifecycle receipt 可见性不等于 grant authoring / fundability / submission readiness。 |
