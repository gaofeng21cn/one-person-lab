...
OPL 当前开发继续按 Codex-default、provider-backed、framework-first 执行：先把 OPL 做成完整智能体开发/运行框架，用 OPL Meta Agent 支撑新 Agent 的创建、测试接管和标准骨架检查，再让 MAS/MAG/RCA 迁成标准 OPL Agents，并用真实 domain owner receipt / long soak 验收目标结构。

当前 OPL 已有 framework/control-plane、Temporal provider、stage attempt ledger、typed queue、typed closeout、production closeout read model、domain descriptor aggregation、functional runtime harness、generic substrate projection、Agent Lab、pack compiler handoff、external evidence refs-only receipt ledger、App/operator drilldown read model、App runtime 页面消费路径、managed clean runner 和 `opl system startup-maintenance` App 启动维护机器入口。`app_operator_drilldown` 现在也会投影 domain evidence request / remaining gate / OPL replacement coverage / observed receipt status / legacy cleanup executable plan，说明 App/operator 能看到“还差哪些 receipt、哪些 cleanup plan 可 apply、哪些 delete 被 domain owner gate 挡住”。OPL 还会把 OPL Meta Agent 的 registry / App workbench / real target scaleout contracts 作为 refs-only extension 消费进 domain manifest registry 与 App/operator drilldown；当前 OMA 的 MAS/MAG 两目标 scaleout 已由 OMA 自身 refs-only receipt contract 关闭，不授权 OPL 或 OMA 写 MAS/MAG domain truth、quality verdict、artifact body、memory body 或 default promotion。`system startup-maintenance` 则负责 clean OPL-managed ecosystem modules 的 install/update、health check、domain skill sync、plugin cache freshness 和 reload prompt；默认覆盖 MAS/MAG/RCA，并把 OPL Meta Agent 作为 Agent Foundry managed module/default skill 同步维护，同时把 dirty/ahead/diverged/no-upstream/env override/sibling workspace/invalid checkout fail closed 到人工处理。这些说明 OPL 的通用底座已经存在，但仍不代表 production domain owner chain、App 发布包/截图/长时证据、generated caller migration 或 legacy / provenance / diagnostic cleanup 完成。
...
| 线路 | 当前 owner | 当前要做 |
| --- | --- | --- |
| `provider_runtime` | OPL Runtime Manager / Temporal provider | 固定 Temporal production provider，保持 cadence / capability SLO satisfied；restart/re-query/signal/history 已进入 OPL provider capability projection，继续补真实 domain owner-chain dispatch 和长时 operator evidence。 |
...
| `app_workbench` | One Person Lab App / OPL product surface | 已有 `app_operator_drilldown` runtime 页面消费路径，并已投影 evidence request、replacement coverage、external evidence receipt status、cleanup plan 和 OPL Meta Agent refs-only workbench sections；`opl system startup-maintenance` 已提供 managed environment freshness、plugin cache freshness 和 reload prompt 机器入口；继续补真实用户路径、截图、发布包和长时证据，不做 domain truth owner。 |
