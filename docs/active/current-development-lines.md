...
2026-06-14 workspace currentness foldback：`redcube` active binding 当前为 `/Users/gaofeng/workspace/redcube-ai`，`medautogrant` active binding 当前为 `/Users/gaofeng/workspace/med-autogrant`；fresh `framework readiness` 已显示 `domain_manifest_stale_binding_project_ids=[]` 与 `domain_manifest_not_configured_project_ids=[]`。这只关闭 workspace binding currentness 尾项，不改变 `framework operating-maturity.status=evidence_required`、`ready_claim_authorized=false`、current owner delta pointer 未闭合、provider capability SLO blocked、App release not-ready、Brand L5 `0/10` 或 physical delete not-authorized。

2026-06-14 provider capability evidence foldback：`provider_long_soak` 的 public ledger payload 现在支持 `capability_requirement_ids`。真实 OPL state 已验证 per-capability refs-only blocker receipt，fresh maturity 将 5 个 provider capability requirement 读为 `refs_observed_not_ready_claim`，`capability_missing_requirement_ids=[]` / `missing_owner_action_ids=[]`。这只让 owner work order 可按 requirement 精确接力；`provider_long_soak_open_count=1`、`long_evidence_ready=false` 和 `ready_claim_authorized=false` 仍保持。
...
- `framework readiness` 当前 `domain_manifest_stale_binding_project_ids=[]`、`domain_manifest_not_configured_project_ids=[]`；`redcube` 与 `medautogrant` 的 active workspace bindings 已回到真实 repo 路径。
...
- `provider_long_soak` 仍是 `owner_evidence_required` / `ready_claim_authorized=false`；readout 已把 `capability_slo_blocked` 细化到 `capability_checklist`、`capability_evidence_observed_requirement_ids` 与 owner work-order `owner_action_checklist`。fresh 5 项 capability requirement 已有 per-capability refs-only blocker/evidence，`capability_missing_requirement_ids=[]`，但 `provider_long_soak_open_count=1` / `long_evidence_ready=false`，仍不能声明 provider production ready。
