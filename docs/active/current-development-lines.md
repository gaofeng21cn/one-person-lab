...
OPL development remains framework-first. The current functional closeout has proven that MAS/MAG/RCA can all enter the OPL task-bound stage-attempt / typed-closeout ledger. The next production acceptance path is still MAS-first: prove that OPL can host the MAS paper-autonomy owner chain without writing MAS truth, while MAG/RCA grant/visual long soaks remain later evidence gates.

Current calibration on 2026-05-14: the framework control plane, shared contracts, local queue / attempt ledger, Temporal provider code, typed closeout gate, MAS/MAG/RCA task-bound attempt bridge, typed closeout memory-ref summary, provider continuous proof projection, latest production proof persistence, runtime tray provider proof operator item, stage-attempt operator item, manifest timeout fail-closed, domain skeleton discovery, stage plane discovery, domain-memory descriptor index, unified domain-agent descriptor, runtime snapshot, Aion stage-attempt workbench, Codex runner repo/test harness, and repo-native Temporal live residency proof have landed as readable/testable surfaces. Fresh `./bin/opl` CLI checks show `agents list --json` at `aligned_count=3`, `missing_count=0`, `drift_detected_count=0`, `physical_skeleton_evidence_observed_count=3`, `physical_skeleton_audit_pending_count=0`; `stages list --json` at `resolved_planes_count=3`, `stages_count=18`; `domain-memory list --json` at `resolved_memory_descriptor_count=3`, `missing_memory_descriptor_count=0`; and `agents descriptors --json` as the unified entry that aggregates entry, skeleton, stage, action, memory, skill, runtime/session/progress/artifact refs, and authority boundaries in one read model with the same physical-skeleton evidence summary. MAS/MAG/RCA descriptor-level migration is complete, and maintainers no longer have to reconstruct a domain agent’s overall state from separate subcommands. The agents read model separates `descriptor_readiness`, `physical_skeleton_layout_audit`, `physical_skeleton_evidence`, and `production_closure_gaps`; MAS/MAG/RCA currently have repo-source anchor evidence observed. OPL only aggregates refs and does not write domain truth or perform domain repo physical migration. The production closeout gate now summarizes consumed/writeback/rejected memory refs from stage typed closeouts, reads Temporal production proof receipts from the runtime ledger, and fails closed on stalled domain manifest commands; `family-runtime intake/tick --hydrate` can now derive the MAS sidecar export command from the active workspace binding and automatically attach the latest persisted Temporal production proof; runtime snapshot derives stage-attempt operator items with controlled apply refs, artifact locator / restore proof, memory refs, writeback receipt refs, and rejected writes; a manifest timeout is a read-model availability blocker. With the operator timeout budget, production closeout now returns `functional_closure_ready_for_live_soak` with `typed_blocker_count=0`, and the stage attempt ledger is `total=4` / `completed=4`, including two MAS guarded-apply typed-blocker attempts.
...
2. Use real MAS paper lines as the first production acceptance path: OPL queue can now bind guarded-apply tasks to stage attempts; start from read-only closeout projection, then let MAS owner gates decide any live provider-hosted guarded apply. Valid results include artifact delta, publication gate replay, AI reviewer update, route decision, human gate, stop-loss, or typed blocker.
...
5. Retire old Hermes/Gateway/frontdoor/local-manager/default-compat wording and duplicate UI/runtime entries as soon as replacement proof and no-default-caller evidence exist; delete or tombstone them instead of keeping compatibility entrypoints.
...
| `2` | `mas_paper_autonomy_acceptance` | OPL provider + MAS owner surfaces | current primary production-closure line. Three MAS real paper lines already have read-only typed closeout projection; OPL queue has created task-bound provider-backed stage attempts for MAS `paper_autonomy/guarded-apply` and ingested two `blocked_no_mas_owner_apply_receipt` typed closeouts. The latest path proves active workspace binding hydrate plus latest persisted proof can automatically form the MAS sidecar export. Next work is live provider-backed attempt -> MAS owner receipt chain under guarded apply, yielding progress delta, human gate, stop-loss, or stable typed blocker without forbidden writes |
| `3` | `domain_framework_migration` | OPL + MAS/MAG/RCA domain repos | descriptor / manifest alignment is complete across the three active domains; `opl agents descriptors` is now the maintainer total entry, while `opl agents inspect`, `opl stages`, `opl actions`, and `opl domain-memory` remain topic drilldowns. OPL now exposes physical skeleton evidence refs / gap projection in the agents read model; current evidence is MAS/MAG/RCA observed. MAG/RCA now have live task-bound sidecar receipt ingestion. Next work is path compatibility audit and continuous proof that direct skill and OPL-hosted paths share owner receipts. Workspace/runtime receipt parity and provider-hosted owner-chain soak remain production-closure work |
| `4` | `feature_partition_and_retirement` | OPL active docs + domain owner docs | lift framework-generic capabilities into OPL, keep domain-specific truth in domains, and retire old Hermes/Gateway/frontdoor/local-manager/MDS-default active-path residue through no-default-caller evidence; when proven unused, delete code/interfaces/tests or move them to history/tombstone rather than preserving compatibility |
| `5` | `opl_app_runtime_workbench` | OPL App / Runtime Manager | show provider readiness, provider proof operator items, stage-attempt operator items, domain status, human gates, action receipts, artifact refs, source refs, memory refs, and rejected writebacks without rewriting domain truth; the stage-attempt workbench now exposes read-only grouping, filter keys, attention counters, controlled apply refs, artifact locator / restore proof, and memory-ref counters for App panels |
...
| old gateway/frontdoor/Hermes-first/local-manager default wording | delete or move to history/tombstone after replacement proof and no-default-caller scan; no compatibility entrypoint remains active |
...
3. Cleanup is migration closeout: old default paths should not linger indefinitely; once no-active-caller and replacement evidence exist, delete the module/interface/test or move it to history/tombstone instead of keeping a compatibility path.
...
| `mas_paper_autonomy_acceptance` | Read-only acceptance is satisfied when three MAS real paper lines each expose OPL-ingestable typed closeout packets and MAS-owned evidence refs without forbidden writes. The OPL task-bound guarded-apply attempt bridge is covered by focused tests. Production acceptance still requires live provider-hosted guarded apply attempt query, MAS owner receipt, progress delta / human gate / stop-loss / typed blocker, and no-forbidden-write proof. |
| `domain_framework_migration` | MAS/MAG/RCA are admitted through shared descriptor/skeleton/locator/receipt surfaces; maintainers can inspect the whole integration through `opl agents descriptors` before drilling into stage/action/memory subcommands; repo-source anchor evidence is observed across all three active domains, while real workspace/runtime receipt proofs and direct/OPL-hosted owner-receipt parity still need follow-through. |
| `feature_partition_and_retirement` | old default dependencies, legacy compatibility, duplicate UI, and stale manager surfaces are classified, replaced, and retired; unused modules/interfaces/tests are deleted or tombstoned; retained items have explicit owner and use; active-path residue tests prove default help/docs no longer advertise legacy operator paths. |
| `opl_app_runtime_workbench` | OPL App can read provider readiness/proof, stage attempt, domain progress, human gate, artifact refs, source refs, safe action receipts, controlled apply refs, restore proof, and stage-attempt grouping/filter summaries in one workbench. |
| `domain_soak_and_acceptance` | MAS/MAG/RCA each produce real or controlled progress delta, quality-gate movement, human gate, stop-loss, domain owner receipt, no-regression evidence, or typed blocker in the migrated target shape. |
