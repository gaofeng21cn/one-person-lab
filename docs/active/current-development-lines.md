...
- `family_runtime_evidence_worklist.summary` 当前读为 0 个 open worklist item、0 个 payload-required safe action、0 个 payload-free safe action、0 个 stage receipt freshness workorder；这只表示当前没有 OPL 可执行 safe-action route，仍不授权 OPL 自造 owner receipt、typed blocker、owner-chain、no-regression、completion、domain ready 或 production ready。closed refs-only item 与 blocked refs-only envelope 数量是动态读数；`zero_open_worklist_guard` 仍读为 `zero_open_worklist_is_completion_claim=false`、`zero_open_worklist_is_domain_ready=false`、`zero_open_worklist_is_production_ready=false`。
