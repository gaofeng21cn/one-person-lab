...
2026-05-15 MAS transition receipt consumption 更新：MAS main `874116ef` 已把 `completion_receipt_consumption` 纳入 MAS-owned `study-state-matrix` transition row、family transition spec receipt 和 matrix case context/expected oracle。覆盖的真实控制面回归包括 DM002 package closure / completed turn closeout 被投成 `completion_receipt_consumed`，防止已完成 work unit 被 redrive；DM003 default executor execution receipt 会 supersede stale blocked closeout，防止旧 user-wait 投影压过更新的 owner execution receipt。OPL 侧只消费这份 domain-owned transition spec / case context，不把 completion receipt consumption 写成 paper closure、publication quality verdict 或 OPL-owned domain decision。
