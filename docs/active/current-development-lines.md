...
| `domain_framework_migration` | MAS/MAG/RCA are admitted through shared descriptor/skeleton/locator/receipt surfaces; maintainers can inspect the whole integration through `opl agents descriptors` before drilling into stage/action/memory subcommands; repo-source layout and real workspace/runtime receipt proofs are complete; direct and OPL-hosted paths share domain owner receipts. |
