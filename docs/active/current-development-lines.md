...
| --- | --- | --- | --- |
| `current_owner_delta_default` | OPL Framework + domain owner | `current-state-vs-ideal-gap.md` | 默认 CLI/App/operator summary 只回答当前 owner、accepted answer shape、hard gate 和下一步 owner action。当前 identity 的 owner-answer handoff 以 readiness current pointer、StageRun closeout binding 和 execution authorization ledger receipt 的 fresh readout 为准；已闭合的 closeout ref 只能关闭 current pointer handoff，不能重复消费旧 refs-only summary，也不能声明 domain ready 或 production ready。新 live identity 出现时必须重新读取 current owner delta / maturity bridge / domain owner truth。raw evidence、worklist、replay、typed-blocker group 和 private residue只进入 full-detail / diagnostic；production-tail 仍回 owner gates。 |
