...
| `family-runtime production-closeout` 等旧 CLI alias | 已被当前 readiness / evidence-worklist / App drilldown route 替代。 | 不保留 compatibility alias；仍被测试或文档引用时迁到当前命令或 negative guard。 |
...
| stale compatibility tests | OPL 当前只需要 no-resurrection guard、contract behavior test 或 migration proof。 | 删除只保护旧路径的测试；保留的测试必须断言当前 contract、no-active-caller、fail-closed 或 tombstone semantics。 |
