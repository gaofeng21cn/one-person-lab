...
4. Generic primitives：已落地的 read-model shell 继续由 OPL/App 产品化和真实 receipt 驱动；domain agent 只提供 authority refs、projection refs、route nodes/edges、review/quality/export refs、receipt 和 typed blocker。缺口集中在真实 live apply、artifact/memory lifecycle evidence、operator SLO 和 App drilldown，而不是重新设计一套通用壳。
...
7. Legacy cleanup：在 full no-active-caller、replacement parity 和 provenance proof 后删除 active-path residue 或迁入 history/tombstone。
