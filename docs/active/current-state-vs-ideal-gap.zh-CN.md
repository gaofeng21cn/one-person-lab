...
- Runtime snapshot 已有 stage-attempt workbench、stage-attempt operator item、attention queue、provider continuous proof operator item、operator action routing shell 和 native-helper execution envelope 投影；stage attempt item 可只读展示 provider completion、domain ready verdict、controlled apply contract、artifact locator / restore proof、memory refs、writeback receipt refs、rejected writes 和明确 owner 的 action route refs。当前 fresh snapshot 里 `stage_attempt_workbench.summary.total=4`、`by_status.completed=4`，分别覆盖 MAS、MAG、RCA；真实 long-running provider-hosted domain activity 仍未形成连续 owner-chain 证据。
...
| OPL generic primitives | OPL 提供跨 domain 可复用的 runtime transport、queue、human gate transport、workspace/source intake shell、memory locator/index、artifact/package lifecycle、restore/retention、artifact gallery、review/repair transport、native-helper execution envelope、workbench shell、route/decision graph renderer、quality/readiness projection shell 和 observability transport | 部分已落地为 provider/attempt ledger、runtime snapshot、stage-attempt workbench、workspace/source intake projection、memory locator/index projection、artifact gallery projection、package/export lifecycle projection、route-decision graph projection、review-repair queue projection、quality/readiness projection、observability/SLO repair-command projection、operator action routing projection、native helper execution envelope projection、domain-memory descriptor、artifact locator refs、closeout gate、native helper state cache 与 App workbench projection | 这些能力仍偏 read-model / receipt-ref / typed-blocker / action-route 层；还缺跨三仓真实 live apply、memory body/writeback transport 泛化、artifact/package lifecycle apply、真实 domain projection ingestion、App productization 和长时 operator SLO；当前 generic read-model shells 不替代 domain owner receipt、quality verdict、source readiness、memory accept/reject、native helper domain mutation 或 export verdict | 把剩余 MAS/MAG/RCA generic-layer 候选继续上收到 OPL：真实 domain projection ingestion、App productization、operator SLO 和 live apply receipt；domain 只回传 authority refs、route/review/quality/export refs、receipt 和 typed blocker |
...
4. Generic primitives：把 workspace/source intake shell、memory locator/index、artifact/package lifecycle、restore/retention、artifact gallery/handoff shell、route/decision graph renderer、review/repair transport、native helper catalog/envelope、workbench shell、attention queue、action routing shell、quality/readiness projection、observability/SLO 和 repair command projection 做成 OPL-owned family primitives；domain agent 只提供 authority refs、route nodes/edges、review/quality/export refs、receipt 和 typed blocker。
...
6. 清理旧 Hermes/Gateway/local-manager 命名和 legacy manager 入口，保留项迁入 explicit proof/provenance/history。
