...
- Runtime snapshot 已有 stage-attempt workbench、stage-attempt operator item、attention queue、provider continuous proof operator item、operator action routing shell、native-helper execution envelope 和 domain projection ingestion 投影；stage attempt item 可只读展示 provider completion、domain ready verdict、controlled apply contract、artifact locator / restore proof、memory refs、writeback receipt refs、rejected writes 和明确 owner 的 action route refs；domain projection ingestion 只汇总 manifest-owned projection refs / surface kind / owner / source refs，不执行 domain projection builder、不读取 domain truth body、不授权 ready / quality / export verdict。当前 fresh snapshot 里 `stage_attempt_workbench.summary.total=4`、`by_status.completed=4`，分别覆盖 MAS、MAG、RCA；真实 long-running provider-hosted domain activity 仍未形成连续 owner-chain 证据。
...
本文对照 [OPL 与 Foundry Agents 理想目标态](../references/runtime-substrate/opl-family-agent-ideal-state.zh-CN.md)。MAS 理想态牵出的跨仓 owner 边界与完善计划见 [MAS 理想态跨仓差距与完善计划](./mas-ideal-state-cross-repo-gap-plan.zh-CN.md)。当前状态和比例判断仍以 [OPL 当前状态](../status.md)、[OPL 架构](../architecture.md)、[OPL Stage-Led Agent Framework Roadmap](../references/runtime-substrate/opl-stage-led-agent-framework-roadmap.zh-CN.md) 与 [OPL 生产级框架闭环差距矩阵](./production-framework-closure-gap-matrix.zh-CN.md) 为准。
...
| `npm run test:fast` | exit 0；`173` tests passed、`0` failed | Fast lane 覆盖 family runtime、provider-hosted attempts、production closeout、runtime snapshot/workbench、descriptor CLI、native smoke、quality details 和 repo hygiene。 |
...
| Agent executor | `Codex CLI` 为默认最小执行器，其他 executor 只能作为独立实验 receipt gate 接入并可审计 | 默认 Codex 口径已收口；Hermes/Gateway 旧兼容面已退出 active/default path | 非默认 executor 不承诺行为等价；真实长时 Codex runner production soak 未完成 | 继续保留 non-equivalence notice；优先证明 Codex CLI long-running activity、heartbeat、typed closeout 和 domain receipt |
...
- MAS/MAG/RCA 理想目标态暴露出一批应由 OPL 上收的 generic primitives：workspace/source intake shell、memory locator/index、artifact/package lifecycle、restore/retention、artifact gallery/handoff shell、route/decision graph renderer、review/repair transport、native helper catalog/envelope、domain projection ingestion、workbench shell、attention queue、quality/readiness projection、observability/SLO 和 repair command projection。OPL 当前已经有 read-model / locator / receipt-ref / native-helper cache 基础；本轮把 workspace/source intake、memory locator/index、artifact gallery / handoff shell、package/export lifecycle shell、route/decision graph renderer、review/repair transport、quality/readiness projection、observability/SLO repair-command projection、operator action routing shell、native helper execution envelope projection 和 domain projection ingestion projection 先落成 stage attempt / workbench / runtime snapshot 的 generic read-model shell。剩余 primitives 还需要继续补 App productization、长时 operator SLO 和跨三仓 live apply，而不是让 MAS/MAG/RCA 分别重复实现。
...
4. Generic primitives：把 workspace/source intake shell、memory locator/index、artifact/package lifecycle、restore/retention、artifact gallery/handoff shell、route/decision graph renderer、review/repair transport、native helper catalog/envelope、domain projection ingestion、workbench shell、attention queue、action routing shell、quality/readiness projection、observability/SLO 和 repair command projection 做成 OPL-owned family primitives；domain agent 只提供 authority refs、projection refs、route nodes/edges、review/quality/export refs、receipt 和 typed blocker。
...
- MAS 理想形态已经把自身收窄为医学研究 `Domain Knowledge / Authority Pack`：医学 stage pack、研究路线知识、publication-route memory policy、AI reviewer / quality rubric、artifact authority contract、owner receipt schema 和 domain projection builder。与此相对，generic runtime、queue、memory locator、artifact lifecycle、restore/retention、workbench shell、route visualization 和 observability primitive 应尽量由 OPL 提供；当前 family 仍有部分能力停在 MAS repo-local surface 或 MAS-specific projection 中，需要上收到 OPL shared layer。
...
5. 将 MAS-specific 的通用能力需求逐项交给 OPL owner：memory locator/index、artifact lifecycle、route graph shell、attention queue、workbench navigation、observability/SLO 和 repair command projection；MAS 保留 route decision、quality verdict、artifact mutation permission 和 owner receipt。
...
- MAG 理想目标态进一步说明哪些 grant-facing 能力不应长期由 MAG 重复维护通用外围：pre-authoring workspace/source intake shell、explicit wakeup / human-gate transport、grant strategy memory locator/writeback transport、submission-ready package locator/gap-report projection、route/decision drilldown、quality/readiness projection shell、artifact lifecycle/restore/retention 和 operator observability 应由 OPL Framework / App 提供通用壳。MAG 保留 funding-call 解释、fundability、specific aims、authoring quality、grant memory body/accept-reject、package readiness 和 export authority。
...
- Submission-ready package 当前是 MAG-owned export gate；OPL/App 仍缺跨 domain 通用 package/export lifecycle shell 来展示 package refs、gap report、external portal/manual submission boundary、restore/provenance 和 owner action，而不推断 export readiness。
...
4. 把 MAG-specific 的通用外围需求交给 OPL owner：workspace/source intake shell、explicit wakeup / human-gate transport、grant strategy memory locator/writeback transport、package/export lifecycle shell、route/decision graph、quality/readiness projection、attention queue 和 observability/SLO；MAG 保留 fundability、authoring quality、route decision、package readiness、export verdict 和 owner receipt。
...
2. `OPL generic primitive absorption`
   先把 MAS/MAG/RCA 理想形态中明确应上收的 generic layer 变成 OPL-owned primitives：workspace/source intake shell、memory locator/index、artifact/package lifecycle、restore/retention、artifact gallery/handoff shell、review/repair transport、native helper catalog/envelope、domain projection ingestion、route/decision graph renderer、workbench shell、attention queue、action routing shell、quality/readiness projection、observability/SLO 和 repair command projection。`workspace/source intake shell` 已以 `opl_workspace_source_intake_projection` 进入 stage-attempt/workbench read model；`memory locator/index` 已以 `opl_memory_locator_index_projection` 进入 read model；`artifact gallery / handoff shell` 已以 `opl_artifact_gallery_projection` 进入 read model；`package/export lifecycle shell` 已以 `opl_package_export_lifecycle_projection` 进入 read model；`route/decision graph renderer` 已以 `opl_route_decision_graph_projection` 进入 read model；`review/repair transport` 已以 `opl_review_repair_queue_projection` 进入 read model；`quality/readiness projection` 已以 `opl_quality_readiness_projection` 进入 read model；`observability/SLO repair-command projection` 已以 `opl_observability_slo_projection` 进入 read model；`action routing shell` 已以 `opl_operator_action_routing_projection` 进入 read model；`native helper execution envelope` 已以 `opl_native_helper_execution_envelope_projection` 进入 runtime snapshot；`domain projection ingestion` 已以 `opl_domain_projection_ingestion_projection` 进入 runtime snapshot。后续继续补 App productization 和 live apply receipt。验收以跨 MAS/MAG/RCA 的 refs/receipts/typed blockers 为准，不要求 OPL 接管 source readiness、memory body、domain truth、quality verdict、review verdict、SLO verdict、domain projection builder execution、domain action execution、native helper artifact mutation 或 export authority。
3. `Temporal provider SLO and continuous proof`
