...
| `node --test --experimental-strip-types tests/src/stale-compat-retirement-guard.test.ts` | exit 0；`7` tests passed、`0` failed | Guard 明确防止 operating-governance 参考重新声称退役 surface matrix JSON 是 active contract，防止 examples/domain-admission 参考把 `domain_gateway` 保留成 active route，防止 examples-corpora 重新链接已退役且当前不存在的 `examples/opl-framework/*.json` artifacts，防止 domain-admission 重新发布不存在的 `candidate-domain-backlog.json` 合同，也防止人读文档把 canonical `hermes_agent` executor adapter 写成 unsupported / retired，或把 Hermes-Agent 整体写成只剩 migration/proof/historical context；本轮还新增 convergence-governance snapshot guard，防止 2026-04 `Gateway / Federation` 快照重新被读成当前 truth。 |
| `npm run test:fast` | exit 0；`184` tests passed、`0` failed | Fast lane 覆盖 family runtime、provider-hosted attempts、production closeout、runtime snapshot/workbench、descriptor CLI、native smoke、quality details、repo hygiene、family structure advisory report alignment、stale compatibility retirement guard、examples/domain-admission/convergence-governance retirement guard 和 executor adapter guard。 |
