...
5. New Agent / OPL Meta Agent 的真实 production consumption：fresh scaffold cohort 与 OMA structural consumption 已通过；owner receipt scaleout 已可由机器 receipt 计为 observed；managed install/update gate 已能在 OMA 非 clean managed-root checkout 时 fail closed 为 manual-required route；App live path gate 已有 OPL refs-only receipt ledger 可消费真实 App/live evidence；clean managed-root receipt 与 long-soak 仍未闭合。
...
| `app_release_user_path` | App/operator drilldown 已能消费 provider cadence、domain evidence、stage evidence、cleanup plan 和 safe action route；默认 `app_release_user_path_evidence` gate 会把 release package、screenshot、reload prompt user path、provider state linkage 和 long operator evidence 五类 required refs 投给 App/live operator 或 release owner，并可通过 OPL refs-only evidence ledger 消费真实 refs 或 typed blocker refs，同时保持 false authority flags。 | 真实 GUI 截图、发布包、reload prompt 用户路径、provider state 联动和长时 operator evidence 未完全闭合；gate open 只说明 payload 缺口可见，ledger refs observed 也只说明对应 refs 已进入读面，不等于 App release/user path 已关闭。 | App release artifact、截图、reload prompt、provider state linkage 与真实用户路径可重复证明；App/OPL 只展示 refs/action/typed blocker route，不写 domain truth、owner receipt、release-ready verdict 或 production verdict。 |
