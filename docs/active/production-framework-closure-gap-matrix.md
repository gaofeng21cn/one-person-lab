...
| `new_domain_admission` | Standard agent skeleton、pack compiler、conformance/readiness 和 template consumption cohort 已有基础；scaffold consumption cohort 已通过，`opl-meta-agent` 真实仓已通过 scaffold validation、generated interface、conformance 和 readiness structural consumption；App/framework 默认读面已把 OMA production-consumption follow-through 固定成 managed install/update、App live path、owner receipt / typed blocker scaleout 和 long-soak 四类 gate，并会把已观测到的 OPL-managed `oplmetaagent` install/update receipt、OPL refs-only App live path receipt、target owner receipt / typed blocker scaleout refs 和 verified long-soak ref 计为 observed。若没有 clean managed-root receipt，且 OMA module inspection 显示 developer/sibling/dirty/ahead 等不能自动维护的状态，managed install/update gate 会投影 manual-required blocker 与 review action。当前本机 App drilldown 已观察到 managed install/update、App live path、owner receipt / typed blocker scaleout refs 和 `long_soak_ref://opl-meta-agent/production-consumption/operator-window/ab2526621c7c8159`，production-consumption open gate 为 0；`oma-production-consumption` 还提供本地 long-soak `start|event|finish` observation workorder commands，方便后续 target cohort 追加受控 event 并产出 hash-bound manifest 与 record payload。 | 当前 OMA production-consumption refs 已 projected，且 open gate 为 0；`oma-production-consumption` ledger 仍只记录/验证 refs，不启动长时运行、不生成 owner receipt、不写 OMA truth。`long-soak start|event|finish` 只处理本地 observation workorder、body-free event log、manifest 和 payload，不直接写 ledger；recorded 但未 verify 的 receipt 只进入 pending verify，typed blocker refs 只说明 owner 显式阻塞，operator evidence refs 只保留审计/监控引用，event-only operator log 不能替代 verified long-soak ref。当前 OMA follow-through closeout 不等于 MAS/MAG/RCA domain ready，也不授权 default promotion 或 global production ready。 | 新 Agent 通过 scaffold validation、pack compiler、conformance/readiness、private surface policy 和 focused tests；production-consumption follow-through 只按 owner receipt / typed blocker / App live evidence / managed install-update / verified long-soak refs 报告，并继续要求后续 target cohort 重复验证。 |
