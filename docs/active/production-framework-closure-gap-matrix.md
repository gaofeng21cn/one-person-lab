...
| `app_release_user_path` | App/operator drilldown 已能消费 provider cadence、domain evidence、stage evidence、cleanup plan 和 safe action route；默认 `app_release_user_path_evidence` gate 会把 release package、screenshot、reload prompt user path、provider state linkage 和 long operator evidence 五类 required refs 投给 App/live operator 或 release owner，并保持 false authority flags。 | 真实 GUI 截图、发布包、reload prompt 用户路径、provider state 联动和长时 operator evidence 未完全闭合；gate open 只说明 payload 缺口可见，不等于 App release/user path 已关闭。 | App release artifact、截图、reload prompt、provider state linkage 与真实用户路径可重复证明；App/OPL 只展示 refs/action/typed blocker route，不写 domain truth、owner receipt 或 production verdict。 |
