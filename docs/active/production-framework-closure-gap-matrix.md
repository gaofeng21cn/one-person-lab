...
- MAS 仍有 `functional_structure_gap_count=5` 的 follow-through；MAG/RCA repo-side generated/handler target consumption 已闭合，但外部默认 caller、App 消费和 live owner-chain evidence 仍需 scaleout；
...
- MAS repo 内 5 个 follow-through 还需要 active caller cutover、refs-only 收薄、physical retirement、App/workbench 与 lifecycle 对账；MAG/RCA 还需要按 no-active-caller proof 做 remaining legacy cleanup evidence；
...
| `generated_surface_production_consumption` | `domain-pack-compiler` 已能投影 CLI/MCP/product-entry/sidecar/status/workbench/harness handoff metadata 与 active-caller proof；MAS/MAG/RCA generated interface descriptor 均可读；MAG/RCA repo-side generated/handler target consumption 已闭合。 | MAS 仍有 generated surface active caller cutover follow-through；MAG/RCA 还缺外部默认 caller / App / live owner-chain evidence，不是 domain ready。 | MAS 关闭 5 个 functional follow-through gate；MAG/RCA 用 release/dist/default caller evidence、App evidence 和 live owner receipt chain 证明生产消费。 |
...
| `developer_mode_agent_lab_repair_route` | `opl system developer-supervisor` 已有系统级配置入口；默认配置包含 `developer_apply_safe` 模式和 `gaofeng21cn` 自动开启 login 候选；Agent Lab 已承接 framework-level eval / improvement control plane。 | App 设置开关、安装期 GitHub login 检测、repo collaborator/write permission 检测、外围 AI 巡检默认挂载、repo 修复提交路径和非 developer fork / PR 路由尚未闭合。 | Developer Mode 开启后，任务默认可触发 Agent Lab 外围巡检；有 repo 权限时走受控 worktree/branch 修复提交和 owner-visible evidence；无 repo 权限时走 fork / PR；全程不改变 managed runtime truth，不写 domain truth。 |
...
- 不能把 MAS/MAG/RCA generated surface production caller 写成已全部切换。
...
4. `Developer Mode / Agent Lab repair route`
