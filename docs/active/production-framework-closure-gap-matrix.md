...
| `owner_delta_first_default_readiness` | Purpose-first 审计后的 compact alias 已落地：`framework readiness`、`family-runtime evidence-worklist`、`runtime app-operator-drilldown` 和 `app state --profile fast` 都暴露 `compact_owner_delta_projection` / `opl_compact_owner_delta_projection`，默认只回答 current owner、required delta、accepted return shapes、next safe action、readiness false flags、count summary 和 full-detail refs。 | 需要继续把 `goal_oracle_missing`、`next_forced_delta` 等细分 steering reason 折进 compact display/source hint，并让 App repo contract/shell 直接消费该 alias；raw refs-only envelope、stage replay packet 和 private residue inventory 继续只保留 audit drilldown。 | 默认读面能直接回答当前是否有 OPL safe action、等待哪个 owner、下一步需要 deliverable delta / quality gate receipt / human gate receipt / owner receipt / no-regression ref / typed blocker，以及是否阻断 domain/App/production readiness；计数清零或 typed blocker verified 不被写成完成。 |
