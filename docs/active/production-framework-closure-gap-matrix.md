...
2026-05-19 的系统评估把下一跳从“继续加 surface”改成“先消除多重语义污染”。闭环计划必须同时处理五个高风险冗余面：provider/runtime readiness 多真相、generated surface drift/block、App/operator drilldown 默认过重、`family-runtime` CLI parser 语义堆叠、legacy vocabulary 在 active 面泄漏。本轮并行 worktree 已把这五项的 OPL-framework 侧实现和文档口径合入集成线；后续 MAS/MAG/RCA production evidence 必须建立在这些单真相 surface 之上。
...
Fresh runtime evidence（2026-05-19）补上了 App/operator 对 domain evidence 与 cleanup plan 的消费面：`opl agents evidence apply` 可记录/验证 OPL-owned refs-only external evidence receipt；`runtime app-operator-drilldown` 现在会投影 domain 声明的 external evidence request、remaining evidence gate、OPL replacement expectation、replacement coverage、已记录/已验证 receipt，以及 physical skeleton follow-through gate 生成的 legacy cleanup executable plan、apply/verify 命令和 no-domain-delete authority。同日 `opl system startup-maintenance` 也补上了 App-managed environment startup maintenance 的 OPL 机器入口：clean MAS/MAG/RCA managed modules 可受控 install/update、repo bootstrap、health check、domain skill sync、plugin cache freshness 和 reload prompt；dirty/ahead/diverged/no-upstream/env override/sibling workspace/invalid checkout fail closed 到 `manual_required`，且不写 domain truth、memory body、artifact body、quality/export verdict 或 domain daemon。
...
| --- | --- | --- | --- |
| `semantic_single_truth_closeout` | 本轮已完成五项 OPL-framework hygiene：provider readiness 共享 lifecycle-aware payload；MAG transition oracle 收成 pack compiler evidence gate；App drilldown 默认 summary-first；`family-runtime` parser 拆分为 scoped modules；主文档限定 legacy vocabulary 到 history/tombstone/provenance。 | 剩余不是框架结构缺口，而是 main 上最终 fresh verification、App release/user path evidence 与 MAS/MAG/RCA production evidence gate。 | readiness、generated surfaces、App projection、CLI grammar、legacy vocabulary 均有唯一 owner payload；派生 surface 不自行推断 truth；相关 focused tests 和 CLI smoke 可重复通过。 |
...
| `generated_surface_production_consumption` | `domain-pack-compiler` 已能投影 CLI/MCP/product-entry/sidecar/status/workbench/harness handoff metadata 与 active-caller proof；MAS/MAG/RCA generated interface descriptor 均可读；MAS/MAG/RCA repo-side generated/handler target consumption 已闭合；MAS runtime transport handoff projection 已把 generic runtime owner 归 OPL。 | MAG/RCA 还缺外部默认 caller / App / live owner-chain evidence；MAS 还缺真实 provider / App / owner receipt evidence，不是 domain ready；MAS physical delete/archive 还需 no-active-caller 与 parity gate。 | MAG/RCA 用 release/dist/default caller evidence、App evidence 和 live owner receipt chain 证明生产消费；MAS 用 paper-line provider/app evidence 证明迁移后目标边界持续成立，并在 parity 成立后清理不再需要的 active-path residue。 |
| `app_operator_drilldown` | runtime snapshot、stage attempt、substrate projection、transition evidence、lifecycle refs、safe action routing 和 production closeout readiness 已有 CLI/runtime read model；One Person Lab App root contract 已绑定 `runtime_tray_snapshot.app_operator_drilldown`，AionUI runtime 页面已接入 `AppOperatorDrilldown` 组件；`app_execution_bridge` 已把可提交 route 指向 `opl runtime action execute`，并声明 domain route 只 queue/approval、不直接执行 domain action；`domain_evidence_request_refs` 与 `domain_legacy_cleanup_plan_refs` 已把 domain request/gate/replacement coverage、external evidence receipt ledger 状态和 legacy cleanup apply/verify plan 投给 App/operator。 | 真实用户路径、截图/发布包证据、长时 operator evidence、release artifact 证据和三仓 owner-aware drilldown 消费仍需闭合。 | App/workbench 能按 owner 展示 provider、attempt、source/artifact/memory refs、blocker、repair、evidence request、verified/recorded receipt、cleanup plan 和 action routing，并且不产生 domain verdict；App 的执行动作必须经 OPL safe-action shell、provider receipt 或 lifecycle apply/reconcile surface；发布包和截图证据证明该路径可用。 |
| `app_operator_drilldown_summary_first` | App/operator drilldown 默认面已改成 summary-first；完整 refs/routes 通过显式 `--detail full` 获取。 | App 真实 GUI 默认页、截图/发布包和长时 operator evidence 仍需闭合。 | 默认 `opl runtime app-operator-drilldown` 返回 summary-first payload；完整 refs/routes 只能通过显式 full/detail 查询；App 默认页不需要拉取巨量数组。 |
...
| `app_managed_environment_startup_maintenance` | `opl system startup-maintenance` 已提供 App 可调用 action；默认只维护 MAS/MAG/RCA clean OPL-managed modules，执行 install/update、repo bootstrap、health check、domain skill sync、plugin cache freshness 投影和 restart/reload prompt；dirty/ahead/diverged/no-upstream/env override/sibling workspace/invalid checkout 进入 `manual_required`；authority boundary 明确禁止写 domain truth、memory body、artifact body、quality/export verdict 或 domain daemon。`opl modules`、`opl module update` 和 `opl skill sync` 仍可作为手动入口。 | App 真实启动自动调用、release artifact / screenshot evidence、provider state 联动、reload prompt 用户路径和长时 operator evidence 仍未闭合。 | App 启动维护默认检查 managed environment 并调用 OPL startup maintenance；clean managed checkout 可受控更新并同步投影；manual_required 状态可见且不会静默覆盖 developer checkout 或 managed runtime；发布包和截图证明该路径可用。 |
...
1. `final mainline verification`：在 main 上验证 runtime readiness 单真相、pack compiler 3/0/24/0/0 drift、App drilldown summary/full、parser tests、contract validate 和 smoke/meta。
2. `external_generated_surface_production_evidence` for MAS/MAG/RCA：repo-side generated/handler target consumption 已闭合；下一步补真实外部 default caller、App、release/dist、live owner-chain 和 direct/hosted parity 证据。
3. `MAS physical thinning closeout`：runtime handoff projection 已落地，继续完成 runner / worker lease / supervisor / workbench projection / SQLite lifecycle writer 的 no-active-caller、parity、delete/archive/tombstone。
4. `App/operator drilldown GUI evidence`
5. `App managed-environment startup maintenance GUI/release evidence`
6. `Developer Mode direct-fix / fork-PR live closeout evidence`
7. `MAS paper-line guarded apply scaleout`
8. `Temporal provider continuous proof / SLO cadence long-window evidence`
9. `transition owner receipt / no-regression evidence on OPL bridge`
10. `domain memory/lifecycle apply generalization`
11. `MAG/RCA controlled soak`
12. `RCA naming contract hygiene tail`
13. `physical skeleton layout`
14. `legacy physical retirement`
