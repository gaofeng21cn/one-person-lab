...
| --- | --- | --- |
| Phase 0: Freeze design target | `covered_active_support` | 目标架构文档已由 `docs/active/README.md` 和文档组合台账纳入 active support；历史 friction 诊断已归档，当前目标态、迁移阶段和验收门由目标架构文档承接。 |
...
统一审计标准已经折回 [OPL Foundry Agent Target Operating Architecture](./opl-foundry-agent-target-operating-architecture.md) 和 [OPL Family 当前状态与理想目标差距](./current-state-vs-ideal-gap.md)：后续不再按“是否补了更多 surface / evidence / doc”判断优化成功，而按 `meets_target`、`needs_demotion`、`needs_retirement` 三类判断。

- `meets_target`: 默认路径更短，`current_owner_delta` 更清楚，真实 progress 回到正确 domain / App / human / provider owner。
- `needs_demotion`: surface 有诊断、审计、history、support 或 production hardening 价值，但不应进入 ordinary App/CLI/operator path。
- `needs_retirement`: surface 已被 generated/hosted surface、App contract 或 domain authority function 替代，应进入 no-active-caller、owner receipt / typed blocker、no-forbidden-write、tombstone/provenance 删除门。

用这个标准读当前状态，OPL core primitives 多数已经是 `meets_target`；剩余高价值工作主要是 `needs_demotion` 的摘要降噪和 `needs_retirement` 的 retained wrapper / support entry cleanup。Domain production evidence tail 不是 Framework architecture gap，必须继续按 domain owner delta 关闭。
...
1. `current_owner_delta_cutover_lane`
   ...
   - 分类：默认路径达到 `meets_target`，旧 alias 属于 `needs_demotion`。
...
2. `summary_de_noise_lane`
   ...
   - 分类：raw worklist / blocked envelope / typed-blocker group / replay count 属于 `needs_demotion`。
...
3. `wrapper_delete_gate_lane`
   ...
   - 分类：满足 gate 的 retained surface 属于 `needs_retirement`；未满足 gate 的留下 owner receipt / typed blocker requirement。
...
4. `real_owner_delta_tail_lane`
   ...
   - 分类：真实 owner answer 属于 `meets_target`；只增长 OPL ledger count 不关闭此 lane。
...
5. `support_entry_cleanup_lane`
   ...
   - 分类：support detail 属于 `needs_demotion`；替代后的 active support wrapper 属于 `needs_retirement`。
