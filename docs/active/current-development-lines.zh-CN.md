...
| `4` | `opl_app_runtime_workbench` | OPL App / Runtime Manager | 展示 provider readiness、stage attempt、domain status、human gate、action receipt、artifact refs 和 source refs；不重写 domain truth。 |
...
4. App workbench 跟随 framework：App 展示 framework/provider + domain owner receipts，不成为第二 truth source。
...
| `opl_app_runtime_workbench` | OPL App 能从一个工作台看到 provider、stage attempt、domain progress、human gate、artifact refs、source refs 和 safe action receipts。 |
