...
| `docs/references/brand-modules/*` | `support_reference` | OPL 九个品牌模块的 north-star、模块边界、设计理念和 L1-L5 成熟度对照。 | 保持支撑参考；核心五件套持有顶层意识和当前状态，active gap 持有当前推进口径；brand module reference 不作为 runtime truth、domain truth、release/install evidence、owner receipt、typed blocker 或 production maturity 证据。 |
