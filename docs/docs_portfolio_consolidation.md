...
`one-person-lab-app/docs/` 治理 One Person Lab App 的产品文档、release、testing、user guides 和 screenshots；`one-person-lab-app/shells/aionui/docs/` 仍按上游 AionUI 依赖文档处理，不主导 App 顶层，也不纳入 OPL/MAS/MAG/RCA 的 canonical docs taxonomy。旧 `opl-aion-shell` 名称只作为历史旧名或 GitHub redirect 出现。OPL 主仓只记录 One Person Lab App/workbench 的目标、消费合同、action routing 和 runtime/domain truth 投影边界。
