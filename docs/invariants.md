...
- Codex-visible skill/plugin cache 只是当前 active source channel 的投影；它不能成为第二真相源。普通用户 active source channel 默认是 managed modules；Developer Mode 或显式 path override 命中时，MAS/MAG/RCA plugin marketplace source 必须跟随对应开发 checkout，OPL Meta Agent 继续跟随 OPL-generated plugin surface。需要刷新或重启 Codex App 才能加载新 metadata 时，App 应明确提示或提供受控刷新入口。
