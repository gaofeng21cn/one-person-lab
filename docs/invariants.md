...
- `Hermes-Agent` 只能是显式可选 hosted/runtime provider adapter；OPL 可以在显式启用时管理、pin、诊断和投影它，但不得把它写成默认 runtime、默认 session/domain readiness 依赖，或 fork/vendor 成 OPL 私有 runtime kernel。
