...
- 文档和合同的 active 叙事必须统一为 `Minimal Trust Kernel + Readiness + Derived Diagnostic Lenses + Surface Budget`。Minimal Trust Kernel 只覆盖 descriptor 形状、组合关系、allowed refs、executor binding、manifest provenance、schema/parity check、authority boundary、expected receipt、audit、replay 与 route-back 下限；Readiness 只聚合 launch/evidence gap；Derived Diagnostic Lenses 只解释 blocker、assumption、cohort、runtime budget、replay 或 failure localization，不拥有 runtime/planner/proof assistant/workflow compiler/domain verdict 角色。
