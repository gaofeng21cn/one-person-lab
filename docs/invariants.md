...
- OPL 必须坚持 AI-first, contract-light。智能体能力提升主要来自 `Codex CLI` 等 AI executor、domain stage pack、prompt、skill、knowledge、rubric 和 quality gate 的演进；合同只负责边界、安全、权限、审计、receipt、阻塞、恢复和 projection，不负责把开放式智能行为、创作策略、评审方法、路线判断或修订逻辑写死。
...
- 文档和合同必须区分 `verified_static_core` 与 `runtime_enforced_boundary`。前者只覆盖 descriptor 形状、组合关系、allowed refs、executor binding、manifest provenance、schema/parity check 和 authority boundary；后者覆盖 AI 输出、人类批准、外部系统结果、artifact mutation、memory writeback、domain verdict、owner receipt 与 long-running provider history。
...
- GraphFlow / GFL 只能作为 stage admission、requires / ensures composition、trust lane 和静态/运行时边界拆分的参考模式；不得被引入为 OPL runtime dependency、provider、executor、domain stage runner、planner、artifact authority、memory owner 或 quality verdict owner。
- 新 GraphFlow / GFL 学习点默认先落入现有 manifest / proof-bundle refs 或 `opl stages readiness` warning / recommendation；capacity budget、domain validity、runtime reliability 这类信号只能作为 folded advisory refs 进入 readiness / proof，不作为 standalone 默认 CLI/schema 目标。只有被 App/operator/queue/runtime 反复消费才升级为 projection，只有影响启动安全或 OPL 越权才升级为 hard gate。
