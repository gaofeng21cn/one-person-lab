...
- `stage_progress_log.user_stage_log` 可以把 domain typed closeout 提供的 `user_stage_log` / `paper_stage_log` / `stage_log_summary` 作为用户可读摘要透出，但 OPL 不得从 artifact body、memory body、publication verdict body 或医学语义中自行生成“问题是什么 / 做了什么”。缺少 domain 语义摘要时必须显式显示 `missing_domain_semantic_summary`，同时继续投影可观测 duration/token/cost 的 observed/missing 状态。
