...
- 任何可由 OPL 启动的 stage pack 必须先通过 admission：stage id、owner、goal、输入/输出 refs、`requires`、`ensures`、prompt refs、skill refs、tool refs、tool affordance boundary、knowledge refs、rubric / evaluation refs、allowed action refs、handoff、trust lane、authority boundary、launch profile 和 selected executor binding 都必须明确。缺失这些准入信息时只能形成 blocker 或 human gate，不能进入默认启动路径。
...
- 标准 OPL Agent 的可持久化 stage attempt 必须按 `Stage Folder + Manifest + Receipt` 物化到外部 runtime artifact root；stage 进度只能由 `physical outputs + manifest validity + receipt authority + current pointer` 推导。目录或文件存在本身不等于完成；缺 owner receipt 的文件是 orphan artifact，receipt 指向文件缺失或 hash 不匹配是 broken artifact，未被 latest/current 指针选中的旧 attempt 只算 historical evidence / provenance。
