...
- 新 GraphFlow / GFL 学习点默认先落入现有 manifest / proof-bundle refs 或 `opl stages readiness` warning / recommendation；只有被 App/operator/queue/runtime 反复消费才升级为 projection，只有影响启动安全或 OPL 越权才升级为 hard gate。
