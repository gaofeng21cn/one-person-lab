...
- Stage admission 必须拆成 `launch admission` 与 `closeout admission` 两次判断。`launch admission` 只阻断 identity、owner、scope、selected executor、authority boundary、required role artifacts、expected receipt / typed blocker shape、forbidden write 和 replay/audit 基础 lineage 这类启动安全与执行边界问题；prompt refs、skill refs、tool refs、knowledge refs、rubric / evaluation refs、optional context、reviewer notes、strategy variants 或 route hints 不完整时，默认进入 advisory warning、route-back recommendation 或 reviewer context，只有同时造成越权、不可执行、不可恢复、不可审计或无法 closeout 时才升级为 hard blocker。`closeout admission` 只判断 required role artifacts、manifest validity、owner receipt / typed blocker、current pointer、content hash、generation 和 lineage 是否足以关闭当前 stage；provider completion、read-model refreshed、stage_progress_log healthy、verified ledger、file presence 或 conformance passed 都不能关闭 stage。
- StageRun execution authorization 必须在 launch admission 后、provider execution 前单独成立：provider attempt、attempt lease、execution authorization decision、workspace/artifact scope、source fingerprint、idempotency 与 forbidden-write guard 缺任一项时，OPL 只能返回 owner=`one-person-lab` 的 runtime blocker。这个 blocker 不是 domain typed blocker，不能改变 domain truth，不能替 domain owner 签 receipt。Closeout receipt binding 必须把 receipt/blocker 绑定到当前 StageRun、stage manifest、current pointer 和 source fingerprint；provider completion、read-model refresh、file presence 或 conformance pass 不能补这个绑定缺口。
...
- 标准 OPL Agent 的可持久化 stage attempt 必须按 `Stage Folder + stage_manifest + role artifacts + owner receipt / typed blocker + minimal StageRun state` 物化到外部 runtime artifact root。StageRun Kernel 只是最小状态壳，承载 generation、attempt、lease、retry/dead-letter、event log、current owner delta 和 projection rebuild，不是第二 controller、readiness worklist、domain scheduler 或 quality verdict owner。stage 进度只能由 Stage Folder、manifest validity、role artifact currentness、receipt / blocker authority、current pointer、content hash 和 lineage 推导。目录或文件存在本身不等于完成；缺 owner receipt 的文件是 orphan artifact，receipt 指向文件缺失或 hash 不匹配是 broken artifact，未被 latest/current 指针选中的旧 attempt 只算 historical evidence / provenance。
...
- OPL 的 route 调度只能表现为 `stage graph + owner-route hydration + reconciliation + attempt ledger`：stage graph 投影 requires/ensures、scope、integrity 和 launch blockers；Route Reconciler 只把 domain owner route refs hydrate 成 typed queue task、stage attempt request、conflict envelope 或 operator projection，并对照 desired route refs 与 actual attempt / provider / receipt / human-gate / dead-letter state 做 reconciliation；attempt ledger 只记录 control metadata、checkpoint、closeout、typed blocker 与 owner receipt refs。Route Reconciler 不执行 stage 内认知策略，不生成候选，不评审或排序，不修订 artifact，不做 meta-learning，不创建 owner receipt，也不声明 ready。这个模型可以借鉴 Temporal event history、LangGraph checkpoint/conditional edges、Kubernetes spec/status reconciliation 和 Dagster graph/op boundary，但不得引入 domain repo 私有 scheduler、第二 truth source 或 OPL-held domain verdict。
...
- `stage_progress_log.user_stage_log` 可以把 domain typed closeout 提供的 `user_stage_log` / `stage_log_summary` / `human_stage_log` / `human_summary` / `paper_stage_log` 作为用户可读摘要透出，canonical 字段是 `stage_work_done` 与 `changed_stage_surfaces`；MAS 论文场景的 `paper_work_done` / `changed_paper_surfaces` 只能作为兼容 alias。OPL 不得从 artifact body、memory body、publication verdict body、fundability/visual verdict body 或领域语义中自行生成“问题是什么 / 做了什么”。缺少 domain 语义摘要时必须显式显示 `missing_domain_semantic_summary`，同时继续投影可观测 duration/token/cost 的 observed/missing 状态。
...
- MAS `domain_owner/default-executor-dispatch` 的 queue `succeeded` 只表示 OPL 已接收 MAS owner handoff 并启动 provider-backed `codex_cli` stage attempt；当前 admission 只接受 MAS 已注册的 default-executor owner：`write`、`ai_reviewer`、`write/ai_reviewer` 和 `gate_clearing_batch`。Temporal provider 不可启动时必须 fail-closed 为 `blocked`。hydrate/export 后续看到同一 dedupe key 的 payload、source fingerprint 或 owner fingerprint 变化时，不得把该 `succeeded` provider-admission task 自动 requeue；只允许刷新 `opl_domain_export_context` 与 `domain_dispatch_evidence_record_payload` 等 refs-only metadata。只有明确 blocked provider transport redrive、retry-budget provider dead-letter operator redrive、或 MAS owner 输出新的 dedupe/work-unit task，才能产生新的 provider attempt。若 MAS owner 在同一 study/action 或同一 dispatch 已有 live linked attempt 时输出新的 dedupe/work-unit task，OPL intake 必须持久化新的 queued task 并记录 deferred/single-flight evidence；single-flight 只能阻止并发 provider launch，不能把新 source fingerprint handoff 消解成纯 idempotent noop 或丢弃。retry-budget provider dead-letter operator redrive 必须要求同一 task 的 stage attempt ledger 具备 `dead_lettered` / `retry_budget_exhausted` 的 Temporal `codex_cli` evidence，并且只能清空 OPL queue retry terminal state、创建新 provider attempt 和记录审计事件；不得手写 queue DB、自动循环重试或修改 MAS truth/export/source fingerprint。若当前 linked attempt 的 Temporal query/inspect/tick 观察到 failed、timed out，或 provider workflow 以 `typed_closeout_packet_required` 等 non-completion blocker 结束，OPL 必须把 linked queue task 反向投影为 `blocked` 并保留 provider-only reason，不能继续显示 clean succeeded；tick 没有 queued work 时也必须同步 scoped running MAS default-executor terminal provider attempt。MAS default-executor single-flight 只允许真实 live linked task/attempt 阻挡新同 dispatch source row 的 provider launch；旧 attempt 即使残留 `running` / `checkpointed` / `human_gate`，只要 linked queue task 已是 `blocked`、`dead_letter` 等非 live 状态，或 provider run 已 terminal，就不能继续阻挡 MAS owner 新 handoff。若同一 task 已存在更新的 queued/running redrive attempt 或更新 accepted typed closeout，旧 terminal observation 只能更新旧 attempt ledger，不能覆盖 task-level currentness。该状态不得被任何 status、tray、workbench 或 runtime read model 表述为 owner attempt completed、MAS owner receipt observed、domain ready、publication ready、artifact ready、package refreshed 或 current manuscript repaired。只有后续 Codex stage attempt closeout、MAS owner receipt / typed blocker、AI reviewer-backed publication eval 或 domain gate receipt 才能关闭对应语义。
- MAS `paper_autonomy/*` provider-hosted task 的 queue `succeeded` 必须绑定 typed closeout packet 或 domain-owned receipt/closeout refs；domain handler 只返回 `accepted`、`paper_autonomy_receipt` 或内部确认字段时，OPL 只能投影为 transport accepted but closeout missing，并 fail-closed 成 `blocked/domain_handler_closeout_required`。历史残留的 `succeeded + checkpointed + closeout_refs=[]` 必须由 tick/read-model repair 收敛到同一 blocker。该状态不得被写成论文已修复、package 已刷新、publication gate 已过或 MAS owner receipt 已观察到。
- MAS `paper_autonomy/*` dead-letter 只能在当前 owner route 仍可执行且 source/work-unit fingerprint 明确更新时由 OPL queue 自动 redrive。若既有 dead-letter 的 domain handler error 已包含 `owner_route_stale` 或 `controller_route_work_unit_unsupported`，后续 hydrate/export 里的 owner fingerprint、source fingerprint 或 export context 变化只能记录 `task_dead_letter_redrive_blocked_by_domain_currentness`，不得把 stale / unsupported work unit 重新排队、启动 provider attempt、修改 MAS truth，或恢复 retired paper-autonomy route。
- App/operator 默认安全动作必须先处理 OPL provider worker start/restart，其次处理 blocked transport redrive，最后才暴露需要 MAS/App/live owner payload 的 handoff refs record。这个顺序只治理 OPL transport/liveness/read-model attention；不得把 worker ready、redrive submitted、provider completed 或 refs-only payload recorded 写成 MAS publication quality、artifact/package authority、owner receipt、publication gate 或 `current_package` closure。
...
- Temporal Event History / Visibility / Updates 与 OPL SQLite attempt ledger / projection 必须分层：Temporal 持有 durable workflow event history、activity retry/timeout、signal/update transport、query/visibility 索引和 replay 所需 provider facts；SQLite ledger 持有 OPL attempt identity、typed queue linkage、idempotency、source fingerprint、checkpoint/closeout refs、owner receipt / typed blocker refs、human-gate/dead-letter state 和 read-model currentness。两者可以互相引用 refs，但不得互相替代：Temporal history 不是 domain truth 或 OPL ledger，SQLite ledger 不是 provider event history，`stage_progress_log` 只是派生 projection。
- `attempt_true_path_proof` 只能证明同一 OPL stage attempt 在 `attempt query`、`queue inspect`、App drilldown、`stage_progress_log`、Temporal visibility 和 Temporal Web UI debug refs 中可追踪；它不得被解释成 long-soak、domain-ready、artifact authority、quality verdict、owner receipt 或 production-ready claim。
- Temporal workflow start / visibility metadata 必须使用 Temporal SDK 可接受的 Search Attributes 形态：每个提交给 `workflow.start` 的 Search Attribute value 都必须是数组；可选 refs 缺失时用空数组表达，不得写成 `undefined` 或 scalar。Search Attribute shape 错误属于 OPL provider-start blocker，必须 fail-closed 到 queue / stage attempt provider reason，不能吞错、手写 queue 状态或回退到 domain owner。
...
- Codex-visible skill/plugin cache 和 OPL-owned marketplace wrapper 只是当前 active source channel 的投影；它们不能成为第二真相源。普通用户 active source channel 默认是 managed modules；Developer Mode 或显式 path override 命中时，MAS/MAG/RCA 的 tracked plugin source 必须跟随对应开发 checkout，Codex config 的 marketplace `source` 必须指向 `OPL_STATE_DIR/codex-plugin-marketplaces/<marketplace-id>` wrapper root，wrapper 内 plugin symlink 才指向 active repo plugin source。OPL 不得在 MAS/MAG/RCA 开发 checkout 写入 `.agents/plugins/marketplace.json` 来刷新 Codex metadata；OPL Meta Agent 继续由 OPL-generated plugin source 加同一 wrapper 暴露。需要刷新或重启 Codex App 才能加载新 metadata 时，App 应明确提示或提供受控刷新入口。
...
2. AI-first / executor-first 执行不被静态合同写死；合同只绑定 prompt、skill、tool affordance boundary、knowledge refs、expected receipt 和 authority boundary；工具 refs 是 affordance catalog，不是 workflow script。
