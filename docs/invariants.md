...
- 可启动 stage pack 的 `runtime_assumptions` 必须能被 `monitor_refs` 或 assumption-local monitor refs 支撑；stale assumption、缺 monitor ref 或缺 owner 必须进入 typed blocker 和 minimal counterexample，不能被写成 App/operator ready。
