...
- Current product-entry truth: `OPL` now exposes a local direct product-entry shell whose default front door is `opl`; `opl <request...>` acts as quick ask, while `opl doctor / ask / chat / resume / sessions / logs / repair-hermes-gateway / frontdesk-manifest` remain the explicit command surface above the external Hermes kernel. `opl web` now lands the local web front desk pilot.
