...
- Current product-entry truth: `OPL` now exposes a local direct product-entry shell whose default front door is `opl`; `opl <request...>` acts as quick ask, while `opl doctor / ask / chat / resume / sessions / logs / repair-hermes-gateway / frontdesk-manifest / frontdesk-service-*` remain the explicit command surface above the external Hermes kernel. `opl web` now lands the local web front desk pilot.
- Current management-surface truth: `opl projects / workspace-status / runtime-status / dashboard` provide the first top-level management layer for project, workspace, session, and runtime visibility, while `opl web` gives that management surface a directly usable browser front door and now also exposes hosted-friendly `health / manifest / sessions / resume / logs` surfaces.
