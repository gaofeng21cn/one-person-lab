...
- Current product-entry truth: `OPL` now exposes a local direct product-entry shell whose default front door is `opl`; `opl <request...>` acts as quick ask, while `opl doctor / ask / chat / resume / sessions / logs / repair-hermes-gateway / frontdesk-manifest / frontdesk-hosted-bundle / session-ledger / handoff-envelope / frontdesk-service-*` remain the explicit command surface above the external Hermes kernel. `opl web` now lands the local web front desk pilot.
- Current management-surface truth: `opl projects / workspace-status / workspace-catalog / workspace-bind|activate|archive / runtime-status / session-ledger / dashboard` provide the current top-level management layer for project, workspace, session, handoff, and runtime visibility, while `opl web` gives that management surface a directly usable browser front door and now also exposes hosted-friendly `health / manifest / hosted-bundle / sessions / resume / logs / handoff-envelope` surfaces.
