...
- Current focus: family-level front desk, hosted-runtime hardening, and domain lightweight direct-entry alignment, with central sync reopened only when admitted-domain deltas or wording drift need it.
- Current truth: the family is no longer at one uniform integration depth. `Med Auto Grant` has landed a real upstream `Hermes-Agent` substrate, `Med Auto Science` has completed external runtime bring-up and is entering real adapter cutover, `RedCube AI` has converged route / managed execution onto local `Codex CLI` host-agent runtime while landing repo-verified `product frontdesk / federated product-entry / session continuity / family-orchestration companion` surfaces, and `OPL` now owns a local direct product-entry shell above the top-level gateway.
- Current product-entry truth: `OPL` now exposes a local direct product-entry shell whose default front door is `opl`; `opl <request...>` acts as quick ask, while `opl doctor / ask / chat / resume / sessions / logs / repair-hermes-gateway / frontdesk-manifest / frontdesk-readiness / frontdesk-domain-wiring / frontdesk-hosted-bundle / frontdesk-hosted-package / frontdesk-librechat-package / session-ledger / handoff-envelope / domain-manifests / paperclip-* / frontdesk-service-*` remain the explicit command surface above the external Hermes kernel. `opl frontdesk-bootstrap --path <workspace>` is now the shortest user-facing bootstrap for the local GUI stack: it installs the OPL workspace console, lands the `OPL Atlas` LibreChat shell, inherits the current local Codex model/reasoning defaults into `OPL Agent`, and mounts the built-in `OPL Cortex` MCP bridge by default. `opl web` continues to land the local web front desk pilot together with a routed `/api/start` surface, a browser-visible start panel, and hosted-friendly `/api/frontdesk-readiness` + `/api/frontdesk-domain-wiring` surfaces for family operator truth; `paperclip-*` remains an optional downstream control-plane bridge rather than a runtime dependency.
- Current management-surface truth: `opl projects / workspace-status / workspace-catalog / workspace-bind|activate|archive / domain-manifests / runtime-status / session-ledger / dashboard` provide the current top-level management layer for project, workspace, session, handoff, and runtime visibility. `workspace-catalog` stays the non-executing registry that stores project-level binding summaries, writable available-action hints, and optional domain-owned `manifest_command` locators; `domain-manifests` is the sibling execution surface that resolves those active bindings into machine-readable product-entry discovery objects, including each domain's `frontdesk_surface`, `operator_loop_surface`, `operator_loop_actions`, `product_entry_shell`, `shared_handoff`, optional `family_orchestration.action_graph`, and recommended command hints; `frontdesk-domain-wiring` now also carries domain-scoped binding parity plus the workspace-registry endpoints needed to repair locator parity from one hosted-friendly surface; `frontdesk-readiness` now pulls local service status, hosted pilot readiness, and domain `product_entry_readiness / preflight` truth into one operator-facing board; `session-ledger` now carries per-session aggregate attribution in addition to raw event entries, and `opl web` gives that management surface a directly usable browser front door while also exposing hosted-friendly `health / manifest / frontdesk-readiness / frontdesk-domain-wiring / domain-manifests / hosted-bundle / hosted-package / librechat-package / start / sessions / resume / logs / handoff-envelope` surfaces.
...
- Current family orchestration truth: the family is now freezing `event envelope`, `checkpoint lineage`, `action graph`, `human gate`, and `product-entry manifest v2` as contract-first companion surfaces; this absorbs the reusable strengths of tools such as `CrewAI` without making `CrewAI` a required runtime dependency.
...
- `references/hermes-native-executor-proof-lane.md` (Chinese-only internal reference)
- `references/family-orchestration-contract-absorb-crewai.md` (Chinese-only internal reference)
...
- `references/family-user-facing-maturity-roadmap.md` (Chinese-only internal reference)
- `references/mas-top-level-cutover-board.md` (Chinese-only internal reference)
- `references/opl-frontdesk-delivery-board.md` (Chinese-only internal reference)
