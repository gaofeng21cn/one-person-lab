...
StageRun controller 只拥有同一 StageRun 内的 Attempt 拓扑，不拥有跨 Stage 专业路由判断。primary-only StageRun 由 producer 作为终局 Codex Attempt 选择 declared next Stage 或 route-back；启用正式 Review 时，只有 terminal reviewer / re-reviewer 承担该选择，repairer 只能返回修复结果和 route recommendation。OPL 必须先校验 Attempt role/终局资格、route shape、decision/recommendation 互斥、legacy 字段缺失、finding-closure 和 declared target，再被动物化通过校验的 Codex route。校验失败只删除无权 route output、记录 route quality debt并保留可消费 artifact；这属于 ABI/authority guard，不是对专业路线的批准、否决或替换，也不恢复已退役的 Stage Transition Authority。
...
Stage / route 的顶层运行语义固定为 `declared stage graph + decisive Codex route selection + route ABI guard + attempt ledger + passive transport persistence`。`Stage` 是 OPL 可启动、可恢复、可审计的 attempt 单元；静态 pack是否完整只影响context质量。`Route` 是终局 Codex Attempt 基于domain artifact、diagnostic、owner recommendation与human input做出的advance、skip、repeat、reverse或route-back选择。Transport reconciler校验输出者是否有终局资格、字段是否规范、目标是否已声明，再把通过校验的选择物化成stage attempt request/projection并对账actual attempt/provider/currentness；它不能生成、批准、否决或自动替换ABI合法route的专业语义。attempt ledger只持久化Codex已选择的stage attempt与外部transport观察，不构成第二控制面。
