...
- `opl install` 默认只安装/修复 headless OPL 基座：Framework、Codex、family runtime provider 与 native helpers；Agent packages 仅由显式 `--modules <...>` 加入，OPL Flow 及其推荐 skills/companion tools 由 `opl packages install opl-flow` 或隐含该 package 的显式 `--with-app` 路径安装，桌面 App 也仅由 `--with-app` 安装或打开。`--headless` 是脚本和宿主可显式声明的同一默认合同；`--no-online-runtime` 只用于开发/离线 degraded diagnostics
