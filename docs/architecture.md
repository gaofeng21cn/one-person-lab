...
物理归位是模块化的起点，真正模块化还要求 public interface 清晰、模块内高聚合、模块间低耦合。模块 public interface 由该模块 `index.ts` 暴露的 export 和对应 contract ref 组成；跨模块 public entrypoint 是 owning module `index.ts` 或 `src/modules/index.ts` 聚合出口，CLI / App / Cloud entrypoint 只把请求转入这些 public entry。模块内部优先使用相对 import 连接同一 owner 下的 parts / cases / helpers；跨模块直接 import 对方内部文件、parts 或 case 文件的历史路径作为 deep import transition debt 管理，记录 owner、目标 public entry、迁移门和删除门，再逐步收敛到 public entry。
