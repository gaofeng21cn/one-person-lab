...
- `stage_progress_log` 统一投影每个 stage attempt 的 intended work、actual work、timeline、usage、Temporal visibility refs、Temporal Web UI debug ref、evidence refs 和 authority boundary。Temporal provider 承担 durable workflow history、activity heartbeat、workflow query 与 searchable visibility；OPL 从 attempt ledger / provider run / activity events / usage projection / closeout packet 派生语义 log，不另建平行 log database，不把 provider completion 解释成 domain ready。
