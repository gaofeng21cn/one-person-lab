...
- OPL-owned workspace topology 由 `contracts/opl-framework/workspace-topology-profile.schema.json` 固定为 `Workspace Group -> Project Unit -> Stage Artifact Unit -> Owner Receipt / Typed Blocker`。`workspace_modes` 只允许 `one_off`、`series`、`portfolio`：默认 `one_off` 也使用 `deliverables/<project-id>/` 的 series-capable skeleton，后续升级 series 不搬已有 project root；RCA 同主题多 deck 默认用 `rca_series` profile，项目集合路径是 `deliverables/<deck-id>/`，共享材料放 `shared/sources`、`shared/brand`、`shared/visual_memory`、`shared/style_system`、`shared/material_inventory`；MAS 多 study / 多 paper line 默认用 `mas_portfolio` profile，项目集合路径是 `studies/<study-id>/`，共享 `data`、`literature`、`memory`，不单列成新框架。Stage Native 的普通用户默认检查面是 `<project-root>/artifacts/stage_outputs/<stage-id>/` 和 domain-owned product views；runtime-state 只做 provider backing / provenance / restore / audit refs，不是普通用户默认查看面，也不能替代 stage folder、owner receipt 或 typed blocker。
