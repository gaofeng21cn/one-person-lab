...
Codex CLI 是 stage 路由的唯一语义控制面。它直接消费上一 stage 的任何可读 artifact、部分结果、阴性结果、review finding、owner/human answer 或非权威 route context，并可选择前进、跳过、重复、逆向或回到任意 declared stage。OPL 只持久化 StageRun、currentness identity、artifact refs 和 Codex-selected route context，并被动投影 current pointer、terminal metadata 与 `current_owner_delta`；Framework 不接受/拒绝 route recommendation，不要求 typed closeout、owner receipt 或 gate receipt 才允许下一 stage 启动，也不运行第二套 transition authority/event-log 裁决面。
...
Stage / route 的顶层调度语义固定为 `declared stage graph + Codex-selected route context + passive transport/currentness projection + attempt ledger`。`Stage` 是 OPL 可启动、可恢复、可审计的 attempt 单元；`Route` 是 Codex 基于当前可读输入做出的下一 stage 选择。Framework 只检查 target 是 declared stage 以及 identity/currentness/permission 边界，再创建 attempt；它不生成、评审、排名、接受、拒绝或覆盖语义路线。owner receipt、typed blocker、human gate 和 quality gate 只约束各自的 ready/export/publication/不可逆声明，不是普通 stage transition 的前置条件。
...
调度表达可以借鉴成熟系统的图、checkpoint、message、durable execution、desired/status 和 read-model 模式，但不引入第二语义 runtime：Temporal history 与 attempt ledger只负责恢复和审计，不能派生 route verdict；OpenAI/agent handoff 只运输结构化上下文；Codex CLI 始终独占 stage 选择。domain truth、quality verdict、artifact authority、memory body 和 owner receipt 继续留在 domain owner。
...
AI-first quality acceptance 需要独立 reviewer / gate attempt。执行 attempt 与审核 attempt 应分开调度、分开上下文、分开 receipt；缺少独立 gate receipt、gate evidence stale 或同 attempt 自审时，只能阻止 quality/export/publication/ready 声明并形成质量债。已有可读 artifact 时，Codex 仍可启动下一 declared stage，也可自主 route-back 重新打开更早 stage。
...
- progress receipt / workbench：Codex stage activity 已有 dry-run / live-dry-run / `codex_cli` runner repo/test harness；typed closeout 是优选信息，不是进度门。普通可读输出会持久化为 raw stage artifact，并由 Framework 派生 progress envelope、hash 与 lineage；`opl runtime snapshot` 输出只读 `stage_attempt_workbench`。
