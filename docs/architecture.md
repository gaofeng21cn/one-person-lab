...
StageRun controller 只拥有同一 StageRun 内的 Attempt 拓扑，不拥有跨 Stage 专业路由判断。primary-only StageRun 由 producer 作为终局 Codex Attempt 选择 declared next Stage 或 route-back；启用正式 Review 时，只有 terminal reviewer / re-reviewer 承担该选择，repairer 只能返回修复结果和 route recommendation。Attempt 的 closeout route output 是一次语义判断结果，不把 Stage graph、current pointer 或 transition authority下放给 Attempt。OPL 必须先校验 Attempt role/终局资格、route shape、decision/recommendation 互斥、legacy 字段缺失、finding-closure 和 declared target，再被动物化通过校验的 Codex route。校验失败只删除无权 route output、记录 route quality debt并保留可消费 artifact；fallback 只沿 action ordered required route，或在没有 action route 时沿唯一 declared successor，绝不按 manifest 文件顺序选择分叉。这属于 ABI/authority guard，不是对专业路线的批准、否决或替换，也不恢复已退役的 Stage Transition Authority。
