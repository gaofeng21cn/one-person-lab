...
`opl-framework` 是 Framework 唯一 JavaScript package surface。它直接导出 `foundry-agent-series-policy`、`reference-design-pattern-packet`、`json-schema-registry`、`reference-build-proof`、`standard-agent-pack-abi` 和 `standard-agent-action-stage-run`，同时承载 `opl` CLI、Temporal-backed provider runtime 与 E2B adapter；本仓不维护第二个静态 npm carrier。标准 Foundry Agent 不在自己的 manifest / lock 中声明或安装该 package；OPL 的 module install/update/reinstall/startup-maintenance workflow 在 checkout 内维护 `node_modules/opl-framework` 到当前 resolved OPL root 的链接，显式诊断与修复入口是 `opl connect agent-packages link-framework --agent-root <repo> [--check|--dry-run] --json`。Domain agent 必须通过公开 subpath 消费 canonical validator、digest primitive、Pack ABI 与 series policy，不得复制 OPL policy/ABI body，也不得通过 `node_modules/**/dist/**` 物理路径读取私有实现。该链接不复制 package、不安装第二份 Temporal tree，也不把 domain truth、artifact、verdict、memory 或 owner receipt authority 交给 OPL。
