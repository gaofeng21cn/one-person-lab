...
`opl-framework-static-contracts` 是 Foundry Agent 的轻量静态消费 carrier，只发布 `reference-design-pattern-packet`、`json-schema-registry`、`reference-build-proof`、`standard-agent-pack-abi` 和 `standard-agent-action-stage-run` 五个同源 subpath，运行依赖只保留 `ajv`。它直接从本仓 canonical source 编译，不复制 policy body，也不持有 CLI/runtime；`opl-framework-shared` 继续承载 `opl` CLI、Temporal-backed provider runtime、E2B adapter 和完整 Framework exports。OMA 等只消费静态合同的仓应切换 package name 并保留原 subpath 名，避免安装完整 runtime 依赖图。

`opl-framework-static-contracts/reference-design-pattern-packet` 是 OPL Workspace 对 `ReferenceDesignPatternPacket` handoff envelope 的 canonical schema validator。OMA 必须先消费该 validator，再执行 source fingerprint、semantic anchor、workflow transfer 和 target-stage planning 等 OMA-owned 语义校验；不得在 OMA 复制第二套 OPL required-fields / authority-boundary ABI。

`opl-framework-static-contracts/json-schema-registry` 是跨 OPL-compatible agent 的公开 JSON Schema compile/validate 边界。Domain agent 可以用它验证自身 action contracts，但不得通过 `node_modules/**/dist/kernel/**` 等物理路径读取 OPL 私有实现，也不得依赖未声明的传递依赖。

`opl-framework-static-contracts/reference-build-proof` 是 reference-driven build 的 canonical digest primitive。它把 AgentPackPlan 的 control/capability/stage semantic refs 解析为 target-agent local file / JSON Pointer，统一使用 raw bytes 或 `canonical_json_without_agent_build_receipt_projection` 计算 SHA-256，从而避免 receipt 写回 capability map / stage control plane 时形成自引用 hash。OMA 负责在写盘后调用该 primitive 生成 receipt；OPL conformance 使用同一 primitive 重算并 fail closed。

`opl-framework-static-contracts/standard-agent-pack-abi` 是标准 Agent Pack ABI 的 public package surface。OMA 和其他 agent builder 必须直接消费该 export，不得在 domain repo 复制 `standard_agent_pack_abi` body；domain repo 只保留本仓拥有的 stage、progress、owner 和 delivery policy。
