...
Foundry Agent profile catalog 是这条 target shape 的能力选择入口。`opl profiles list|inspect|select|conformance --json` 由 OPL Framework 持有，返回 refs-only profile catalog、natural-language intent selection receipt、stage/capability/knowledge/tool/evaluation requirements 和 structural conformance result；它不写 target domain truth、不签 owner receipt、不声明 target agent ready。OMA / OPL Meta Agent 作为 profile consumer：新建 agent 时先消费 selection / inspect readback，再把 `selected_profile_refs`、`profile_selection_receipt_ref` 和 `profile_requirements` 写入 target descriptor、capability map、stage control plane 和 generated stage pack。
