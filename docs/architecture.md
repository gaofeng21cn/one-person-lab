...
Foundry Agent profile catalog 是这条 target shape 的能力选择入口，但不是智能体设计的模板上限。`opl profiles list|inspect|select|conformance --json` 由 OPL Framework 持有，返回 refs-only profile catalog、natural-language intent selection receipt、stage/capability/knowledge/tool/evaluation requirements 和 structural conformance result；它不写 target domain truth、不签 owner receipt、不声明 target agent ready。匹配到内置 profile 时，OMA / OPL Meta Agent 把 `selected_profile_refs`、`profile_selection_receipt_ref` 和 `profile_requirements` 写入 target descriptor、capability map、stage control plane 和 generated stage pack；没有内置匹配但用户提供论文/PDF/repo/产品案例等 reference design 时，`profiles select` 必须返回 `profile_selection_mode=source_derived_design` 的 refs-only route receipt，要求 OMA 从 source refs / pattern packet refs 提炼可迁移 stage graph、grounding、tool orchestration、rubric、validation、handoff 和 failure taxonomy，再生成目标 agent pack。内置 profile 只保下限 guardrail；source-derived route 保留现学设计思路的上限。
