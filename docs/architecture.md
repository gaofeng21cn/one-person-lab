...
- `opl install` 默认安装或复用 Codex、family runtime provider、MAS/MAG/RCA domain modules、推荐 skills/companion tools、native helpers，并安装或打开桌面 App；`opl install --headless --modules <...>` 使用同一 Framework installer，但明确排除 App 下载、安装和打开。`--skip-gui-open` 只是桌面安装流程的跳过动作开关，不等同 headless 基座合同；`--no-online-runtime` 只用于开发/离线 degraded diagnostics
