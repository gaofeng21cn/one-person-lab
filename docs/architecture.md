...
- OPL-owned workspace topology 由 `contracts/opl-framework/workspace-topology-profile.schema.json` 固定为 `Workspace Group -> Project Unit -> Stage Artifact Unit -> Owner Receipt / Typed Blocker`。`workspace_mode` 只允许 `one_off`、`series`、`portfolio`：MAS 多 study / 多 paper line 按 portfolio profile 读取，不单列成新框架；RCA 同主题多 deck 按 series profile 读取。Stage Native 的普通用户默认检查面是 project-local `artifacts/stage_outputs` 和 domain-owned product views；runtime-state 只做 execution backing / provenance / restore refs，不是普通用户默认查看面，也不能替代 stage folder、owner receipt 或 typed blocker。
