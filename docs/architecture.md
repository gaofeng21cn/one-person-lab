...
StageRun 的创建入口只有 pack-bound path：普通 `opl family-runtime attempt create` 从已安装或显式 domain pack 编译 `stage_quality_cycle_policy_ref`、四类 role prompt、rubric、source/goal/lineage refs 和 manifest SHA，生成 `opl_pack_bound_stage_quality_runtime_binding` 后才可启动 `StageRunWorkflow`。`stage_run_id` 只绑定 domain、stage 与 durable invocation；manifest SHA、pack closure 和其他精确不可变输入进入 `stage_run_spec_sha256`，波动 currentness observation 不进入二者。Runway 在 Temporal start 前把 exact input 登记到 `stage_run_launches`，以支持幂等 closed replay、spec drift fail-closed 与 pre/post-start crash recovery。raw `family-runtime stage-run start` 已退役；`stage-run` CLI 只提供 query。producer / repairer 的本地 artifact 由 OPL transport 对最终 bytes 重算 SHA，并生成 content-addressed `opl_transport_artifact_identity_receipt`；外部 artifact 由可独立读取的 domain identity receipt 提供 owner 绑定。review receipt 再绑定 exact artifact refs/hashes、fresh session、rubric、finding lineage和 controller verdict。
