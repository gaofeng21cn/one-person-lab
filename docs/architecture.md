...
OPL Framework 允许使用 sandbox provider，但框架职责归 OPL：stage attempt lifecycle、typed queue、handoff、human gate、retry/dead-letter、observability、artifact/file lifecycle 与 operator projection。标准 OPL Agent 的默认长跑路径是 OPL/Temporal 托管自治执行：domain agent 不内置通用 daemon、scheduler 或 attempt loop，任务启动后由 OPL/Temporal 负责持续唤醒、resume/re-query、retry/dead-letter 和 attempt ledger；Codex App 只是启动、观察、介入和展示入口。Agent sandbox 默认由 `OPL Runway` 使用本地 Dev Containers / Docker 承接 selected stage executor 的 isolated workspace / process substrate；E2B / Daytona / Modal 是显式 remote sandbox provider option，不是默认路径，也不替换 OPL 基座。OPL 继续持有 stage、Runway、owner boundary、receipt、readback 和 false-ready guard。
...
| `OPL Runway` | Durable execution、control-loop runtime、`DomainProgressTransitionRuntime`、stage execution sandbox provider selection、typed queue、attempt、lease、retry/dead-letter、wakeup、human gate、runtime blocker、readiness / reconcile / handoff-gates / recovery-repair 读面和 desired/current Progress Reconciler。 | stage pack launch request、local Dev Containers / Docker sandbox refs、optional remote sandbox provider profile、domain owner route refs、current owner delta、worker/scheduler/provider refs、domain command/outbox/event refs。 | domain truth、owner receipt、artifact readiness、quality verdict、production readiness、L5 long-soak closure。 |
...
- Agent sandbox provider 与 Temporal 是两层：Temporal 仍是 durable workflow / wakeup / retry / human-gate substrate；Runway 默认使用本地 Dev Containers / Docker 承接 stage executor 的隔离 workspace、filesystem、process、network 和 resource substrate。E2B / Daytona / Modal 只作为显式 remote sandbox provider option；它们不替换 OPL Runway，也不归 Connect 主执行。当前 local Docker/devcontainer slice 已进入 Codex stage runner 的 selected executor process transport，E2B slice 保留为显式 remote provider；sandbox provider 的 live Docker run、remote credential run、provider long-soak 和 App release cohort 仍是后置 evidence，不能由 docs、contracts、mocked local Docker path、mocked E2B path 或 focused tests 替代。
...
- MAS executor-first paper path 下，paper-progress SSOT 保持在 MAS `PaperMissionRun`：OPL Runway 只承载 mission attempt / queue / provider lifecycle，OPL Pack / Connect 只分发 generated descriptors 和 domain task refs，OPL Console 只投影 `current_owner_delta` 与 mission refs，OPL Ledger 只保存 refs-only evidence。`paper-mission`、`paper_mission/start_or_resume`、`artifact_first_mission_summary.paper_mission_run`、legacy truth import pack 和 consume-candidate verdict 的语义解释由 MAS 持有；OPL 不把这些 projection 改写成 OPL-owned paper mission truth、publication verdict、artifact authority 或 owner receipt。
...
- family runtime provider 的 provision / version pin / profile wiring、readiness
  ...
  surface。
- agent sandbox provider 的 profile wiring、local Dev Containers / Docker default、
  optional remote credential preflight、sandbox run-context binding、provider receipt
  projection 和 selected executor process transport。local Docker/devcontainer slice
  已接入 Codex stage runner：默认通过 git workspace transport 在 sandbox 内运行
  Codex/default executor，并收集 JSONL / stderr / diff refs 到 receipt。显式选择 E2B
  时才进入 remote provider adapter。E2B / Daytona / Modal 这类 provider 只作为 selected
  stage executor 的 isolated workspace / process substrate option；OPL 不把它们写成
  workflow owner、Runway 替代品、
