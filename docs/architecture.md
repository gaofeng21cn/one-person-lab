...
`OPL Runway` 的 Temporal `StageRunWorkflow` 是非模型、durable 父级 controller，不再是 `StageAttemptWorkflow` alias。它运行时动态创建 `producer -> reviewer -> (repairer -> re_reviewer) x 0..3` child workflows；Attempt role 是 Framework 有限枚举，domain 只提供专业 prompt、rubric、必要顺序和 route-back 语义。每个 child workflow 启动新的 `codex exec` 和独立 thread/session，只通过 exact artifact/source/rubric refs、finding、repair map 与 lineage 通信。`parent_attempt_ref` 记录上一质量角色，`artifact_producer_attempt_ref` 独立记录当前 artifact bytes 的真实生产 Attempt：初审指向 producer，repairer 延续当前生产者，re-review 指向实际产出本轮新 artifact identity 的 repairer；repairer 没有产出新 artifact 时保留旧可消费产物并终止为质量债，不启动伪 re-review，也不生成伪 receipt。same-thread 自检只能记为 `in_thread_refinement`；缺 typed closeout 时的 same-thread resume 只能记为 `protocol_closeout_resume`。该 resume 强制 read-only，只补既有 Attempt 的 JSON closeout，返回包通过 Attempt identity 校验且期间没有 command / unsupported function event 后才记成功；两者都不是正式 Review、不产生 review receipt、不消耗质量修复轮次。

StageRun controller 只拥有同一 StageRun 内的 Attempt 拓扑，不拥有跨 Stage 专业路由判断。primary-only StageRun 由 producer 作为终局 Codex Attempt 选择 declared next Stage 或 route-back；启用正式 Review 时，只有 terminal reviewer / re-reviewer 承担该选择，repairer 只能返回修复结果和 route recommendation。Attempt 的 closeout route output 是一次语义判断结果，不把 Stage graph、current pointer 或 transition authority下放给 Attempt。OPL 必须先校验 Attempt role/终局资格、route shape、decision/recommendation 互斥、legacy 字段缺失、finding-closure 和 declared target，再由 controller 物化通过校验的 Codex route。校验失败只删除无权 route output、记录 route quality debt并保留可消费 artifact；fallback 只沿 action ordered required route，或在没有 action route 时沿唯一 declared successor，绝不按 manifest 文件顺序选择分叉。这属于 ABI/authority guard，不是对专业路线的批准、否决或替换，也不恢复已退役的 Stage Transition Authority。

StageRun 的创建入口只有 pack-bound path：普通 `opl family-runtime attempt create` 从已安装或显式 domain pack 编译 `stage_quality_cycle_policy_ref`、四类 role prompt、rubric、source/goal/lineage refs 和 manifest SHA，生成 `opl_pack_bound_stage_quality_runtime_binding` 后才可启动 `StageRunWorkflow`。`stage_run_id` 只绑定 domain、stage 与 durable invocation；manifest SHA、pack closure 和其他精确不可变输入进入 `stage_run_spec_sha256`，波动 currentness observation 不进入二者。Runway 在 Temporal start 前把 exact input 登记到 `stage_run_launches`，以支持幂等 closed replay、spec drift fail-closed 与 pre/post-start crash recovery。raw `family-runtime stage-run start` 已退役；`stage-run` CLI 只提供 query。producer / repairer 的本地 artifact 由 OPL transport 分块读取稳定的最终 bytes、重算 SHA，并生成文件名绑定 receipt exact bytes 的 content-addressed `opl_transport_artifact_identity_receipt`；外部 artifact 由可信 domain authority root 下的 content-addressed identity receipt 提供 owner 绑定。每个 reviewer、repairer、re-reviewer 在 child workflow 物化前重新校验当前 artifact bytes、receipt 和 `artifact_producer_attempt_ref`，防止 closeout 后篡改或错误 lineage。review receipt 再绑定 exact artifact refs/hashes、fresh session、rubric、finding lineage 和 controller verdict。Temporal workflow 行为变化使用 `patched` marker，并以旧 Event History fixture 做 replay gate，避免升级破坏既有 StageRun/StageAttempt history。
...
`OPL Pack` 的 family action catalog v2 显式区分 required、optional 与 workspace locator 字段，并把执行 ABI 收敛为 closed `handler_ref` / `stage_binding` union。`OPL Runway` 的 `opl agents run` 只从 `OPL Connect` 已解析的 exact managed package source 加载 action catalog、handler registry 与 stage manifest：普通用户链路以 `latest-stable` 选 Release Set，但以 digest lock 和 exact tree identity 作为安装/启动真相；handler 在只读 sandbox 中执行并校验 input/output schema，stage binding 则把已持久化 request ref 与 SHA 绑定到 Temporal StageRun。请求/输出 exact bytes 落到 Workspace，Ledger 只记 refs；Stage `started` 只表示已启动，不等于 domain completion、owner receipt 或 ready。
