...
StageRun controller 只拥有同一 StageRun 内的 Attempt 拓扑，不拥有跨 Stage 语义路由。primary-only StageRun 由 producer 作为终局 Codex Attempt 选择 declared next Stage 或 route-back；启用正式 Review 时，只有 terminal reviewer / re-reviewer 承担该选择，repairer 只能返回修复结果和 route recommendation。OPL transport 被动物化这个 Codex-selected route，不批准、否决或替换它，也不恢复已退役的 Stage Transition Authority。
