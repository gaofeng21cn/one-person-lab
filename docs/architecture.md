...
该 profile 的当前机器入口是 `opl foundry evidence-profile inspect --json`：读回应同时暴露 Pack ABI、Charter forbidden-claim boundary、Stagecraft / Runway policy、Ledger / Connect / Workspace substrate、Atlas catalog、Console drilldown ref 与 Foundry Lab eval / promotion gate。这个入口只证明非-live 功能 / 结构 surface 可读；Live Evidence、owner acceptance、release / production readiness 和真实 domain verdict 仍回各 owner surface。
