...
`opl-framework-shared` 是 Framework 唯一 JavaScript package surface，并通过 Git `latest-stable` channel 供 family consumer 安装。它直接导出 `foundry-agent-series-policy`、`reference-design-pattern-packet`、`json-schema-registry`、`reference-build-proof`、`standard-agent-pack-abi` 和 `standard-agent-action-stage-run`，同时承载 `opl` CLI、Temporal-backed provider runtime 与 E2B adapter；本仓不维护第二个静态 npm carrier。Domain agent 必须通过这些公开 subpath 消费 canonical validator、digest primitive、Pack ABI 与 series policy，不得复制 OPL policy/ABI body，也不得通过 `node_modules/**/dist/**` 物理路径读取私有实现。
