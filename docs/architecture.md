...
- Runway current-control/provider admission：`family-runtime intake` 当前只消费 domain emit 的 generic current-control command/outbox/event record，把其中 aggregate identity、study/work-unit identity、idempotency key、source generation、expected version、postcondition/outcome、route identity 和 attempt identity 送入 OPL queue / provider attempt。OPL 不根据 domain read-model、action_queue 形状或 recovery 文案自行推导下一步 domain action；domain state 与 transition policy 仍由 domain owner surface 决定。
