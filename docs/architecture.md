...
`opl-framework-shared/reference-design-pattern-packet` 是 OPL Workspace 对 `ReferenceDesignPatternPacket` handoff envelope 的 canonical schema validator。OMA 必须先消费该 validator，再执行 source fingerprint、semantic anchor、workflow transfer 和 target-stage planning 等 OMA-owned 语义校验；不得在 OMA 复制第二套 OPL required-fields / authority-boundary ABI。

`opl-framework-shared/json-schema-registry` 是跨 OPL-compatible agent 的公开 JSON Schema compile/validate 边界。Domain agent 可以用它验证自身 action contracts，但不得通过 `node_modules/**/dist/kernel/**` 等物理路径读取 OPL 私有实现，也不得依赖未声明的传递依赖。
