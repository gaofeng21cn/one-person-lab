...
`opl-framework` 是 Framework 唯一 JavaScript package surface。它直接导出 `foundry-agent-series-policy`、`reference-design-pattern-packet`、`json-schema-registry`、`reference-build-proof`、`standard-agent-pack-abi` 和 `standard-agent-action-stage-run`，同时承载 `opl` CLI、Temporal-backed provider runtime 与 E2B adapter；本仓不维护第二个静态 npm carrier。Framework 的 Python helper 直接位于 `python/opl_framework`，不是独立 package，也没有独立 manifest、lock 或发布 channel。标准 Foundry Agent 不在自己的 manifest / lock 中声明或安装 Framework；OPL 的 module install/update/reinstall/startup-maintenance workflow 在 checkout 内维护所需的 `node_modules/opl-framework` 和 `src/opl_framework` 链接，显式诊断与修复入口是 `opl connect agent-packages link-framework --agent-root <repo> [--check|--dry-run] --json`。Domain agent 必须通过公开 subpath 或 `opl_framework` namespace 消费 canonical helper，不得复制 OPL policy/ABI body。该链接不复制 package、不安装第二份 Temporal tree，也不把 domain truth、artifact、verdict、memory 或 owner receipt authority 交给 OPL。
