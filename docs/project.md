...
- `OPL Runtime Manager` 薄管理层：只在显式启用时把外部 `Hermes-Agent` hosted/runtime provider adapter / online-management gateway 纳入 OPL 产品级 profile、任务注册、诊断和状态投影
- 执行引擎与模块注册表
- 工作空间、会话、进度、交付物等接口面
...
其中 `OPL Runtime Manager` 位于默认运行时层与显式激活层之间。它是产品级管理/投影层，不是新的 runtime kernel：默认 session/runtime 仍归 `Codex`；`Hermes-Agent` 只在显式 online-management / hosted provider adapter 路径中持有长期在线 session、scheduler、wakeup、interrupt/resume、memory、delivery/cron 与 gateway；`OPL Runtime Manager` 只负责把这个可选外部 provider 的受支持版本、profile、domain task registration、诊断、恢复入口、可选 native helper 与高频状态索引统一投影进 `sessions / progress / artifacts / attention queue`。

## 项目目标

- 给 `opl`、`opl exec`、`opl resume`、直接 `Codex` 使用和外部壳提供稳定一致的 Codex-default session/runtime 合同
- 冻结 `OPL Runtime Manager` 的薄管理层合同，让 OPL 能在显式启用时管理外部 `Hermes-Agent` provider adapter，而不复制一套 scheduler/session/memory kernel
- 让 `opl install` 默认走 Codex + MAS/MAG/RCA domain modules + 推荐 companion tools；Hermes gateway 只作为显式启用后的 online-management 能力呈现，避免成为首启核心/domain 工作依赖
...
- `OPL Runtime Manager` 负责显式可选 provider adapter provisioning、profile wiring、task registration hydration、diagnostics、status projection、native helper catalog 与 state index catalog
- `OPL Runtime Manager` 不拥有 scheduler、session store、memory store、domain truth 或 concrete executor
- Hermes online-management gateway 是可选系统服务，由 Hermes installer/gateway command 管理；OPL 只在显式安装/修复路径中触发、检查并报告该 gateway 的 readiness
- `OPL` 的默认 runtime 只有一个：`Codex`
- `Hermes-Agent` 保留为显式可选 hosted/runtime provider adapter / online-management gateway；执行语义只在显式切换或长跑托管语境中进入
- Hermes 缺失或 gateway 未 loaded 只表示可选 online-management readiness 尚未完成；当 Codex 和已准入 domain 模块 ready 时，它不应成为首屏核心/domain blocker
- `OPL` 不持有领域运行时所有权
- `OPL` 不替代各个领域仓的智能体逻辑
