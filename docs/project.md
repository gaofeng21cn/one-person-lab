...
当前 runtime / product / policy 入口的归位方式固定为：`OPL Workspace` 提供 Project / Stage Artifact Unit 的用户检查根；`OPL Stagecraft` 提供 stage pack、Stage Attempt Runtime、Stage Transition Authority 和 route-as-transition；`OPL Runway` 提供 Temporal-backed provider、queue、lease、retry/dead-letter 和 worker lifecycle；`OPL Atlas` 提供 route graph、decision map、workspace/resource index 与 cross-agent topology telemetry；`OPL Vault` 提供 refs-only evidence、receipt、trace、replay、long-soak、cleanup 和 no-regression telemetry；`OPL Console` 提供 App/operator owner-delta-first cockpit 和 action boundary；`OPL Foundry Lab` 提供 Agent Lab / OMA improvement control plane；`OPL Connect` 提供 managed modules、skills、plugin metadata 和 domain discovery；`OPL Charter` 冻结 owner split、surface budget 和 forbidden claims。

2026-06-10 后，当前十模块共同承载 family-level Foundry Agent OS pattern：`OPL Agent OS + Domain Declarative Pack + Domain Minimal Authority Kernel + Domain Capability Registry`。其中 `OPL Pack` 负责 Domain Pack / capability registry ABI / generated surfaces，`OPL Atlas` 负责 capability registry catalog，`OPL Stagecraft` 负责 capability use policy，`OPL Console` 负责 `current_owner_delta` 默认读根，`OPL Runway` 和 `OPL Vault` 分别负责 durable execution 与 refs-only evidence。Capability Registry 不新增品牌模块，不执行 capability，不生成 owner receipt、typed blocker 或 domain verdict。
