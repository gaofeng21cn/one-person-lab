...
- 用 OPL Meta Agent 支撑新 Agent 的创建、测试接管和标准骨架检查，并推进 MAS/MAG/RCA/Book Forge 按统一 domain-agent skeleton 做 descriptor、sidecar、receipt schema、projection builder、artifact locator contract、workspace / runtime artifact root locator 和 repo-source 目录结构收敛；业务内部允许保持领域差异
