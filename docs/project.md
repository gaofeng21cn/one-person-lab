...
- `OPL Runtime Manager` 产品控制面：把 family runtime provider 纳入 OPL 产品级 profile、typed family queue、domain dispatch、任务注册、诊断和状态投影；Temporal-backed provider 是当前长期在线 / durable orchestration 的生产 substrate 候选，Hermes/local provider 可作为显式 provider、Agent executor adapter 或诊断面保留
...
- Full OPL family readiness 要求已配置的 family runtime provider ready。Temporal-backed provider 是生产目标；Hermes gateway readiness 只在显式 provider / executor adapter 或诊断语境中出现
