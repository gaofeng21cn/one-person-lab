...
`OPL Framework` 的内部能力长期按当前十个品牌模块组织：`OPL Charter`、`OPL Atlas`、`OPL Workspace`、`OPL Pack`、`OPL Stagecraft`、`OPL Runway`、`OPL Ledger`、`OPL Console`、`OPL Foundry Lab` 和 `OPL Connect`。这组模块是顶层 taxonomy、owner boundary 和成熟度管理语言，不替代 `OPL Framework -> One Person Lab App -> Foundry Agents` 三层产品认知，也不把 reference 文档写成机器真相。
...
源码组织也按这十个模块对齐：`src/modules/<module_id>/` 是新增模块代码的默认物理入口，`contracts/opl-framework/source-module-map.json` 记录现有 root-level `src/` 文件的模块归属和共享 kernel 归属。历史文件可以在保持行为不变时逐步迁入对应目录；新代码默认先选择模块目录，避免继续把能力堆回 `src/` 根层。

当前 runtime / product / policy 入口的归位方式固定为：`OPL Workspace` 提供 Project / Stage Artifact Unit 的用户检查根；`OPL Stagecraft` 提供 stage pack、Stage Attempt Runtime、Stage Transition Authority、route-as-transition 和 capability use policy；`OPL Runway` 提供 Temporal-backed provider、queue、lease、retry/dead-letter 和 worker lifecycle；`OPL Atlas` 提供 route graph、decision map、workspace/resource index、capability / tool-card catalog 与 cross-agent topology telemetry；`OPL Ledger` 提供 refs-only evidence、receipt、trace、replay、long-soak、cleanup 和 no-regression telemetry；`OPL Console` 提供 App/operator owner-delta-first cockpit、invocation-plan projection 和 action boundary；`OPL Foundry Lab` 提供 Agent Lab / OMA improvement control plane；`OPL Connect` 提供 managed modules、skills、plugin metadata、MCP/OpenAI/AI SDK descriptors 和 domain discovery；`OPL Charter` 冻结 owner split、surface budget 和 forbidden claims。
