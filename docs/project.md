...
安装、管理和更新也只使用三层用户对象：`OPL Base` 是可独立使用的 Framework runtime，`OPL App` 是可替换 GUI/部署载体，`OPL Package` 是可自由组合的安装单元。Skill、Tool、Plugin、workflow 和 entrypoint 是 Package 可发现 capability，不是平行生命周期。目标态由 Codex Plugin Manager、Git 或实际 carrier 平台负责 Package install/list/update/remove，Framework 只提供薄 adapter、动态 descriptor discovery 与 runtime projection；不自建 resolver、lock、payload、LKG、receipt 或固定 registry。该目标当前为 `planned`，现有 `opl packages` 仅作迁移期 compatibility，详见 [`OPL Package 平台组合迁移计划`](./active/opl-package-platform-composition-migration.md)。
