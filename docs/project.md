...
当前 repo 分工固定为：`one-person-lab` 是 OPL Framework / runtime / CLI / contracts owner，不持有 GUI shell implementation；`one-person-lab-app` 是 GUI product truth、release、updater、用户教程、页面状态和 active-shell validation owner；`opl-aion-shell` 是当前 AionUI shell implementation carrier，负责实现 App-owned GUI contract；MAS/MAG/RCA/BookForge 等 domain repos 是 domain app/runtime authority，持有自己的 stage 语义、prompt、skill、领域逻辑、质量 gate、truth reducer、运行规则、交付物真相、owner receipt 和 direct skill 入口。
...
3. `Foundry Agents`
   MAS/MAG/RCA/BookForge 以及后续 Patent/Award/Thesis/Review 等基于 OPL Framework 开发的领域智能体。它们以 OPL-compatible package / repo 暴露 descriptor、skill、stage、quality gate、artifact locator 和 projection，可被 App 托管运行，也保留 direct Codex/app-skill 入口。OPL Meta Agent 作为 Agent Foundry managed module 支撑这些智能体的创建、测试接管和机制改进，不持有领域真相或交付物权威。
...
- 用 OPL Meta Agent 支撑新 Agent 的创建、测试接管和标准骨架检查，并推进 MAS/MAG/RCA/BookForge 按统一 domain-agent skeleton 做 descriptor、sidecar、receipt schema、projection builder、artifact locator contract、workspace / runtime artifact root locator 和 repo-source 目录结构收敛；业务内部允许保持领域差异
...
- `Med Auto Science`、`Med Auto Grant`、`RedCube AI`、`OPL BookForge` 等仓继续是独立 `domain agent`
