...
One Person Lab App 继续放在独立的界面仓中维护，作为普通用户使用 OPL Framework 与 Foundry Agents 的工作台产品。App 仓持有 GUI runtime bridge 产品合同、active shell contract、release 包装和验证；`opl-aion-shell` 只持有当前 AionUI shell adapter 实现，未来可替换为其他 GUI shell 而不改变 OPL runtime protocol。
...
2. `One Person Lab App`
   普通用户使用的工作台产品。它消费 OPL Framework 的 runtime/activation truth，把通用工作和 Foundry Agents 呈现为可直接使用的桌面体验；它持有 GUI runtime bridge 产品合同和 active shell validation，不持有 domain truth，也不复制 runtime/provider 实现。
3. `Foundry Agents`
