...
安装、管理和更新也只使用三层用户对象：`OPL Base` 是 Framework runtime 与 companion dependency/integration，统一走 `opl update`；`OPL App` 的 bundle/carrier 更新归 App updater 与 host route；`OPL Packages` 是 Foundry Agent/capability package 的 lock、digest、projection、profile migration 与 receipt transaction，统一走 `opl packages`。`runtime_substrate`、`installation_carrier`、`capability_packages` 只作为内部 provider id，不是用户需要选择的产品层。
