...
Pack OS 是这条边界里的 landed 通用 transport surface：`contracts/opl-framework/pack-os-contract.json` 与 `src/pack-os.ts` 支持 `opl pack os inspect|lock|validate --descriptor <path> --json`，从 descriptor 派生 refs-only lock，记录 descriptor ref/hash、present local file 的 content hash、artifact locator refs、lifecycle state refs、review receipt refs、provenance 和 authority false flags。它也支持 `opl pack os mas-display-smoke --contract <path> --json` 消费 MAS Display Pack v2 contract，生成 refs-only lock 与 audit smoke。它可以承载 MAS Display Pack v2 等 domain-declared refs，也可以服务未来 MAG/RCA/deck/report/app UI capability packs；它不复制 artifact body，不改写 domain artifact，不把 review receipt transport 解释成 quality verdict，不签 domain owner receipt，也不授权 publication/grant/visual/App ready 或 production maturity。
...
| Pack OS descriptor / lock | OPL 只把 capability pack descriptor、resource refs、content hashes、artifact lifecycle refs 和 review receipt refs 投影成 refs-only lock；MAS Display Pack v2 smoke 只消费 contract refs 并返回审计对象；不存 artifact body，不生成 owner receipt，不判断 quality / publication / export readiness。 | `contracts/opl-framework/pack-os-contract.json`、`src/pack-os.ts`、`opl pack os inspect --descriptor <path> --json`、`opl pack os lock --descriptor <path> --json`、`opl pack os validate --descriptor <path> --json`、`opl pack os mas-display-smoke --contract <path> --json`。 |
