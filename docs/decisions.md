...
- 2026-05-22 追加：当 MAS sidecar export 暴露 `domain_owner/default-executor-dispatch` task 时，OPL family runtime 负责把该 task 入队、建立 `codex_cli` stage attempt，并在 Temporal provider 可启动时直接启动该 Codex writer workflow。workspace locator 保留 `dispatch_ref`、action type、dispatch authority、source refs 和 `authority_boundary=mas_default_executor_dispatch_request_only`。该 queue task 只有在 Temporal start receipt 已记录后才可标记为 `succeeded`；Temporal address / worker / stage packet 不可启动时必须 fail-closed 为 `blocked`，不能只留下 queued attempt。
- 2026-05-22 追加：`domain_owner/default-executor-dispatch` 的 queue `succeeded` 只表示 OPL 已接收 MAS writer handoff 并启动 provider-backed Codex writer attempt；不得把它解释成 Codex writer attempt 已完成、MAS owner receipt 已产生、论文质量已关闭或 package/current manuscript 已刷新。
