...
- OPL 持有 `DomainProgressTransitionRuntime`：append-only authority event log、command/outbox/event intake、fixed-point reconciler、exactly-one transition、StageRun identity、idempotency、projection metadata、replay、NonAdvancingApply 分类、human gate resume、closeout transport 和 operator projection。
...
- OPL 只能消费 domain emit 的 command/outbox/event、owner answer、typed blocker、human gate 和 provider observation；不得根据 action_queue、read-model 形状或 recovery 文案自行推导下一步 domain action，也不得签 domain owner receipt、创建 domain typed blocker、写 domain study/runtime artifacts、声明 domain ready/production ready。
...
- 2026-06-16 follow-through：current-control/provider admission 进一步收紧为 OPL generic DomainProgressTransitionRuntime command/outbox/event consumer，不新增第 11 品牌模块；Runway 负责 current-control/provider admission 与 exactly-one/NonAdvancingApply guard，Pack 负责 command/outbox ABI，Stagecraft 负责 StageRun identity/replay semantics，Console 负责 read-model metadata，Vault 负责 append-only outbox/event/replay refs。被 OPL hydrate 的 candidate 必须携带 `current_control_command` / `current_control_command_outbox_record`，其中包含 aggregate identity（至少 `aggregate_kind`、`aggregate_id`、`study_id`、`work_unit_id`）、`idempotency_key`、`source_generation`、`expected_version` 和 `postcondition` / `outcome`。缺 command record、identity、postcondition 或与 candidate 的 study/work unit 不一致时，hydrate 必须 fail closed 为 `current_control_provider_admission_command_record_missing`、`current_control_provider_admission_command_identity_missing`、`current_control_provider_admission_command_identity_mismatch` 或 `current_control_provider_admission_command_postcondition_missing`。OPL 不根据 domain read-model、action_queue 形状或业务 recovery 语义自行推导下一步 domain action；只消费 domain emit 的 command/event/outbox record，把它原样带入 queue payload 与 `provider_admission_identity`。
