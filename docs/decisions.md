...
- 2026-06-18 follow-through：完整 physical transaction 还必须证明 command、event 与 outbox item 三处携带同一 `stage_run_identity`。`readDomainProgressStageRunIdentity` 是 Runway-owned readback helper；缺 `stage_run_id`、`route_identity_key`、`attempt_idempotency_key`、`provider_attempt_ref` 或 `attempt_lease_ref`，或三处 StageRun identity 不一致时，`readDomainProgressTransitionRuntimeReadbackJsonl` 必须继续返回 `incomplete_transaction` / `blocked_incomplete_transaction`。这只关闭 OPL outbox / StageRun identity readback 证据缺口，不声明 provider-backed soak、domain owner receipt、typed blocker、human answer、publication ready 或 production ready。
- 2026-06-19 follow-through：`publishCurrentControlProviderAdmissionReadback` 也必须只接受 `opl_domain_progress_transition_runtime_live_readback` 的完整事务读回：`runtime_readback_status=complete_transaction`、`transaction_complete=true`、同一 transaction event/outbox、同一 outbox identity 和同一 StageRun identity。缺任一项时返回 `transition_runtime_live_readback_incomplete`，不得把 provider admission pending 写回 current-control。这只证明 provider-admission projection 消费了完整 OPL runtime readback，不声明 provider 已运行、domain owner answer、paper progress 或 production readiness。
- 2026-06-20 current-control intake follow-through：`family-runtime hydrate/tick` 消费 current-control provider admission 时必须向 queue payload 与 `provider_admission_identity` 写入 canonical `current_control_command_outbox_record`。历史 live readback 只暴露 `current_control_command` 时，OPL 只有在同一 candidate 同时携带完整 `domain_progress_transition_runtime`、完整 `opl_domain_progress_transition_runtime_live_readback`，且 command id / idempotency key 与 runtime readback 一致时，才可把该 command 提升为 canonical outbox record；裸 alias、缺 live readback 或 identity 不一致继续 fail closed 为 `current_control_provider_admission_command_record_missing`。同一 transition 同时来自 current-control 和 pending export 时必须按 attempt / route / idempotency identity 合并，避免重复入队。若旧 queue 行已因同 dedupe key 标记 `succeeded`，但没有 StageAttempt、没有 canonical command/outbox/readback，且新 current-control candidate 携带完整 command readback 并仍要求 provider lease，OPL 必须重新入队该 admission，而不是把旧 transport-only 成功行当作已消费终态。已有同 identity terminal StageAttempt 仍保持 consumed noop。这只关闭 current-control readback/intake 自洽缺口，不声明 provider running、domain owner answer、paper progress、domain ready 或 production readiness。
- 2026-06-20 queue consumption follow-through：`family-runtime enqueue` 必须在入队和 dedupe reconcile 前规范化 current-control command readback payload。若完整 `domain_progress_transition_runtime` 和完整 `opl_domain_progress_transition_runtime_live_readback` 暂时只出现在 `provider_admission_identity` 内，且 command id / idempotency key 与 runtime readback 同一 identity，enqueue 必须把它提升到 queue payload 顶层和 `provider_admission_identity.current_control_command_outbox_record`；否则旧 transport-only `succeeded` 行会吞掉真正可消费的 current-control admission。该提升只是 OPL queue payload canonicalization，不写 domain truth、不启动 provider、不签 owner receipt、不创建 typed blocker、不声明 paper progress 或 production readiness。
- 2026-06-20 live-readback identity proof follow-through：current-control provider admission 和 enqueue payload canonicalization 共享同一个完整 live readback 判定。若 physical JSONL live readback 的 `latest_transaction_readback` 已证明同一 transaction event/outbox，但 `same_outbox_identity` 或 `same_stage_run_identity` 顶层布尔未投影，OPL 可用 `read_model_readback.latest_transaction_identity`、`latest_outbox_identity` 和 `latest_stage_run_identity` 证明同一 outbox item、transition event 与 StageRun identity；任一 identity 缺失、错配或显式 false 仍 fail closed，不得把裸 alias、stale read-model 或半事务提升为 current-control provider admission。这仍只是 OPL readback consumption guard，不声明 provider running、domain owner answer、paper progress、domain ready 或 production readiness。
- 2026-06-20 terminal consumed readback follow-through：同一 study 与 provider-admission idempotency key 的 current-control publication 必须保持 exactly-one outcome。若同一轮 intake 同时看到 volatile provider-admission pending replay 与已有 terminal StageAttempt / accepted typed closeout，`provider_admission_terminal_consumed` 必须覆盖 pending publication，并从 current-control pending candidates 中移除该 identity。该 consumed readback 只表示 OPL provider admission 已被 terminal stage attempt 消费；不等于 provider running、domain owner answer、paper progress、domain ready 或 production readiness。
- 2026-06-20 provider-admission intake replay follow-through：当 MAS current-control export 已携带完整 same-identity OPL `complete_transaction` readback 时，Runway intake 必须优先消费该 readback，而不是把同 key MAS request-only carrier 再当作新 intent。完整 readback 可在缺冗余顶层 `same_stage_run_identity` / `same_outbox_identity` 投影时，由 `identity.stage_run_identity`、`latest_transaction_readback`、`latest_outbox_identity` 和 command/event/outbox present 共同证明；不同 intent 的 idempotency replay 仍 fail closed。由 complete readback 派生的 provider-admission command 的 runtime owner 继续是 `one-person-lab`，domain next executable owner 只作为 payload/handoff 字段传递，不能把 domain owner 写成 OPL transition owner。这关闭的是 current-control replay/intake 卡住缺口，不声明 provider running、domain owner answer、paper progress、domain ready 或 production readiness。
- 2026-06-19 projection demotion follow-through：`readDomainProgressTransitionRuntimeReadbackJsonl` 的 `projection_metadata` 只有在完整事务时才可标记 `projection_role=complete_runtime_readback` 和 `read_model_projection_consumable=true`；不完整或空 readback 必须降权为 `projection_role=fail_closed_runtime_readback`、`read_model_projection_consumable=false`，并保留 source read-model metadata 仅作诊断。下游不得绕过 `runtime_readback_status` / `transaction_complete` gate，把 stale read-model projection 当成 current provider admission、running proof 或 domain progress。
- 2026-06-19 replay-audit follow-through：`auditDomainProgressTransitionReplay` 是同一 physical JSONL log 的可消费性审计面；只有 latest transaction 同时满足 command、event、outbox item、outbox identity 和 StageRun identity 完整一致时，才可返回 `replay_status=replay_ready` 与 `read_model_projection_consumable=true`。不完整、outbox identity 错配或 StageRun identity 错配必须返回 `replay_status=fail_closed`，并把 fail-closed reason 投影到 live readback / current-control metadata。下游只能消费该 audit 作为 OPL runtime primitive readback；不能把 replay-ready 写成 provider running、domain owner answer、paper progress 或 production readiness。
- 2026-06-19 App/operator drilldown follow-through：`runtime app-operator-drilldown` 可以读取 MAS workspace 内的 `runtime/artifacts/supervision/opl_current_control_state/latest.json`，把完整 OPL `NonAdvancingApply` current-control readback 投影到 `current_control_state` summary/full detail。该投影只能显示 `transition_non_advancing_apply_recorded`、`complete_transaction`、`replay_audit_consumable=true` 与 no-authority flags；`provider_admission_allowed`、`current_executable_owner_action_allowed` 和 `paper_progress_delta` 必须保持 false，不能升级为 provider admission、owner action、paper progress、domain ready 或 production readiness。
...
- `contracts/opl-framework/target-operating-architecture-contract.json#foundry_agent_os_standard.new_agent_baseline_handoff_policy` 固定 OMA 新建 target agent 的 family-level handoff gate；`opl agents conformance` 必须投影同一 policy。OPL ScholarSkills 保持 Foundry public projection 下的 `framework_capability_package`，不是 standard domain agent；其 capability refs 只作为 domain owner consumption / typed blocker 输入，不能成为第二 runtime、第二 truth 或 production-ready evidence。
...
- 2026-06-17 follow-through：`DomainProgressTransitionRuntime` append 语义进一步收紧为 idempotent command/event/outbox transaction boundary：同一 `idempotency_key` + 同 intent 只返回 `idempotent_replay` 和既有 event/outbox/read-model readback，不追加第二组 log entry；同 key 不同 intent fail closed 为 `domain_progress_transition_idempotency_key_reused_for_different_intent`；缺完整 command/event/outbox transaction 的旧记录 fail closed 为 incomplete transaction。current-control provider admission 现在使用 workspace physical JSONL runtime append 结果（`runtime/artifacts/supervision/domain_progress_transition_runtime/command_event_log.jsonl`），而不是未落账的 result envelope，因此 queue payload 的 `domain_progress_transition_runtime`、`provider_admission_identity` 和 `domain_progress_transition_log_append` 同时携带 append 后的 `idempotency_readback`、aggregate version、read-model readback、log ref 与 persisted append status；重复 intake 同 key 同 intent 不追加第二组 command/event/outbox。该 follow-through 关闭的是 OPL runtime idempotency / durable readback / live intake 持久化语义缺口；仍不声明真实 provider-backed soak、domain owner receipt、domain typed blocker、publication ready 或 production ready。
- 2026-06-17 追加：MAS `domain-handler export` 若只给出 `pending_family_tasks[].payload.opl_domain_progress_transition_request` 且 current-control 仍显示 `transition_request_pending`，OPL Runway intake 必须把该 pending task 提升为 `transition_request_pending_task` provider-admission candidate，normalize 成 OPL command/event/outbox，向 workspace `domain_progress_transition_runtime/command_event_log.jsonl` append，并把 `domain_progress_transition_log_ref`、append readback、transition event/outbox、projection/read-model metadata 回灌到 queue payload 与 `provider_admission_identity`。同 study 普通 default-executor pending input 必须被抑制，避免 transition request 旁路成没有 OPL runtime readback 的 domain-dispatch payload；该路径仍不让 OPL 推导 domain next action、签 owner receipt、创建 typed blocker 或声明 domain progress。
- 2026-06-19 current-transition primitive follow-through：MAS 导出的 current authorized stage packet 若明确要求 OPL transition runtime / execution authorization，但缺少 `opl_domain_progress_transition_request` 或 OPL-native command record，OPL Runway intake 不得把它入队为 provider task，也不得要求 MAS selector 特判；必须在同一 workspace `domain_progress_transition_runtime/command_event_log.jsonl` 记录完整 `NonAdvancingApply` command/event/outbox transaction，读回 `runtime_readback_status=complete_transaction` 后把 current-control 投影为 `transition_non_advancing_apply_recorded`、`current_executable_owner_action=null`、provider admission / executable owner action / paper progress delta 均为 false，并压制同 study stale default-executor pending residue。该结果是平台控制面 typed blocker/readback，不是 provider running、owner receipt、domain progress、paper progress 或 live readiness。
- 2026-06-20 repo/control-plane hardening：`DomainProgressTransitionRuntime` 的 physical JSONL append 必须把同一 command/event/outbox transaction 作为单次 append chunk 写入并回传 physical append metadata；OPL-native / current-control command record 必须显式携带 `command_id`，MAS request-only transition request 仍只能由 OPL normalize 成内部 command identity；current-control candidate intake 必须合并 root `provider_admission_candidates` 与 nested `action_queue` / per-study provider candidates 并按 identity 去重；`NonAdvancingApply` current-transition readback 必须同时匹配当前 route identity、attempt idempotency 与 stage packet identity，stale authorized packet 只能 fail closed，不能写 runtime transaction。该 hardening 关闭 repo/source/control-plane 缺口，不声明 live DM002/DM003 acceptance、provider running、paper progress、domain ready 或 publication ready。
...
- 2026-06-30 追加：MAG/RCA/OMA/BookForge 的 default CLI、Skill/plugin、App/product-entry、status read model、workbench 和 conformance readback 必须把 ordinary owner route 表达为 `StageRun + current_owner_delta`。repo-local runner、private wrapper、generic owner surface 或旧 product/status shell 只能作为 migration residue / deletion gate / diagnostic，不得成为默认 owner；OPL StageRun 也不能签 domain owner receipt、创建 typed blocker、授权 quality/export verdict 或声明 runtime/domain ready。
...
- `core_engines.codex.runtime_toolchain_updater` 是机器可读的 updater/readiness surface，必须暴露 runtime root、current binary、staging root、version status、latest status、platform package materialization policy 和 `global_toolchain_mutation_allowed=false`。
...
- `settings_repair_model_access`、`settings_verify_workspace`、`settings_sync_capabilities`、`settings_apply_opl_packages`、`settings_reload_codex_surface`、`settings_check_app_update`、`settings_prune_runtime_roots_dry_run` 和 `settings_rollback_runtime_toolchain` 只通过既有 `opl app action execute` envelope 暴露；apply 类动作只能委托现有 OPL-owned system/module/skill routes，workspace/update check 为只读投影，rollback 只暴露显式 preflight/metadata，cleanup 只提供 dry-run plan，不删除 runtime roots。
...
- 当前实现只在显式 `--apply` 下写 OPL-managed state：`materialize --apply` 写 `${OPL_STATE_DIR}/runtime-environment` 下的 runtime root、lock、manifest、env、receipt 和 selected pointer；`cache prune --apply` 只删除未被 current / rollback pointer 保护且带 receipt 的 stale runtime root；`prepare --apply` 只把缺失语言包安装进 OPL-managed library path，并向显式 paper root 写 dependency lock / receipt / run-context。`run-context --paper-root <path>` 只读取这些 refs 并输出 consumer preflight；缺 run-context、target identity 不匹配或未提供 paper root 时 fail closed，消费者不得回落到宿主机包环境。它不写 development checkout、不写 domain truth / memory body / artifact body、不调度 domain stage、不签 owner receipt / typed blocker。
