...
- 2026-06-16 follow-through：current-control/provider admission 进一步收紧为 OPL generic DomainProgressTransitionRuntime command/outbox consumer，不新增第 11 品牌模块；Runway 负责 current-control/provider admission 与 exactly-one/NonAdvancingApply guard，Pack 负责 command/outbox ABI，Stagecraft 负责 StageRun identity/replay semantics，Console 负责 read-model metadata，Vault 负责 append-only command/event/outbox/replay refs。被 OPL hydrate 的 candidate 必须携带 `current_control_command_outbox_record`，其中包含 aggregate identity（至少 `aggregate_kind`、`aggregate_id`、`study_id`、`work_unit_id`）、`idempotency_key`、`source_generation`、`expected_version` 和 `postcondition` / `outcome`。缺 command record、identity、postcondition 或与 candidate 的 study/work unit 不一致时，hydrate 必须 fail closed 为 `current_control_provider_admission_command_record_missing`、`current_control_provider_admission_command_identity_missing`、`current_control_provider_admission_command_identity_mismatch` 或 `current_control_provider_admission_command_postcondition_missing`。OPL 不根据 domain read-model、action_queue 形状或业务 recovery 语义自行推导下一步 domain action；只消费 domain emit 的 `current_control_command_outbox_record`，将其 normalize 成 OPL transition command/event/outbox runtime result 后带入 queue payload 与 `provider_admission_identity`。
