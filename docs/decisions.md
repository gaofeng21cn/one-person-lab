...
- 核心五件套必须把九模块读作 OPL Framework 的长期架构语言；详细 north-star 继续留在 `docs/references/brand-modules/*`。
...
- 2026-06-07 追加：`contracts/opl-framework/workspace-index.schema.json` 是 workspace-local `workspace_index.json` 的实例级合同。`workspace_index.json` 必须同时提供领域命名和统一语义层：MAS 继续用 `studies/<study-id>`，RCA/MAG/OMA 继续用 `deliverables/<project-id>`，但 `canonical_topology` 统一映射到 `workspace_group -> project_units -> stage_artifact_unit -> owner_receipt_or_typed_blocker`，`display_labels` 保留领域显示名，`shared_resources` 给 shared roots 明确 role / manifest ref / owner / user-visible / domain-truth-owner，`projects` 给每个 project/study 明确 stage outputs root manifest ref 与 lifecycle。
- 2026-06-07 追加：`opl workspace validate --workspace <path>` 是 fail-closed gate；`opl workspace doctor --workspace <path>` 是同检查的只读诊断；`opl workspace adopt --agent ... --workspace ... --dry-run|--apply` 支持既有目录采用，apply 只写 OPL-owned metadata / manifests / map / health，不写 domain truth、不绑定 registry、不迁移 artifact body；`opl workspace upgrade --workspace ... --apply` 原地刷新 generated refs，不移动 project root；`opl workspace project archive --workspace ... --project-id ... --apply` 只更新 indexed project lifecycle，不删除文件，也不等价于 registry binding archive；`opl workspace export-map` 和 `opl workspace health` 提供只读用户检查投影。`opl workspace interfaces`、App action catalog 和 App action execute 必须暴露这些管理面，避免 workspace 合同只有叙事没有可调用接口。
