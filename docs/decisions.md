...
- 2026-05-25 追加：Temporal terminal failure / timeout 的 read-model sync 必须以 `workflow.describe()` 的 terminal `workflow_status` 为先。对于已经 `FAILED` 或 `TIMED_OUT` 的 workflow，custom `StageAttemptQuery` 可能不可用或长时间无响应；OPL 不得把这种 terminal query failure 误投影成 `temporal_service_unreachable`，也不得因此让本地 `stage_attempts` / linked MAS default-executor queue task 停留在假 `running`。该同步只关闭 OPL provider transport liveness，不生成 MAS owner receipt、不写 domain truth、不授权 publication quality、artifact gate、paper package 或 `current_package`。
