...
- 2026-05-25 追加：跨 task 的 `domain_owner/default-executor-dispatch` single-flight 只能把已经 provider-started 的 live attempt 视作占用，也就是本地状态 `running`、`checkpointed` 或 `human_gate`。同一 task 内的 `queued` attempt 仍用于防重复创建；但跨 task 的 `queued` / `registered` ledger residue 不能挡住 refreshed task 启动，否则 Temporal workflow 未创建或不存在时会把 MAS owner handoff 永久卡成假 live。该规则仍只修 OPL queue/provider liveness，不写 MAS truth、不授权 artifact gate、publication quality、paper package 或 `current_package`。
