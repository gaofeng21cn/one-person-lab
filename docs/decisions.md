...
- 2026-05-27 追加：`stage_progress_log.user_stage_log` 是面向用户询问“哪个 stage、问题是什么、论文/交付物做了什么、耗时和 token 多少”的统一读面。OPL 只提供容器、stage identity、duration/token/cost telemetry、fallback wall-clock、refs 和显式 missing/null；“问题是什么”和“做了什么”的人话语义只能来自 domain typed closeout 的 `user_stage_log` / `paper_stage_log` / `stage_log_summary`。domain 未提供时必须投影 `missing_domain_semantic_summary`，不能由 OPL 读取 artifact body、猜论文改动或把 provider completed 解释成质量完成。
