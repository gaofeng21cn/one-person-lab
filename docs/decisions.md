...
- 当前读法：`mas_default_executor_superseded_by_current_source` 是历史 currentness blocker，只能用于阻止旧 task / attempt 继续 redrive，不能反过来作为 current source 去遮蔽 fresh MAS provider admission。哪些 attempt status 可参与“当前源”裁决属于 family-runtime source/tests/read-model 事实；canceled / terminal residue 只能作为 audit 或 stale-redrive filter 的比较对象。
...
- 当前 Framework 的默认环境 profile 是 `fast_local_env`：doctor / prepare / run-context 负责确认 R/Python/MAS display 这类本机依赖是否可消费；缺 run-context、target identity mismatch 或 doctor failure 必须 fail closed，不能偷偷回落到未声明的宿主机包环境。
- Local Sandbox / Docker slice 保留为显式 local provider：选择 `local_sandbox` / `local_docker` profile 时，Runway 通过 Docker/devcontainer preflight、workspace transport、executor run 和 `sandbox_execution` receipt 证明隔离执行路径；缺本地 image / Docker preflight 或 workspace transport 时只输出 repair / preflight work order。
- E2B slice 保留为显式 remote provider：选择 `remote_sandbox` / `e2b` profile 时，Runway 会创建或连接 E2B sandbox，并保留 credential-ref / provider-receipt-ref / no-secret-log guard。该 slice 不覆盖 Daytona / Modal，不读取、打印或转发 host secret material，不在缺 E2B credential 或缺 git workspace transport 时回落到 Fast Local Env，也不声明 provider ready / runtime ready / domain ready。
