...
- 标准 Agent 的用户级叙事统一为 `OPL Agent Package`；Codex Plugin、OPL App module、Capability Pack、MCP/Web/native surface 只是 carrier / projection。统一抽象不得被实现成“把所有 carrier 的 Skill metadata 全局注册到用户 Codex”。
