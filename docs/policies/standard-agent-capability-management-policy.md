...
required dependency 只检查 presence，optional 缺失不阻断，不以 version、ABI、lock、
payload、LKG、receipt 或 materialization transaction 准入。Git history 与 retained
compatibility 字段中的 `materializer`、`activation transaction`、Package lock/LKG
和固定 ScholarSkills 数量只解释旧实现，不再定义目标 policy。迁移 owner 与删除门见
...
- 常见 `runtime_projection`：配置的 Codex carrier 投影、Codex App plugin entry、OPL App Package drilldown。
- `sync_policy`：owner descriptor 声明 primary skill entrypoint；配置的 carrier 从 repo-owned source 安装或投影，Framework 只聚合 fresh installed/callable readback。生成物和 cache 不是 source of truth。
...
- Primary skill 必须由 owner descriptor 指向 repo-owned `agent/primary_skill/SKILL.md` 或等价 source；配置的 carrier 负责安装或投影到 Codex。Codex carrier、plugin cache 和 marketplace wrapper 都只是投影，不是第二真相源。
- Domain skill declaration 可以被 generated surface 或 stage control plane 引用；只有配置的 carrier fresh readback 证明其标准 `SKILL.md` 入口存在且可发现时，才进入 Codex Skill discovery。
...
- 标准 Agent 的用户级叙事统一为 `OPL Agent Package`；Codex Plugin、OPL App module、Capability Pack、MCP/Web/native surface 只是 carrier / projection。标准 domain agent 的 Codex carrier 统一消费 owner descriptor 指向的 repo-owned primary skill，不再保留 MAS/MAG/RCA 与 OMA/OBF 两套身份路径。统一抽象不得被实现成“把所有 carrier 的 Skill metadata 全局注册到用户 Codex”。
...
Skill 的物理 source、carrier 安装面、Codex metadata 和任务上下文是四个不同层面。source 存在不代表已安装；carrier 有 bytes 不代表已注册；注册 metadata 存在也不代表正文应该进入当前任务。默认按最窄可用层级暴露：
...
| `quest_local` | 单个 quest / stage worktree 的 `.codex/skills` 子集。 | 论文 quest、mission、stage-specific specialist pack。 | 需要 owner descriptor 声明的 domain capability。 |
| `domain_profile` | domain agent 的 owner descriptor 与 Package presence dependency。 | MAS Scholar Skills 等 domain professional capability Package。 | 需要配置的 carrier 投影到 executor discovery。 |
...
- Codex-visible surface：由配置的 carrier 从 source-only Skill 或 contract pack 安装/投影；生成物不得成为 source of truth，且必须保留 `exposure_scope` / `activation_gate` 读法。
...
- Package / Full seed：进入 Package carrier、Full runtime 或 managed companion 路径时，只作为安装来源或缓存；当前状态以对应 carrier 的 fresh installed/callable readback 为准。
...
| `opl-agent-package-lifecycle-reviewer` | `OPL Pack` + `OPL Connect` | Package owner descriptor、presence dependency、carrier exposure、provenance、install/update/remove、Codex reload proof 和 owner route 审查。 | Package owner 持有 identity/capability/dependency intent；carrier 持有物理 lifecycle/readback；Pack/Connect 不建立 registry、lock 或 install ledger。 |
...
- 不把 `~/.codex/skills`、Codex plugin cache、workspace copy、generated plugin source 或 carrier bytes 当作 canonical source。
...
| MAS Scholar Skills professional skills | `professional_skill` | canonical `mas-scholar-skills` source repo | `mas` owner descriptor 只声明 required Package identity；ScholarSkills owner descriptor 与实际 installed source 动态声明 Skill/capability exports。配置的 carrier 负责 install/update 与 workspace/quest discovery，Framework 只检查 Package presence 和 MAS 所需入口 callability，不冻结 Skill 数量或 readiness floor。 | 只提供 Package owner 声明的专业 playbook 与 candidate refs；不签 MAS owner receipt、typed blocker、quality verdict、artifact authority、runtime readiness 或 publication readiness。 |
...
| ScholarSkills descriptor、schema 与 capability catalog | `contract_module` | `mas-scholar-skills` Package owner | 内容合同留在 owner Package；OPL 只通过 owner descriptor、required identity、carrier installed/callable readback 消费。 | OPL 不复制 catalog/validator，不提供 `opl scholar-skills` 私有命令，也不授权 domain authority。 |

外部 Package 中由配置的 carrier 安装且 fresh readback 可发现的 Skill，可以按 workspace / quest-local policy 进入 executor discovery；是否 required/optional、属于哪类医学方法，以及何时 route-back 都由 Package/domain owner 声明。executable runtime、数据访问、artifact mutation 和 readiness 仍需独立 owner / source / projection / authority；不得在 OPL 内创建第二套正文、Skill 清单或 profile。
