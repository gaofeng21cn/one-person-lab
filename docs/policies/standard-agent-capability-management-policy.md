...
Tool connector 负责资源访问、标准化 source refs、调用记录、限流/凭据边界、receipt candidate 和 no-authority readback。通用数据库、Crossref/OpenAlex、HPC、渲染器、存储、软件环境和外部 API 可以由 OPL Connect 承担；PubMed search 这类领域专用 client 留在 MAS/domain owner。
...
同一个功能可以同时使用三层。例如文献能力由 `medical-research-lit` 做检索策略和证据判断，`kernel.py` 做 DOI/PMID 归一化和 citation lint，MAS owner surface 负责 PubMed search client，OPL Connect 负责通用 metadata verifier、Crossref/OpenAlex transport 和 no-authority receipt。设计时先定位子职责所在层，再决定物理文件；不要因为需要一个小工具就把专业 Skill 脚本化，也不要因为有专业 Skill 就把同步、凭据、provider lifecycle 或 owner gate 下放给 Skill。
...
| source API / renderer / environment 等稳定外部资源入口 | `tool_connector` | 通用 provider 归 OPL Connect / Fabric；PubMed 等领域专用 client 归 MAS/domain source surface | connector 输出 normalized refs、invocation receipt 和 no-authority flags；MAS PubMed 由 `adapters/literature/pubmed.py` 持有。 | connector 负责资源访问和 receipt，不承接文献筛选、临床判断、source readiness verdict 或 reviewer verdict。 |
