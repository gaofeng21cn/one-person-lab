...
OPL 不把 `plugins/opl-foundation-skills/skills/**` 的全部 metadata 默认提升到 `global_user`。如果 support Skill 数量增长，应优先使用一个极薄的 router / search / inspect 入口，再按 workspace、quest、domain 或 developer profile 同步需要的子集；不得为了“方便发现”把 100 个 framework 小 Skill 变成日常 Codex 全局候选。
...
- workspace / quest local discovery：通过 `opl connect sync-skills ... --scope workspace|quest` 或等价 managed profile 复制 refs-only 子集到目标 `.codex/skills/`；不得默认复制完整 foundation pack。
