...
| `opl-external-scientific-skill-router` | `OPL Connect` | `opl-external-specialist-skill-router` 的 scientific specialization / compatibility entry，仅用于罕见科研工具、数据库、工作流或方法。 | canonical generic router 仍是 `opl-external-specialist-skill-router`；scientific entry 不创建第二套 external Skill truth。 |
