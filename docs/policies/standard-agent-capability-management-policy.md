...
| --- | --- | --- | --- |
| `opl-runway-compute-operator` | `OPL Runway` + `OPL Connect` | 环境/SSH/SLURM/E2B/generic external endpoint 路线判断、provider failure 分类、handoff/receipt briefing。 | Runway / Connect 继续持有 credential、submit/wait/harvest、endpoint registration、provider receipt 和 runtime readback；未落地 executor 的具体 provider 不得声明支持。 |
