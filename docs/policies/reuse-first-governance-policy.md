...
2. **full scan 是无豁免存量盘点，strict diff gate 是新增守门。** full scan 只输出当前 finding，不维护历史分类或 owner worklist；`reuse-first:scan:diff --strict` 只证明当前 diff 没有新增未解释的重复造轮子。
...
- 对 full-scan finding 的处理优先级是：替换 active caller、迁移到共享 owner boundary、收敛为 generated / hosted surface、记录明确 owner decision、最后才允许保留 projection boundary；执行状态回现有 active plan 或 owner surface，不写回扫描器。
...
- final report 明确 full scan 仍可见的 findings 和 forbidden claims；
- 没有把 unrelated dirty diff、active worktree 或 sibling owner lane 混入本次完成声明。

若 full scan 仍有 findings，只能说本切片完成，不能说全量风险清零。
