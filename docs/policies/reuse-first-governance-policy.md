...
State: `active_policy`
Machine boundary: 本文是人读维护政策。可执行守门继续归 `contracts/opl-framework/reuse-first-governance.json`、`contracts/opl-framework/reuse-first-historical-worklist.json`、`scripts/reuse-first-scan.mjs`、`./scripts/verify.sh reuse-first`、source、tests 和 runtime/readback surface。
...
2. **full scan 是存量盘点，strict diff gate 是新增守门。** `decisioned_finding_count` 表示历史 finding 已分类，不表示风险已修复；`reuse-first:scan:diff --strict` 只证明当前 diff 没有新增未解释的重复造轮子。
...
| 可执行扫描、diff gate、historical worklist 分类 | `contracts/opl-framework/reuse-first-*.json` 与 `scripts/reuse-first-scan.mjs` | 只写 prose checklist |
...
- 对历史 finding 的处理优先级是：替换 active caller、迁移到共享 owner boundary、收敛为 generated / hosted surface、记录明确 owner decision、最后才允许保留 projection boundary。
...
- `reuse-first:scan:diff --strict` 没有新增未解释 hard finding；
- 对应 focused tests / typecheck / source-module / docs check 按风险通过；
- final report 明确剩余 historical findings 和 forbidden claims；
- 没有把 unrelated dirty diff、active worktree 或 sibling owner lane 混入本次完成声明。

若仍有 `must_migrate`、`owner_decision_required`、`accepted_migration_worklist` 或 allowed projection findings，只能说本切片完成，不能说全量风险清零。
