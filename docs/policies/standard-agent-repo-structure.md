...
2. **OPL 生成 / 托管层**：OPL Pack / Stagecraft / Connect 根据合同生成 CLI、MCP、Skill descriptor、OpenAI / AI SDK tool descriptor、App projection、workspace projection 和 conformance readback；Codex App carrier 统一由 `agent/primary_skill/SKILL.md` 物化为 OPL-owned Codex plugin，不再按 MAS/MAG/RCA 与 OMA/OBF 分两套物理路径。
