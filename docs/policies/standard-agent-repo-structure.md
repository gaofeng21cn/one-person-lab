...
目录语义是稳定边界，不随命名偏好移动：`agent/skills/` 表示 domain pack 内的 skill declaration / generated-surface 输入；`agent/professional_skills/<skill-id>/SKILL.md` 表示 Codex-style 一等专业方法 Skill。若某个 repo 暂时采用等价目录，必须在 `contracts/capability_map.json` 中声明 `capability_kind`、canonical source 和 runtime projection，并保证不会降低 Codex discovery 或 OPL sync 能力。
