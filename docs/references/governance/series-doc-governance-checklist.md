...
本清单用于把 OPL series 6 个默认仓：`one-person-lab`、`med-autoscience`、`med-autogrant`、`redcube-ai`、`opl-meta-agent` 和 `one-person-lab-app` 放进统一文档管理口径里做巡检。
...
- `opl-meta-agent` 是 Agent Foundry / new-agent builder/tester module，按 target-agent refs、work-order、mechanism proposal、owner receipt 或 typed blocker 边界治理；它不进入 MAS/MAG/RCA 当前 active domain truth owner 列表，也不能写目标 agent 的 domain truth、artifact、memory body、quality/export verdict 或 owner receipt body。
