...
本清单用于把 `One Person Lab` 放进 `One Person Lab`、`Med Auto Science`、`Med Auto Grant`、`RedCube AI` 和 `opl-meta-agent` 这组系列项目与 OPL-compatible repo 的统一文档管理口径里做巡检。
...
- 系列项目名称与角色要与 admitted domain 当前真相同步：`Med Auto Science` 对应 `Research Ops`，`Med Auto Grant` 对应 `Grant Ops` 业务仓，`RedCube AI` 对应 visual-deliverable / `Presentation Ops`。`opl-meta-agent` 是 OPL-compatible Foundry Agent / target-agent builder，按 target-agent refs、work-order、mechanism proposal 和 typed blocker 边界治理；它不进入 MAS/MAG/RCA 当前 active domain truth owner 列表，也不能写目标 agent 的 domain truth、artifact、memory body、quality/export verdict 或 owner receipt body。
