...
| [OPL Atlas](./atlas.md) | Agent、capability、surface、owner、dependency 和 lifecycle catalog。 | OPL Framework |
| [OPL Workspace](./workspace.md) | 用户项目空间、共享素材、stage outputs、handoff 和可检查文件结构。 | OPL Framework + domain workspace owner |
| [OPL Pack](./pack.md) | Declarative Domain Pack、authority ABI、pack compiler、generated/hosted surfaces 和 standard authority functions。 | OPL Framework + Foundry Agent owners |
| [OPL Stagecraft](./stagecraft.md) | Stage 设计、认知计算、tool affordance、quality gate 和 handoff。 | OPL Framework + Foundry Agent |
...
| [OPL Console](./console.md) | App/operator 工作台、current owner、next action、阻塞、产物和 drilldown。 | One Person Lab App |
| [OPL Foundry Lab](./foundry-lab.md) | Agent 创建、测试接管、机制改进、canary、promotion 和 rollback。 | OPL Framework + OPL Meta Agent |
| [OPL Connect](./connect.md) | CLI、MCP、OpenAI/AI SDK tools、Skill/plugin、release/install 分发。 | OPL Framework + App release owner |
...
- `Atlas` 是可发现目录，不执行、不签 receipt、不拥有 domain truth。
- `Workspace` 是用户和 Agent 共同检查文件的默认落点。
- `Pack` 固定 domain pack、authority ABI、pack compiler 和 generated-surface 输入，不接管 domain handler 或 owner verdict。
- `Stagecraft` 是 stage 内认知工作设计，不承担 durable runtime。
...
- `Console` 只消费 projection，不成为第二 runtime 或第二 domain truth。
- `Foundry Lab` 只改进 agent 机制和生成 work order，不接管 domain authority。
- `Connect` 只把同一合同投给不同外部调用面，不重新解释语义。
