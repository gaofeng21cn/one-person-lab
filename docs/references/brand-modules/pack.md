...
`OPL Pack` 是 OPL 的 Domain Pack、Capability Invocation ABI 与 Authority ABI 模块。它把一个 Foundry Agent 应该声明什么、Agent executor 怎样发现和调用能力、哪些 authority function 可以被 OPL 调用、哪些 surfaces 可以由 OPL generated / hosted、以及 pack compiler 如何判断 drift 和 conformance，收成独立 bounded context。
