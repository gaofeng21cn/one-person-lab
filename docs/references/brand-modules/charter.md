...
未来如需拆出独立 Charter 子命令，可在同一 registry/command contract 下派生 `opl charter modules|authority|terms|validate|status`，但这些命令不是当前 L4 结构基线的机器验收入口。
...
- `brand_module_registry` 只证明对应模块已有统一 `brand-modules` inspection、descriptor 和 validation surface，不证明未来独立子命令已拆出。
...
- `opl brand-modules list|inspect|maturity|validate|interfaces` 与 App descriptors 从同一 registry 派生。
