...
`Vault` 当前已达到 Workspace 级 `L4_structural_baseline`。判断 SSOT 不是 registry 文案本身，而是 `contracts/opl-framework/brand-module-surfaces.json#modules.vault`、`opl vault status|inspect|interfaces|validate|doctor --json`、App action/read-model refs、focused tests 和 [完成度对照](./current-maturity-against-workspace.md) 的组合。
