...
Currentness policy: 本文不冻结 Vault L4 状态、evidence ledger counts、verified refs、fresh index 状态、restore proof、no-regression refs、App projection 或 production maturity。当前 Vault 结构状态必须从 fresh `opl vault status|validate|doctor --json`、evidence contracts、runtime ledgers、source/tests、App read-model 和 domain owner surfaces 读取。
...
Fresh readback can support reading `Vault` as Workspace 级 `L4_structural_baseline`。判断 SSOT 不是 registry 文案本身，而是 `contracts/opl-framework/brand-module-surfaces.json#modules.vault`、`opl vault status|inspect|interfaces|validate|doctor --json`、App action/read-model refs、focused tests 和 [完成度对照](./current-maturity-against-workspace.md) 的组合。

该声明只覆盖结构完成度：Vault 有独立对象模型、schema/contract、模块级 CLI family、App/operator projection、validate/doctor、测试和状态文档。它不表示 artifact/memory body authority 已迁给 OPL，不表示 domain owner 已签收，也不表示 quality verdict、artifact ready、domain ready 或 production ready。`L5 production operating maturity` 继续归 L5 evidence contract 与真实 owner evidence。
...
当前 L4 由专属 CLI family 承担，并继续把底层实现委托给 evidence-vault/state-index/stage-artifact 真实 source。
...
这些 surface 是 Vault 的 source/delegate。模块级 L4 以 `opl vault status|inspect|interfaces|validate|doctor --json` 和 `brand-module-surfaces.json#modules.vault` 为验收入口，底层 delegate 不能单独替代模块自身读面。

## App Action / Read-Model

当前 L4 给 App 和 operator 提供 Vault 自身读面：
