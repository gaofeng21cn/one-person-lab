...
| 层面 | 目标 refs |
| --- | --- |
| `schema / contract` | `contracts/opl-framework/family-product-operator-projection.json`、`contracts/opl-framework/current-owner-delta.schema.json`、`contracts/opl-framework/public-surface-index.json`、`contracts/opl-framework/agent-platform-surface-ownership-contract.json`。 |
...
| `current delegate CLI` | `opl app state --profile fast --json`、`opl app state --profile full --json`、`opl app action execute --json`、`opl runtime app-operator-drilldown --detail full --json`、`opl brand-modules inspect --module console --json`。 |
| `App action / read-model` | `app_projection:current_owner_delta`、`app_projection:evidence_next_steps`、`app_action:workspace_ensure`、`app_action:provider_scheduler_status`；后续 `opl console interfaces` 必须把这些字段和 action shape 输出给 App。 |
...
- `interfaces`：输出 App 可以消费的 read-model 字段、safe action payload schema、diagnostic drilldown descriptor 和 CLI command descriptor。
