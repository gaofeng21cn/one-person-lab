...
| `opl runway recovery-repair --json` | worker/provider/scheduler/lease/dead-letter 的 OPL repair plan 和 refs-only blocker。 | 不把 repair 成功写成 domain progress、production long-soak complete 或 App release ready。 |
...
| `opl runway recovery-repair --json` | 返回 worker/provider/scheduler/lease/dead-letter repair plan；repair command 只作用于 OPL runtime/provider。 |
...
- `opl runway readiness|reconcile|handoff-gates|recovery-repair|control-loop status --json` 的 direct command binding、live control-loop projection、next safe action 和 false-authority flags。
