...
- Runtime environment / sandbox execution：Runway 拥有 Runtime Environment Substrate、Fast Local Env doctor / prepare / run-context、selected stage executor 的 sandbox provider selection、create-or-restore、executor run、timeout / retry / blocker 和 receipt projection。默认环境路径是 Fast Local Env；Local Docker / Devcontainer 是显式 local sandbox provider；E2B 是当前已实现的 remote provider option，其他 provider 只能通过 generic external adapter shape 接入后再声明支持。OPL Connect 只在需要时提供 provider discovery、configuration、package/install 或 connector 分发辅助，不拥有 stage execution sandbox。
