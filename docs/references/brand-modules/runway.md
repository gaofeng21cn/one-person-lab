...
- Module-owned surface：`contracts/opl-framework/brand-module-surfaces.json#modules.runway` 与 `opl runway status|inspect|interfaces|validate|doctor|readiness|reconcile|handoff-gates|recovery-repair|control-loop status --json` 已证明 Runway 当前达到 Workspace 级 `L4_structural_baseline`；`opl brand-modules ...` 只作为聚合目录。
