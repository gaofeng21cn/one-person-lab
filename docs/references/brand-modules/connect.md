...
- `dependent_guard` 直接来自 installed lock graph 的 required dependents；有 dependent 时 disable/uninstall 都 fail closed。repair 与 activation 分别复用已存在的 `agent_package_repair` 和 `agent_package_activate` App action、payload、permission、transaction 与 receipt 语义，不增加同义 action alias。
- fast/full 都提供上述 canonical 字段；fast 将未完成 live verification 的正向 readiness 降为 `verification_deferred`，full 只增加 owner detail。App 与 Shell 只能显示 status projection，并通过 directory `available_actions[].action_ref` 执行 mutation，不从 status `command_ref`、raw package status、静态 package list、历史 workspace binding 或 carrier path 推断 truth。
- 单个 package status read 不可用时，projector 仍保留 installed lock 已证明的 installed/exposure truth，但 dependency readiness 与 repair/activation action 全部 fail closed 为 `status_unavailable`；`disabled` package 不得投影可执行 activation。
