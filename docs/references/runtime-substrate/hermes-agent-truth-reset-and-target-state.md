...
本文保留为 Hermes 相关历史和迁移边界说明，不冻结当前 executor、provider、repo 状态或 proof 结果。当前机器真相按下列入口读取：

- `contracts/opl-framework/family-executor-adapter-defaults.json`
- `contracts/opl-framework/runtime-manager-contract.json`
- `src/agent-executor.ts`
- stage attempt launch gate / executor binding tests
- `opl executor doctor --executor codex_cli --json`
- `opl executor doctor --executor hermes_agent --json`
...
当前长期口径是：OPL family runtime 的 production online 路径收敛为 provider-backed runtime，Temporal-backed provider 是必需 substrate。旧 Hermes provider / Gateway / proof-provider / readiness / compatibility surface 不再承担 provider、默认 executor、Codex CLI 备线、可选安装模块或目标 session/wakeup substrate 角色。`hermes_agent` 是 canonical 显式非默认 executor adapter/backend，只承诺接口连接、生命周期、回执、审计和 fail-closed；它不承诺行为、质量、工具语义或 resume 与 `Codex CLI` 等价。缺少 binary、binding ref、receipt 或 full-loop proof 时必须 fail-closed，不能静默回落到 Codex。旧 Hermes provider / Gateway / proof-provider / readiness / compatibility surface 只保留为历史 provenance、诊断语料或负向 guard。最新 provider 支撑边界见 [Temporal Family Runtime Provider 支撑参考](./temporal-family-runtime-provider-plan.md)。
...
本节保留的是 Hermes-first 重置时的历史判断。当前 `OPL` 已经收敛为 stage-led、provider-backed，并以 Agent executor 为最小执行单位的 family agent framework owner；具体当前状态必须回到核心五件套、contracts/source/tests 和 live CLI/read-model 读取。`gateway / federation` 只作为 history/provenance/diagnostic vocabulary 或负向 guard 阅读。`OPL` 不能声称自己已经拥有上游 `Hermes-Agent` runtime，也不能把 Hermes 写成默认 session/wakeup substrate、provider、proof-provider、readiness path 或兼容接口。`hermes_agent` executor adapter/backend 是单独的显式非默认接口，不能被解读成 provider 或默认路径。

### 2.2 RedCube AI

- 历史上 RedCube AI 已有可运行、可验证的视觉交付主线。
- 当时所谓 `Hermes` 运行层，本质上仍是仓内本地实现与本地控制面。
- 这类实现可以被称为 `repo-local managed runtime pilot`，不能被称为“已接入上游 Hermes-Agent”。

### 2.3 Med Auto Science

`Med Auto Science` 当前是独立医学科研 domain agent；具体当前 truth 归 MAS 仓自己的 active plan、contracts/source/tests 和 CLI/read-model。外部 `MDS / DeepScientist` 已降为 MAS 显式声明的 archive / provenance / backend audit / explicit archive import / upstream intake / diagnostic / parity oracle reference。历史上对 `controlled MedDeepScientist backend`、repo-side Hermes seam / shim 和 outer-loop contract 的描述只保留为迁移来源；它不定义 MAS 默认 runtime owner，也不等于“已接入上游 Hermes-Agent”。

### 2.4 Med Auto Grant

- 历史上 Med Auto Grant 已有可运行的本地 `CLI` grant runtime 基线。
- 当时仓内所谓 `Hermes` 主线，本质仍是 repo-local runtime helper / migration scaffold。
...
本节同样保留为历史背景。当前 provider 支撑边界以 [Temporal Family Runtime Provider 支撑参考](./temporal-family-runtime-provider-plan.md) 为准；该参考只保存 stable owner split、语义映射、动态读取入口和 negative boundary。旧 Hermes-first 默认口径清理规则仍按核心五件套、runtime naming boundary 和 live machine surfaces 读取。
