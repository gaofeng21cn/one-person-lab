...
本文只定义 `hermes_agent` 作为显式非默认 executor adapter/backend 时的评估规则，不冻结某天的本机安装状态、binary 可用性、proof 结果、模型配置、reasoning effort 或 family conformance 计数。当前机器真相按 `contracts/opl-framework/family-executor-adapter-defaults.json`、`src/agent-executor.ts`、`src/agent-lab-stage-executor-policy.ts`、stage attempt launch gate / executor binding tests、agent-executor tests、`opl executor doctor --executor <kind> --json`、`opl agents conformance --family-defaults --json` 和 `opl framework readiness --family-defaults --json` 读取。
