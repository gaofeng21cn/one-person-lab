...
Currentness policy: 本文不冻结 executor binary readiness、本机模型配置、reasoning effort、评估结果或 closeout 结论。当前 executor 集合、默认/非默认边界和 full-loop receipt gate 读取 `contracts/opl-framework/family-executor-adapter-defaults.json`、`src/agent-executor.ts`、`src/agent-lab-stage-executor-policy.ts`、`opl executor doctor --executor <kind> --json`、agent-executor tests 和 live receipt/audit evidence。

当前口径：`hermes_agent` 已是 OPL family canonical executor backend 之一，但只作为显式非默认 adapter/backend 使用。它和 `claude_code`、`antigravity_cli` 一样只承诺接口连接、生命周期、receipt、audit 与 fail-closed；不承诺行为、质量、工具语义或 resume 与 `Codex CLI` 等价。本文的评估规则用于守住这个非等价边界；其他非默认 adapter 也受同一 receipt/audit 非等价边界约束。旧 Hermes provider / Gateway / readiness / compatibility surface 已退出 active/default path，不属于本文要保留的执行器接口。
...
- family 默认执行模式由 contract/source/doctor read model 读取；本文不冻结本机模型、reasoning effort 或 thinking 配置
...
2. 先选与 `Codex CLI` 默认 executor path 同类的任务目标
