...
当前状态说明（2026-05-14）：本文只保留当前有效的 executor 默认口径。`Codex CLI` 是默认且第一公民 executor；`hermes_agent` 与 `claude_code` 都是显式非默认 canonical backend/interface。非默认 backend 只承诺连接、生命周期、回执、审计和 fail-closed 边界，不承诺行为、质量、工具语义或 resume 与 `Codex CLI` 等价。旧 Hermes provider / Gateway / frontdoor / proof-provider / readiness / compatibility surface、`frontdoor`、`product frontdoor`、`federated product entry` 和 MDS 默认执行语境只按历史/tombstone 背景阅读，不保留兼容接口。
...
- 家族一等 executor backend 冻结为 `codex_cli`、`hermes_agent` 与 `claude_code`
