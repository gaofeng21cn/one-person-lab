...
当前状态说明（2026-05-20）：本文只保留当前有效的 executor 默认口径。`Codex CLI` 是默认且第一公民 executor；`hermes_agent`、`claude_code` 与 `antigravity_cli` 都是显式非默认 canonical backend/interface。非默认 backend 只承诺连接、生命周期、回执、审计和 fail-closed 边界，不承诺行为、质量、工具语义或 resume 与 `Codex CLI` 等价。`antigravity_cli` 只作为 experimental / non-default explicit adapter，用于类似 `RCA` HTML route 选择 `Gemini flash/high` 的 stage-level selection 示例；它不成为默认执行器，也不声明与 `Codex CLI` 的质量、工具或 resume 等价。旧 Hermes provider / Gateway / frontdoor / proof-provider / readiness / compatibility surface、`frontdoor`、`product frontdoor`、`federated product entry` 和 MDS 默认执行语境只按历史/tombstone 背景阅读，不保留兼容接口。
...
- 家族一等 executor backend 冻结为 `codex_cli`、`hermes_agent`、`claude_code` 与 `antigravity_cli`
...
Stage pack 可以显式声明 stage-level executor/model/reasoning policy，但默认路线仍继承 `Codex CLI` 第一公民语义。当前机器合同把这层字段冻结为：
...
可以保留 `hermes_agent`、`claude_code` 这类 canonical 非默认 backend 的配置示例，但示例必须标记为 `example_only_not_default_active`。默认激活 backend 仍是 `codex_cli`；非默认 backend 只有在 request explicit executor 或 `OPL Runtime Manager` / handoff default executor 显式配置时才生效。
...
- 任何旧 `Hermes` / `Gateway` / `frontdoor` 名称不得重新成为安装、更新、readiness、provider、default executor 或兼容接口
...
- `RedCube AI`
  ...
  - 当前 repo-tracked truth 以 visual-deliverable domain agent、product-entry/session continuity 和 OPL-consumable descriptor/projection 为准；历史 `product frontdoor / federated product entry` 只按 history/provenance 背景阅读
  - 历史 Hermes `/v1/runs` relay 不再作为 active bridge
- `Med Auto Grant`
  - 历史 Hermes provider proof 记录只作为 history/provenance/negative-guard 参考
