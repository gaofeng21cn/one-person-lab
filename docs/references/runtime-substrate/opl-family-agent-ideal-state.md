...
理想状态下，`OPL Framework` 是完整生产级智能体开发与运行框架。它负责开发接入、domain admission、stage-led runtime、provider-backed durable workflow、typed queue、attempt ledger、human gate、retry/dead-letter、memory/artifact/lifecycle locator、operator projection、App/workbench 投影、质量证据承载和跨 domain 审计。
...
它不是自带一套运行平台。Domain repo 默认声明 stage 内需要做什么、使用哪些 knowledge / prompt / skill refs、如何判断质量、谁能写 truth、哪些 artifact 可变更、以及完成或阻塞时返回什么 receipt。只有领域裁决无法可靠声明化时，domain repo 才保留最小 authority function，例如 quality verdict、artifact mutation authorization、memory accept/reject、source readiness verdict、owner receipt signer 或 domain-specific native helper implementation。
...
OPL 的运行逻辑以专家阶段 `stage` 为中心。每个 stage 应接近真实专家完成复杂工作的一个阶段，而不是单个工具调用。
...
- `prompt_refs`、`skill_refs`、`knowledge_refs`、`tool_refs`：阶段所需知识与工具入口。
- `quality_gates`：domain-owned review / quality / export / publication gate。
...
AI-first quality gate 的理想实现是“双 attempt”：执行 attempt 产出 artifact/source/evidence refs；审核 attempt 在独立 invocation、独立 context 和独立 task record 中读取这些 refs，产出 review/audit receipt。不能让同一个 agent 在同一上下文中先执行再自审并关闭质量门。
