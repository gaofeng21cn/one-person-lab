...
2026-05-17 fresh 总控读模型显示，这条目标态已经落到 `functional_privatization_audit` 的机器口径：三仓合计仍有 `53` 个非知识功能审计项，但 `opl_owned_replacement=0`、`retire_tombstone=0`、`temporary_migration_bridge=0`、`active_private_generic_residue=0`、`blocker=0`。剩余项只允许落入 OPL hosted/generated surface、declarative pack、minimal authority function、refs-only adapter、diagnostic cleanup path 或 provenance/fixture；这表示“清空”不等于审计项数量归零，而是清掉 domain repo 对 generic runtime/platform 的长期 owner claim。
...
- 提供统一 `domain-agent skeleton` 与 `Agent Pack Compiler`，让新 Agent 按稳定目录、声明式 spec 和 contract 暴露能力；action catalog、CLI/MCP/product-entry/sidecar/status/projection/harness 默认从 pack metadata 派生，而不是由每个 repo 手写。
