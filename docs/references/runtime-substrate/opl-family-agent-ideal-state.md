...
- `provider SLO executor`：把当前 Temporal production proof / `operator_slo_repair_loop` 从 read-model 推进到 supervised cadence tick、execution receipt、overdue repair receipt、restart/re-query/signal/history 长时证据。当前 `family-runtime provider-slo tick --provider temporal` 已能在 due 时执行 production proof、fresh 时写 skipped cadence receipt、`--force` 时强制重跑；下一步仍需长时窗口中的周期性证据。该能力只证明 provider residency，不证明 domain ready。
