...
- `最短路径`：先冻结 standard domain-agent skeleton 与 OPL generic primitives；当前 P0 已落地 OPL `family_scheduler_replacement`，本轮补齐 OPL `functional-agent-runtime-harness`、`functional_privatization_audit` 和 `family-runtime-lifecycle-index` refs-only SQLite sidecar index，后续迁移/退役 domain repo 中的通用功能残留，随后做 focused parity / no-forbidden-write / receipt 对账，最后跑 integration、provider SLO 和 live soak。
