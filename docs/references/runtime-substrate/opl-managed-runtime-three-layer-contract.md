...
Current status note (2026-05-14): this document keeps the three-layer owner split as useful content, but the old `runtime_owner = upstream_hermes_agent` mapping is obsolete. The current OPL production online path is provider-backed runtime; the Temporal-backed provider is the required substrate. `hermes_agent` is retained only as an explicit non-default executor adapter/backend; Hermes provider / Gateway / readiness / compatibility surfaces remain historical provenance, diagnostic vocabulary, or negative guards. The current implementation order lives in [OPL stage-led agent framework roadmap](./opl-stage-led-agent-framework-roadmap.zh-CN.md) and [Temporal Family Runtime Provider plan](./temporal-family-runtime-provider-plan.zh-CN.md).
