...
本文不冻结当前 adoption 顺序、repo 状态、readiness 计数或 production evidence。OPL 当前职责是 shared contracts、descriptor discovery、stage pack admission、typed queue、stage attempt、refs-only receipt / projection、safe action route 和 domain authority boundary；它不接管 CrewAI runtime、domain truth、memory body、artifact authority、owner receipt 或 quality/export verdict。
...
| `family-stage-control-plane`、`family-stage-admission` | OPL Stage Kernel | 只做 stage descriptor、owner boundary、allowed refs、selected executor binding、expected receipt、audit、handoff 和 route-back 下限；不执行 stage。 |
