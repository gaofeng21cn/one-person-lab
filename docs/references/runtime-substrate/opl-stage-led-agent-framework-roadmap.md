...
| Domain descriptor / adapter | `standard_conformance_passed_with_tail` | Standard skeleton discovery / validation 已落地；当前读法只从 `opl agents conformance --family-defaults --json`、`opl agents default-callers --family-defaults --json` 和 domain-owned contracts 读取。Fresh conformance/default-caller counters 只记录在 live read-model 或过程 ledger，不冻结进本文；这仍是 descriptor / replacement / deletion-evidence read-model，不授权 physical skeleton layout 完成、domain ready、production ready 或 domain repo physical delete。旧 `opl agents list` 三仓 aligned 数字只作为历史 discovery proof。 |
...
| Provider-backed execution | `temporal_required_provider_path_with_evidence_tail` | Temporal SDK、workflow/activity/signal/query、worker lifecycle contract、CLI start/query/signal、provider receipt、fail-closed readiness 和 replay / residency proof surfaces 已落地；Temporal 已是 production online required substrate。周期性长时 SLO、production retry 运行证据和 real domain soak 仍未完成。 |
...
| Agent Lab / eval improvement control plane | `usable_workbench_export_optimize_refs_only_loop_landed` | Agent Lab 已从 refs-only complete surface 推进到实际可用机器面：`contracts/opl-framework/agent-lab-contract.json`、`src/agent-lab.ts`、`src/agent-lab-complete.ts`、`opl agent-lab sample --json`、`opl agent-lab longline --json`、`opl agent-lab complete --json`、`opl agent-lab run --suite <suite.json> --json`、`opl agent-lab workbench --json`、`opl agent-lab export --target <inspect-ai|openinference|langfuse|phoenix|json> --json` 和 `opl agent-lab optimize --suite <suite.json> --json` 均已落地。sample suite 覆盖 MAS/MAG/RCA fixture eval run；longline suite 覆盖 MAS paper owner-chain、MAG grant controlled soak、RCA visual controlled soak 三个 provider-hosted longline task、七个 recovery probe、repo test disposition 和 no-forbidden-write promotion gate；complete surface 冻结 Inspect AI optional adapter、METR task-standard reference、OpenInference/OpenTelemetry refs、Langfuse/Phoenix optional connector refs、optimizer loop 与 RL transition boundary；workbench surface 输出 App 可消费 read model，`app_workbench_consumption_ready=true`；export surface 输出 connector-shaped refs-only envelope，不上传外部服务，不读取 domain body；optimize surface 运行 external suite 后输出 gated optimizer candidate set 和 RL transition refs，不训练或部署权重；fixture/no-current-failure 只进入 `regression_guard_only`，default agent 自动推广还必须具备真实 failure/evidence delta、真实 independent AI review、promotion receipt、rollback/no-forbidden-write refs 和中风险 canary refs。2026-05-17 MAS/MAG/RCA 已各自加入 repo-native Agent Lab longline migration guard，用本仓验证入口消费 `opl agent-lab longline --json` 的 disposition，并保留 scorer、owner receipt fixture 与 artifact authority checks 在 domain 仓。它仍只消费 descriptor、stage attempt、provider receipt、domain-owned eval/proof refs 与 operator blocker，不持有 domain truth、quality verdict、artifact authority、memory body、model weight deployment 或 owner receipt authority。 |
| Real domain soak / retirement | `task_bound_bridge_landed_owner_chain_soak_pending` | 统一 Agent Executor Adapter、receipt/fail-closed、默认 Codex caller、Hermes/Claude opt-in adapter、MAG/RCA OPL receipt consumption、MAS requirement/receipt boundary 和 App/TUI read-model 边界已落地。MAS DM002/DM003/Obesity 三条真实 paper line 已有 read-only typed closeout projection 和 no-forbidden-write proof；DM002 已有 publication-route memory consumed/writeback receipt refs。MAS guarded-apply 已进入 provider-hosted task-bound attempt，并已观察到 MAS owner stable blocker receipt；MAG/RCA 已有 OPL task-bound receipt / no-regression evidence ingestion。尚未完成的是 MAS owner receipt 连续 scaleout、progress delta / human gate / stop-loss 证据、Codex 长时 activity、grant/visual long soak、memory body / writeback apply 泛化和旧面物理删除。旧 Hermes/Gateway/frontdoor/local-manager 默认 caller 已移走，保留项只按 explicit adapter、diagnostic、fixture、provenance 或 history 读取。 |
...
- Standard skeleton discovery / conformance 当前只从 `opl agents conformance --family-defaults --json`、`opl agents default-callers --family-defaults --json` 和 domain-owned contracts 读取。旧 `opl agents list` 三仓 aligned 数字只作为历史 discovery proof，不作为当前完成状态；descriptor/conformance 可读仍不等于 physical delete authority、domain ready 或 production ready。
- `MDS` / DeepScientist 只按 archive / diagnostic / upstream-intake reference 阅读；它不属于当前 active OPL conformance / default-caller / production evidence scope。
...
- `opl agents descriptors|descriptor --domain <domain>` 已成为 admitted domain agent 的统一只读 descriptor：它聚合 domain entry、standard skeleton、action catalog、stage control plane、domain memory descriptor、skill catalog、runtime/session/progress/artifact refs、parity 和 authority boundary。它是维护者/App 的总入口，不替代 `opl stages`、`opl actions`、`opl domain-memory` 的专题 drilldown，也不嵌入 memory 正文、prompt 长正文或 domain verdict；这些 surfaces 都应从同一份 canonical action/stage metadata 派生，而不是各自维护一套 CLI/MCP/Skill/product-entry/sidecar 映射。
- `family-transition-runner` 已具备最小 generic runner / matrix runner 机器边界：OPL 执行 domain-declared transition spec、匹配 guard、fail-closed、生成 receipt/projection envelope 和 matrix summary；domain agent 持有 transition table、guard 语义、owner receipt、质量 verdict、publication/fundability/visual/export 判断和 artifact authority。MAS 接入该面前必须提供 owner-signed `family_transition_spec`、`family_transition_matrix_cases`、owner receipt refs、source locator refs，以及明确的 forbidden-write / no-artifact-mutation authority boundary；只有 descriptor 时 OPL 继续投影 `descriptor_only` / `refresh_required`，不把 MAS publication verdict 或 `current_package` 状态推断成 OPL ready。
...
- `opl runtime snapshot --json` 与 `opl runtime app-operator-drilldown --json` 已输出 stage attempt / App operator refs-only read model；Aion runtime workbench 已展示 provider completion 与 domain ready verdict 的边界，并把 human gate、dead letter、rejected writeback 作为独立 operator closure axes。
- Agent Lab 边界与完整可运行入口已进入 runtime 文档：`docs/runtime/opl-agent-lab-control-plane.md` 把 OPL 内部 eval / improvement / evolution harness control plane 限定为 refs-only 评估、机制编辑候选与改进组织面，并记录 `opl agent-lab sample --json` / `opl agent-lab longline --json` / `opl agent-lab complete --json` / `opl agent-lab run --suite <suite.json> --json` / `opl agent-lab workbench --json` / `opl agent-lab mechanism --json` / `opl agent-lab export --target ... --json` / `opl agent-lab optimize --suite <suite.json> --json` / `opl agent-lab evolve --suite <suite.json> --json`。合同、CLI、connector 或 App projection 不得把 lab score、provider completion、longline suite pass、external suite pass、mechanism candidate、evolution segment、optimizer candidate、transition ref 或 harness pass 升级成 domain verdict。
- MAS/MAG/RCA 已各自声明 stage/action/projection descriptor，OPL 只消费 descriptor，不写 domain truth。

尚未完成的生产闭环：

- Temporal provider code 已落地，repo-native live proof 已完成 worker restart/re-query 代码路径证明；Temporal 已被确认为默认 Full online runtime 的必需 substrate。还没有闭合的是外部 production Temporal server/worker deployment、readiness 运维证明和真实 domain soak。
...
- OPL App/operator 读面已有 stage attempt workbench、provider completion/domain ready verdict 边界、human gate、resume、dead-letter repair 与 rejected writeback axes；provider-level signal 操作已接入白名单 bridge，其中 human gate signal payload 必须绑定当前 attempt id。后续 operation lane 聚焦按 domain/stage/blocker/memory refs 的更细下钻和真实 MAS domain owner-chain 执行证明。
...
- Local provider 只作为 dev/CI/offline diagnostic baseline；`hermes_agent` executor adapter/backend 只在显式选择时生效。旧 Hermes runtime-provider 面归 history/provenance/diagnostic source ref、fixture 或 negative guard。默认 caller 和 active-path wording 已完成收口；剩余旧名若保留，必须能解释为 explicit executor adapter、diagnostic、fixture、provenance、history、negative guard 或 tombstone，不能重新成为默认 runtime、默认 executor、provider interface、provider proof surface、readiness blocker 或兼容入口；确认 no-active-caller 后直接物理删除或迁入 tombstone。

因此，对外和开发文档应把“Agent Executor Adapter 计划已落地”和“生产级 domain soak 未闭合”分开写。准确口径是：OPL family framework 的控制面骨架、local queue/attempt ledger、Temporal provider code、domain adapter discovery、standard skeleton validation 机制、domain memory descriptor read model、Codex stage runner repo/test harness、Agent Executor Adapter、typed receipt/fail-closed gate、runtime snapshot、App/operator drilldown 和 Aion stage attempt signal workbench 已落地；standard conformance 只按 fresh `opl agents conformance` / `opl agents default-callers` 摘要读取，且仍停在 descriptor / replacement / deletion-evidence read-model 层。剩余验收是外部 production provider deployment、真实长时 domain stage execution / soak、真实 domain memory apply、repo-source 物理目录标准化，以及旧接口在无 active caller 且有 owner-side delete authority 之后的物理删除或 history/tombstone 归档。
...
| 旧面退役 | 默认语义已从 Hermes provider/Gateway/MDS/local-manager 转向 stage-led / Agent executor / Codex CLI 默认执行器 / provider-backed；public help / command spec、Product Entry runtime 和 runtime status 不再把 Hermes provider、Gateway cron、gateway readiness 或 compatibility alias 放在默认示例或 active field 中。`hermes_agent` executor adapter/backend 仍是显式非默认接口。active-path residue scan / no-default-caller evidence 已完成当前收口；保留旧名只允许作为 explicit executor adapter、diagnostic、fixture、provenance、history、negative guard 或 tombstone。剩余无 active caller 的实现、接口和测试应直接物理删除或 history/tombstone 归档，不保留兼容入口。 |
...
6. `Retirement cleanup`：当前已清理 active/default caller、public-help residue 和可安全落地的 active-path wording，并以 focused scan 证明 retained legacy wording 只存在于 explicit executor adapter、diagnostic、fixture、provenance、history 或 tombstone 语境；旧 vocabulary、legacy manager、过时测试和非标准 skeleton 入口一旦有 no-active-caller 证据，应直接物理删除或迁入 tombstone，不保留兼容入口。
...
1. Provider abstraction freeze：把 OPL family runtime provider 显式枚举为 `local_sqlite | temporal`，并声明一致的 readiness、attempt、signal、query、receipt 字段；旧 `hermes_legacy` provider selection 必须 fail-closed。
...
当前落地校准：普通 `Product Entry`、`opl resume` 与 `opl session resume` 已收敛为 Codex-default executor；provider-backed family runtime、Hermes provider / Gateway / readiness 退役语境和 explicit executor adapter 边界回到核心五件套、runtime boundary contract、live CLI/read-model 与 history/provenance 读取。旧 `hermes_legacy` provider selection 必须 fail-closed；`hermes_agent` executor 相关输出只在显式 executor selection、receipt、fixture 或 fail-closed proof 中出现。Descriptor、stage、domain-memory、conformance 和 default-caller readiness 的具体 counters 只从 fresh CLI/read-model 摘要读取；它们用于区分 descriptor/conformance 可读、physical morphology / deletion-evidence gate 和 production evidence tail，不授权 domain ready 或 production ready。真实 Temporal server/worker residency、长时 domain activity soak、真实 paper/grant/visual provider-hosted apply、memory body apply receipt、物理 skeleton 重组与物理旧面删除仍属于 production closure。

结构质量 gate 的当前语义也按这个边界执行：`sentrux gate .` baseline drift 只作为 advisory，并触发 OPL quality details 输出帮助定位结构变化；line budget 与 `sentrux check .` explicit rules 仍是 blocking。后续结构治理可以先做 audit、gap projection 和文档同步，不能把 advisory baseline drift 写成阻塞发布的 hard gate，也不能把 line budget / explicit rules 降级成 advisory。
...
以下 master/lane 小节保留为 implementation roadmap / provenance，不再是 active execution queue。当前执行 baton 回到 `docs/active/current-state-vs-ideal-gap.md`；每个 `状态：` 或 `待完成：` 都必须用 live contracts/source/tests/CLI/read-model 重新校准后才能作为当前结论。
...
状态：OPL 已落地 `standard-domain-agent-skeleton-contract.json`、alias-aware manifest normalizer 和 `opl agents list|inspect`；MAS/MAG/RCA 的当前 manifest / conformance 状态只从 fresh `opl agents conformance --family-defaults --json`、`opl agents default-callers --family-defaults --json` 与 artifact locator surface 读取。这是 descriptor / manifest 层对齐，不是 physical repo-source 目录重组。
...
- 已完成：MAS/MAG/RCA 生成并暴露可被 OPL 校验的 skeleton mapping：现有 repo-source 文件/manifest 到 `agent/`、`contracts/`、`runtime/`、`docs/` 的对应关系，以及 workspace artifact root / runtime artifact root 的 locator contract。
- 已完成：MAS 补充可被 `opl agents list|inspect` 读取的 standard skeleton descriptor 与 artifact locator surface；当前 OPL 读模型是否 aligned 只能从 fresh conformance/default-caller 摘要读取。
- 已完成：OPL normalizer 消费标准 skeleton 字段并 fail-closed 报告 missing / drift；缺少 artifact locator surface 时不会被误判为可托管。
...
状态：已落地到 `opl family-runtime attempt create|list|inspect` 和 `${OPL_STATE_DIR}/family-runtime/queue.sqlite#stage_attempts`。当前实现覆盖本地 ledger、provider receipt、task-bound lifecycle projection，并已进入 `opl runtime snapshot --json` / `opl runtime app-operator-drilldown --json` 的 refs-only operator read model；产品体验继续由 App/operator visibility lane 打磨。
