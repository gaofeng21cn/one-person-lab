...
| Agent Lab / eval improvement control plane | `complete_control_plane_landed_refs_only` | Agent Lab 已从边界定义推进到完整可运行合同与 CLI：`contracts/opl-framework/agent-lab-contract.json`、`src/agent-lab.ts`、`src/agent-lab-complete.ts`、`opl agent-lab sample --json`、`opl agent-lab longline --json`、`opl agent-lab complete --json` 和 `opl agents meta-builder plan --json` 均已落地。sample suite 覆盖 MAS/MAG/RCA fixture eval run；longline suite 覆盖 MAS paper owner-chain、MAG grant controlled soak、RCA visual controlled soak 三个 provider-hosted longline task、七个 recovery probe、repo test disposition 和 no-forbidden-write promotion gate；complete surface 进一步冻结 Inspect AI optional adapter、METR task-standard reference、OpenInference/OpenTelemetry refs、Langfuse/Phoenix optional connector refs、optimizer loop、RL transition boundary 与元智能体 builder plan。它仍只消费 descriptor、stage attempt、provider receipt、domain-owned eval/proof refs 与 operator blocker，不持有 domain truth、quality verdict、artifact authority、memory body、model weight deployment 或 owner receipt authority。下一步是让 App/workbench 消费 complete read model，并按需实现 optional external connectors。 |
...
- Agent Lab 边界与完整可运行入口已进入 runtime 文档：`docs/runtime/opl-agent-lab-control-plane.md` 把 OPL 内部 eval / improvement control plane 限定为 refs-only 评估与改进组织面，并记录 `opl agent-lab sample --json` / `opl agent-lab longline --json` / `opl agent-lab complete --json` / `opl agents meta-builder plan --json`。合同、CLI、connector 或 App projection 不得把 lab score、provider completion、longline suite pass、optimizer candidate 或 harness pass 升级成 domain verdict。
