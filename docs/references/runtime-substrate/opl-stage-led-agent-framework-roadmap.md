...
| Agent Lab / eval improvement control plane | `boundary_defined_refs_only` | Agent Lab 已冻结为 OPL Framework 内部统一 eval / improvement control plane：它组织 lab run、improvement candidate、acceptance evidence、owner route 和 follow-up projection，只消费 descriptor、stage attempt、provider receipt、domain-owned eval/proof refs 与 operator blocker；它不持有 domain truth、quality verdict、artifact authority、memory body 或 owner receipt authority。下一步是补最小 machine-readable contract、CLI/App projection 和 refs-only 验证 lane。 |
...
- Agent Lab 边界已进入 runtime 文档：`docs/runtime/opl-agent-lab-control-plane.md` 把 OPL 内部 eval / improvement control plane 限定为 refs-only 评估与改进组织面，后续合同/CLI/App projection 不得把 lab score、provider completion 或 harness pass 升级成 domain verdict。
