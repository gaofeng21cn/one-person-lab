...
- Agent Lab 边界与完整可运行入口已进入 runtime 文档：`docs/runtime/opl-agent-lab-control-plane.md` 把 OPL 内部 eval / improvement control plane 限定为 refs-only 评估与改进组织面，并记录 `opl agent-lab sample --json` / `opl agent-lab longline --json` / `opl agent-lab complete --json` / `opl agent-lab run --suite <suite.json> --json` / `opl agent-lab workbench --json` / `opl agent-lab export --target ... --json` / `opl agent-lab optimize --suite <suite.json> --json`。合同、CLI、connector 或 App projection 不得把 lab score、provider completion、longline suite pass、external suite pass、optimizer candidate、transition ref 或 harness pass 升级成 domain verdict。
