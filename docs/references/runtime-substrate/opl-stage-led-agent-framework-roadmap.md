...
| Agent Lab / eval improvement control plane | `sample_and_longline_suite_landed_refs_only` | Agent Lab 已从边界定义推进到可运行合同与 CLI：`contracts/opl-framework/agent-lab-contract.json`、`src/agent-lab.ts`、`opl agent-lab sample --json` 和 `opl agent-lab longline --json` 均已落地。sample suite 覆盖 MAS/MAG/RCA fixture eval run；longline suite 覆盖 MAS paper owner-chain、MAG grant controlled soak、RCA visual controlled soak 三个 provider-hosted longline task、七个 recovery probe、repo test disposition 和 no-forbidden-write promotion gate。它仍只消费 descriptor、stage attempt、provider receipt、domain-owned eval/proof refs 与 operator blocker，不持有 domain truth、quality verdict、artifact authority、memory body 或 owner receipt authority。下一步是让 App/workbench 消费 longline read model，并让 domain repo 把重复的 longline orchestration 测试迁到 OPL，保留各自 scorer/receipt/artifact authority 测试。 |
...
- Agent Lab 边界与可运行入口已进入 runtime 文档：`docs/runtime/opl-agent-lab-control-plane.md` 把 OPL 内部 eval / improvement control plane 限定为 refs-only 评估与改进组织面，并记录 `opl agent-lab sample --json` / `opl agent-lab longline --json`。合同、CLI 或 App projection 不得把 lab score、provider completion、longline suite pass 或 harness pass 升级成 domain verdict。
