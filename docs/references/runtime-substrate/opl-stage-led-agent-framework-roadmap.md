...
| Agent Lab / eval improvement control plane | `complete_control_plane_landed_refs_only` | Agent Lab 已从边界定义推进到完整可运行合同与 CLI：`contracts/opl-framework/agent-lab-contract.json`、`src/agent-lab.ts`、`src/agent-lab-complete.ts`、`opl agent-lab sample --json`、`opl agent-lab longline --json`、`opl agent-lab complete --json` 和 `opl agent-lab run --suite <suite.json> --json` 均已落地。sample suite 覆盖 MAS/MAG/RCA fixture eval run；longline suite 覆盖 MAS paper owner-chain、MAG grant controlled soak、RCA visual controlled soak 三个 provider-hosted longline task、七个 recovery probe、repo test disposition 和 no-forbidden-write promotion gate；complete surface 进一步冻结 Inspect AI optional adapter、METR task-standard reference、OpenInference/OpenTelemetry refs、Langfuse/Phoenix optional connector refs、optimizer loop 与 RL transition boundary；external suite runner 允许独立 Foundry Agent 生成 OPL-compatible suite 后交回 Agent Lab 运行。它仍只消费 descriptor、stage attempt、provider receipt、domain-owned eval/proof refs 与 operator blocker，不持有 domain truth、quality verdict、artifact authority、memory body、model weight deployment 或 owner receipt authority。下一步是让 App/workbench 消费 complete read model，并按需实现 optional external connectors。 |
