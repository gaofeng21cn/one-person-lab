...
- `opl family-runtime evidence-worklist --family-defaults --provider temporal --executor-kind codex_cli --detail full --json`：open worklist、closed refs-only items 与 zero-open envelope counters 是动态读数，只能从 fresh CLI/read-model 读取；`domain_ready_authorized=false`、`production_ready_authorized=false` 是固定 authority 边界。zero-open worklist 只表示当前没有可执行 OPL evidence workorder，不表示 domain owner-chain、artifact authority、App release 或 long-soak 完成。
...
- `node dist/cli.js agents list --json`：`total_projects_count=3`、`aligned_count=3`、`missing_count=0`、`drift_detected_count=0`、`blocked_count=0`。MAS/MAG/RCA 当前 skeleton 均 aligned 且带 artifact locator surface；这表示 descriptor-level admission surface 已齐，不表示物理目录已重组或 production soak 已完成。
...
- MAS closeout projection 已覆盖 DM002、DM003、Obesity 三篇真实 workspace。
...
- 这证明 OPL 可以消费 MAS owner closeout refs 和 route-memory receipt refs；Temporal worker residency 代码路径已由 live proof 与本机 production proof 覆盖。尚未证明的是长时 Codex/domain activity、MAS owner-chain guarded apply 推动论文前进，以及真实 token/cost/progress 观测校准。
...
- 旧 Hermes/Gateway/local-manager/default-compat active/default caller 已移走；保留名称只允许出现在 diagnostic、fixture、provenance、history、negative guard 或 tombstone 语境。该 closeout 不等于真实 production-hosted soak 已完成。
...
| Provider-backed execution | `production_residency_proven_temporal_required` | Temporal SDK、workflow/activity/signal/query、worker lifecycle contract、CLI start/query/signal、provider receipt、fail-closed readiness、repo-native Temporal live residency proof 与本机 managed production proof 已落地；Temporal 已是 production online required substrate。周期性长时 SLO、production retry 运行证据和 real domain soak 仍未完成。 |
...
当前落地校准（2026-05-14）：普通 `Product Entry`、`opl resume` 与 `opl session resume` 已收敛为 Codex-default executor；`opl status runtime` 顶层已经报告 provider-backed family runtime。Hermes provider / Gateway / readiness 相关输出仅保留在 diagnostics/provenance/test fixture 与负向 guard 语境；旧 `hermes_legacy` provider selection 必须 fail-closed。`hermes_agent` executor 相关输出只在显式 executor selection、receipt、fixture 或 fail-closed proof 中出现。OPL 当前读模型为 agents `aligned_count=3` / `missing_count=0`、stages `resolved_planes_count=3` / `stages_count=18`、domain-memory `resolved_memory_descriptor_count=3` / `missing_memory_descriptor_count=0`。agents read model 已显式暴露 `descriptor_readiness`、`physical_skeleton_layout_audit` 与 `production_closure_gaps`，用于区分 descriptor aligned、物理 layout audit 待证据闭合、以及平台成熟后才能关闭的 production gaps。下一步不需要等待“完整智能体平台”才能继续做 descriptor / gate / projection / Codex runner / runtime status / path retirement audit 的收口；真实 Temporal server/worker residency、长时 domain activity soak、真实 paper/grant/visual provider-hosted apply、memory body apply receipt、物理 skeleton 重组与物理旧面删除仍属于平台成熟后的 production closure。
...
4. Framework guardrail 与 domain quality gate 还需要明确分层：OPL 只能把 contract completeness、forbidden writes、freshness 和 owner boundary 投影成下限检查、advisory 或 blocker localization；domain 才能判断论文、基金、视觉交付质量。
...
状态：OPL 已落地 `standard-domain-agent-skeleton-contract.json`、alias-aware manifest normalizer 和 `opl agents list|inspect`；MAS/MAG/RCA 当前真实 manifest smoke 均为 aligned，且通过 artifact locator surface 证明真实 artifact 不进入 domain repo source skeleton。这是 descriptor / manifest 层对齐，不是 physical repo-source 目录重组。
...
- 已完成：MAS 补充可被当前 `opl agents list|inspect` 读取的 standard skeleton descriptor 与 artifact locator surface；当前 OPL 读模型把 MAS 报告为 aligned，source field 为 `standard_domain_agent_skeleton`。
