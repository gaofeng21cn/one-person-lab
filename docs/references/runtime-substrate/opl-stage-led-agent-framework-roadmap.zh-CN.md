...
| Domain descriptor / adapter | `mostly_landed_one_skeleton_gap` | MAS/MAG/RCA 已暴露 stage/action/projection descriptor；MAG/RCA 的 standard skeleton aligned，MAS 当前缺 standard skeleton descriptor，OPL 当前真实 `opl agents list` 为 2 aligned / 1 missing。 |
...
- MAG/RCA 的 standard skeleton adapter 当前通过 OPL 真实 manifest smoke 对齐；MAS 当前 manifest 缺 standard skeleton surface，因此 fresh `opl agents list --json` 为 `aligned_count=2`、`missing_count=1`、`drift_detected_count=0`、`blocked_count=0`。OPL 消费的 source fields 当前为 MAG `standard_domain_agent_skeleton`、RCA `standard_domain_agent_skeleton`，两者均带 artifact locator surface；MAS 需要在 MAS 侧补 standard skeleton descriptor 后回到 aligned。
