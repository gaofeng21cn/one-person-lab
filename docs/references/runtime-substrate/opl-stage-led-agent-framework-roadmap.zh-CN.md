...
状态：已落地 stage attempt workbench 核心投影与 Aion 白名单 signal bridge；真实 worker/domain 执行证明、按 refs 分组的操作体验和跨 domain soak 仍待完成。
