...
| Domain descriptor / adapter | `all_active_domains_aligned_descriptor_level` | MAS/MAG/RCA 已暴露 stage/action/projection descriptor；三仓 standard skeleton discovery 当前均 aligned，OPL 当前真实 `opl agents list` 为 3 aligned / 0 missing。 |
...
- MAS/MAG/RCA 的 standard skeleton adapter 当前均通过 OPL 真实 manifest smoke 对齐；fresh `opl agents list --json` 为 `aligned_count=3`、`missing_count=0`、`drift_detected_count=0`、`blocked_count=0`。OPL 消费的 source fields 当前为 MAG `standard_domain_agent_skeleton`、RCA `standard_domain_agent_skeleton` 与 MAS `opl_domain_agent_skeleton_mapping` alias；三者均带 artifact locator surface。
