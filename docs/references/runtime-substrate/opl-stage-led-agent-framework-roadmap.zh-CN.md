...
结论：前一轮 stage-led / provider-backed 计划已经落到一批可调用 surface；stage、domain-memory descriptor 与 standard domain-agent skeleton discovery / validation 当前均已在 MAS/MAG/RCA 三仓 resolved/aligned。Temporal TypeScript SDK、`StageAttemptWorkflow`、activity、signal/query、CLI start/query/signal、worker lifecycle contract、typed closeout ingestion、Codex stage runner repo/test harness、stage attempt workbench 和 Aion 白名单 signal bridge 均已落地。typed closeout ledger 已具备严格幂等 / fail-closed 规则：同一 `closeout_id` 的同一 packet 重放是 no-op，冲突 packet 不会污染 closeout refs 或 activity ledger；Aion workbench 也已把 provider completion、domain ready verdict、human gate、dead letter、rejected writeback 拆成独立 operator 状态轴，并加严 human gate signal payload 必须绑定当前 `stage_attempt_id`。2026-05-12 新增的 OPL proof tests 进一步覆盖了 Codex live runner timeout / checkpoint heartbeat / process output summary，以及 Temporal worker resident state re-query / restart already-ready / stop 后 worker-not-ready projection。当前 production closure 的优先级已调整为先验证 MAS 三篇真实 paper line，MAG/RCA controlled soak 暂缓且只要求 descriptor/index 不退化。

2026-05-12 fresh CLI 读模型校准：
...
| Provider-backed execution | `production_provider_minimal_loop_landed_temporal_required` | Temporal SDK、workflow/activity/signal/query、worker lifecycle contract、CLI start/query/signal、provider receipt、fail-closed readiness 和 repo-native Temporal live residency proof 已落地；Temporal 已是 production online required substrate。外部 production service provisioning、production retry 运行证据和 domain soak 仍未完成。 |
...
| 目标项 | 当前距离 |
| --- | --- |
| OPL 作为完整智能体框架 | 控制面、合同、队列、attempt ledger、Temporal provider code、repo-native Temporal live residency proof、Codex runner repo/test harness、typed closeout gate、snapshot/workbench 已落地；MAS 三篇真实 paper line 已证明 read-only typed closeout 可被 OPL 消费。还差把 Temporal 作为必需依赖完成安装/修复/监控/readiness 的 production residency，长时 domain activity soak、human gate/resume 进入 domain owner chain 的运行证明和生产 cutover。 |
...
User / Codex App / OPL GUI / CLI
  ...
  -> family runtime provider (Temporal required; Hermes/local legacy)
...
当前落地校准（2026-05-12）：普通 `Product Entry`、`opl resume` 与 `opl session resume` 已收敛为 Codex-default executor；`opl status runtime` 顶层已经报告 provider-backed family runtime。Hermes 相关输出仅保留在 `hermes_legacy` provider、diagnostics/provenance/test fixture 与可选安装模块语境。OPL 当前读模型为 agents `aligned_count=3` / `missing_count=0`、stages `resolved_planes_count=3` / `stages_count=18`、domain-memory `resolved_memory_descriptor_count=3` / `missing_memory_descriptor_count=0`。agents read model 已显式暴露 `descriptor_readiness`、`physical_skeleton_layout_audit` 与 `production_closure_gaps`，用于区分 descriptor aligned、物理 layout audit 待证据闭合、以及平台成熟后才能关闭的 production gaps。下一步不需要等待“完整智能体平台”才能继续做 descriptor / gate / projection / Codex runner / runtime status / path compatibility audit 的收口；真实 Temporal server/worker residency、长时 domain activity soak、真实 paper/grant/visual provider-hosted apply、memory body apply receipt、物理 skeleton 重组与物理旧面删除仍属于平台成熟后的 production closure。
