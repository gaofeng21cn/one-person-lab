...
本参考说明当前测试入口语义。机器真相在 `scripts/test-lanes.mjs`、`scripts/run-parallel-test-lanes.sh`、`scripts/verify.sh`、`package.json` 和 GitHub workflow；本文只解释维护口径，不冻结 lane 细节或 CI 通过状态。
