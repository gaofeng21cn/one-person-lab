...
MAS Scholar Skills 的 canonical source 是外部 `mas-scholar-skills` package；其 `.codex-plugin/plugin.json`、`skills/*/SKILL.md` 和 package-owned contracts 决定当前可用能力。OPL 不再保留 thin plugin mirror、医学 Skill ID catalog 或 required/default profile，只通过统一 capability package channel 安装/更新/回滚，并由 `opl connect skills|sync-skills` 校验实际目录、按显式 workspace / quest target 同步、记录 source head 与 materialized skill ids。MAS 的 stage prompt 与 domain truth 留在 MAS，专业 Skill 内容与清单留在 package owner；OPL receipt 只证明分发和 provenance，不声明 owner gate、quality/domain truth、clinical data readiness、typed blocker、runtime queue 或 publication/export readiness。
...
- 新版 OPL App Full 不再打包或自动链接 Superpowers。历史 runtime payload 仍可被显式 `--superpowers full` 兼容读取，但默认 `system configure-codex` 使用 `keep`，不创建、覆盖或删除用户的 Superpowers。
