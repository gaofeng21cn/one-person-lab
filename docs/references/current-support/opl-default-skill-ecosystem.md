...
MAS Scholar Skills 的 canonical source 是外部 `mas-scholar-skills` package；其 `.codex-plugin/plugin.json`、`skills/*/SKILL.md` 和 package-owned contracts 决定当前可用能力。OPL 不再保留 thin plugin mirror、医学 Skill ID catalog 或 required/default profile，只通过统一 `opl packages` channel 安装/更新/回滚，并由 Packages activation transaction 校验实际目录、按 workspace / quest target 物化当前发布包声明的全部 35 Skills、记录 provider lock、core/full digests、Skill ids 和 lifecycle receipt。MAS 的 stage prompt 与 domain truth 留在 MAS，专业 Skill 内容与清单留在 package owner；OPL receipt 只证明分发和 provenance，不声明 owner gate、quality/domain truth、clinical data readiness、typed blocker、runtime queue 或 publication/export readiness。
