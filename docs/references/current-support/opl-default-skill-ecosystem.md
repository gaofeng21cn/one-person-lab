...
| OPL Flow 推荐能力 | agent-reach、ui-ux-pro-max、officecli skills/CLI、MinerU skill/CLI | OPL Flow manifest 声明，Framework 优先复用已有兼容能力，缺失时投影通用安装动作 | `opl packages install|update opl-flow` 解析开放组合；App Full 可携带已有兼容 payload，但缺失不阻断，App 不维护第二份清单 |

OPL Base 对 OPL-managed OfficeCLI 与 `mineru-open-api` 维护独立 current/latest 状态：`opl update check|plan` 做有界上游查询，启动或定时 managed apply 只对确认落后的 OPL-managed root 静默升级并验证，普通 `opl app state --profile fast` 只读 receipt / 进程缓存。Homebrew、global npm 或普通 PATH 中的 Codex / Temporal CLI 不进入后台 apply；只有能够从 binary realpath 证明 Homebrew formula、Homebrew cask 或 global npm owner 时，App 才能展示需要二次确认的原渠道委托动作，无法证明 owner 时只显示原安装渠道 guidance。Temporal SDK 继续随 Framework generation 更新；本机 Temporal CLI 可按已证明的 Homebrew owner比较，远端或系统 Temporal Server 版本不由 OPL 猜测。
| Codex bundled skills | Documents、Presentations、Spreadsheets | Codex plugin cache | 只检测可用性，不复制到 `~/.codex/skills` |

OPL family domain agent 默认不写入各自的顶层 `[mcp_servers.*]` standalone server。MAS、MAG、RCA 的 Codex App 可见面统一来自 Codex plugin / family plugin registry / OPL-generated interface；OPL Meta Agent 的 Codex App 可见面来自 OPL-generated Codex surface。repo-local MCP server 只能作为 domain handler target、direct protocol adapter 或 proof lane 保留。`opl install` 和 `opl connect sync-skills` 在刷新 family plugin registry 时，会移除旧的 family standalone MCP server 段，并注册 Framework-owned `[mcp_servers.opl-connect]`，其当前精选工具仅覆盖只读 scientific search 和 reference verification；同时退役旧的 `~/.codex/skills/{mas,mag,rca}` 裸 mirror，保留 Sentrux、Playwright 等非 OPL MCP server 配置。
...
MAS Scholar Skills 的 canonical source 是外部 `mas-scholar-skills` package；其 `.codex-plugin/plugin.json`、`skills/*/SKILL.md` 和 package-owned contracts 决定当前可用能力。OPL 不再保留 thin plugin mirror、医学 Skill ID catalog 或 required/default profile，只通过统一 `opl packages` channel 管理 current/LKG generation。每个 hosted action / 新 child Attempt 首次启动时，Packages 校验实际 source，按 source policy 解析最新可运行闭包，把完整 35 Skills 写入 Attempt 自有的只读 `.agents/skills` generation，并把 workspace / quest `.codex/skills/` 仅作为 Codex discovery/兼容投影；provider lock、digests、Skill ids 和 lifecycle receipt 只记录 provenance。MAS 的 stage prompt 与 domain truth 留在 MAS，专业 Skill 内容与清单留在 package owner；OPL receipt 不声明 owner gate、quality/domain truth、clinical data readiness、typed blocker、runtime queue 或 publication/export readiness。
...
| `src/modules/connect/agent-package-registry-parts/managed-policy-surface.ts` / `opl packages install|update opl-flow` | 在通用 Package 事务内校验 owner policy/schema，安装推荐依赖，备份并移出冲突 surface，处理 profile/model projection，并写入统一 lock/receipt/LKG。 | 每次执行的具体结果与 fresh inventory。 |
...
MAS ScholarSkills 不再暴露独立安装、activate 或 scope sync 入口。`opl packages install mas` 建立 required closure 与首个 LKG；之后每个 hosted action / 新 child Attempt 按用户 source policy 静默追最新，验证失败自动继续使用 LKG。`opl packages status --package-id mas --scope ...` 只读取依赖、generation 与 provenance observations；缺失、漂移或更新失败不要求用户先 repair/reload，App state 也不发布 activation action。Console/App/Shell 不维护第二套 gate。
