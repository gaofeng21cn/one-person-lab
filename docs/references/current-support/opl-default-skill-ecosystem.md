...
MAS professional skill dependency 只从 MAS agent package manifest 读取。`mas-scholar-skills` 是 MAS required capability package，但 repo 仍独立维护；OPL Connect / App 只把 manifest 中的 dependency graph 投影为 managed package 安装、更新、Developer Mode source checkout 和 workspace / quest skill sync，不在 OPL 侧写死 MAS 私有依赖。
