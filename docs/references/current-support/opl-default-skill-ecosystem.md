...
| `src/system-installation/codex-plugin-registry.ts` | 作为 Codex Plugin carrier adapter，在 `OPL_STATE_DIR/codex-plugin-marketplaces/*` 生成 OPL-owned marketplace wrapper，注册 `mas@mas-local`、`mag@mag-local`、`rca@rca-local`、`oma@oma-local` 和 `obf@obf-local`，并移除旧 family standalone MCP server blocks。它不持有 Agent Package package core。 | 用户 config 当前内容、wrapper marketplace path 是否存在、wrapper plugin symlink target、实际移除数量。 |
