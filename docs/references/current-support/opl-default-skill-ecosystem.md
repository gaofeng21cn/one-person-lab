...
- `opl-external-scientific-skill-router`：默认 MAS / professional pack 覆盖不到罕见科学工具、数据库、工作流或方法时，薄路由到 OPL Connect external-skills search/inspect/sync；只允许 workspace / quest-local 按需同步，不进入全局 Codex metadata。
