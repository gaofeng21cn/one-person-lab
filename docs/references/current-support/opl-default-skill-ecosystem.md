...
| `src/opl-skills.ts` / `opl skill list` / `opl skill sync` | 检查 MAS/MAG/RCA tracked plugin manifest / skill source，生成 OMA Codex plugin source，并调用 Codex plugin registry。 | 当前 sibling/managed repo 是否存在、tracked plugin source path、generated plugin cache path、sync count。 |
