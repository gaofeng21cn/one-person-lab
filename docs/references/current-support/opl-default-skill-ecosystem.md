...
| --- | --- | --- | --- |
| OPL family domain skills | MAS、MAG、RCA、OPL Meta Agent | MAS/MAG/RCA 走 Codex plugin cache / family plugin registry；OPL Meta Agent 走 OPL-generated Skill surface | MAS/MAG/RCA 由各 active domain-agent 仓维护 plugin metadata，OPL 只负责刷新 Codex plugin registry，不复制到 `~/.codex/skills`；OPL Meta Agent 不持有 repo-local plugin wrapper，由 OPL 从 contract pack materialize `~/.codex/skills/opl-meta-agent` |
...
OPL family domain agent 默认不写入顶层 `[mcp_servers.*]` standalone server。MAS、MAG、RCA 的 Codex App 可见面统一来自 Codex plugin / family plugin registry / OPL-generated interface；OPL Meta Agent 的 Codex App 可见面来自 OPL-generated Skill surface。repo-local MCP server 只能作为 domain handler target、direct protocol adapter 或 proof lane 保留。`opl install` 和 `opl skill sync` 在刷新 MAS/MAG/RCA plugin registry 时，会移除旧的 family standalone MCP server 段，并退役旧的 `~/.codex/skills/{mas,mag,rca}` 裸 mirror，保留 Sentrux、Playwright 等非 OPL MCP server 配置。
