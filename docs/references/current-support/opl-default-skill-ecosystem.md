...
OPL family domain agent 默认不写入顶层 `[mcp_servers.*]` standalone server。MAS、MAG、RCA 的 Codex App 可见面统一来自 Codex plugin / family skill sync / OPL-generated interface；repo-local MCP server 只能作为 domain handler target、direct protocol adapter 或 proof lane 保留。`opl install` 和 `opl skill sync` 在刷新 MAS/MAG/RCA plugin registry 时，会移除旧的 family standalone MCP server 段，保留 Sentrux、Playwright 等非 OPL MCP server 配置。
