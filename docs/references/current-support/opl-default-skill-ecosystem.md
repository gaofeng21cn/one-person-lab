...
`opl-runway-compute-operator` 与 `opl-runtime-soak-and-recovery-auditor` 可以审阅外部 remote-compute env spec、using-model-endpoint provider 经验、runtime environment bundle 和 native-helper diagnostic refs，但 OPL 的机器 truth 只来自 `contracts/opl-framework/runtime-environment-substrate-contract.json` 与 `opl runtime env * --json` readback。Skill helper 只生成 no-authority handoff / reviewer notes；当前只有 E2B 拥有 Runway executor，其他 provider 只能使用 generic external adapter shape，不能在 endpoint/ref、credential ref、provider receipt ref 或 lifecycle 禁止项之外声明实现支持。
...
| `src/modules/connect/opl-skills.ts` / `opl connect skills` / `opl connect sync-skills` | 检查标准 agent repo-owned primary skill source，读取 agent package manifest dependency graph，按统一 materializer 生成 Codex plugin carrier，并调用 Codex plugin registry。 | 当前 sibling/managed repo 是否存在、primary skill source path、required capability package 是否 materialized、generated plugin cache path、sync count。 |
...
- 原版 Codex App 独立安装 MAS 时，MAS 使用 repo-local full-copy plugin carrier；ScholarSkills required capability payload 仍通过 MAS agent package manifest 的 dependency graph、managed package receipt 与 workspace / quest sync 分发，不塞回 MAS plugin carrier 作为第二真相源。
