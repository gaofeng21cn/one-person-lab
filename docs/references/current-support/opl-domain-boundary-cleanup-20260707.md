...
| P0 | ScholarSkills source truth | 外部 `mas-scholar-skills` package 持有 plugin manifest、Skill 清单、内容合同与正文；OPL 只保留 generic package spec、validation、install/update/rollback、target-bound sync 与 provenance receipt。 | OPL-local plugin mirror、医学 catalog/validator/profile/engine 均不再需要。 |
