...
| --- | --- | --- | --- | --- |
| `mas-scholar-skills` | 用户级 Codex Skill，源语义来自 external `mas-scholar-skills` repo。 | `domain_profile` -> `workspace_local` / `quest_local`；`global_user` 仅限显式个人安装。 | partial | OPL 默认不应要求全局安装；Connect profile/readback 应继续以 package / workspace / quest sync 为主。 |
...
- `mas-scholar-skills` 的默认证据来自 OPL Connect profile/readback、source repo pack inventory 和 workspace / quest sync receipt，不来自用户全局 registry。
