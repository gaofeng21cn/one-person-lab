...
- runtime payload 包含 MAS/MAG/RCA、当前 family runtime provider 所需 payload / manifest、`officecli` CLI binary、MAS/MAG/RCA domain skills、officecli skill 组与 `ui-ux-pro-max`；Temporal-backed provider 是 production online runtime 的必需 substrate。Hermes-Agent、Claude Code 等只作为显式 opt-in executor adapter / proof / diagnostic payload 保留，不作为 Full 默认在线底座。App 首启仍执行统一 `opl install --skip-gui-open`，把模块、provider profile/readiness 与 skills 同步到标准 OPL state / Codex 可见目录。
