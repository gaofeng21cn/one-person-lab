...
- runtime payload 包含 MAS/MAG/RCA、Hermes lean runtime、`officecli` CLI binary、MAS/MAG/RCA domain skills、officecli skill 组与 `ui-ux-pro-max`；App 首启仍执行统一 `opl install --skip-gui-open`，把模块和 skills 同步到标准 OPL state / Codex 可见目录。
