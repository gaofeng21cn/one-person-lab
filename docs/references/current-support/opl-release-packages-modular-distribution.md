...
- `opl packages manifest` 输出 active native helper GHCR OCI prebuild 坐标、MAS/MAG/RCA/OPL Meta Agent active module package 坐标，以及 `one-person-lab-manifest` channel manifest 坐标；App-owned WebUI 镜像坐标从 App release/contracts/evidence 读取。
