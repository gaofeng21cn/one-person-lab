...
- First-party OPL agent package 的 dependency graph 来自对应 agent package manifest 的 package core；Codex surface、App surface、MCP/Web/native 等只作为 carrier adapter 投影。MAS manifest 若声明 `mas-scholar-skills` 为 required capability package，OPL 只消费这条声明生成 channel/package/readback；不得把该依赖散落成 OPL 私有常量、App 专属列表、单一 Codex carrier manifest 或 hard-coded MAS 知识。
