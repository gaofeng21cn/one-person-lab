...
`opl update status --json` 是 OPL 基座统一的受管组件更新状态面。它把 App binary、App-owned runtime/toolchain、GHCR agent package channel、Codex skill/plugin exposure 投影成同一套 `manifest -> current_state -> plan -> fetch -> verify -> stage -> activate -> post_apply -> receipt -> status_projection -> repair_action` 生命周期。
