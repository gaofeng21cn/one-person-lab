...
First-party carrier payload 由 `scripts/first-party-package-payload.mjs` 从 canonical package manifest、Framework-owned `package-payload-allowlists/<canonical-id>.json`、绑定 origin 的 owner worktree 与 exact commit 生成。package/plugin/source/output identity 不能由调用者覆盖；allowlist 是唯一文件集合，`source_root: "."` 也不触发递归全仓发现。已存在的 SemVer payload 只允许 exact-byte 幂等 readback，不同 bytes、可执行位、symlink、非规范路径或并发冲突均 fail closed。
...
npm run packages:payload -- --manifest contracts/opl-framework/packages/<canonical-id>.json --allowlist contracts/opl-framework/package-payload-allowlists/<canonical-id>.json --repo <owner-repo-root> --source-commit <full-sha>
