...
- `.github/workflows/packages.yml` 是 Framework module source archive / manifest publish workflow；它允许 release gate 通过 `workflow_call` 自动发布，并保留手动 `workflow_dispatch` 做修复。它发布 `one-person-lab-modules/*` 和 `one-person-lab-manifest` 到 GHCR，同时上传 workflow artifact；它不代表 MAS/MAG/RCA/OPL Meta Agent 各 repo 已经各自维护独立 GHCR/Packages 发布面。
...
| `.github/workflows/packages.yml` | 由 release gate 通过 `workflow_call` 自动构建 module source archive、运行 release-discipline gate、发布 module archive / release manifest 到 GHCR，并上传 release manifest / channel manifest / checksum artifact；`workflow_dispatch` 仅保留为人工修复入口；不构建或发布 WebUI image。 | workflow run 状态、artifact URL、远端 package 可见性和本机 package build 结果。 |
