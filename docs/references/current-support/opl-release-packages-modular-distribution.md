...
本文冻结 release / Packages / App / package 分发 owner boundary，不冻结日期、OPL 版本号、GUI 版本号、archive size、checksum、package source head SHA、release artifact、workflow run、GHCR digest、branch/SHA state、本机 package build 结果、Full payload layout、App first-run sequence、签名/公证模式或 VM gate 结果。当前 package 坐标、release discipline、channel manifest shape 和 install/update source 必须从 fresh `opl connect packages manifest`、`src/package-distribution.ts`、`scripts/package-archives.mjs`、`scripts/package-release-discipline.mjs`、`.github/workflows/release-package-channel.yml`、`.github/workflows/packages.yml` 与相关测试读取；App 下载资产、Full DMG、updater metadata、Full manifest、first-run matrix、签名/公证结果和 release evidence 继续归 `one-person-lab-app`。

稳定读法是：`GitHub Releases` 是用户下载入口，`GitHub Packages/GHCR` 是机器制品通道。first-party package canonical ids 固定为 `mas`、`mag`、`rca`、`oma`、`obf`、`mas-scholar-skills`、`opl-flow`，每个对象只有一个 OCI repository：`ghcr.io/<owner>/one-person-lab-packages/<canonical-id>`。moving channel 只允许 `candidate` 与 `latest-stable`；不可变 SemVer tag 必须绑定唯一 digest，普通安装只消费 `latest-stable`。Framework `.github/workflows/release-package-channel.yml` 与 `.github/workflows/daily-package-channel.yml` 只把发生 source/content fingerprint 变化的 package 送入 candidate；package gates 通过后才逐包提升 latest-stable。内容变化未推进 owner manifest SemVer、同 canonical id/version 漂到不同 digest、manifest/channel version 不一致或 unchanged package 被重发时必须 fail closed。旧 `one-person-lab-modules/*`、`one-person-lab-manifest:*`、repo-slug OCI 与 `latest` 只作 migration/history locator，不得作为当前安装源。Developer Mode 是 App/system settings 中启用 GitHub repo / local checkout package 来源的正式入口；低层 path/repo URL override 不改变普通 latest-stable 来源。
...
- `.github/workflows/release-package-channel.yml` 是 release gate；`.github/workflows/daily-package-channel.yml` 是 changed-package-only gate。daily 逐包比较 source/content fingerprint，只把有变化且 owner manifest SemVer 已合法推进的 package 发布为 immutable version 并移动 `candidate`；完整 gates 通过后再移动该 package 的 `latest-stable`。任何 SemVer/digest/channel drift 都 fail closed，无变化 package 不发布。`.github/workflows/packages.yml` 复用同一 canonical OCI namespace 并上传同源 manifest/checksum artifact，不为各 repo 创建第二发布面。
- `scripts/package-archives.mjs` 会生成 `opl-release-manifest.json`、`opl-channel-manifest.json` 与 `SHA256SUMS`，并把每个 package的 `source_git.head_sha`、源码包大小和 `sha256` 写入 manifest；Framework core 另投影 Homebrew 所需的 manifest-owned version、source head 与 immutable commit archive URL，tap 只负责下载后计算 sha256。
...
- App 更新后的首次启动会按当前 package-channel 目标补齐缺失模块；显式开发 override 由 operator 负责保持可用，不由 package/App 默认更新强行覆盖。
...
| `.github/workflows/daily-package-channel.yml` | 每天逐包比较 latest-stable 与候选 source/content fingerprint；只为 changed package 验证 owner SemVer、生成 immutable artifact 并移动 `candidate`，通过 gates 后再提升 `latest-stable`。 | 当天调度 run、changed package set、candidate/latest-stable digest、是否实际发布。 |
| `.github/workflows/packages.yml` | 通过 reusable `workflow_call` 构建 changed package source archive、运行 SemVer/digest/release-discipline gate，发布到唯一 canonical OCI 并上传 manifest/checksum artifact；直接 `workflow_dispatch` 仅作人工修复入口；不构建或发布 WebUI image。 | workflow run 状态、artifact URL、远端 package 可见性和本机 package build 结果。 |
...
| Package catalog / checksum artifact | packages workflow artifact 与 Base-owned readback | canonical ids、versions、OCI refs、digests、dependency closure 与 promotion result | `opl packages` 解析；不创建额外 `one-person-lab-manifest` OCI namespace |
...
- Daily package channel 只响应 package source/content fingerprint 变化。仅 `generated_at` 或 channel metadata 变化不得触发发布；changed package 必须先通过 owner manifest SemVer 与 immutable digest gate。
