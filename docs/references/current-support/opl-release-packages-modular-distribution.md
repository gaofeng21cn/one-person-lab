...
- GitHub Packages 面向机器消费，适合承载可独立更新的 OPL 内核包、App-owned WebUI/Docker 镜像坐标、domain module 预备制品与预构建 helper。
...
稳定读法是：`GitHub Releases` 是用户下载入口，`GitHub Packages/GHCR` 是机器制品通道；当前 `module_install_update_source` 仍是 `git_checkout`，`package_consumption_status` 仍是 `packages_defined_not_consumed_by_install_update`。Framework packages workflow 现在只通过 `workflow_dispatch` 准备 module source archive、release manifest、channel manifest 与 `SHA256SUMS` artifact；不会在 tag push 时自动推送 `one-person-lab-modules/*` 或 `one-person-lab-manifest` 到 GHCR，也不会发布 WebUI image。远端已存在的 `one-person-lab-modules/*` 与 `one-person-lab-manifest` package 应读成 `prepared_only_deprecated` 历史/预备通道，不能读成当前安装更新来源。Packages/GHCR 只有在 `opl module install/update` 或 App 环境管理真的消费 channel manifest 之后，才能写成当前安装更新机制。
...
| MAS | 当前 git checkout / sibling repo；模块 source archive 坐标是 prepared-only 实验通道 | 否 | domain agent 独立演进，按环境管理安装/更新 |
...
| MAG | 当前 git checkout / sibling repo；模块 source archive 坐标是 prepared-only 实验通道 | 否 | domain agent 独立演进，按环境管理安装/更新 |
| RCA | 当前 git checkout / sibling repo；模块 source archive 坐标是 prepared-only 实验通道 | 否 | 交付物链路可能较大，不应拖慢 App 更新 |
...
- `opl packages manifest` 输出 App-owned WebUI 镜像 reference、active native helper GHCR OCI prebuild 坐标、MAS/MAG/RCA/OPL Meta Agent 模块 source archive 预备坐标。
...
- `.github/workflows/packages.yml` 是 Framework module source archive / manifest prepared-artifact workflow；它只允许手动 `workflow_dispatch`，不在 tag push 上自动发布远端 GHCR channel，也不代表 MAS/MAG/RCA/OPL Meta Agent 各 repo 已经各自维护独立 GHCR/Packages 发布面。
...
- `scripts/package-release-discipline.mjs` 是 CI gate：检查 channel manifest、artifact build、checksum、rollback、旧版本清理策略与 prepared-only/manual 语义是否齐全。缺 checksum、缺回退策略、缺清理策略，或误称 tag push 自动发布 / active GHCR channel 时，package workflow 应直接失败。
...
| `scripts/cleanup-ghcr-package-versions.mjs` | 按 manifest 的 native-helper active policy 与 modules/manifest `prepared_only_deprecated` lifecycle 做 GHCR retention dry-run；只有 `--execute` 才删除候选 package versions。 | 当前远端 package version 列表、候选删除列表、rollback tag、实际删除结果。 |
| `.github/workflows/packages.yml` | 手动构建 module source archive、运行 release-discipline gate、上传 prepared release manifest / channel manifest / checksum artifact；不推送 module archive / release manifest 到 GHCR，不构建或发布 WebUI image。 | workflow run 状态、artifact URL、远端 package 可见性和本机 package build 结果。 |
...
| 模块源码包 | `ghcr.io/gaofeng21cn/one-person-lab-modules/<module>:<version>` | 不含 `.git`、缓存、venv、node_modules 的模块源码归档；当前 workflow 只生成 prepared artifact，远端历史 packages 读作 `prepared_only_deprecated` | 后续 `opl module install/update` 接入 manifest 后消费；当前不作为正式安装更新来源 |
...
| Release manifest | Prepared workflow artifact；`one-person-lab-manifest:<opl_version>` 是 `prepared_only_deprecated` 历史/预备 package，不是当前 active auto-published channel | 制品版本、URL、sha256、回滚目标 | App 环境管理与 CLI；当前只有被 install/update 或 App 环境管理 live source 消费后才成为安装更新来源 |
