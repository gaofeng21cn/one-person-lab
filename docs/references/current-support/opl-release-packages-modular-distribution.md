...
Connect 安装时再次校验 manifest、Release Set catalog 与 payload 的 carrier SHA 完全一致，并只把 payload admission 已验证的 SHA 写入 package lock、install/update/repair receipt、dependency closure 和 use binding。新 manifest 缺失或漂移时 fail closed，旧 lock 的 `owner_source_commit` 不得补位；dependency closure digest/readiness 同时绑定 provider SHA，activation/use receipt 对缺失或漂移的 first-party lock 拒绝继续。`owner_source_commit` 只证明 immutable carrier source currentness，不等于 domain truth、Live reload、runtime launch 或 installed Stable readback。
...
- `.github/workflows/release-package-channel.yml` 是 exact-digest stable promotion gate；`.github/workflows/daily-package-channel.yml` 是 changed-component candidate gate。daily 比较 Base、App 与 Packages 的 versioned source/content fingerprint；内容变化却未推进 owner version 时 fail closed。`.github/workflows/packages.yml` 只构建 candidate，stable gate 从 immutable generation 读取完整 BOM 后逐组件 retag 精确 digest，不重新构建或重新解析 owner HEAD。
