...
稳定读法是：`GitHub Releases` 是用户下载入口，`GitHub Packages/GHCR` 是机器制品通道。first-party package canonical ids 固定为 `mas`、`mag`、`rca`、`oma`、`obf`、`mas-scholar-skills`、`opl-flow`，每个对象只有一个 OCI repository：`ghcr.io/<owner>/one-person-lab-packages/<canonical-id>`。moving channel 只允许 `candidate` 与 `latest-stable`；不可变 SemVer tag 必须绑定唯一 digest，普通安装只消费 `latest-stable`。Framework `.github/workflows/release-package-channel.yml` 与 `.github/workflows/daily-package-channel.yml` 只发布发生 source/content fingerprint 变化且 SemVer 已推进的 Package；未变化成员在 Release Set 中复用上一精确 digest。完整 BOM 通过 readback 后才发布 `ghcr.io/<owner>/one-person-lab-manifest:<release-set-generation>` catalog carrier。该 carrier 不是第八个 Package，也不进入普通安装列表。旧 `one-person-lab-modules/*`、repo-slug OCI 与裸 `latest` 只作 migration/history locator，不得作为当前安装源。Developer Mode 是 App/system settings 中启用 GitHub repo / local checkout package 来源的正式入口；低层 path/repo URL override 不改变普通 latest-stable 来源。
...
- Release Set 使用 UTC CalVer generation `YY.M.D`，同日修订使用 `YY.M.D-rN`。它是七成员的精确 BOM，记录各自 SemVer、owner commit、OCI ref 与 digest，不是 App 版本，也不要求七个 Package 同时更新。
- `one-person-lab-manifest` 是 Release Set catalog carrier，不是 Package identity。尚无 stable baseline 时，daily workflow 会把 canonical 七成员初始化为 candidate；之后只处理 changed Package 或显式 repair。GitHub Release `published` gate 才提升 `latest-stable`。普通用户无需手动选择 generation，`opl packages` 从 stable catalog 解析并锁定精确成员。
...
- `.github/workflows/release-package-channel.yml` 是 stable release gate；`.github/workflows/daily-package-channel.yml` 是 changed-package-only candidate gate。daily 逐包比较 source/content fingerprint，只把有变化且 owner manifest SemVer 已合法推进的 Package 发布为 immutable version 并移动 `candidate`；GitHub Release `published` 后才把验证后的 Package 与完整 Release Set 提升到 `latest-stable`。任何 SemVer/digest/channel drift 都 fail closed，无变化 Package 不重发。`.github/workflows/packages.yml` 复用同一 canonical OCI namespace，不为各 repo 创建第二发布面。
- `scripts/package-archives.mjs` 会生成 `opl-release-manifest.json`、`opl-channel-manifest.json` 与 `SHA256SUMS`，并把每个 Package 的 `source_git.head_sha`、源码包大小和 `sha256` 写入 manifest；Release Set 同时记录精确 SemVer、owner commit、OCI ref 与 digest。Framework core 另投影 Homebrew 所需的 manifest-owned version、source head 与 immutable commit archive URL，tap 只负责下载后计算 sha256。
...
- Daily package channel 只响应 Package source/content fingerprint 变化或显式 Release Set repair。仅 `generated_at` 或 channel metadata 变化不得触发 Package 重发；changed Package 必须先通过 owner manifest SemVer 与 immutable digest gate，unchanged Package 复用上一精确 digest。
