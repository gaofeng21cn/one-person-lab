...
稳定读法是：`GitHub Releases` 是用户下载入口，`GitHub Packages/GHCR` 是机器制品通道；当前默认 stable/App install/update 通过 GHCR `one-person-lab-manifest` channel manifest 消费 `one-person-lab-modules/*`。Framework packages workflow 只通过 `workflow_dispatch` 发布 module source archive 与 release/channel manifest 到 GHCR，并同步上传 `opl-release-manifest.json`、`opl-channel-manifest.json` 与 `SHA256SUMS` artifact；不会在 tag push 时自动发布，也不会发布 `one-person-lab-webui`。dev/operator 可显式使用 `OPL_MODULE_SOURCE_MODE=git_checkout`、`OPL_MODULE_PATH_<MODULE_ID>` 或 `OPL_MODULE_REPO_URL_<MODULE_ID>` override 模块来源；这些 override 不改变 stable/App 默认 package-channel 来源。旧文档中 `packages_defined_not_consumed_by_install_update`、`prepared_only_deprecated` 或 prepared-only module channel 只描述历史状态，不能作为当前 package channel 语义。
...
| MAS | 默认 GHCR `one-person-lab-modules/med-autoscience`；dev/operator 可显式 override 到 git checkout / path / repo URL | 否 | domain agent 独立演进，按环境管理安装/更新 |
...
| MAG | 默认 GHCR `one-person-lab-modules/med-autogrant`；dev/operator 可显式 override 到 git checkout / path / repo URL | 否 | domain agent 独立演进，按环境管理安装/更新 |
| RCA | 默认 GHCR `one-person-lab-modules/redcube-ai`；dev/operator 可显式 override 到 git checkout / path / repo URL | 否 | 交付物链路可能较大，不应拖慢 App 更新 |
...
- `.github/workflows/packages.yml` 是 Framework module source archive / manifest publish workflow；它只允许手动 `workflow_dispatch`，发布 `one-person-lab-modules/*` 和 `one-person-lab-manifest` 到 GHCR，同时上传 workflow artifact；它不代表 MAS/MAG/RCA/OPL Meta Agent 各 repo 已经各自维护独立 GHCR/Packages 发布面。
...
- `scripts/package-release-discipline.mjs` 是 CI gate：检查 channel manifest、artifact build、checksum、rollback、旧版本清理策略与 active package channel/manual dispatch 语义是否齐全。缺 checksum、缺回退策略、缺清理策略，或误称 tag push 自动发布 / Framework-owned WebUI publish 时，package workflow 应直接失败。
...
- 默认 stable/App install/update 从 GHCR channel manifest 解析目标模块包；dev/operator 可用 `OPL_MODULE_SOURCE_MODE=git_checkout`、`OPL_MODULE_PATH_<MODULE_ID>` 或 `OPL_MODULE_REPO_URL_<MODULE_ID>` 显式改用本地 checkout、指定路径或指定 repo URL。
...
| `.github/workflows/packages.yml` | 手动构建 module source archive、运行 release-discipline gate、发布 module archive / release manifest 到 GHCR，并上传 release manifest / channel manifest / checksum artifact；不构建或发布 WebUI image。 | workflow run 状态、artifact URL、远端 package 可见性和本机 package build 结果。 |
...
| 模块源码包 | `ghcr.io/gaofeng21cn/one-person-lab-modules/<module>:<version>` | 不含 `.git`、缓存、venv、node_modules 的模块源码归档；当前 lifecycle 为 `active_release_channel` | stable/App install/update 通过 GHCR channel manifest 消费；dev/operator 可显式 override |
...
- stable/App 环境管理通过 GHCR channel manifest 判断目标模块版本；dev/operator override 模式下由显式 checkout/path/repo URL 决定来源。
...
- `OPL Meta Agent` 当前源码已纳入 Framework package manifest 的 active module package 集合；实际可拉取性以 workflow 发布结果和 GHCR package 可见性为准。
