...
本文冻结的是 Release / Packages / Full 首次安装包 / domain module 的 owner 与分发边界，不冻结具体版本、日期、包大小、sha256、branch/SHA、release artifact 状态、App release readiness、GitHub Actions 候选 cohort、updater metadata 或 Packages 消费状态。

当前状态必须从 live machine surface 读取：

- `opl packages manifest --json`：读取当前 OPL package manifest、module 坐标、`module_install_update_source`、`package_consumption_status`、WebUI/native helper/package refs。
- `npm run packages:manifest -- --version <opl_version>`：生成 release/channel manifest、module source archive 与 `SHA256SUMS`；该命令会写入输出目录，执行前确认输出路径和工作树边界。
- `npm run packages:release-discipline -- --manifest <manifest>`：验证 channel manifest、artifact build、checksum、rollback 与 cleanup discipline。
- `opl runtime app-operator-drilldown --json` 与 `opl framework readiness --family-defaults --json`：读取 App release/user-path refs-only evidence gate、release/prod claim flag 与 OPL authority boundary。
- `one-person-lab-app` 的 `contracts/app-release-channel.json`、`docs/release/README.md`、release evidence bundle、GitHub Release assets 和 updater metadata：读取标准包、Full DMG、VM smoke、one-shot installer 与 Docker/WebUI release truth。

本文的样例 JSON 只描述 manifest shape。任何带版本、日期、大小、sha256 或 receipt/cohort 的当前判断都应进入 `docs/history/process/plans/` 的 tranche ledger、runtime ledger 或 release evidence bundle，不写回本文作为长期事实。
...
本地完整工作树大小不能代表分发体积。分发时不带 `.git`、未跟踪文件、缓存、虚拟环境和构建产物。下面只保留历史源码归档体积 provenance，用于提醒“工作树大小 != 分发体积”；发布前必须重新用 `scripts/package-module-archives.mjs` / `opl packages manifest --json` / release evidence 读取当前版本、体积、sha256 和 source git。
...
下面是 shape example，不是当前版本或当前 release snapshot：
...
```json
{
  ...
  "release_automation": {
    ...
    "rollback": {
      ...
      "previous_version": "<previous_opl_version_or_null>",
  ...
  "modules": {
    "medautoscience": {
      ...
      "artifact": "ghcr.io/gaofeng21cn/one-person-lab-modules/med-autoscience:<opl_version>",
      ...
      "source_archive": {
        "file_name": "med-autoscience-<opl_version>.tar.gz",
        "size": 1234567,
        "sha256": "<sha256>"
      },
      "source_git": {
