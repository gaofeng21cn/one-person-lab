...
| OPL ScholarSkills | 默认 GHCR `one-person-lab-modules/opl-scholarskills`；Developer Mode 可显式切到 local checkout / path | 否 | Framework capability package，随 capability packages channel 安装和更新，供 MAS/MAG/RCA/OMA 等 agent 同步到 workspace / quest 使用 |
...
| 状态 | 能做什么 | 典型对象 | 不能声称 |
| --- | --- | --- | --- |
| `auto_apply` | clean managed target 可以由 Framework 下载、校验、stage、activate、post-apply 并写 receipt。 | `capability_packages`：MAS/MAG/RCA/OMA/OPL ScholarSkills package channel。 | domain truth、owner receipt、quality/export verdict、App release ready。 |
...
- `codex_exposure_status_adapter` 只投影由模块状态派生的 Codex skill/plugin 可见面和 reload guidance，不拥有 domain truth，也不是 `opl update apply --component` 的 mutation target。
...
- Codex plugin registry、plugin-packaged skills 和 OMA generated plugin surface 是 package update 后的 post-apply projection/reload guidance，不是第二套 domain truth，也不通过单独的 `codex_surface` apply 完成。
