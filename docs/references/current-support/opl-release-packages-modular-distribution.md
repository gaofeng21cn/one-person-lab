...
- First-party OPL agent package 的 dependency graph 来自对应 agent package manifest。MAS manifest 若声明 `mas-scholar-skills` 为 required capability package，OPL 只消费这条声明生成 channel/package/readback；不得把该依赖散落成 OPL 私有常量、App 专属列表或 hard-coded MAS 知识。
...
- Codex standalone 与 OPL managed 托管不是两套依赖真相。Standalone fat plugin 只是为了让原版 Codex App 独立安装时可用；OPL managed thin package 通过 manifest dependency graph 分开 materialize MAS package 与 `mas-scholar-skills` package，Developer Mode source checkout 只覆盖显式选中的 source channel。
...
- MAS/MAG/RCA/OPL Meta Agent/MAS Scholar Skills 的普通用户来源仍是 GHCR `one-person-lab-manifest` 与 `one-person-lab-modules/*` package channel；`stable/latest` tag 只作为 channel selector，安装回执必须记录 digest、sha256、source fingerprint 或 git head。
