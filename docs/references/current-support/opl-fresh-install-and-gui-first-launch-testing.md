...
执行面由 `gaofeng21cn/one-person-lab-app` 持有，并在 CI / 本地检出 `gaofeng21cn/opl-aion-shell` 到 `shells/aionui`。当前命令形状由 `contracts/opl-framework/fresh-install-test-matrix.json`、App release/testing 文档和 App-owned workflow 共同约束：
...
nightly/self-hosted Mac 入口属于 App repo workflow。runner labels、Tart source VM、guest SSH 用户、Node 注入方式、签名/公证模式、release profile 和 artifact 上传策略是 App-owned release/test truth；本页只保留 OPL 侧需要消费的合同字段和基础 evidence 字段。
...
完整 artifact set、文件名、上传路径、release cohort 和 readiness gate 归 App release/testing 合同、workflow 与 release evidence。当前 App Standard / Full release gate 还把 `artifacts/assistant-route-smoke-summary.json` 作为 VM evidence：它验证 packaged GUI 中 MAS、MAG、RCA purpose entry、badge、普通 selector 隐藏和 Codex CLI route receipt。该工件属于 App release evidence，不属于 OPL 主仓 CLI clean-room lane。
