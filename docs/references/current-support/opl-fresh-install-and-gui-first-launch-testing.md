...
无桌面 App 的机器使用 `opl install --headless --modules rca`（或其他显式 module selection）。该模式仍执行 Framework/Codex runtime、configured family runtime provider、选定 modules、Codex plugin/skills、companion tools 与 native helper 安装/修复，但不会下载 release DMG、挂载或复制 `.app`、也不会调用 App open。`--skip-gui-open` 保留为已有桌面安装流程的动作跳过选项，不是 headless 安装声明。
