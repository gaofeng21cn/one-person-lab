...
nightly/self-hosted Mac 入口属于 App repo workflow。runner labels、Tart source VM、guest SSH 用户、Node 注入方式和 artifact 上传策略是 App-owned release/test truth；本页只保留 OPL 侧需要消费的合同字段和工件清单。
...
App Standard / Full release gate 还把 `artifacts/assistant-route-smoke-summary.json` 作为 VM evidence：它验证 packaged GUI 中 MAS、MAG、RCA purpose entry、badge、普通 selector 隐藏和 Codex CLI route receipt。该工件属于 App release evidence，不属于 OPL 主仓 CLI clean-room lane。
