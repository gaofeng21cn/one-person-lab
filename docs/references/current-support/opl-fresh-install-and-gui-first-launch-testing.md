...
`opl install` 默认安装 headless OPL 基座；`opl install --modules rca` 在同一基座上显式加入 RCA。两者都不会下载或打开 App；只有 `--with-app` 才加入 GUI。宿主脚本可显式传 `--headless --skip-modules` 表达 base-only reconcile。
