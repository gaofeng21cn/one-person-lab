...
| --- | --- | --- |
| `opl-gui-shell-adapter-boundary.md` | GUI adapter 归属与 upstream-sync 边界 | 当前 `opl-aion-shell` 持有 GUI code；目标是 `one-person-lab-app/shells/aionui` 持有 upstream-backed GUI adapter。OPL 持有 CLI-backed runtime/contracts/projection surfaces。 |
...
| `opl-fresh-install-and-gui-first-launch-testing.md` | Fresh install 与 GUI 首启证据计划 | OPL 主仓持有 CLI clean-room truth；release App VM proof 留在 `opl-aion-shell`。 |
