...
| --- | --- | --- |
| `opl-gui-shell-adapter-boundary.md` | GUI adapter 归属与 upstream-sync 边界 | 当前 `opl-aion-shell` 持有 upstream-backed GUI adapter。OPL 持有 CLI-backed runtime/contracts/projection surfaces。 |
| `../../active/one-person-lab-app-repo-split-plan.md` | App repo 拆分和 AionUI shell 独立化 closeout | 固定 Framework repo、clean App repo、history-rich shell repo 和 AionUI upstream intake owner split。 |
...
修改操作支撑参考前，先确认底层 owner 是 OPL CLI/source/contracts、`opl-aion-shell`、App release artifact 还是 domain repo。行为变化应先更新 owner surface。
