...
| `opl-agent-capability-admission-audit-2026-07-07.md` | 标准 Agent 能力准入与 Skill 暴露审计 | 按能力模块先行、三层实现、按需暴露审计 OPL foundation、MAS、ScholarSkills、MAG、RCA、OMA、BookForge 和 App companion skills；只做人读治理，不声明 runtime/domain/release ready。 |
...
修改操作支撑参考前，先确认底层 owner 是 OPL CLI/source/contracts、`opl-aion-shell`、App release artifact 还是 domain repo。GUI 相关工作默认只推进 AionUI 主线或 Hermes Desktop foreground alternative；AGUI 相关材料只做归档读取或显式 replay。行为变化应先更新 owner surface。
