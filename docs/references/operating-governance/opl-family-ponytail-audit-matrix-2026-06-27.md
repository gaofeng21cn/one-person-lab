...
| Repo | Work package | Child candidates | Result | Fresh evidence | Residual |
| --- | --- | --- | --- | --- | --- |
| `med-autoscience` | `mas-paper-recovery-owner-callable-split-round58` | `tests/domain_action_request_materializer_cases/test_paper_recovery_owner_callable.py` paper-recovery callable / anti-loop owner-gate family | 将 anti-loop owner gate 与 execution dispatch cases 移入 `tests/domain_action_request_materializer_cases/test_paper_recovery_owner_callable_cases/`，原 entry 继续通过 explicit aggregate import 暴露。 | Pushed MAS commit `5547dd9621c4cf6189f7b8e2960305c52673d7c9`; focused aggregate passed 48/48 before rebase and 61/61 after rebase with collection hygiene; `git diff --check origin/main..HEAD` passed before absorption; post-round `scripts/run-python-clean.sh scripts/line_budget.py` reported 78 advisory issues. Line readback: entry 1015 -> 835 lines; new cases 187 and 216 lines. | Test/projection structure evidence only；不声明 paper progress、runtime readiness、publication readiness、owner receipt validity、typed blocker validity、provider readiness、current-package authority、App release readiness 或 production readiness。 |
| `med-autoscience` | `mas-ai-reviewer-publication-eval-command-split-round58` | `tests/test_cli_cases/ai_reviewer_publication_eval_command.py` identity / payload currentness guard family | 将 identity guard 与 payload currentness guard cases 移入 `tests/test_cli_cases/ai_reviewer_publication_eval_command_cases/`，原 CLI command entry 继续通过 explicit aggregate import 暴露。 | Same MAS commit `5547dd962`; focused aggregate included in 61/61 verification；collection hygiene passed 13/13；collect-only readback reported 61 tests collected across selected aggregate entries. Line readback: entry 1188 -> 925 lines; new cases 153 and 270 lines. | Test/projection structure evidence only。Reopen only if this CLI publication-eval guard family regrows or fresh failures point to this split boundary. |
| `med-autoscience` | `mas-runtime-health-kernel-split-round58` | `tests/test_runtime_health_kernel.py` explicit resume / new-run grace / reconcile activity-timeout family | 将 explicit-resume、new-run grace 与 reconcile/activity-timeout cases 移入 `tests/test_runtime_health_kernel_cases/test_*_cases.py`，原 runtime health entry 继续通过 explicit aggregate import 暴露。 | Same MAS commit `5547dd962`; focused aggregate included in 61/61 verification；line-budget advisory count reduced from 81 to 78. Line readback: entry 1116 -> 885 lines; new cases 126、160、122 lines. | Runtime-health test structure evidence only；不声明 runtime truth、worker readiness、provider long-soak、domain progress 或 production readiness。 |
| `med-autoscience` | `mas-nested-case-collection-hygiene-round58` | `tests/conftest.py`、`tests/test_collection_hygiene.py`、existing provider-admission nested case marker | 补齐新 nested case families 的 `collect_ignore_glob` / aggregate coverage / explicit import hygiene，并给既有 provider-admission nested case 增加 marker classification，防止 case split 后默认 pytest 重复收集或漏收集。 | Same MAS commit `5547dd962`; `tests/test_collection_hygiene.py` passed 13/13；collect-only readback reported 61 tests collected from selected aggregate entries；MAS root `main...origin/main` clean/current and Round 58 worktree/branch removed. | Collection hygiene evidence only；不改变 provider-admission behavior，也不关闭 unrelated behavior failure或 domain authority tail。 |
