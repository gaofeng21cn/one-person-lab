...
| `needs_behavior_fix_first` | Required display catalog coverage / renderer registry mismatch | MAS publication/display registry owner | Baseline root failure in `tests/medical_publication_surface_cases/figure_narrative_and_renderer_contracts.py`. | `scripts/run-pytest-clean.sh tests/medical_publication_surface_cases/figure_narrative_and_renderer_contracts.py -q`. | Existing failure on unmodified root: `2 failed, 14 passed`. Do not hide this under fixture cleanup; fix as behavior/registry lane. |
