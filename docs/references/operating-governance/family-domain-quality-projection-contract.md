...
`OPL` read-model、worklist、App/operator drilldown、refs-only receipt 或 `projection_only` summary 只能说明投影、路由、freshness 和可见性状态。即使当前没有 open safe-action route，或某个 refs-only receipt 已被 record / verify，也不能写成 MAS paper closure、MAG fundability / submission readiness、RCA visual ready / export proof、domain ready、production ready、artifact authority、quality/export verdict 或 publication judgment。质量门只能由对应 domain-owned eval / proof / package pointer 和 `assessment_owner=domain_owned` 关闭。
