...
2026-05-26 live read-model 口径：`opl runtime app-operator-drilldown --json` 当前可读，provider cadence/capability SLO satisfied，App user-path production user path ready，但 `app_release_user_path_release_ready_claimed=false`、`app_release_user_path_production_ready_claimed=false`；Codex App 只承担 start / observe / intervene / display，不驱动 long-running task loop。Operator projection 的角色是展示 OPL / Temporal / domain-owned refs 和 next gate，不关闭 domain owner-chain、quality gate、artifact authority、memory writeback apply、App release 或 production ready。
