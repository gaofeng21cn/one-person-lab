...
2026-05-28 live read-model 校准：本目录仍是 support reference，不是 active truth owner。Fresh `opl agents conformance --family-defaults --json` 读为 4/4 structural pass；`opl stages list --json` 读为 4 个 resolved planes、19 个 admitted stages、0 blocked stages；`opl stages readiness --family-defaults --json` 读为 `launch_warning`、hard blocker 0、warning 78，且 `can_claim_domain_ready=false`、`can_claim_artifact_authority=false`、`can_claim_production_ready=false`；`opl agents default-callers --family-defaults --json` 读为 32 个 generated/default caller surfaces、0 blocked，但状态仍是 `ready_domain_evidence_required`。同轮 `framework readiness`、`family-runtime evidence-worklist` 和 `app-operator-drilldown` 聚合命令在当前 checkout fail-closed，错误为 `Unexpected non-whitespace character after JSON at position 664920`，因此不能继承旧 frozen counters 或把上轮 worklist / App projection 状态写成当前完成事实。本文档组只能作为 lifecycle、定位收敛、shared release、stage descriptor adoption 和文档 intake 支撑，不授权 domain ready、production ready、App release ready、quality/export verdict、artifact authority、memory body/apply、owner receipt closeout 或 physical delete。
