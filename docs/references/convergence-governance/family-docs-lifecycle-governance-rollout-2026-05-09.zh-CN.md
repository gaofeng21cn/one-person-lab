...
- `OPL` 可以使用 `docs/active/`、`docs/public/`、`docs/specs/` 来表达 activation 与 shared boundary。
- `MAS` 可以使用 `docs/runtime/`、`docs/program/`、`docs/capabilities/`、`docs/policies/` 来表达科研 runtime、program 队列、能力族与规则层。
- `RCA` 可以使用 `docs/product/`、`docs/runtime/`、`docs/delivery/`、`docs/source/` 来表达视觉交付 domain 的产品、运行、交付和来源层。
- `MAG` 可以继续保留 path-stable `docs/specs/*.md`，但必须用 README/index 层把 active specs、reference specs 和 historical specs 分清；当 current-program 和历史审计不再引用旧路径时，再做物理迁移。

标准要求的是 role-equivalent information architecture，不是机械统一目录名。
...
MAS 已完成 2026-05-09 docs lifecycle restructure。它是当前 family 的完整参考实现：

- `docs/runtime/` 拆为 contracts、control、projections、display、designs。
- `docs/program/` 保持当前执行队列与 program index。
- `docs/capabilities/` 承接能力族材料。
...
- 新的 active work 不再继续堆进 dated specs 根层；应进入 current owner surface、plans、references 或 history。
