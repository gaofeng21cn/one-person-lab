...
> 2026-05-14 current note: 本文保留 2026-04-14 snapshot。当前长期目录名以
> `active/public/product/runtime/delivery/source/policies/specs/references/history`
> 为准；MAS 旧 `program` / `capabilities`、MAG 旧 `plans`、RCA 旧
> `program` 能迁移就直接迁入 canonical 目录，暂留旧路径只作历史 provenance、
...
- 系列定位：stage-led、以 Agent executor 为最小执行单位的 OPL family agent framework
- 公开/开发骨架：根层 `README*` + 中文 `docs/README.md` + 核心五件套
- 内部对齐重点：canonical docs taxonomy、reference-grade supporting docs 与 admitted-domain delta intake
- 默认 docs 审计：`scripts/verify.sh meta`

### Med Auto Science

- 系列定位：`Research Ops` medical research domain agent
- 公开/开发骨架：根层 `README*` + 中文 `docs/README.md` + 核心五件套
- 内部对齐重点：`docs/runtime/`、`docs/active/`、`docs/delivery/medical-display/`、`docs/references/`、`docs/policies/`
- 默认 docs 审计：`scripts/verify.sh meta`

### Med Auto Grant

- 系列定位：医学 `Grant Ops` author-side / proposal-facing domain agent
- 公开/开发骨架：根层 `README*` + 中文 `docs/README.md` + 核心五件套
...
- 系列定位：visual-deliverable domain agent
- 公开/开发骨架：根层 `README*` + 中文 `docs/README.md` + 核心五件套
...
- 再看根层 `README*`、`docs/README.md` 与核心五件套是否继续显式链接、显式分层
