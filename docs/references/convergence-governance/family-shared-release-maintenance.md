...
Currentness policy：本文的 `Current Contract And Live Alignment` 是 owner-pin / drift-attention reference，不是当前 App release、domain production release 或 family production-ready claim，也不冻结某次 consumer pin 检查结果。shared release alignment 只证明 shared helper / contract / consumer pin 一致；它不授权 MAS/MAG/RCA/OMA domain ready、quality/export verdict、artifact authority、owner-chain closeout、App release-ready 或 production-ready。具体 aligned / stale consumer、owner commit 和 pin hash 必须从 `contracts/family-release/shared-owner-release.json` 与 fresh `npm run family:shared-release -- check` 读取，并把单轮结果记录到 ledger。
...
- owner commit source：`contracts/family-release/shared-owner-release.json#/owner_commit`
- package locator source：`contracts/family-release/shared-owner-release.json#/packages`
- live check command：`npm run family:shared-release -- check`
- alignment state source：command exit `0` means all declared consumers are aligned；non-zero output is `drift_attention` and must be read per consumer / target file before claiming closeout
- current-state source of truth：rerun the live check command before claiming alignment, drift, stale pins or closeout
