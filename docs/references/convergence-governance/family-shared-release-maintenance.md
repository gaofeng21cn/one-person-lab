...
- latest audited alignment at 2026-05-28 CST：`med-autogrant` and `redcube-ai` aligned；`med-autoscience` still pins `e3fd0b6be41e858958d42ea400a3e63c4205ff8a` in `pyproject.toml` and `uv.lock`
- current state：`drift_attention`
- current-state source of truth：rerun the live check command before claiming alignment or closeout
