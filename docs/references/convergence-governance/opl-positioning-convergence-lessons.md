...
Currentness policy：本文是 support reference，不是 active truth owner，也不冻结任何一次 CLI/read-model 计数。定位、术语和 owner-boundary 方法可以复用；current roadmap、release、production、artifact、quality、owner-chain、framework readiness、family-runtime worklist 和 App/operator projection 必须回到核心五件套、contracts、source、tests、runtime ledger、provider receipt、domain-owned manifest、App/workbench projection 和 fresh CLI/read-model。

## 用途

这份参考材料沉淀两次仓库调研的核心经验：

- 从 commit 历史看 `OPL` 如何从 gateway / federation 蓝图，逐步收敛为 `Codex CLI first-class executor + explicit activation layer + stage-attempt request/projection / Temporal-backed provider + family shared modules/contracts/indexes`。
...
Read-model guard：`agents conformance`、`stages list|readiness`、`agents default-callers`、`framework readiness`、`family-runtime evidence-worklist` 和 `app-operator-drilldown` 的输出都必须 fresh read。它们即使返回 structural pass、admission pass、readiness warning、replacement surface、operator attention 或 refs-only worklist，也只证明对应机器面当下可读；不能从本文继承旧 counters、旧 fail-closed 状态或旧 App/operator projection。本文所有历史经验都只能用来做定位收敛和污染防线，不授权 current roadmap、release、production、artifact、quality 或 owner-chain 完成声明。
...
- Temporal-backed provider 是 production online runtime 的必需 substrate；`OPL Runtime Manager` 做产品级 provider profile、stage-attempt request/projection、诊断、恢复入口、状态投影和 native helper catalog。
- `hermes_agent`、`claude_code` 与 `antigravity_cli` 同属 canonical 显式非默认 executor adapter/backend，只承诺接口连接、生命周期、回执、审计和 fail-closed；不承诺行为、质量、工具语义或 resume 与 `Codex CLI` 等价。旧 Hermes provider / Gateway / readiness / compatibility surface 只作为 history、provenance、diagnostic、fixture 或 negative guard 阅读。
- One Person Lab App / GUI 是用户工作台 consumer，主仓提供 CLI/API-backed machine-readable surfaces、release/install surface 与 runtime truth；App 只 start / observe / intervene / display，不驱动 long-running task loop，也不持有 domain truth、artifact body 或 quality verdict。
...
- 用户入口：`opl` CLI、One Person Lab App / external shell、WebUI / ACP-compatible shell。
- 开发编排：worktree、OMX、subagent、CI、release scripts。

`OPL` 的当前收敛方式是：

- 主仓保留 runtime truth、CLI、contracts、release/install surface。
- App / GUI shell 通过 CLI/API-backed machine-readable surfaces 消费 OPL。
- 长跑 stage runtime 由 configured family runtime provider 承担；当前 production online substrate 是 Temporal-backed provider，`OPL Runtime Manager` 做 provider profile、stage-attempt request/projection、诊断和投影。
...
- `hermes_agent` / `claude_code` / `antigravity_cli`：canonical 显式非默认 executor adapter/backend；OPL 只保证接口连接、生命周期、receipt、audit 和 fail-closed，不承诺质量或行为与 `Codex CLI` 等价。
...
- Full first-install：首次安装资产，可带 MAS/MAG/RCA/OMA、OfficeCLI、companion skills，但不进入 `latest*.yml`；`hermes_agent` 只作为显式非默认 executor adapter，MDS 只作为 MAS 显式 provenance/backend/audit/oracle companion，不进入默认 Full payload。
