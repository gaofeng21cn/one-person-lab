...
2026-05-26 live read-model guard：`framework readiness` 读为 `framework_control_plane_available_with_blocked_refs_only_attention`，hard blocker 为 0，但 domain-blocked attention 与 refs-only evidence envelope 仍在；`agents conformance` 4 repo structural pass 不等于 physical thinning complete；`app-operator-drilldown` 可用但 release/prod ready claimed 都是 false；`family-runtime evidence-worklist` zero-open 仍不授权 domain ready 或 production ready。本文所有历史经验都只能用来做定位收敛和污染防线，不授权 current roadmap、release、production、artifact、quality 或 owner-chain 完成声明。
