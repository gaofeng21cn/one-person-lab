...
- Full first-install：首次安装资产，可带 MAS/MAG/RCA、可选 Hermes provider-adapter payload、OfficeCLI、companion skills，但不进入 `latest*.yml`；MDS 只作为 MAS 显式可选 companion，不进入默认 Full payload。
