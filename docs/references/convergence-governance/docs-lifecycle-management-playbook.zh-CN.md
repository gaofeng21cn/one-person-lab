...
旧 `gateway / federation / routed-action` 文档曾经是 OPL 的顶层公开集成语言。当前主线已经是 `Codex-default session/runtime + explicit activation + domain agent skill entry`，所以旧语料进入 `docs/history/compatibility/gateway-federation/`。active 文档只把它称为历史兼容材料，并指向 `contracts/opl-gateway/` 的 machine-readable compatibility surface 或当前 core truth。
