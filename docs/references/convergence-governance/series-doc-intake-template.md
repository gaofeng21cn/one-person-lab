...
2026-05-26 读法：当前 active family coverage 必须显式区分 `one-person-lab` 的 framework/App/operator projection、MAS/MAG/RCA 的 domain-owned truth，以及 OMA/new-agent builder/tester 线路。涉及 OMA、App release、provider SLO、long-soak、memory/artifact lifecycle 或 physical thinning 时，应在“影响仓范围”和“验证结果”中单独写 owner、read-model、receipt/typed-blocker refs 与 non-authority boundary，不能用模板行数、conformance pass 或 zero-open worklist 代替证据。
