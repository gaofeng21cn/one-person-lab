...
- `submit_stage_attempt_request(request, opl_bin=None, timeout_seconds=120, runner=None)`：请求必须包含 `domain_id`、`stage_id`、非空 `workspace_locator` 和 `start=true`；可带 `action_id`、`source_fingerprint`。client 固定调用 `family-runtime attempt create --provider temporal --start`；运行端仍校验 Workspace、Stage 和执行身份，client 不复制准入逻辑或专业判断。
