...
`opl-framework-shared/stage-run-orchestration` 是 StageRun Kernel 的纯 refs event reducer，主模块是 `OPL Stagecraft`，协同模块是 `OPL Runway`。Domain agent 以声明式 cycle manifest 提供 canonical `target_agent_ref`、`descriptor_ref`、`run_ref`、stage binding 和 cycle / attempt 预算；generated control-plane adapter 复用 canonical `normalizeFamilyStageControlPlane`，校验 canonical Agent alias 与 control-plane domain owner 一致，把 `plane_id`、`target_domain_id`、owner 和完整 normalized plane fingerprint 绑定进 manifest identity，并注入唯一 OPL canonical launch owner 与 Agent runner ref，不接受 caller 自填第二 scheduler。Manifest identity 由 target、descriptor、run、control-plane binding、input、stage binding 和 budget refs 稳定派生，并在读取时用同一 helper 重算，caller 不能另填第二身份。每个 route/effect event 必须同时绑定 `manifest_id`、`stage_run_id`、`cycle_index` 与 `attempt_index`，跨 run、跨 cycle 或跨 attempt 重放 fail closed。Reducer 只接受 manifest 与有序 route/effect events，并从头重建投影，不接收 caller mutable state；route decision 与 effect status 只接受各自 discriminant 允许的字段，夹带会被忽略的 checkpoint、blocker 或 output refs 一律 fail closed。`validateStageRunPersistedState` 只用同一 replay 结果校验外部 persisted projection，没有 active persistence consumer，因此不能宣称 runtime persistence enforcement 已落地。当前没有 canonical admission consumer 的 no-progress signal 只属于 reducer 外 advisory，不进入 manifest/effect/status，也不能阻断或耗尽 StageRun。
