...
`opl-framework-shared/stage-run-orchestration` 是 StageRun Kernel 的通用执行扩展，主模块是 `OPL Stagecraft`，协同模块是 `OPL Runway`。Domain agent 以声明式 cycle manifest 提供 canonical `target_agent_ref`、`descriptor_ref`、`run_ref`、允许的 stage 到 single-pass handler 绑定，以及 cycle / attempt / no-progress 预算；generated control-plane adapter 复用 canonical `normalizeFamilyStageControlPlane`，直接从验证后的 `FamilyStageControlPlane.stages[].stage_id` 形成 binding，不在 OPL 手抄 MAG/RCA 等 domain stage。Manifest identity 由 plane、target、descriptor 和 run refs 稳定派生，caller 不能另填第二身份。`target_agent_ref` 必须来自标准 Agent identity，不能从 `target_domain_id` 推断。domain route oracle 只回答 `dispatch / accepted / rollback / blocked` 和相应 refs。OPL 负责重复调用单次 handler、按预算重试、冻结 no-progress、投影 exhaustion、验证 accepted / rollback checkpoint、生成跨 target 隔离的 StageRun / step identity，并把每个 step 放入稳定的 `<output_root>/<stage_run_id>/cycle-*/attempt-*` 目录。

Typed runner dispatch 只接受显式 executor kind/ref、argv、env 和 input/output refs。argv 只持久化 fingerprint；env 只持久化 key 与绑定 value 的不可逆 fingerprint，不持久化 value。raw stdout/stderr 默认不落盘，只记录 SHA-256 与 byte count；因此 domain CLI 即使把 workspace body 写到 stdout，也不能进入 OPL receipt。同一 step idempotency key 已有同 dispatch receipt 时直接回放，env value、command、timeout 或声明 refs 任一变化都会 fail closed；重放前会重新校验 receipt envelope、identity、dispatch binding、refs 与日志 metadata，损坏 JSON 不能触发覆盖重跑。任意 callback handler 本身不被 OPL 假装成可重放执行：有副作用的 handler 必须调用 typed runner dispatch，或由 domain 自己提供幂等入口。每个 step 的 `output-manifest.json`、single-pass / runner receipt 和最终 `stage-run-closeout.json` 都是 refs-only transport evidence：它们可以记录 process exit、output refs、checkpoint refs、single-pass closeout refs、domain 已提供的 typed blocker / human gate refs，但不能携带 artifact、workspace、domain result、quality verdict、owner receipt 或 typed blocker body。Checkpoint-only 成功不会丢弃合法输入 refs；blocked route 只合并既有与 owner 新提供的 blocker/gate refs。Process/handler completion、目录存在、manifest 写入和 OPL exhaustion 都不关闭 domain stage，也不授权 domain ready；最终 Stage current pointer 与 terminal state仍只由 Stage Transition Authority 消费合法 domain owner answer 后派生。
