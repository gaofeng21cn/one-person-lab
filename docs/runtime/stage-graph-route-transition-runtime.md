...
`stage_run_id` 只由 `domain_id + stage_id + stage_run_invocation_id` 派生。`stage_run_spec_sha256` 单独绑定不可变的 pack closure、Stage manifest、quality policy、source/checkpoint/input artifact hash、rubric、lineage、executor 与 parent route；其中 prompt、rubric、policy、manifest、pack-local source/lineage 必须记录实际文件 bytes 的 SHA-256，全部 input artifact hash 必须是规范 SHA-256，checkpoint 必须绑定 input artifact hash 或 ref 内嵌 SHA-256，package closure 必须带 root `content_digest`。普通 ref 本身不算 immutable binding。launch registration/replay 和每次 Attempt materialization 都会 fresh 回读 pack bytes 并核对这些 digest，注册后原位更新 checkout 会在进入 provider/executor 前 typed fail closed。`checked_at`、`use_receipt_ref`、checkout path、currentness receipt 等波动观察不参与 Run ID 或 spec hash。

Runway 在启动 Temporal 前，先把 exact StageRun input 写入 `${OPL_STATE_DIR}/family-runtime/queue.sqlite#stage_run_launches`。真正调用 provider 前再用 `BEGIN IMMEDIATE` 做 `registered|start_failed -> starting` compare-and-swap。`starting` 明确表示 provider start 的结果未知：进程可能尚未调用、调用失败，或 Temporal 已接受但本地 receipt 尚未持久化，因此恢复时允许按同一个 deterministic `workflow_id` 重投 start RPC。RPC 是 at-least-once；Temporal 的 `USE_EXISTING + REJECT_DUPLICATE` 才是 exactly-one StageRun execution 的去重边界。并发 success/failure 回写必须保持 `started` 不降级为 `start_failed`，`closed` 永久终局且 late receipt/failure 不得重开。同 invocation + 同 spec 的 running 或 closed Run 幂等返回；同 invocation + 不同 spec 以 `stage_run_invocation_spec_conflict` 失败，不得覆盖原输入。
