...
在一个 StageRun 内，primary-only Stage 的 producer，或启用正式 Review 后真正 terminalize 的 reviewer / re-reviewer，提交 `route_impact.stage_route_decision`；其他 Attempt 最多提交 recommendation。hard stop 只提交 typed blocker 或 human gate，不能再夹带跨 Stage route。这个 closeout 输出是 Codex 的一次语义判断，不把 Stage graph、current pointer 或 transition authority 下放给 Attempt。OPL 只校验输出者资格、route ABI、finding closure 与 declared target。
