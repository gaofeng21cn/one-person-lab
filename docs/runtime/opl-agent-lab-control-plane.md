...
| Work-order primitive | 执行 OMA developer patch work order，拥有 target worktree lifecycle、Codex execution、verification、absorption、cleanup 和 refs-only receipt；旧 `opl agent-lab execute-work-order` alias 已退役。 | `src/agent-lab-work-order-execution.ts`、`tests/src/cli/cases/work-order-execution.test.ts`、`opl work-order execute --work-order <developer-patch-work-order.json> --json`。 |
