...
可执行 developer work order 的运行 primitive 归 OPL Framework。canonical CLI 是 `opl work-order execute --work-order <developer-patch-work-order.json>`；`opl agent-lab execute-work-order` 只作为 Agent Lab self-evolution / external-suite 消费入口和兼容 alias。该 primitive 会校验 OMA work order、Codex CLI executor lease、target checkout cleanliness 与 `.worktrees` ignore policy，然后由 OPL 在目标仓打开临时 worktree，调用 Codex CLI 执行 source patch，运行显式和推导出的 verification command，提交 patch commit，fast-forward 吸收到目标 base branch，最后清理临时 worktree / branch 并写 refs-only execution receipt。失败时 OPL 必须 fail-closed、写 failure cleanup receipt，并强制清理自己打开的 target worktree / branch。
...
- CLI work-order primitive：`opl work-order execute --work-order <developer-patch-work-order.json> [--target-agent-dir <dir>] [--suite <suite.json>] [--output-dir <dir>] [--verification-command <command>] --json`，把 OMA developer patch work order 交给 Codex CLI 在目标临时 worktree 中执行，完成 target verification、commit、fast-forward absorption、worktree / branch cleanup 和 refs-only receipt。该命令只拥有 source patch execution / verification / absorption / cleanup lifecycle，不拥有目标 truth、artifact、memory、quality/export verdict 或 owner receipt authority。`opl agent-lab execute-work-order` 保留为 Agent Lab 兼容 alias，输出会标记其为 `agent_lab_alias=true`，不能被理解为 Agent Lab 拥有通用执行器或 target worktree lifecycle。
