...
- CLI efficiency：`opl agent-lab efficiency --json`，输出通用 `opl_agent_lab_efficiency_nonregression_read_model`，从 generic suite refs 和显式输入 refs 汇总效率非回归证据；complete/workbench 同步投影该 read model 的 readiness。
