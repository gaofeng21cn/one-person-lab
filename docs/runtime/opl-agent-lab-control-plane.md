...
`developer_mode_repair_routes.live_closeout_evidence` 是 Developer Mode / Agent Lab repair closeout 的 refs-only 读模型。它固定两类 base drill：具备 repo developer 权限时的真实 repo direct-fix route，以及无 owner 写权限时的 non-owner fork / pull request route。每条 drill 只表达 `route_eligibility`、patrol observation、diff、focused verification、no-forbidden-write、commit 或 fork/PR、external owner acceptance 或 repo contract fixture refs；OPL 不把这些 refs 升级成 owner receipt，也不把 route closeout 写成 managed runtime truth 或 domain truth。缺少外部 owner acceptance ref，或 fork/PR owner acceptance ref 未绑定 GitHub PR 时，closeout 只能停在 evidence incomplete、owner gate route 或明确的 repo fixture drill，不得由 OPL 伪造 owner acceptance。机器面必须同时暴露 `closeout_claim_status` 与 `owner_acceptance_status`，让 repo fixture drill 固定读成 `fixture_drill_owner_acceptance_open` / `fixture_drill_not_owner_acceptance`，不能被 consumer 当成 external owner closeout。`live_external_owner_acceptance_count` 只统计 verified Developer Mode closeout ledger 中的 live external owner refs；静态 direct-fix drill 即使带有示例 external ref，也只表示 drill coverage，不进入 live acceptance 计数。

同一 read model 现在还投影 `scaleout_followthrough`。base direct-fix 与 fork-PR verified receipt 都出现前，它读为 `waiting_for_base_live_route_closeout_refs`；base route ready 后，它按 `route_repetition_refs`、`risk_tier_auto_promotion_refs` 和 `app_patrol_mount_refs` 三类 follow-through 计算 open gate。`route_repetition_refs` 可以来自 verified ledger payload，也可以由 Agent Lab 从多个 verified live ledger closeout receipts 的 target repo 或 patrol observation 覆盖派生为 body-free `developer-mode-route-repetition-ref:*`；App patrol mounting 必须由显式 verified ledger ref 或同等 typed blocker 关闭；risk-tier auto-promotion 必须由 first-class Agent Lab promotion evidence 关闭，且 evidence 需要证明真实 failure/evidence delta、独立 AI review、rollback target、canary observation 和 no-forbidden-write boundary，不能用未校验字符串、fixture、scorecard pass、provider completion 或只有 rollback/canary 的弱 ref 填充。App/operator attention 与 framework readiness 会把缺失的 scaleout refs 作为 record guidance，而不是把 base route closeout 误写成 Developer Mode global closeout。当前本机 App patrol mounting 已由 `opl://developer-mode-closeout/one-person-lab/app-patrol-mount-2026-05-29` verified receipt 投影为 `app-patrol-mount-ref:one-person-lab-app/developer-mode-live-closeout-evidence-fast-full-20260529`，因此 scaleout open gate 只剩 `risk_tier_auto_promotion_refs`。scaleout refs 的含义分别是重复 direct-fix 或 fork-PR closeout 的 route repetition、带独立 AI review / rollback / canary 的 risk-tier promotion evidence，以及 App/default caller 可见的 patrol mounting refs；它们都是 follow-through evidence，不是 owner receipt、release-ready verdict、domain-ready verdict 或 production-ready verdict。
...
- Developer Mode scaleout follow-through refs：显式或派生 `route_repetition_refs`、`risk_tier_auto_promotion_refs` 与 `app_patrol_mount_refs`。
...
- CLI efficiency：`opl agent-lab efficiency --json` 输出默认通用 `opl_agent_lab_efficiency_nonregression_read_model`；`opl agent-lab run/efficiency --suite <suite.json> --json` 先运行外部 suite，再从 suite result 与 refs-only handoff 投影效率非回归 read model。两者都只汇总 generic suite refs / handoff refs / 显式输入 refs；complete/workbench 同步投影该 read model 的 readiness。
- CLI stage executor policy：`opl agent-lab stage-executor-policy --json`，输出 refs-only stage executor policy read model。该 surface 记录默认 `codex_cli` baseline、显式非默认 backend set、RCA HTML route 上 `antigravity_cli` + `gemini-3.5-flash` + `high` 的 controlled-trial candidate、必需 test refs、binding-ref gate、no-equivalence policy 和 typed blocker。它只支持后续比较与测试设计，不直接启动非默认 executor，不把速度或连接 receipt 写成质量、工具语义或 resume 等价，也不切换默认 executor。
...
- CLI work-order primitive：`opl work-order execute --work-order <developer-patch-work-order.json> [--target-agent-dir <dir>] [--suite <suite.json>] [--output-dir <dir>] [--verification-command <command>] --json`，把 OMA developer patch work order 交给 Codex CLI 在目标临时 worktree 中执行，完成 target verification、commit、fast-forward absorption、worktree / branch cleanup 和 refs-only receipt。该命令只拥有 source patch execution / verification / absorption / cleanup lifecycle，不拥有目标 truth、artifact、memory、quality/export verdict 或 owner receipt authority。Agent Lab 只能读取该 canonical receipt 与 re-evaluation refs；旧 `opl agent-lab execute-work-order` alias 已退役，保留负向 guard 防止兼容入口复活。
...
`opl agent-lab run --suite <suite.json> --json` 是当前外部 suite 闭环入口。它允许 `opl-meta-agent` 这类独立 Foundry Agent 生成候选 agent 的 baseline suite，再交回 OPL Agent Lab 运行。该入口只读取 suite JSON 中的 refs、scorecard refs、recovery probes、trajectory refs、improvement candidate refs、promotion gates 和结构化 independent AI reviewer receipt refs；它不会读取或写入 domain truth、memory body、artifact body 或 owner receipt body。suite passed 只能进入风险分级 promotion pipeline；低/中风险自动推广必须同时具备真实 failure/evidence delta、independent AI review assessment、promotion receipt、tests/canary/no-forbidden-write/provenance 和 rollback，缺 delta 或缺 receipt 时只能停在 `regression_guard_only`、`review_pending` 或 `blocked_from_auto_promotion`，高风险必须 owner/human gate。

domain agent production evidence suite 是这个 external suite 入口的 refs-only 特化输入。OPL Agent Lab 只识别 `agent_production_evidence_suite` 的 `production_evidence_gate` 中的 gate ids、owner route、no-forbidden-write proof、typed blocker、required receipt 和 gate result refs，把它们作为 `production_evidence_gate_result` 回传给声明兼容的 domain agent 或 `opl-meta-agent` 接力；domain owner receipt、domain truth、quality verdict、artifact body、memory body 仍只能由对应 domain owner surface 产生。
...
| Evolution harness | `next_mechanism_candidate_ready` | `opl agent-lab evolve --suite ... --json` 输出 mechanism evolution segment，不写 domain truth/memory/artifact，不训练权重；低/中风险只有在 promotion safety assessment 通过后自动推广，高风险路由 owner/human。 |
| Risk review / version ledger | `auto_evolution_gate_ready` | 真实 failure/evidence delta、独立 AI reviewer、direct evidence、promotion receipt、no-forbidden-write、version ledger、canary 和 rollback 是常态 gate。 |
...
| Runtime-budget usage/cost companion | `schema_ready_refs_only` | `family-stage-runtime-budget` 可选消费 usage/cost estimate refs、model pricing refs、calibration refs、visual-stage profile refs 和 artifact locator refs；它只是 runtime budget 的预算 projection，不授权 spending、domain ready、quality verdict、artifact mutation 或 owner receipt。 |
...
| Optimizer external suite | `review_pending` / `gated_candidate_set_ready` | `opl agent-lab optimize --suite ... --json` 输出 gated candidate、risk review、version ledger 和 transition refs，不训练/部署权重；缺真实 failure/evidence delta 或 independent AI review assessment 时只停在 `regression_guard_only`、`review_pending` 或 `blocked_from_auto_promotion`，promotion safety refs 齐全后才允许按风险 gate 推广。 |
...
4. Optimizer / RL 只生成 candidate config、candidate branch、transition refs 和 risk-classified promotion refs；低风险在真实 failure/evidence delta、independent AI approve、promotion receipt、rollback 与 no-forbidden-write refs 齐全后可 auto-promote，中风险还需 tests pass 和 canary，高风险只路由 owner/human gate，不允许 OPL 直接改写 domain truth、artifact、memory body、memory accept/reject、credential/network/write policy 或 owner verdict。
