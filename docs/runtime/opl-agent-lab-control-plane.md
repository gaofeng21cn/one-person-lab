...
可执行 developer work order 的运行 primitive 归 OPL Agent Lab。`opl agent-lab execute-work-order --work-order <developer-patch-work-order.json>` 会校验 OMA work order、Codex CLI executor lease、target checkout cleanliness 与 `.worktrees` ignore policy，然后由 OPL 在目标仓打开临时 worktree，调用 Codex CLI 执行 source patch，运行显式和推导出的 verification command，提交 patch commit，fast-forward 吸收到目标 base branch，最后清理临时 worktree / branch 并写 refs-only execution receipt。失败时 OPL 必须 fail-closed、写 failure cleanup receipt，并强制清理自己打开的 target worktree / branch。

这条 primitive 是执行器和 target worktree lifecycle owner，不是 domain authority owner。它只能执行 OMA work order 允许的 target source/test/docs patch，并输出 target verification、patch absorption、cleanup、no-forbidden-write 和 target owner receipt-or-typed-blocker refs；它不能写目标 domain truth、visual truth、artifact body、memory body、quality/export verdict 或 owner receipt，也不能把 Agent Lab re-evaluation score 写成目标 quality verdict。OMA 侧只能生成 work order 或薄委托到该 OPL primitive，不拥有 generic runner、queue、target worktree lifecycle、absorption 或 cleanup。
...
- CLI execute work order：`opl agent-lab execute-work-order --work-order <developer-patch-work-order.json> [--target-agent-dir <dir>] [--suite <suite.json>] [--output-dir <dir>] [--verification-command <command>] --json`，把 OMA developer patch work order 交给 Codex CLI 在目标临时 worktree 中执行，完成 target verification、commit、fast-forward absorption、worktree / branch cleanup 和 refs-only receipt。该命令只拥有 source patch execution / verification / absorption / cleanup lifecycle，不拥有目标 truth、artifact、memory、quality/export verdict 或 owner receipt authority。
