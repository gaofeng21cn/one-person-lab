...
- CLI optimize：`opl agent-lab optimize --suite <suite.json> --json`，运行外部 suite 后输出 risk-classified optimizer candidate set、review refs、version ledger refs 和 RL transition refs。该命令不训练或部署权重，不写 memory body，也不覆盖 domain owner verdict；fixture/domain scorecard pass 只代表 suite gate 通过，只有真实 `independent_ai_review_assessment` 带 reviewer/provenance/no-shared-context/no-forbidden-write evidence 且 approved 时，低/中风险候选才可进入自动推广，高风险只输出 owner/human gate route。
- CLI evolve：`opl agent-lab evolve --suite <suite.json> --json`，运行外部 suite 后输出 mechanism evolution segment、meta edit receipt、evidence delta 和 next mechanism candidate。candidate refs 会进入同一套风险分级 promotion pipeline；缺真实 independent AI reviewer receipt 时只能 `review_pending` 或 `blocked_from_auto_promotion`，低/中风险自动推广必须由真实 AI review assessment 授权，高风险不得由 OPL 自动推广。
...
`opl agent-lab run --suite <suite.json> --json` 是当前外部 suite 闭环入口。它允许 `opl-meta-agent` 这类独立 Foundry Agent 生成候选 agent 的 baseline suite，再交回 OPL Agent Lab 运行。该入口只读取 suite JSON 中的 refs、scorecard refs、recovery probes、trajectory refs、improvement candidate refs、promotion gates 和结构化 independent AI reviewer receipt refs；它不会读取或写入 domain truth、memory body、artifact body 或 owner receipt body。suite passed 只能进入风险分级 promotion pipeline；低/中风险自动推广必须同时具备真实 independent AI review assessment、tests/canary/no-forbidden-write/provenance，缺 receipt 或 generated/fixture receipt 只能停在 `review_pending` / `blocked_from_auto_promotion`，高风险必须 owner/human gate。
...
| Optimizer external suite | `review_pending` / `gated_candidate_set_ready` | `opl agent-lab optimize --suite ... --json` 输出 gated candidate、risk review、version ledger 和 transition refs，不训练/部署权重；缺真实 independent AI review assessment 时只停在 `review_pending` 或 `blocked_from_auto_promotion`，具备真实 reviewer provenance 后才允许按风险 gate 推广。 |
