...
Agent Lab 可把 `domain_stage_completion_policy` 当作 quality/ready conformance 输入，但不能把它变成运行时推进门。Stage executor 的任意可读 artifact 都可供 Codex 选择下一 stage；标准 closeout packet、owner receipt 与 reviewer evidence只决定 accepted/promotion/ready 声明。

Agent Lab 对 contract 本身不符合 canonical progress-first floor 的情况输出 conformance blocker：缺失 `stage_completion_policy_ref`、`surface_kind` 不是 `domain_stage_completion_policy`、`completion_judgment_owner` 不是 `domain_stage`、`closeout_packet_required` 不是 `false`、`raw_artifact_sufficient_for_progress` 不是 `true`、`provider_completion_is_domain_completion=true`、`opl_content_judgment_allowed=true`、`semantic_route_decision_owner` 不是 `decisive_codex_attempt`、`stage_transition_materialization_owner` 不是 `opl_stage_run_controller`，或 authority boundary 允许 Framework 按专业语义批准、否决、排序、覆盖 ABI 合法的 Codex route。终局 Codex Attempt 只输出语义 route decision，不写 Stage current pointer，也不物化 transition；Framework 对越权、畸形、legacy-field 或 undeclared-target route output 的 ABI 拒绝和 StageRun controller 的 declared-Stage transition materialization仍是必需 guard。该 blocker 只拒绝错误的 agent contract/currentness 声明，不能阻止任何已有可读 artifact 的 Stage 推进，也不是目标 domain owner receipt。
