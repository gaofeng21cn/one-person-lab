...
Agent Lab / Foundry Lab 的 dynamic workflow template 只沉在 suite topology / verifier / work-order draft ref 层。它把 `classify_and_act`、`fan_out_and_synthesize`、`adversarial_verification`、`generate_and_filter`、`tournament`、`loop_until_done`、`model_routing`、`worktree_isolation` 这些模式表达成可读 catalog，让 App/workbench、OMA 或 operator 选择评估拓扑、验证 refs 和开发 work order 草稿。

该机器面不是普通用户 workflow compiler，不编译任意业务流程，不替代 Runway、Temporal 或 family-runtime substrate，不执行非默认 executor，也不改变 stage/runtime lifecycle。它只能输出 `suite_topology_ref`、`verifier_ref` 和 `work_order_draft_ref`，并且固定禁止声明 `runtime_substrate`、`ordinary_workflow_compiler`、`domain_truth`、`quality_verdict` 和 `owner_receipt`。

当前机器入口是 `src/agent-lab-workflow-templates.ts` 与 `opl agent-lab workflow-template --json`。consumer 必须把它当成 refs-only Foundry Lab template catalog；任何 template pass、variant selection、loop completion 或 worktree isolation proof 都不能升级成 domain ready、quality verdict、owner receipt、default executor promotion 或 runtime substrate readiness。
...
- `opl_agent_lab_rho_backend_plan`：RHO no-apply backend sidecar 计划面，只输出 trajectory digest、diagnosis、candidate harness、self-preference score、winner、candidate diff、work-order draft 和 promotion evidence refs；它不调用外部 RHO CLI、不写 target repo、不直接 apply、不签 owner receipt、不持有 domain truth，也不能提升 default agent。
...
| 读取面 | 稳定职责 | 当前机器入口 |
| --- | --- | --- |
| Contract / authority | 定义 Agent Lab input/output refs、suite result、Developer Mode route、risk-tier promotion、forbidden authority flags 和 refs-only 边界。 | `contracts/opl-framework/agent-lab-contract.json`、`src/agent-lab-authority.ts`、`tests/src/agent-lab-developer-mode-contract.test.ts`。 |
| Suite / read model | 组织 sample、longline、external suite、production evidence suite、complete/workbench read model、ref summary 和 authority boundary。 | `package.json` exports、`src/agent-lab.ts`、`src/agent-lab-complete.ts`、`src/agent-lab-longline.ts`、`opl agent-lab sample|longline|run|complete|workbench --json`。 |
| Mechanism evolution | 投影 mechanism、AHE evidence、variant comparison、log-driven candidate、risk review、version ledger、rollback/canary 和 promotion gate。 | `src/agent-lab-promotion.ts`、`src/agent-lab-control-read-models.ts`、`src/agent-lab-ahe-evidence.ts`、`src/agent-lab-variant-comparison.ts`、`opl agent-lab mechanism|optimize|evolve --json`。 |
| RHO backend plan | 投影 RHO no-apply sidecar 的候选生成计划、证据 refs、candidate diff refs 与 work-order draft refs；只用于后续 OMA/work-order/risk gate 消费，不执行 apply、不成为 runtime substrate 或 truth source。 | `src/agent-lab-rho-backend.ts`、`tests/src/cli/cases/agent-lab.test.ts`、`opl agent-lab rho --project <dir> --json`。 |
| Dynamic workflow template | 投影 Foundry Lab suite topology / verifier / work-order draft refs 的 workflow template catalog；不作为普通用户 workflow compiler 或 runtime substrate。 | `src/agent-lab-workflow-templates.ts`、`opl agent-lab workflow-template --json`。 |
| Developer Mode / repair route | 投影 direct-fix、fork/PR、owner acceptance、scaleout follow-through、risk-tier promotion receipt 和 App/operator drilldown refs。 | `src/agent-lab-developer-mode.ts`、`src/runtime-tray-app-operator-drilldown.ts`、`opl app state --profile fast|full --json`、`opl runtime app-operator-drilldown --json`、`opl agent-lab risk-tier-promotion record|verify|list --json`。 |
| Work-order primitive | 执行 OMA developer patch work order，拥有 target worktree lifecycle、Codex execution、verification、absorption、cleanup、OPL-owned execution plan/report artifact 和 refs-only receipt；旧 `opl agent-lab execute-work-order` alias 已退役。 | `src/agent-lab-work-order-execution.ts`、`tests/src/cli/cases/work-order-execution.test.ts`、`opl work-order execute --work-order <developer-patch-work-order.json> --json`。 |
| Efficiency / cost / executor policy | 投影效率非回归、token/cost estimate、executor capability lease 和 stage executor policy；只支持预算、比较、trial design 与 refs-only readiness。 | `src/agent-lab-efficiency-nonregression.ts`、`src/agent-lab-token-cost-estimate.ts`、`src/agent-lab-executor-capability-aperture.ts`、`src/agent-lab-stage-executor-policy.ts`、`opl agent-lab efficiency|stage-executor-policy --json`。 |
| Optional connector export | 输出 Inspect AI / OpenInference / Langfuse / Phoenix / JSON connector-shaped refs-only envelope；core 不依赖这些外部 runtime。 | `src/agent-lab-complete.ts`、`opl agent-lab export --target <inspect-ai|openinference|langfuse|phoenix|json> --json`。 |
