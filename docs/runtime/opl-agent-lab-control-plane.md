...
当前 OPL App/operator consumption 已接上该闭环的 refs-only closeout 端：OMA workbench projection 可以把 blocked suite、developer patch work order、patch traceability matrix、target verification、runtime/read-model consumption、workspace environment proof、no-forbidden-write proof、target owner receipt or typed blocker、patch absorption、worktree cleanup 和 Agent Lab re-evaluation 作为 11 个机器 refs 展示给 operator。App 只提供 drilldown 与审计，不接管 OMA work order 生成、不执行 target patch、不签 target owner receipt，也不改变 Agent Lab risk gate。
