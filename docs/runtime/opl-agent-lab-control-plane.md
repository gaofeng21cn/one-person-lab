...
- Meta-agent plan：`opl agents meta-builder plan --json`，输出面向“用 OPL 开发智能体”的元智能体阶段计划：intent intake、web research、stage decomposition、agent skeleton build、eval suite build、baseline run、optimizer iteration、baseline delivery 和 online learning。
...
1. App/workbench 消费 `opl agent-lab complete --json` 与 `opl agents meta-builder plan --json`，展示 eval adapters、observability exports、optimizer candidates、promotion gates 和 online learning refs。
