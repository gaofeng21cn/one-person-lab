...
- CLI external suite：`opl agent-lab run --suite <suite.json> --json`，运行 domain agent 或 OPL-compatible meta-agent 仓生成的 OPL-compatible Agent Lab suite JSON，返回同一套 refs-only suite result、ref summary 和 authority boundary。`mas_production_evidence_suite` 会额外投影 MAS production evidence gate result refs、gate ids、owner route refs、no-forbidden-write refs、typed blocker refs 和 required receipt refs，并固定 `domain_verdict_claimed=false`。
...
MAS production evidence suite 是这个 external suite 入口的 refs-only 特化输入。OPL Agent Lab 只识别 `mas_production_evidence_gate` / `production_evidence_gate` 中的 gate ids、owner route、no-forbidden-write proof、typed blocker、required receipt 和 gate result refs，把它们作为 `production_evidence_gate_result` 回传给 MAS 或 `opl-meta-agent` 接力；MAS owner receipt、domain truth、quality verdict、artifact body、memory body 仍只能由 MAS owner surface 产生。
