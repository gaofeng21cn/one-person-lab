...
| External sandbox provider | E2B / Daytona / Modal 这类成熟 sandbox provider 是 selected stage executor 的 isolated workspace / process substrate，不是 OPL Framework、Runway 或 Temporal 的替代品。OPL 持有 provider profile、credential preflight、run-context binding、sandbox receipt projection 和 false-ready guard；不自研底层 VM/container sandbox，不把 sandbox run 写成 domain truth、owner receipt、App release ready 或 production ready。 | 当前机器真相是 `opl.external_sandbox_provider_adapter.v1` 的 provider profile / credential-ref / provider-receipt-ref preflight 与 OPL sandbox binding receipt；live credential run、provider long-soak 和 App release cohort 仍是后置 evidence。 |
