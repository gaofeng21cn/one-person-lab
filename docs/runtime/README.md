...
- [StructuredCloseoutGate 与 repair-redrive runbook](./structured-closeout-gate-runbook.md)
...
| Stage currentness 谁裁决？ | OPL runtime | [OPL Stage Graph 与 Route-as-Transition Runtime](./stage-graph-route-transition-runtime.md)；只有 Stage Transition Authority 可从 append-only authority event log 派生 current pointer、terminal state 和 `current_owner_delta`。`DomainProgressTransitionRuntime` 是这一边界的 Runway runtime form；domain 仓只提供 policy adapter。 |
...
| Domain progress transition | OPL `DomainProgressTransitionRuntime` 承载 command/outbox/event intake、fixed-point reconcile、exactly-one transition、StageRun identity、replay、NonAdvancingApply 和 projection demotion；MAS/MAG/RCA/OMA 只提供 Domain Progress Policy Adapter。MAS first consumer 名称是 `PaperProgressPolicyAdapter`。 | `docs/runtime/stage-graph-route-transition-runtime.md`、`docs/architecture.md`、`docs/decisions.md`、`docs/active/current-state-vs-ideal-gap.md`。 |
...
- E2B 是当前已实现的 remote provider option；缺 endpoint、credential ref、provider receipt 或 workspace transport 时 fail closed，不回落到 host。其他 provider 必须先接入 generic external adapter 并落地 executor，之后才能声明支持。
...
| `structured-closeout-gate-runbook.md` | `active_support` | OPL Runway owner | 解释 StructuredCloseoutGate、typed JSON closeout、格式漂移恢复、provider-runtime closeout blocker 和 repair-redrive query/decision 边界；不声明 runtime ready、Live Evidence ready、domain ready 或 production ready。 |
...
| `stage-graph-route-transition-runtime.md` | `active_support` | OPL runtime owner | 解释复杂 domain agent 的 stage graph、route-as-transition、`DomainProgressTransitionRuntime`、Domain Progress Policy Adapter、Stage Folder Contract、child graph、human gate、executor/reviewer split 和 MAS/RCA 承载方式；已落地运行面与剩余 production evidence gate 分开读取，不把 transition pass、provider completion、route graph projection 或目录存在写成 domain ready / stage complete。 |
