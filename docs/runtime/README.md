...
| Runtime environment substrate | OPL 持有 dependency descriptor / lock / bundle manifest / managed runtime root / run-context / cache inventory；domain repo 只声明 dependency intent。当前源码入口是 thin `src/runtime-environment-substrate.ts` facade，加上 `runtime-environment-substrate-parts/{contract,target-state,package-profile,projection-cache}.ts`；`shared.ts` 只做 re-export facade。Materialization receipt、run-context 或 cache hit 都不能声明 provider ready、domain ready、App release ready 或 production ready。 | `contracts/opl-framework/runtime-environment-substrate-contract.json`、`src/runtime-environment-substrate.ts`、`tests/src/runtime-environment-substrate.test.ts`、`tests/src/cli/cases/runtime-environment-substrate-command-surface.test.ts`、`opl runtime env inspect|lock|build|prepare|materialize|verify|cache|doctor|run-context|contract --json`。 |
| Runway agent sandbox provider | Runway 默认用本地 Dev Containers / Docker 作为 selected stage executor 的 isolated workspace / process substrate；E2B / Daytona / Modal 是显式 remote sandbox provider option，不是 OPL Framework、Runway 或 Temporal 的替代品。OPL 持有 provider selection、local image/template refs、optional remote credential preflight、run-context binding、sandbox receipt projection 和 false-ready guard；不把 sandbox run 写成 domain truth、owner receipt、App release ready 或 production ready。 | 当前机器真相是 Codex stage runner 的 `sandbox_execution` receipt、`contracts/opl-framework/runtime-environment-substrate-contract.json` 的 `default_provider_kind=local_devcontainer`、以及 E2B remote adapter 的 credential-ref / provider-receipt-ref preflight。live Docker/devcontainer run、remote credential run、provider long-soak 和 App release cohort 仍是后置 evidence。 |
