...
**Goal:** 把 `OPL` 从历史 `frontdesk` 公开语义重置为新的 `Product API` 模型，并先完成主仓文档收束，再推进主线接口与实现重组。

**Architecture:** 公开产品模型统一收敛为 `system / engines / modules / agents / workspaces / sessions / progress / artifacts`。`frontdesk`、`readiness`、`domain-wiring` 等历史概念退出当前主线，只保留为内部实现遗留或历史归档。各 domain 仓继续保持独立 agent entry，`OPL` 只持有 family-level shared runtime 与 API truth。
...
- [ ] 在 `docs/decisions.md` 与 `docs/status.md` 里冻结“`frontdesk` 公开语义退役”的当前决策。
- [ ] 把旧 `frontdesk / handoff / readiness` 相关表述从当前主线文档中删干净。
- [ ] 验证：`rg -n "frontdesk|readiness|entry-guide|domain-wiring|hosted-bundle|hosted-package" README* docs/{README*,project.md,status.md,architecture.md,decisions.md,invariants.md}`

### Task 2: 清理 OPL 主仓历史文档入口，避免继续污染主线

**Files:**
- Modify: `docs/history/README.md`
- Modify: `docs/history/frontdesk-legacy/README.md`
...
- Modify: `docs/plans/2026-04-19-opl-initialize-and-onyx-overlay-implementation.md`
...
- [ ] 统一写法：README 面向潜在使用者，突出“解决什么问题、适合什么场景、怎么开始”，不暴露底层 `frontdesk / handoff` 历史术语。
- [ ] 明确它们与 `OPL` 的关系：可被 `OPL` 调用，也可被 `Codex` 直接调用，行为语义一致。
- [ ] 验证：逐仓 `rg -n "frontdesk|handoff|readiness|entry-guide|domain-wiring"` 仅允许出现在历史/参考区。
...
- [ ] 保留旧实现文件时，只允许作为过渡内部实现，不再把 `frontdesk` 作为当前公开产品命名。
...
- [ ] 验证：核心类型在 TypeScript 层收束，不再用 `frontdesk_*` 作为主线 public type。
