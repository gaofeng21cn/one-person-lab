...
它应被理解为建立在已冻结 `Phase 1` gateway 基线之上、并继续服务当前 `Minimal admitted-domain federation activation package` 的 CLI-first / read-only discovery 辅助索引。
...
repo-tracked 的 `Phase 1` candidate-domain closeout 顺序已冻结为 `Review Ops` 然后 `Thesis Ops`，已吸收的前序 follow-on 仍是 `Phase 1 exit + next-stage activation package freeze`，而当前 active follow-on 已切到 `Minimal admitted-domain federation activation package`。
...
- [OPL Routed Action Gateway](./opl-routed-action-gateway.zh-CN.md)（当前仅作为 planning-level 合同参考；repo-tracked `Phase 1 / G3 thin handoff planning freeze hardening` 已完成 closeout，`Review Ops -> Thesis Ops` candidate-domain closeout 继续停留在 admission / discovery / routing readiness 之下，已 absorb 的前序 follow-on 是 `Phase 1 exit + next-stage activation package freeze`，而当前 active follow-on 已切到 `Minimal admitted-domain federation activation package`）
