...
已完成的 `G2` closeout 已把单一、repo-tracked 的顶层 `G2` CLI-first / read-only baseline 固定下来；当前这一棒是 `Phase 1 / G3 thin handoff planning freeze hardening`。在这一步里，`G3` 仅处于 `thin handoff planning` 预冻结，也就是 planning-level contract，而不是已激活的 routed-action runtime。
...
- [OPL Routed Action Gateway](./opl-routed-action-gateway.zh-CN.md)（当前仅作为 planning-level 合同参考；对应 `Phase 1 / G3 thin handoff planning freeze hardening`）
...
如果某个界面属于 routed-action prose 或 schema 这一层，那么在当前 `Phase 1 / G3 thin handoff planning freeze hardening` 里，它也只保持为 planning-level contract / planning dependency，不会升级成 launcher 或 live runtime control plane。
