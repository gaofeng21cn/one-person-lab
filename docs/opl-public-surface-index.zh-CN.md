...
它应被理解为建立在当前本地 gateway baseline 之上的 CLI-first / read-only discovery 辅助索引。
这条基线仍然是当前 `OPL` 的 formal entry contract 与 public system surface。
当前 repo-tracked follow-on 是 `S1 / shared runtime substrate v1 contract freeze`；它的作用，是让公开界面对 shared runtime substrate 语言达成一致，而不是把这个索引升级成 runtime registry、launcher 或 runtime-authority surface。
更早的 phase / activation-package 冻结仍保留为 reference-grade 历史锚点，但不再定义当前 active follow-on。
当前仓库成熟度与跨仓状态判断，以 [生态四仓统一状态总表](./references/ecosystem-status-matrix.md) 作为内部参考锚点。
...
- [OPL Routed Action Gateway](./opl-routed-action-gateway.zh-CN.md)（当前仅作为 planning-level 合同参考；`G3` 继续停留在 planning-only 状态，当前 repo-tracked follow-on 是 `S1 / shared runtime substrate v1 contract freeze`，而不是 routed-action activation）
