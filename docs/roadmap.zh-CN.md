...
- [`MedAutoScience`](https://github.com/gaofeng21cn/med-autoscience) 是当前 active 的 `Research Foundry` domain gateway 与 harness
- [`RedCube AI`](https://github.com/gaofeng21cn/redcube-ai) 是当前已 admitted 的视觉交付 domain gateway 与 harness
- `ppt_deck` 是当前最直接映射到 `Presentation Foundry` 的 family
- `Thesis Foundry` 与 `Review Foundry` 仍处于定义阶段
- `Grant Foundry -> Med Auto Grant` 已是当前活跃的医学 `Grant Foundry` 业务仓路径；在 OPL 顶层，它的 federation admission / handoff wording 仍单独门控，而可见里程碑继续是 registry material、`G2` discovery readiness、`G3` routed-action readiness 与 domain-onboarding evidence
...
- 保持 `MedAutoScience` 明确为 `Research Foundry` 的 domain surface
...
这个顺序本来就只表示边界定义的先后，现在已经被吸收到当前 `Phase 1 exit + next-stage activation package freeze` 中。`Grant Foundry` 已经拥有通过 `Grant Foundry -> Med Auto Grant` 表达的活跃业务仓路径，但其顶层 federation admission / handoff wording 仍单独门控；后续里程碑聚焦在 registry material、discovery readiness、routing readiness 与 onboarding evidence。
`Review Foundry` 继续作为 under-definition semantic bundle 推进，review truth 保持 domain-owned，并沿 `execution_model`、`discovery_readiness`、`routing_readiness` 与 `cross_domain_wording` 四类 package 收口。
`Thesis Foundry` 也沿同一组 package 推进，同时保持自身的 dissertation / defense 角色，与 `Research Foundry` 的 manuscript flow 以及 `Presentation Foundry` / `RedCube AI` 的 deck production 清楚分层。
...
这份已 absorbed 的 federation package 继续针对 `MedAutoScience` + `RedCube AI` 这两条 admitted domain surface 收紧顶层 federation wording，同时保持 formal entry 不变，仍然是本地 `TypeScript CLI`-first / gateway contract surface；`Grant Foundry`、`Review Foundry` 与 `Thesis Foundry` 继续沿各自的 candidate-definition 路径推进。
...
- 外界是否能理解 `MedAutoScience` 仍是独立的 `Research Foundry` domain gateway 与 harness？
- 外界是否能理解 `RedCube AI` 仍是独立的视觉交付 domain gateway 与 harness？
- 外界是否能理解 `ppt_deck` 直接映射 `Presentation Foundry`，而 `xiaohongshu` 在 OPL 顶层保留独立 visual family 语义？
