...
- 当前主线交互路径以 Codex-default 为先：本地 `opl`、直接 `Codex` 使用，以及未来外部壳都消费同一套 session/runtime truth。需要把家族 domain skill pack 装进 Codex 时，先运行 `opl skill sync`；显式 runtime 切换继续是可选行为。
