...
<p align="center">以「任务阶段」为单位，让论文、基金、汇报、专利等复杂成果持续推进，并在每个专家阶段内部承载开放式认知计算</p>
...
它把复杂知识工作拆成一个个清晰的「任务阶段」：准备材料、执行创作、质量审核、修订完善、交付收口。Route 编排负责守住外部边界：当前 owner 是谁、下一步应启动哪个 stage attempt、缺什么 receipt 或 blocker、什么时候必须 route back。每个 stage 内部则由被选中的 AI executor 进行生成、反思、比较、演化、工具调用、知识读取和质量门检查。OPL 把这个 stage 内部循环称为认知计算内核，让 AI 不只是“聊天”，而是真正围绕成果持续工作。
...
论文、基金、汇报、专利不是一次问答能完成的。OPL 把它们组织成可持续推进的任务阶段，并让每个阶段内部承载专家生成、批评、比较、修订、工具使用、知识扎根和质量审核。
...
医学研究、基金写作、视觉汇报和 Agent 构建分别由不同 Foundry Agent 承担。用户看到的是统一工作台，背后每个专业 Agent 保留自己的 domain pack、skill、knowledge、quality gate、判断标准和交付权威。
...
OPL 把 route 编排和 stage 内部认知分开。Route 说明下一步应该发生什么：启动一个 stage attempt、等待 human gate、记录 owner receipt、暴露 typed blocker，或带着证据 route back。Stage attempt 则是真正发生专家工作的地方。

在一个 stage 内部，认知计算内核允许被选中的 executor 运行开放式工作模式：生成候选结果、根据 source 反思、比较替代方案、演化出更强版本、总结反复出现的 review 问题、调用工具、读取 knowledge refs，并把结果交给独立质量门。OPL 记录这项工作周围的输入、输出、receipt、blocker 和 handoff refs；它不把专家推理写死成固定脚本。

Foundry Agents 提供这个内核的领域侧能力。它们声明 domain pack、stage 目标、skill、可用工具 affordance、knowledge refs、rubric、quality gate、artifact locator 和 authority receipt。OPL 负责托管和观察 attempt；MAS、MAG、RCA、OPL Meta Agent 以及后续智能体继续持有自己的 domain truth 和交付裁决。

## 和 workflow-style agents 的区别

Workflow-style agents 很适合程序自动化：工具调用、函数输入输出、节点编排和确定性路由。复杂知识交付需要换一个工作单元。论文、基金、汇报或专利不是因为某个节点跑完了就真正推进，而是因为一个专家阶段完成了定义、资料扎根、执行、审阅、修订和交付，并留下可见证据。在 OPL 中，Route 保持为轻量的 owner 与恢复边界，Stage 才是认知计算边界。
...
| 层级 | 面向对象 | 作用 |
| --- | --- | --- |
| **OPL Framework** | 开发者、技术操作者、产品集成 | 负责编排 route 与 stage attempt、托管认知计算内核、记录进展和证据，并支持恢复、重试和人工介入。 |
| **One Person Lab App** | 终端用户 | 桌面工作台。用户从这里选择任务、查看进度、打开文件、处理阻塞和获取更新。 |
| **Foundry Agents** | 专业工作场景 | MAS、MAG、RCA 等专业 Agent 分别提供 domain pack、skill、knowledge、quality gate、authority receipt 和高价值知识工作的交付物。 |
...
- 将阶段式运行时扩展为更清晰的认知计算内核，让 route 恢复、重试、人工确认、阶段质量门和进度投影更加完整。
