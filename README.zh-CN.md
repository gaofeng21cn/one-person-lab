...
> 安装并配置这个 OPL 仓库：clone 仓库、安装 OPL CLI、运行 `opl install`，并确保当前第一公民执行器、Temporal-backed OPL 家族运行时提供者、MAS/MAG/RCA、推荐技能、`officecli` 这类必要配套命令行工具、One Person Lab App 和浏览器入口都可用；如果缺任何东西，直接修复或报告精确阻塞点。当前打包第一公民执行器是 Codex CLI。Temporal-backed provider 是 durable stage attempt、human-gate signal、retry、query 和 workflow history 的生产必需 substrate；Hermes-Agent、Claude Code 等同类工具是可显式选择的 Agent executor adapter，必须通过回执和审计面证明接入。

### 安装后常用命令

```bash
opl system initialize   # 检查执行器策略、family runtime provider、模块、skills、GUI 和工作目录状态
...
opl skill sync          # 把 OPL 家族 skills 同步到 agent 可见路径
...
2. 技术规划、架构判断和方向同步，继续读 [文档索引](./docs/README.md)，再读 [项目概览](./docs/project.md)、[当前状态](./docs/status.md)、[架构](./docs/architecture.md)、[硬约束](./docs/invariants.md)、[关键决策](./docs/decisions.md)。
3. 开发者和维护者，继续读 [合同目录说明](./contracts/README.md)、[规格索引](./docs/specs/README.md)、[参考级索引](./docs/references/README.md)，以及 [历史归档索引](./docs/history/README.md) 下的过程归档材料。
...
- OPL family runtime 的生产路径是 Temporal-backed。Temporal 是 durable stage-attempt workflow、activity retry/timeout、human-gate signal、status query 与 execution history 的生产必需 substrate；local provider 只用于 dev/CI/offline diagnostics。`hermes_agent` 是 canonical 显式非默认 Agent executor adapter/backend，可作为 proof lane 显式选择；旧 Hermes provider / Gateway / readiness / compat 面不再是目标长期 session/wakeup substrate。
...
- `Med Auto Science` 对 OPL 和操作者环境只暴露一个 MAS domain app skill。OPL 同步并消费这个 skill 以及 MAS-owned projection 作为共享工作台 surface；医学科研 runtime、controller truth、质量 authority 和 publication gate 继续由 MAS 持有。
...
- [任务版图](./docs/public/task-map.md)
- [运行模型](./docs/public/operating-model.md)
- [统一工程基座](./docs/public/unified-harness-engineering-substrate.md)
- [文档索引](./docs/README.md)
