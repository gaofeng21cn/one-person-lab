...
对 OPL framework、domain agent、App 或交付型任务，真实进度按目标交付物、owner decision、human gate、可接力 artifact delta 或用户可用结果计算；runtime、queue、stage、read model、测试、docs、provider liveness 和平台修复只是支持证据。OPL 执行基座是首选路径，不是目标交付的前置条件：基座顺畅时走基座，基座卡住时，前端执行者继续推进合法的目标交付增量，并把暴露出的基座问题作为 side repair lane 记录或修复。只有继续动作会越权写 authority surface、缺少必要内容/数据源、或同一写集 ownership 冲突且无法隔离时，才暂停具体目标动作。repair lane 不能吞掉目标主线，除非用户目标本身就是平台修复。
...
3. **抓大放小**
   ...
   `抓大` 用于守住不会走歪的大边界：owner、authority、stage lifecycle、workspace topology、selected executor、single ordinary route、launch / execution / closeout admission、accepted owner answer shape、release / delete authority 和 no-second-truth。缺失这些边界会导致错误启动、越权、不可审计、不可恢复、误闭合或不可逆 mutation，必须 fail closed。`放小` 用于避免细节反向卡住 ordinary progress：prompt / skill / tool / knowledge / rubric refs 完整性、projection mirror、diagnostic proof、receipt accounting、L5 evidence item、provider ops detail 和 release cohort diagnostic 默认进入 advisory、audit、cleanup 或 production evidence lane；只有它们实际破坏大边界时，才升级为 hard blocker。
