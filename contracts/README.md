...
- `contracts/opl-framework/target-operating-architecture-contract.json`：OPL family 顶层目标操作架构合同；它冻结标准资源模型、单一 Stage Transition Authority、Domain Pack + generated surfaces + authority ABI、surface lane、小 reconciler、Atlas/Ledger telemetry、App Console 默认字段与 Agent Lab refs-only improvement 边界，但不声明 domain ready、App release ready、production ready、quality verdict、artifact authority、owner receipt 或 typed blocker。
