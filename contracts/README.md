...
9. capacity、monitor、assumption、cohort-loop、replay 和 domain-owner review 信号只进入 `opl framework readiness --family-defaults --json` 与 `opl stages readiness --family-defaults --json` 的 attention / drilldown refs；需要单仓 stage 级展开时再读 `opl stages readiness --domain <domain>`，不作为独立 launch-authority schema 或默认真相入口。
...
- `contracts/opl-framework/brand-module-registry.json`：九个 OPL 品牌模块的机器注册表；`opl brand-modules list|inspect|maturity|validate|interfaces --json` 从这里派生，作为聚合目录与成熟度总览。
- `contracts/opl-framework/brand-cli-governance.json`：九模块 command surface 与 domain-agent internal module spine 治理；它约束 `opl agents modules * --json` 和 Workspace validate/doctor/interfaces 的碰撞边界。
- `contracts/opl-framework/brand-module-surfaces.json`：九个 OPL 品牌模块的自身 executable surface 合同；`opl charter|atlas|workspace|stagecraft|runway|vault|console|foundry-lab|connect status|inspect|interfaces|validate|doctor --json` 从这里派生，证明模块级 Workspace-level `L4_structural_baseline`，但不声明 domain ready、quality verdict、artifact authority、production ready、owner receipt、typed blocker 或 App release truth。
- `contracts/opl-framework/brand-module-l5-operating-evidence.json`：九个 OPL 品牌模块的 L5 operating-evidence 矩阵；`opl brand-modules l5-status|l5-validate|l5-interfaces --json` 与 `opl <module> l5-status --json` 从这里派生。它只证明 L5 证据门可执行、可验证、可维护，不能把 docs foldback、contract validation、provider completion、App projection 或 conformance pass 单独升级成 `L5 production operating maturity`。
...
- `contracts/opl-framework/stage-route-scheduler-contract.json`：OPL stage/route 顶层调度合同；它固定 stage graph、owner-route hydration、reconciliation loop 与 attempt ledger 四层模型，明确 stage 是 OPL attempt 单元，route 是 domain owner 语义，不是小 stage
...
- `opl index doctor|rebuild|checkpoint|integrity-check|backup --json` 是该合同的 OPL-owned SQLite sidecar 维护面；它只维护 `${OPL_STATE_DIR}/family-runtime/{queue,lifecycle-index,artifact-index,read-model}.sqlite`，并把 `artifact-index.sqlite` / `read-model.sqlite` 初始化为 refs-only projection tables。标准 Foundry Agent 通过 `contracts/state_index_kernel_adoption.json` 声明 SQLite 分工，`opl agents conformance` 会阻止 domain repo 把 SQLite 声明为 truth store、artifact body store、owner receipt authority 或私有 generic persistence engine
...
- `contracts/family-orchestration/research-hypothesis-portfolio.schema.json`：OPL-owned refs-only hypothesis portfolio / research hypothesis evidence-pack companion；它只承载 hypothesis candidate、assumption decomposition、novelty/provenance check、negative/failed path、ranking/proximity/advisory metric 与 human review refs/status，不持有 domain truth、quality verdict、artifact authority、hypothesis acceptance 或 owner receipt authority
