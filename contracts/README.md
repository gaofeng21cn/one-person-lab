...
- 当前公开默认主路径是 `external shell or CLI -> Codex CLI first-class executor -> OPL typed family queue / activation layer -> Temporal-backed family runtime provider -> selected domain agent entry`；Full OPL family readiness 的 online runtime substrate 是已配置且 ready 的 Temporal provider，Hermes-Agent 只作为 `hermes_legacy` provider 或显式 Agent executor/proof lane，local provider 只作为 dev/CI/offline diagnostic baseline
...
- `contracts/opl-framework/family-runtime-online-substrate-contract.json`：provider-backed family runtime 合同；它冻结 `local_sqlite`、`hermes_legacy`、`temporal` 的 owner split、queue state、stage attempt ledger、degraded diagnostic mode 与 forbidden authority
