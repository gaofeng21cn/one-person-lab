...
- `opl index doctor|rebuild|checkpoint|integrity-check|backup --json` 是该合同的 OPL-owned SQLite sidecar 维护面；它只维护 `${OPL_STATE_DIR}/family-runtime/{queue,lifecycle-index,artifact-index,read-model}.sqlite`，其中 `queue.sqlite` 是兼容文件名，语义是 `stage_attempt_index`；`artifact-index.sqlite` / `read-model.sqlite` 初始化为 refs-only projection tables。标准 Foundry Agent 通过 `contracts/state_index_kernel_adoption.json` 声明 SQLite 分工，`opl agents conformance` 会阻止 domain repo 把 SQLite 声明为 truth store、artifact body store、owner receipt authority 或私有 generic persistence engine
