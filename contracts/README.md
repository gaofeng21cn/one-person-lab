...
- `contracts/opl-framework/stage-route-transport-contract.json`：OPL Stage/route 顶层调度合同；它固定 Stage graph、StageRun、Attempt、owner-route 输入、ABI guard 与 attempt ledger 的分层，明确 Stage 是主要开放语义判断，Attempt 是独立 executor 调用，权威跨 Stage route 只能由 progress-terminal decisive Codex Attempt 输出
