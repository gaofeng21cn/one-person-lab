...
- `family-runtime-attempt-contract.json` 同时定义 `stage_execution_log` 作为 OPL 统一 stage attempt 可观测面。该读面把 intended work、actual work、timeline、usage、evidence refs 和 authority boundary 投到 `attempt query`、operator visibility 与 runtime-tray workbench summary。这个 log 只从 OPL ledger、provider 观测和 domain receipt refs 派生；它不执行 domain action、不写 domain truth，也不授权 quality 或 domain-ready verdict。
