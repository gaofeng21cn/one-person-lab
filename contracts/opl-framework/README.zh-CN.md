...
- cognitive computation kernel 治理，让每个 Stage 声明 generation、reflection、comparative selection、evolution、strategy retrospective、tool affordance boundary、knowledge 与独立 quality gate refs；工具目录是 affordance catalog，不是 workflow script；Route 仍是 domain owner 语义，不是小 Stage
...
- `stage-quality-cycle-contract.json`：定义非模型 Temporal `StageRunWorkflow` 父级 controller，以及有界的 `producer | reviewer | repairer | re_reviewer` child Attempt。同一目标的多次生成、审阅和修复是 Attempt；目标、owner、质量门或交付关系发生变化，就是新的 Stage。controller 只管理同一 StageRun 内的 child Attempt 拓扑；跨 Stage 语义路由仍归 Codex CLI。
...
- `official-knowledge-deliverable-quality-profile.json`：为 MAS、MAG、RCA、OMA、OBF 启用上下文隔离的 Stage Review和独立顶层 Meta Review，不强制其他 OPL Agent 启用 Meta Review。
- 正式 Review 必须使用新的 child workflow和 provider session；`in_thread_refinement` 与 `protocol_closeout_resume` 都不是 Review。三轮修复预算与 provider/activity/structured-output/runtime retry完全分账。
- StageRun 创建必须绑定 domain pack：`opl family-runtime attempt create` 把已编译 Stage manifest 与 quality policy解析成 `opl_pack_bound_stage_quality_runtime_binding`，manifest SHA参与 StageRun identity。raw `family-runtime stage-run start` 已退役；`stage-run` 只保留 query。
- 进入正式 Review 前，producer / repairer 输出必须提供带 SHA-256 metadata 的 domain-owned artifact identity refs；正式 review receipt 绑定这些 exact refs/hashes、fresh producer/reviewer session、rubric 与 verdict，不能由文件存在或 provider completion 合成。
