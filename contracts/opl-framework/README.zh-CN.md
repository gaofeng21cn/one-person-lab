...
- Pack OS 读取通用 capability-pack descriptor，生成 install registry entry、content-addressed cache manifest、refs-only distribution bundle、refs-only lock、content hash、artifact locator refs、lifecycle refs 和 review receipt transport，不把 pack lock、review receipt transport 或 provider completion 升级成 domain ready、quality verdict、artifact authority 或 publication/export readiness
...
- brand-module governance 通过 registry、模块自有品牌 CLI command surface、read-model projection、validation gate 和 false-authority boundary，让 Charter、Atlas、Workspace、Pack、Stagecraft、Runway、Vault、Console、Foundry Lab 与 Connect 保持 Workspace-level structural baseline
...
- 单写者 Stage transition authority 边界：多个模块可以提交 transition intent 或 observation，但只有 OPL 能折叠成 Stage current pointer、StageRun terminal state 与 `current_owner_delta`
...
- target operating architecture 治理，把理想资源模型、单一 Stage Transition Authority、Declarative Domain Pack / authority ABI、surface lane、小 reconciler、Atlas/Vault telemetry、App Console 收薄与 Agent Lab improvement 边界冻结成顶层机器合同，同时不把诊断细节过度硬门化
- target architecture contract kernels，把 current owner delta、stage artifact unit、owner answer、passive evidence vault、ordinary golden path、stop-loss、guardrail tiers、progress truth、wrapper retirement 与 default surface budget 落成机器面，同时不把 domain truth 移入 OPL
...
- advisory knowledge boundary 治理 domain-owned Markdown memory，让 Atlas / Pack / Stagecraft / Runway / Vault / Console / Connect 只承载 refs、prompt-context refs、receipts 和 projection，不变成 route scorer、winning-path generator、memory-body owner、quality gate 或 export / publication verdict owner
...
- `target-operating-architecture-contract.json`：顶层目标操作架构合同。它冻结 `Agent / DomainPack / WorkspaceGroup / ProjectUnit / StageRun / StageArtifactUnit / OwnerAnswer / EvidenceRef / ReleaseCohort / ImprovementWorkOrder` 资源词汇，要求 `spec/status/conditions/ownerRefs/finalizers`，保持 Stage Transition Authority 单写者，保持 Domain Pack + generated surfaces + authority functions 作为标准 agent ABI，把小细节默认放入 advisory / audit / diagnostic / cleanup / production-evidence lane，把 runtime 拆成小 reconciler，并保持 Atlas / Vault / Console / Foundry Lab 边界 refs-only 与 false authority flags。
...
- `stop-loss-policy.schema.json`：lineage repeat、receipt-only、platform-repair-only、read-model-reconcile-only 与 stale-route 的冻结 / 释放政策。

### Stage Artifact Unit

- `stage-run-kernel-contract.json`：refs-only StageRun spec/status/event-log/read-model substrate、observed_generation、retry budget、hold scope、execution authorization、closeout receipt binding 与 domain adoption boundary。
- `stage-transition-authority-contract.json`：单写者 Stage transition authority 合同，固定 append-only transition events、accepted intent kinds、producer boundaries，以及 stale-generation / provider-only / read-model-only fail-closed 语义。
- `stage-transition-intent.schema.json`：refs-only transition intent envelope，承载 domain owner answer、typed blocker、human-gate decision、provider observation、read-model/worklist observation、Agent Lab observation 与 evidence observation。
...
- `stage-artifact-progress-truth-policy.json`：deliverable progress 必须同时具备 physical output、valid manifest、owner answer 与 current pointer 的 family-level 规则。
...
- `pack-os-contract.json`：通用 Pack OS lifecycle 合同，覆盖 capability-pack descriptors、install registry entries、content-addressed cache manifests、refs-only distribution bundles、refs-only lock projection、artifact lifecycle refs 与 review receipt transport。它由 `src/pack-os.ts` 与 `opl pack os inspect|install|registry|cache|distribute|lock|validate --json` 消费，并通过 `opl pack os mas-display-smoke --contract <path> --json` 消费 MAS Display Pack v2 contract。lock 和 distribution bundle 只记录 descriptor refs、本地存在文件的 content hashes、artifact locator refs、lifecycle state refs、review receipt refs、provenance 与 authority-boundary flags；MAS smoke 将 `med-autoscience:contracts/display-pack-contract.v2.json` 作为 refs 消费，可按请求写 refs-only lock 并返回 audit object，但不存 artifact body、不改 domain artifact、不签 owner receipt、不把 review receipt transport 升级成 quality verdict、不授权 publication/grant/visual/App readiness，也不把 provider completion 说成 pack-quality-ready。

### Evidence Vault

- `evidence-vault-event.schema.json`：raw evidence、provider trace、replay、receipt ledger、typed blocker group、soak、no-regression、cleanup 与 diagnostic refs 的 passive audit-only event envelope。
...
- `contracts/family-orchestration/family-stage-proof-bundle.schema.json`、`contracts/family-orchestration/family-stage-graph-projection.schema.json` 和 `contracts/family-orchestration/family-stage-integrity-metadata.schema.json` 是 stage-pack proof、graph、integrity、citation-support、evidence-handoff、data-access 与 human-checkpoint metadata 的 companion contracts。它们属于 family orchestration，因为 MAS/MAG/RCA 把 domain projection 或 adapter 投影到这些 schema，同时把 domain truth 与 verdict authority 留在各自仓库。
