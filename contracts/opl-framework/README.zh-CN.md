...
- `cli-command-registry.json`：受保护 CLI 命令的 metadata registry，记录 parser adapter、options、output schema ref 与 authority-boundary flags。当前覆盖 `connect scientific search`、`connect references verify`、`connect external-skills sources add|list|search|inspect|sync` 和必需 Connect module action 命令（`install`、`update`、`reinstall`、`remove`）；该 registry 只描述命令，不执行命令，也不声明 readiness。
