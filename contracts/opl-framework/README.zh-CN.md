...
- `native-renderer-execution-contract.json` 与 `stage-native-renderer-request.schema.json`：Stage/Attempt 声明的 macOS native renderer capability。`opl pack native-helper render --request <request.json> --json` 只通过 Runway `native_helper_carrier` 执行固定的 `qlmanage`、`soffice` 或官方 `render_docx.py` 命令集；绑定 stage/attempt、cwd、输入/输出根、argv/env、超时、工具版本和 executable digest，拒绝 shell、任意 executable、路径/符号链接越界，并返回不授权视觉质量、PDF proof、导出 readiness 或 domain truth 的 typed execution receipt。
