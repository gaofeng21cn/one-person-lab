...
- `cli-command-registry.json`：受保护 CLI 命令的 metadata registry，记录 parser adapter、options、output schema ref 与 authority-boundary flags。第一条受保护命令是 `connect pubmed search`；该 registry 只描述命令，不执行命令，也不声明 readiness。
