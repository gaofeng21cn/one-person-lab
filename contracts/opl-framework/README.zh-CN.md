...
- `workspace-topology-profile.schema.json`：`Workspace Group -> Project Unit -> Stage Artifact Unit -> Owner Receipt / Typed Blocker` profile schema，固定 `workspace_modes=one_off|series|portfolio`、默认 `deliverables/<project-id>`、RCA series `deliverables/<deck-id>`、MAS portfolio `studies/<study-id>`、`project_stage_outputs_root=artifacts/stage_outputs` 与 runtime-state-as-provider-backing/provenance 边界。
...
- `workstreams.json`、`domains.json`、`stage-selection-vocabulary.json`、`task-topology.json`、`public-surface-index.json`、`brand-module-registry.json`、`brand-cli-governance.json` 和 `surface-budget-policy.json` 定义当前活跃的 stage-led framework 选择面、Framework / App / Foundry 的产品层 owner split、九个 OPL brand module registry、模块自有 brand CLI frontdoor、default surface 升级预算，以及 OPL-compatible agents 用来定位外部 framework 运行依赖的 `opl_framework_locator` surface。`brand-module-registry.json` 由 `opl brand-modules list|inspect|maturity|validate|interfaces --json` 消费；`brand-cli-governance.json` 由非 Workspace 的 `opl <brand-module> status|inspect|interfaces|validate|doctor --json`、Workspace 的 `opl workspace status|inspect --json` 和 `opl agents modules list|inspect|interfaces|validate|doctor --json` 消费。它们共同只证明 `L4_structural_baseline` 结构，不声明 domain ready、quality verdict、artifact authority、production ready、domain truth write、owner receipt、typed blocker 或 App release truth。`public-surface-index.json` 还为每个 active public surface 绑定 `surface_budget` envelope，包括 default-state reason、promotion evidence refs、consumer refs，以及不得声明 domain ready、quality verdict、artifact authority、production ready、executor planning 或 domain owner 的 authority false flags。
