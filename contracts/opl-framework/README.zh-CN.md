...
- `workspace-topology-profile.schema.json`：`Workspace Group -> Project Unit -> Stage Artifact Unit -> Owner Receipt / Typed Blocker` profile schema，固定 `workspace_mode=one_off|series|portfolio`、`collection_path`、`shared_resource_roots`、`project_stage_outputs_root=artifacts/stage_outputs` 与 runtime-state-as-backing/provenance 边界。
