...
- target operating architecture 治理，把理想资源模型、单一 Stage Transition Authority、Declarative Domain Pack / authority ABI、surface lane、小 reconciler、Atlas/Ledger telemetry、App Console 收薄与 Agent Lab improvement 边界冻结成顶层机器合同，同时不把诊断细节过度硬门化
- target architecture contract kernels，把 current owner delta、stage artifact unit、owner answer、passive evidence ledger、ordinary golden path、stop-loss、guardrail tiers、progress truth、wrapper retirement 与 default surface budget 落成机器面，同时不把 domain truth 移入 OPL
...
- `cli-command-registry.json`：受保护 CLI 命令的 metadata registry，记录 parser adapter、options、output schema ref 与 authority-boundary flags。当前覆盖 `connect pubmed search`、`connect scientific search`、`connect references verify`、`connect external-skills sources add|list|search|inspect|sync` 和必需 Connect module action 命令（`install`、`update`、`reinstall`、`remove`）；该 registry 只描述命令，不执行命令，也不声明 readiness。
