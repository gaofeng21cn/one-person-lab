...
- `family-runtime-attempt-contract.json` 同时定义 `stage_progress_log` 作为 OPL 统一 stage attempt 可观测面。该读面把 intended work、actual work、timeline、usage、Temporal visibility refs、evidence refs 和 authority boundary 投到 `attempt query`、operator visibility 与 runtime-tray workbench summary。这个 log 只从 OPL ledger、provider 观测和 domain receipt refs 派生；它不执行 domain action、不写 domain truth，也不授权 quality 或 domain-ready verdict。Temporal provider 持有 durable workflow history、activity heartbeat、workflow query 与 searchable visibility；OPL 只把 `temporal_visibility` / `temporal_webui_ref` 投影为 refs-only metadata，Temporal Web UI ref 只用于 operator debug，不是 App 主状态页。
