...
- `workspace-topology-profile.schema.json`: Workspace Group -> Project Unit -> Stage Artifact Unit -> Owner Receipt / Typed Blocker profile schema with `workspace_modes=one_off|series|portfolio`, default physical `project_collection_path=projects`, canonical `projects/<project-id>` roots for RCA/MAG/OMA deliverables and MAS studies, `deliverables` / `studies` as display or legacy aliases only, `project_stage_outputs_root=artifacts/stage_outputs`, and runtime-state-as-provider-backing/provenance boundary.
