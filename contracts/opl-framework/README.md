...
- `okf-context-bundle-contract.json`: OPL-owned OKF context bundle boundary for Google OKF v0.1-compatible Markdown/YAML/crosslink directories. It is consumed by `src/okf-context-bundle.ts` and `opl okf validate|inspect|project-pack --json` as an advisory, body-free context interchange layer over Foundry Agent pack refs and memory locator refs. It is not part of the framework startup required-contract set and does not grant runtime scheduling, domain truth, memory body, owner receipt, typed blocker, quality/export verdict, artifact authority, domain-ready, App-release-ready, or production-ready authority.
