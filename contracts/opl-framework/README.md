...
- `family-runtime-attempt-contract.json` also defines `stage_progress_log` as the canonical shared OPL attempt observability surface with `surface_kind=opl_stage_progress_log`. It projects intended work, actual work, timeline, usage, evidence refs, and authority boundary into `attempt query`, operator visibility, and runtime-tray workbench summaries. This log is a refs-only OPL read-model over ledger/provider/domain receipt refs; it does not execute domain actions, write domain truth, or authorize quality/domain-ready verdicts.
