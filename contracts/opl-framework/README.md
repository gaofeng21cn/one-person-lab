...
- `agent-workspace-norm-contract.json`: executable OPL Agent workspace norm that binds `opl workspace ensure`, App `workspace_ensure`, descriptor-only MCP/Skill/OpenAI/AI SDK delegates, workspace registry writes, user inspection paths, canonical generated projection/report roots, workspace report, full workspace_norm projection drift, runtime-state boundary, lifecycle ownership, and conformance anti-overclaim flags.
