...
- `family-runtime-attempt-contract.json` also defines `stage_progress_log` as the canonical OPL family-runtime attempt/progress projection with `surface_kind=opl_stage_progress_log`. It projects intended work, actual work, timeline, usage, evidence refs, authority boundary, provider status refs, and domain receipt refs into `attempt query`, operator visibility, Agent Lab improvement inputs, and runtime-tray workbench summaries. Agent Lab consumes these refs as evidence for evaluation, root-cause, candidate fix, and follow-up read models; it does not own the runtime log, execute domain actions, write domain truth, or authorize quality/domain-ready verdicts. Historical `stage_execution_log` wording is retired and may appear only in tombstone/provenance contexts.
