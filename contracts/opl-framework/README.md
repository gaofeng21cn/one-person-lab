...
- `family-runtime-attempt-contract.json` also defines `stage_execution_log` as the shared OPL attempt observability surface. It projects intended work, actual work, timeline, usage, evidence refs, and authority boundary into `attempt query`, operator visibility, and runtime-tray workbench summaries. This log is an OPL read-model over ledger/provider/domain receipt refs only; it does not execute domain actions, write domain truth, or authorize quality/domain-ready verdicts.
