...
- generic Pack OS governance for refs-only capability-pack descriptor intake, resolver/lock projection, artifact lifecycle refs, and review receipt transport without moving domain truth, quality verdicts, publication readiness, artifact body ownership, owner receipt signing, or production maturity into OPL
...
- `pack-os-contract.json`: generic Pack OS lifecycle contract for capability-pack descriptors, refs-only lock projection, artifact lifecycle refs, and review receipt transport. It is consumed by `src/pack-os.ts` and `opl pack os inspect|lock|validate --descriptor <path> --json`, plus `opl pack os mas-display-smoke --contract <path> --json` for MAS Display Pack v2 contract consumption. These surfaces are folded into the `OPL Pack` brand module surface. The lock records descriptor refs, content hashes for present local files, artifact locator refs, lifecycle state refs, review receipt refs, provenance, and authority-boundary flags. The MAS smoke consumes `med-autoscience:contracts/display-pack-contract.v2.json` as refs, writes a refs-only lock when requested, and returns an audit object; it does not store artifact bodies, mutate domain artifacts, sign owner receipts, turn review receipt transport into a quality verdict, authorize publication/grant/visual/App readiness, or make provider completion pack-quality-ready.
