...
- brand-module governance that keeps Charter, Atlas, Workspace, Pack, Stagecraft, Runway, Vault, Console, Foundry Lab, and Connect at the Workspace-level structural baseline through one registry, one command-surface governance contract, one module-surface contract, native per-module CLI/read-model families, validation/doctor gates, and false-authority boundary
...
- advisory knowledge boundary governance for domain-owned Markdown memory, so Atlas / Pack / Stagecraft / Runway / Vault / Console / Connect can carry refs, prompt-context refs, receipts, and projections without becoming route scorers, winning-path generators, memory-body owners, quality gates, or export / publication verdict owners
...
- `pack-os-contract.json`: generic Pack OS lifecycle contract for capability-pack descriptors, install registry entries, content-addressed cache manifests, refs-only distribution bundles, refs-only lock projection, artifact lifecycle refs, and review receipt transport. It is consumed by `src/pack-os.ts` and `opl pack os inspect|install|registry|cache|distribute|lock|validate --json`, plus `opl pack os mas-display-smoke --contract <path> --json` for MAS Display Pack v2 contract consumption. These surfaces are folded into the `OPL Pack` brand module surface. The lock and distribution bundle record descriptor refs, content hashes for present local files, artifact locator refs, lifecycle state refs, review receipt refs, provenance, and authority-boundary flags. The MAS smoke consumes `med-autoscience:contracts/display-pack-contract.v2.json` as refs, writes a refs-only lock when requested, and returns an audit object; it does not store artifact bodies in the contract, mutate domain artifacts, sign owner receipts, turn review receipt transport into a quality verdict, authorize publication/grant/visual/App readiness, or make provider completion pack-quality-ready.
