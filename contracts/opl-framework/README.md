...
- `workspace-index.schema.json`: workspace-local `workspace_index.json` instance schema for the canonical topology layer, domain display labels, shared resource roles, indexed project roots, user inspection roots, and authority false flags materialized by `opl workspace init|ensure` and checked by `opl workspace validate|doctor`.
