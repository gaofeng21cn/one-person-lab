...
- `pack-os-contract.json`: generic Pack OS lifecycle contract for capability-pack descriptors, refs-only lock projection, artifact lifecycle refs, and review receipt transport. It is consumed by `src/pack-os.ts` and `opl pack os inspect|lock|validate --descriptor <path> --json`, and is folded into the `OPL Pack` brand module surface. The lock records descriptor refs, content hashes for present local files, artifact locator refs, lifecycle state refs, review receipt refs, provenance, and authority-boundary flags. It does not store artifact bodies, mutate domain artifacts, sign owner receipts, turn review receipt transport into a quality verdict, authorize publication/grant/visual/App readiness, or make provider completion pack-quality-ready.
