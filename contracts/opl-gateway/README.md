...
As of `2026-04-19`, the active public `opl-mainline` has already moved into the family-level front desk / hosted-entry hardening line around the local CLI product entry, the local `opl` shell shortcuts, and the `opl web` adapter service, while the repo-tracked formal entry still remains the `Phase 1` local `TypeScript CLI`-first gateway contract baseline that reads the frozen contract artifacts in this directory.
The current public default path is `external GUI overlay or CLI -> Codex -> OPL gateway surfaces`: `opl web` exposes the local adapter service, `/` returns a machine-readable root payload for external GUI overlays, and this directory freezes the gateway surfaces behind that front door without declaring Codex as the product runtime substrate owner.
...
The current GUI mainline is an external overlay repo that consumes these adapter surfaces, while `Hermes-Agent` stays an explicit alternate mode and online gateway.
That delivery target keeps the existing top-level contract language executable through external GUI overlays and local CLI surfaces, while any honest upstream `Hermes-Agent` rollout remains a domain-side migration target rather than a current OPL-layer fact.
