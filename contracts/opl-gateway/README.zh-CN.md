...
截至 `2026-04-19`，当前 `opl-mainline` 的活跃公开主线已经进入围绕本地 CLI product entry、本地 `opl` shell shortcut 与 `opl web` adapter service 的 family-level front desk / hosted-entry hardening，而 repo-tracked 的 formal entry 仍然是 `Phase 1` 那条本地 `TypeScript CLI`-first gateway contract baseline；它只读取这个目录中已经冻结的 contract 工件。
当前公开默认主路径已经统一为 `外部 GUI overlay 或 CLI -> Codex -> OPL gateway surfaces`：`opl web` 暴露本地 adapter service，根路由 `/` 返回给外部 GUI overlay 消费的 machine-readable root payload，而当前目录冻结的是这层前台背后的 gateway surfaces，同时继续保持 Codex 不被写成产品 runtime substrate owner。
...
而位于这层合同之上的当前用户前台，已经以 CLI product entry 与 `opl web` adapter service 为中心；其中 hosted-friendly 的 `opl frontdesk manifest`、`opl frontdesk entry-guide`、`opl frontdesk readiness`、`opl frontdesk domain-wiring`、`opl frontdesk hosted-bundle` 等 surface 已作为当前前台 contract 的一部分落地。
...
当前 GUI 主线已经冻结为消费这些 adapter surfaces 的独立 overlay 仓；`Hermes-Agent` 继续只保留显式备用模式与 online gateway 角色。
这个交付目标把已有的顶层 contract language 继续维持为外部 GUI overlay 与本地 CLI surface 可读取的合同入口；任何诚实的上游 `Hermes-Agent` rollout，仍然属于 domain 侧迁移目标，而不是当前 OPL 层既成事实。
...
- [家族 Executor Adapter 默认口径](../../docs/references/family-executor-adapter-defaults.md) — 当前家族执行器命名、默认模式与 `Hermes-Agent` guardrail（中文内部参考）
...
`Review Ops`、`Thesis Ops` 这类 planned workstream，在对应 domain 边界明确冻结之前，继续停留在 candidate-definition 路径上。
如果当前公开文档里提到 `Grant Foundry -> Med Auto Grant`，应把它表达成由 `MedAutoGrant` 承接的已收录 `Grant Ops` domain gateway 入口；路由仍然只走 `domain_gateway`，grant truth 也继续留在该 domain 内部。
