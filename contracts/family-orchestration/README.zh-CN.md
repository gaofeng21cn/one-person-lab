...
- `research-hypothesis-portfolio.schema.json`
  - 冻结 refs-only hypothesis portfolio / evidence-pack companion，承载 hypothesis candidate refs、assumption decomposition、novelty / provenance check refs、negative / failed path refs、advisory ranking / proximity metric refs 与 human review refs；OPL 只投影 refs 和状态，domain 仓继续持有 hypothesis truth、quality verdict、artifact authority 与 owner receipt authority
...
stage graph 与 owner-route 的关系由 `contracts/opl-framework/stage-route-scheduler-contract.json` 冻结：stage graph 描述 OPL 可启动的 stage attempt topology；`family-owner-route` 描述 domain owner 的下一步、route-back、typed blocker、safe action 或 owner receipt refs。OPL route hydration 只把这些 refs 转成 typed queue task、stage attempt request、conflict envelope 或 operator projection。route 不是 graph 里的隐藏小 stage，也不能绕过 stage attempt ledger 执行。
...
`research-hypothesis-portfolio.schema.json` 冻结 OPL-owned refs-only hypothesis portfolio / research hypothesis evidence-pack companion。它吸收 Co-Scientist 风格的 hypothesis candidate 生成、假设拆解和评审循环经验，但只作为 contract-first projection surface：candidate、assumption decomposition、novelty / provenance check、negative 或 failed path、ranking / proximity / advisory metric ref、human review ref 都只以 refs 和状态进入 OPL。

该 portfolio 不是 domain hypothesis store、scientific truth reducer、quality gate、artifact authority 或 owner receipt signer。advisory ranking 与 proximity metrics 只作为 review-routing 输入。domain 仓继续持有 hypothesis body、evidence body、accept/reject decision、domain truth、quality verdict、publication / fundability / visual authority、artifact body authority 和 owner receipt。
