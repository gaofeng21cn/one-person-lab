...
- `family-stage-runtime-budget.schema.json`
  - 冻结 runtime reliability / capacity advisory 的 refs-only 投影，覆盖 boundary count、runtime guard count、monitor / metric refs、unmonitored boundary count、expected-success 或 boundary-success-rate refs；capacity-budget 证据折叠为 readiness warning / recommendation 和 drilldown ref，OPL 不计算未证实概率、不授权 domain readiness、不引入 GraphFlow runtime
...
GraphFlow / GFL 的 `verified-core` vs `durable-runtime` 区分在 admission、graph projection 和 proof bundle 中落成 `mode_tags`：`verified_core_eligible` 标记可静态校验且不跨 effect/runtime guard 的 stage；`durable_runtime_only` 标记必须依赖运行时事件、owner receipt、provider / executor 或外部边界的 stage；`runtime_boundary_required` 是 scheduler / App 可以直接消费的启动边界信号。旧 `guarantee_mode` 保留兼容，不作为 domain verdict。失败定位落成 `failure_localization` 投影，把 blocker / warning 按 `ai`、`human`、`external`、`provider`、`runtime`、`domain`、`artifact`、`source`、`monitor`、`executor` lane 分类，并暴露 `source_ref` 与 minimal counterexample；它只服务 operator drilldown 和 launch-readiness，不替代 domain owner receipt 或质量判断。
...
GraphFlow / GFL 的 diagram-as-spec / generated artifacts 模式在 proof bundle 内落成 `generated_artifact_manifest`：OPL 只记录从同一 diagram / stage pack 推导出来的 code / test / proof / schema / artifact refs、`source_stage_pack_ref`、`graph_projection_ref`、`stage_pack_hash`、per-ref source hash binding、`regeneration_policy` 与 `drift_status`。diagram / stage pack 是 source of truth；generated code、tests、proofs、schemas 与 artifacts 是可再生供应链输入。source hash 不一致时机器状态是 `needs_regeneration`，generated ref 没有声明 source hash 时机器状态是 `missing_source_hash`。manifest 仍是 refs-only build/review input：不扫描真实文件 hash、不执行 stage、不写 artifact body、不引入 GraphFlow runtime dependency，也不把 generated refs 升级成 domain ready 或 quality verdict。
