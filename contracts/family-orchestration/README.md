...
`contracts/opl-framework/stage-route-scheduler-contract.json` freezes how the stage graph relates to owner routes. The stage graph describes OPL-launchable stage attempt topology. `family-owner-route` describes domain-owner next-step, route-back, typed-blocker, safe-action, or owner-receipt refs. OPL route hydration only turns those refs into stage attempt requests, provider-backed dispatch intents, conflict envelopes, or operator projections. A route is not a hidden small stage in the graph and cannot execute around the stage attempt ledger.
