...
GraphFlow / GFL's diagram-as-spec / generated-artifacts pattern lands inside the proof bundle as `generated_artifact_manifest`. OPL records code / test / proof / schema / artifact refs derived from the same stage pack plus `source_stage_pack_ref`, `graph_projection_ref`, `stage_pack_hash`, and per-ref regeneration metadata. Each generated ref can declare the source stage-pack hash it was generated from; matching hashes are `regenerated`, changed hashes are `needs_regeneration`, and missing hashes are `missing_source_hash`. This is refs-only supply-chain drift detection, not artifact body generation or file hashing. The manifest is a build/review input only; it does not execute the stage, write artifact bodies, or turn generated refs into domain readiness or quality verdicts.
