...
2. AI-first execution stays outside static contracts; contracts bind expected receipts and authority boundaries, while prompt/tool/knowledge/rubric refs remain recommended AI strategy refs rather than an OPL launch hard gate.
...
`contracts/opl-framework/stage-route-scheduler-contract.json` freezes how the stage graph relates to owner routes. The stage graph describes OPL-launchable stage attempt topology. `family-owner-route` describes domain-owner next-step, route-back, typed-blocker, safe-action, or owner-receipt refs. OPL route hydration only turns those refs into typed queue tasks, stage attempt requests, conflict envelopes, or operator projections. A route is not a hidden small stage in the graph and cannot execute around the stage attempt ledger.
...
`opl_stage_launch_invocation` is a Stage Kernel activity event. An agent/operator in `invocation` mode may only retrieve, select, bind, launch, or deploy an approved / admitted stage pack, then bind source fingerprint, workspace locator, idempotency key, and selected executor before handing launch to the OPL provider/executor path. `authoring` mode and AI proposal flows may emit only `bounded_edit_ref` proposals; under `--require-stage-admission`, authoring without that ref fails closed with `agent_authoring_requires_bounded_edit_ref`, and unadmitted agent-generated stage packs must not be launched. The default `codex_cli` records `default_codex_cli`; non-default executors such as `hermes_agent`, `claude_code`, or `antigravity_cli` must carry an `executor_binding_ref` when `--require-stage-admission` is used, or launch fails closed with `non_default_executor_binding_ref_missing`. Stage-level executor policy may also carry `model`, `reasoning_effort`, `provider`, `executor_labels`, `required_capabilities`, and `receipt_requirements`; these fields prove launch binding and audit refs only, and do not claim behavioral or quality equivalence with Codex CLI.
