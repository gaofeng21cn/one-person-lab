...
- Freeze `Agent-first`, explicit formal-entry matrix semantics, and `Auto-only current repo + future HITL layering` semantics at the OPL layer before domains hydrate them.
...
- Current read-only control-plane phases may defer runtime build-out, but they must not weaken the long-term `Agent-first` plus `Auto-only current repo + future HITL layering` architecture.
...
- Future domains must expose an `Agent-first` execution story, an explicit formal-entry matrix, and a credible path from a current `Auto-only` domain repository toward a compatible `Human-in-the-loop` sibling or upper-layer product before being treated as formally aligned with `OPL`.
