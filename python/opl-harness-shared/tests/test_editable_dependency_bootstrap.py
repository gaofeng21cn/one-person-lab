...
def test_shared_bootstrap_resolves_workspace_level_sibling_src_for_worktree_roots(
    monkeypatch,
    tmp_path: Path,
) -> None:
    fake_shared_src = tmp_path / "one-person-lab" / "python" / "opl-harness-shared" / "src"
    ...
    candidate_root_str = str(fake_shared_src)
    repo_root = tmp_path / "med-autoscience" / ".worktrees" / "family-final-reuse-mas"
    ...
    try:
        added = module.ensure_editable_dependency_paths(repo_root=repo_root)
    finally:
        sys.path[:] = original_sys_path
