...
def resolve_family_product_entry_preset(
    *,
    ...
    federated_node_id: str | None,
    progress_node_id: str | None,
    direct_transition_event: str | None,
    federated_transition_event: str | None,
    direct_progress_event: str | None,
    federated_progress_event: str | None,
    federated_title: str | None,
    federated_surface_kind: str | None,
) -> dict[str, str | None]:
    resolved_federated_title = _text(federated_title)
    resolved_federated_surface_kind = _text(federated_surface_kind)
    if bool(resolved_federated_title) != bool(resolved_federated_surface_kind):
        raise ValueError("family orchestration federated step requires both title and surface_kind")
    return {
        ...
        "federated_node_id": _text(federated_node_id) or "step:opl_bridge_handoff",
        ...
        "federated_transition_event": _text(federated_transition_event) or "enter_via_opl_bridge",
        "direct_progress_event": _text(direct_progress_event) or "session_started",
        "federated_progress_event": _text(federated_progress_event) or "handoff_completed",
        "federated_title": resolved_federated_title,
        "federated_surface_kind": resolved_federated_surface_kind,
    }
...
def build_family_product_entry_preset_graph_parts(
    *,
    ...
    resolved_federated_node_id = _require_string(resolved.get("federated_node_id"), "federated_node_id")
    ...
    resolved_federated_transition_event = _require_string(
        resolved.get("federated_transition_event"), "federated_transition_event"
    )
    resolved_direct_progress_event = _require_string(resolved.get("direct_progress_event"), "direct_progress_event")
    resolved_federated_progress_event = _require_string(
        resolved.get("federated_progress_event"), "federated_progress_event"
    )
    resolved_federated_title = resolved.get("federated_title")
    resolved_federated_surface_kind = resolved.get("federated_surface_kind")
    ...
    has_federated_entry = bool(resolved_federated_title and resolved_federated_surface_kind)

    if has_federated_entry:
        nodes.append(
            {
                "node_id": resolved_federated_node_id,
                "node_kind": "federated_entry",
                "title": resolved_federated_title,
                "surface_kind": resolved_federated_surface_kind,
                "produces_checkpoint": True,
            }
        )
        edges.append(
        ...
        checkpoint_nodes.append(resolved_federated_node_id)
    ...
    if has_federated_entry:
        edges.append(
            {
                "from": resolved_federated_node_id,
                "to": resolved_progress_node_id,
                "on": resolved_federated_progress_event,
            }
        )
